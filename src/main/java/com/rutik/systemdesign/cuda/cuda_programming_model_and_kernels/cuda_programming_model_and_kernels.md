...
<!-- study-paths
senior: cuda_programming_model_and_kernels.md
...
[occupancy_and_launch_configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md)), while
...
- **Kernel launches are asynchronous**: `kernelName<<<...>>>(...)` returns control
  ...
  [memory_management_and_data_transfer](../memory_management_and_data_transfer/memory_management_and_data_transfer.md) for
...
- **Occupancy and warp size are downstream of these choices but not this module's
  ...
  [warps_and_simt_execution](../warps_and_simt_execution/warps_and_simt_execution.md)); how many resident
...
Example: 8x8 image, blockDim = (4, 4) -> gridDim = (2, 2) = four blocks B(x,y)
           ...
           col->  0    1    2    3    4    5    6    7
  row  0  [ B00 ][ B00 ][ B00 ][ B00 ][ B10 ][ B10 ][ B10 ][ B10 ]
  row  1  [ B00 ][ B00 ][ B00 ][ B00 ][ B10 ][ B10 ][ B10 ][ B10 ]
  row  2  [ B00 ][ B00 ][ B00 ][ B00 ][ B10 ][ B10 ][ B10 ][ B10 ]
  row  3  [ B00 ][ B00 ][ B00 ][ B00 ][ B10 ][ B10 ][ B10 ][ B10 ]
  row  4  [ B01 ][ B01 ][ B01 ][ B01 ][ B11 ][ B11 ][ B11 ][ B11 ]
  row  5  [ B01 ][ B01 ][ B01 ][ B01 ][ B11 ][ B11 ][ B11 ][ B11 ]
  row  6  [ B01 ][ B01 ][ B01 ][ B01 ][ B11 ][ B11 ][ B11 ][ B11 ]
  row  7  [ B01 ][ B01 ][ B01 ][ B01 ][ B11 ][ B11 ][ B11 ][ B11 ]

  B(x,y) = block with blockIdx.x=x, blockIdx.y=y -- x selects the COLUMN band,
  y selects the ROW band. Thread (tx=2, ty=1) inside
...
| Typical `blockDim.x` | **128 or 256** | Sweet spot balancing occupancy against per-block resource use for most kernels; tuned per-kernel in [occupancy_and_launch_configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md) |
| Max `gridDim.x` | **2^31 - 1** (~2.1 billion) | Effectively unbounded for the x dimension on every architecture CUDA still supports |
...
- **Thrust and CUB**: NVIDIA's C++ template libraries generate kernels with this
  ...
  See [cuda_math_and_dnn_libraries](../cuda_math_and_dnn_libraries/cuda_math_and_dnn_libraries.md).
...
- The operation is not already provided by a tuned library (cuBLAS, cuDNN,
  Thrust) — see [cuda_math_and_dnn_libraries](../cuda_math_and_dnn_libraries/cuda_math_and_dnn_libraries.md) for
...
- The dataset is small enough that host<->device transfer time dominates any
  compute savings — see [memory_management_and_data_transfer](../memory_management_and_data_transfer/memory_management_and_data_transfer.md).
...
The fix is one line and effectively free. Every warp except the last one has all 32
lanes pass the test, so the compare costs a single instruction and nothing else; in the
one partial warp where lanes disagree, `ptxas` implements the guard by *predicating* the
load/store rather than branching, so the failing lanes are masked off in the same
instruction the passing lanes execute. There is no performance reason to omit the guard,
only the mistake of forgetting it.
...
3. **Choosing `blockDim` not a multiple of 32.** A block of 100 threads still
   ...
   single instruction issue. See [warps_and_simt_execution](../warps_and_simt_execution/warps_and_simt_execution.md).
...
|---------------|------|-------|
| `nvcc` | Compiles `.cu` files (CUDA C++) to PTX/SASS | See [cuda_toolkit_and_compilation](../cuda_toolkit_and_compilation/cuda_toolkit_and_compilation.md) |
...
| CuPy (`cp.RawKernel`, `cp.ElementwiseKernel`) | Embed/generate CUDA C++ kernels from Python, operate on `cupy.ndarray` | See [python_gpu_ecosystem](../python_gpu_ecosystem/python_gpu_ecosystem.md) for the full ecosystem comparison |
| `cuda-gdb` / `compute-sanitizer` | Debug out-of-bounds accesses and races at the kernel level | The `memcheck` tool catches exactly the Section 10 bug at runtime — see [debugging_correctness_and_numerics](../debugging_correctness_and_numerics/debugging_correctness_and_numerics.md) |
| Nsight Compute / Nsight Systems | Profile a kernel's actual occupancy, memory throughput, launch overhead | See [profiling_and_performance_analysis](../profiling_and_performance_analysis/profiling_and_performance_analysis.md) |
...
2. **Default to `blockDim = 256`** (or 128) as a starting point; it is a multiple
   ...
   [occupancy_and_launch_configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md) for
...
- Crop-window crash rate: 100% of non-multiple-of-16 crops corrupted memory before
  the fix -> 0% after.
- Kernel latency for the 3840x2160 frame: unchanged to within measurement noise (the
  guard adds one predicated compare per thread) -> still comfortably inside the
  16.6 ms/frame budget that 60 fps allows.
...
**See also**: [warps_and_simt_execution](../warps_and_simt_execution/warps_and_simt_execution.md) for how
...
[occupancy_and_launch_configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md) for how
