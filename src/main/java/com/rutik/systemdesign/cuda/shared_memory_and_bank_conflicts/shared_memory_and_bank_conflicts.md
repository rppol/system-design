...
See [Memory Coalescing & Access Patterns](../memory_coalescing_and_access_patterns/memory_coalescing_and_access_patterns.md) for the
global-memory-side analog of this problem (coalesced vs strided access), and
[Synchronization & Atomics](../synchronization_and_atomics/synchronization_and_atomics.md) for the `__syncthreads()`
...
### Bank index and conflict degree — the two-line rule behind both grids

Both grids above are generated by the same pair of expressions. Writing them out turns the
`[32][32]` disaster and the `[32][33]` fix into one arithmetic fact instead of two facts to
memorize:

```
bank(word_index) = word_index mod 32           where word_index = byte_address / 4

For a 2D tile with row stride S words, read down a column at fixed col:
  bank(row, col) = (row * S + col) mod 32

  distinct banks touched by 32 rows = 32 / gcd(S, 32)
  conflict degree N                 = gcd(S, 32)
  cycles for that one instruction   = N
```

**Put simply.** "A column access spreads across all 32 banks exactly when the row stride shares no factor of 2 with 32 — so make the stride odd and you are permanently safe."

The greatest-common-divisor form is the part worth carrying into an interview. It says the
conflict is not about *how big* the stride is, it is about how much of the factor `32 = 2^5`
the stride contains. That is why `S = 33` and `S = 65` and `S = 17` are all equally fine
while `S = 32` and `S = 64` are equally catastrophic.

| Symbol | What it is |
|--------|------------|
| `word_index` | Byte address divided by 4. Shared memory is banked per 32-bit word, not per byte |
| `mod 32` | There are exactly 32 banks; the bank is the low 5 bits of the word index |
| `S` | Row stride in words — `32` for `tile[32][32]`, `33` for `tile[32][33]` |
| `row * S` | The term that should vary the bank across threads. It vanishes whenever `S` is a multiple of 32 |
| `gcd(S, 32)` | How many threads pile onto each bank. `1` means conflict-free, `32` means fully serialized |
| `32 / gcd(S, 32)` | How many distinct banks the warp actually reaches out of the 32 available |

**Walk one example.** Run a stride ladder through `gcd(S, 32)` — the value of `S` is the only input:

```
  Column read, 32 rows, fixed col. N = gcd(S, 32).

  stride S    gcd(S,32)   distinct banks    conflict     cycles    throughput
  (words)                 32 / gcd          degree N               1/N of peak
  --------    ---------   --------------    --------     ------    -----------
      1            1            32              1           1        100.000%
      2            2            16              2           2         50.000%
      3            1            32              1           1        100.000%
      ...
      8            8             4              8           8         12.500%
     16           16             2             16          16          6.250%
     32           32             1             32          32          3.125%   <- tile[32][32]
     33            1            32              1           1        100.000%   <- tile[32][33]
     64           32             1             32          32          3.125%
     65            1            32              1           1        100.000%
```

The two rows to stare at are `S = 32` and `S = 33`. One extra unused word per row moves the
conflict degree from 32 to 1 — a **32x** change in cycles for that instruction — because
`gcd(33, 32) = 1`. Nothing about the data moved, nothing about the algorithm changed; only
the spacing did.

**What the padding actually costs.** Exactly one word per row, and nothing else:

```
  unpadded  tile[32][32] = 32 x 32 x 4 B = 4,096 B
  padded    tile[32][33] = 32 x 33 x 4 B = 4,224 B
  overhead  =  128 B  =  3.125% more shared memory per tile

  The GEMM kernel in Section 6.1 holds two such tiles (As + Bs):
    unpadded 8,192 B/block   ->   padded 8,448 B/block   (+256 B)
```

Spending 3.125% more shared memory to remove a 32x serialization is the most lopsided trade
in CUDA, which is why `[N][N+1]` is a reflex rather than a decision. Note the symmetry: the
padding overhead (`1/32 = 3.125%`) and the 32-way conflict's throughput (`1/32 = 3.125%`)
are the same fraction — you pay one thirty-second of the memory to avoid running at one
thirty-second of the speed.

**Why `+1` and not `+2` or `+16`.** Any `S` coprime with 32 works, but `+1` is the smallest
such shift and every odd offset is coprime with a power of two, so `+1` is both minimal and
guaranteed. Adding `+2` would give `S = 34`, `gcd(34, 32) = 2` — a 2-way conflict, better
than 32-way but still not free. This is the trap in the "why not just pad by any amount"
follow-up question: **the padding must make the stride odd**, not merely different.
...
#### 6.5.1 When a conflict cancels the entire reason you used shared memory

Section 1 says a bad conflict "silently degrades toward global-memory levels." That is not a
figure of speech — it is a two-number comparison you can do in your head:

```
cycles(shared access) = N x base_shared_latency        N = conflict degree
                      = N x (20 to 30 cycles)

cycles(global load)   = 400 to 800 cycles (uncached)

break-even N = global_latency / base_shared_latency
```

**The idea behind it.** "Each extra conflicting thread costs you one more full shared-memory access, so past a certain conflict degree the on-chip scratchpad is no longer buying you anything over just reading DRAM."

This is the number that makes bank conflicts an urgent bug rather than a micro-optimization.
Tiling is only worth its `__syncthreads()` barriers and its occupancy cost because shared
memory is ~20x faster than global; a 32-way conflict spends that entire advantage.

| Symbol | What it is |
|--------|------------|
| `N` | Conflict degree — `gcd(S, 32)` from the rule above, or just "how many threads share the worst bank" |
| `base_shared_latency` | Cost of one conflict-free shared access, `20-30` cycles |
| `N x base_shared_latency` | Total cost, because the hardware issues the N groups back to back |
| `400-800` cycles | Uncached global-memory latency — the thing tiling exists to avoid |
| break-even `N` | The conflict degree at which shared memory stops being faster than global |

**Walk one example.** Take the conservative end of each range (`25` cycles shared, `600` cycles global):

```
  conflict     shared cycles      vs global      verdict
  degree N     N x 25 cycles      600 cycles
  --------     -------------      ---------      -------------------------------
      1              25              600         24x faster -- tiling pays off
      2              50              600         12x faster -- still clearly worth it
      4             100              600          6x faster -- worth it
      8             200              600          3x faster -- shrinking margin
     16             400              600          1.5x faster -- barely worth it
     32             800              600          SLOWER than just reading global

  break-even: N = 600 / 25 = 24-way conflict
```

At `N = 24` the shared-memory path and the global path cost the same, and the block paid for
barriers, occupancy pressure, and staging code to get nothing. Past that — and the
`tile[32][32]` column access sits at `N = 32`, well past it — **the tiled kernel is slower
than the naive un-tiled one it replaced.** That is the war story behind every "we added
shared memory and it got slower" ticket.

**Why the ranges do not ruin the argument.** Slide to the optimistic end (`20` cycles shared,
`800` global) and break-even moves to `N = 40`, which the 32-bank hardware cannot even reach;
slide to the pessimistic end (`30` shared, `400` global) and break-even drops to `N = 13.3`.
So the conclusion holds across the whole range: small conflicts (2-4 way) are a tax worth
paying attention to, and a 32-way conflict is somewhere between "no benefit" and "actively
worse." Either way the fix costs 3.125% of a tile.
...
**FIX — add the barrier (see [Synchronization & Atomics](../synchronization_and_atomics/synchronization_and_atomics.md) for
...
**Short:** Two or more threads in a warp hitting different addresses in the same one of 32 banks, forcing those accesses to serialize instead of completing in one cycle.
...
**Short:** The row stride of 32 words is a multiple of the 32 banks, so the bank formula reduces to just the column — every thread in the warp lands on the identical bank.
...
**Short:** Padding shifts the row stride to 33, making the bank formula row-dependent so 32 consecutive rows sweep through all 32 banks, costing 4 wasted bytes per row.
...
back?**
**Short:** A data race — threads read stale or partially-written values because no barrier guarantees every write completed first, often passing tests and failing in production.
You get a data race: some threads may read stale or partially-written shared-memory
...
specifically to catch this class of bug; see [Synchronization & Atomics](../synchronization_and_atomics/synchronization_and_atomics.md)
for the full barrier semantics.

**Q: What is broadcast access and why is it conflict-free regardless of warp size?**
**Short:** Every thread in a warp reading the exact same shared-memory word; the hardware detects this and services all 32 reads as one conflict-free transaction.
Broadcast is
...
**Short:** Static shared memory's size is fixed at compile time and bounds-checked; dynamic shared memory takes its size as a runtime launch argument, serving multiple tile sizes.
Static
...
**Short:** Shared memory is on-chip SRAM at 20-30 cycle latency versus global memory's 400-800 cycle off-chip HBM round trip — a locality difference, not just clock speed.
...
**Short:** Tiling requires cross-thread cooperation — one thread loads an element another later consumes — which only works if every thread in the block sees the same memory.
...
**Short:** The SM's total capacity is architecture-fixed (up to 228 KB on Hopper), but a single block's claim defaults to 48 KB and can be raised via `cudaFuncSetAttribute`.
...
**Short:** More shared memory per block leaves room for fewer resident blocks on the SM, so doubling the per-block claim can halve the number of resident warps.
A
...
[Occupancy & Launch Configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md) for how to check
...
hand?**
**Short:** Profile with Nsight Compute and check the Shared Memory Bank Conflicts line, backed by the `l1tex__data_bank_conflicts_pipe_lsu` counter, localized to the source line.
Profile the kernel with Nsight Compute and check the "Shared Memory Bank Conflicts"
...
**Short:** Each bank services only one address per cycle, so cost scales linearly with the size of the largest colliding group, not a flat conflict tax.
Because
...
difference?**
**Short:** L1 cache is hardware-managed with no explicit addressing, while shared memory is entirely programmer-declared and controlled, and the two share physical capacity.
L1 cache is hardware-managed while shared memory is entirely programmer-managed.
...
**Short:** Yes — a bank is 4 bytes, so a double spans 2 banks per element and a float4 spans 4, changing what stride triggers a collision and how much padding fixes it.
Yes —
...
**Short:** No — one extra element only fixes a 4-byte element with a stride that's a multiple of 32; wider types need padding recomputed to be coprime with 32.
...
**Short:** No — coalescing is global addresses fitting few 128-byte DRAM transactions, while bank conflicts are shared-memory addresses hitting few of 32 SRAM banks.
No — they are
...
[Memory Coalescing & Access Patterns](../memory_coalescing_and_access_patterns/memory_coalescing_and_access_patterns.md) for the
global-memory side.

**Q: How do you handle bank conflicts in a shared-memory transpose kernel, where both the read
and write directions matter?**
**Short:** Pad the shared tile to `[TILE][TILE+1]` once, which resolves both directions simultaneously since only one of row-major and column-major access was conflicting.
Pad the shared tile once to `[TILE][TILE+1]`, which resolves
...
bug?**
**Short:** It cannot statically prove which access pattern a kernel will use — the same `tile[32][32]` might be row-major in one kernel and column-major in another.
Because the compiler cannot always statically prove which access pattern the kernel
...
actually see it?**
**Short:** A 32-way conflict costs roughly 32x the cycles of a conflict-free access — exactly what an unpadded `[32][32]` tile under a pure column access produces naturally.
The theoretical maximum is a 32-way conflict, costing roughly 32x the
...
shared-memory tile "to be safe"?**
**Short:** Shared memory is a hard per-SM capacity limit shared across resident blocks, so an oversized tile can silently cap resident blocks and regress occupancy.
Because shared memory is a hard per-SM capacity limit
