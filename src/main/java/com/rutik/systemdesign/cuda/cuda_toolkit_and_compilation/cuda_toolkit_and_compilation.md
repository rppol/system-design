...
Read this every time you touch a hot kernel: `16 registers` bounds how many warps can be resident per SM (see [occupancy_and_launch_configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md) for the occupancy math this feeds); `0 bytes spill stores/loads` confirms the register allocator did not have to spill to slow local memory — a nonzero spill count here is a silent performance cliff that never shows up as a compiler error.
