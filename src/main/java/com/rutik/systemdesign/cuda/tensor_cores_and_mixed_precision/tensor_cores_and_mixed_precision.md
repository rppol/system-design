...
<!-- study-paths
senior: tensor_cores_and_mixed_precision.md
...
| Fine-tuning / LoRA | BF16 autocast, often with FP32 master weights for small trainable adapters | Base weights frequently loaded in a compressed format (FP16/INT4 via QLoRA) while adapter gradients stay in a higher-precision path — see [`../../llm/fine_tuning/`](../../llm/fine_tuning/fine_tuning.md) |
...
| Post-training quantized inference (weights + activations) | INT8 (with calibration) or FP8 E4M3 | Highest throughput; requires calibration or quantization-aware training to bound accuracy loss — see [`../../llm/optimization_and_quantization/`](../../llm/optimization_and_quantization/optimization_and_quantization.md) |
...
- You are serving inference at scale where throughput per dollar dominates and you can afford a calibration or quantization-aware fine-tuning step to recover accuracy — see [`../../ml/model_compression_and_efficiency/`](../../ml/model_compression_and_efficiency/model_compression_and_efficiency.md).
...
| Triton | Python-embedded kernel DSL | Compiles block-level matmuls down through `mma.sync` automatically; trades hand-tuning control for much faster kernel authoring — see [`triton_and_kernel_dsls`](../triton_and_kernel_dsls/triton_and_kernel_dsls.md) |
...
**Short:** Usually a non-16-byte-aligned dimension forces an unvectorized edge kernel rather than dropping Tensor Cores entirely; the real dropout cause is an unsupported dtype like FP32 or FP64.
...
**Short:** TF32 truncates FP32 to about FP16 precision and is on by default for convolutions and cuBLAS GEMMs on Ampere+, though `torch.matmul` leaves it off unless enabled explicitly.
...
**Short:** FP16's narrow 5-bit exponent range lets small gradients underflow to zero, eventually producing NaN/Inf; fix with `GradScaler` loss scaling or switch to BF16, which needs none.
...
**Short:** A dedicated hardware unit computing a fixed-shape matrix-multiply-accumulate as one warp-level `mma.sync` instruction, with each lane holding a fragment of the tile.
...
**Short:** A portable C++ abstraction over `mma.sync`'s undocumented per-thread register layout, keeping kernel source portable across GPU generations while the compiler lowers it.
...
**Short:** The hardware only implements a small fixed set of tile shapes per precision; a GEMM of any size is built by tiling the K loop and padding M/N/K to the nearest multiple.
...
**Short:** Summing many products in the narrow input format would compound rounding error and could overflow FP16's ~65504 limit; a wider accumulator keeps precision until the final cast.
...
**Short:** It multiplies the loss up before backward and divides gradients back down, shifting values into FP16's narrow range; BF16 shares FP32's exponent range so it never underflows.
...
**Short:** Static fixes one scale factor requiring manual tuning; dynamic starts large, halves on detecting Inf/NaN while skipping that step, and grows back after clean runs.
...
**Short:** A 19-bit compute format Tensor Cores consume from ordinary FP32 storage with zero source changes, trading FP16-level mantissa precision for 4-8x throughput on Ampere+.
...
**Short:** E4M3 has more mantissa precision and a smaller range, suiting forward-pass weights and activations; E5M2 trades precision for wider range, suiting backward-pass gradients.
...
**Short:** Since cuBLAS 11.0 it uses Tensor Cores for any supported dtype with no dimension requirement outside FP8; alignment only decides which Tensor Core kernel, not whether one runs.
...
**Short:** NVIDIA's open-source template library implementing the full tiling hierarchy that cuBLAS itself is built from; reach for it when you need a fusion cuBLAS's epilogues don't offer.
...
**Short:** It keeps a per-op allowlist running FLOP-dominant matmuls and convolutions in reduced precision while numerically sensitive ops like softmax and layernorm stay in FP32.
...
**Short:** FP16's narrow exponent range rounds small values to zero, eventually causing division by near-zero; BF16 shares FP32's exponent range so the same values stay representable.
...
**Short:** A Hopper/Blackwell feature automating FP8 mixed-precision training by selecting E4M3 vs E5M2 and computing per-tensor scaling factors dynamically, reaching roughly 2x BF16 throughput.
...
**Short:** Check a metric like `sm__pipe_tensor_op_hmma_cycles_active` — near-zero means an unsupported dtype, low-but-nonzero means an alignment problem, not a dtype miss.
...
**Short:** For small precision-sensitive reductions like softmax or layernorm where accuracy risk outweighs negligible time saved, or when a workload demands bit-exact FP32 reproducibility.
...
**Short:** INT8 Tensor Cores compute integer matmuls at roughly double FP16 throughput but need a calibrated per-tensor or per-channel scale factor, unlike a drop-in FP16/BF16 dtype cast.
...
**Short:** Compilation fails outright with a compile-time error rather than silently degrading, a safer failure mode than a misaligned cuBLAS call that only regresses throughput.
...
**Short:** They're independent, multiplicative levers — 2:4 sparsity roughly doubles throughput on top of whatever precision speedup is in effect, not a substitute for one.
...
**Short:** Yes — Tensor Cores raise the compute ceiling, but a memory-bandwidth-bound kernel below the ridge point, like single-token decode, gains little from a ceiling it never reaches.
...
**Related reading**: [`../cuda_math_and_dnn_libraries/`](../cuda_math_and_dnn_libraries/cuda_math_and_dnn_libraries.md) for how cuBLAS/cuDNN/CUTLASS build on the primitives in this module; [`../../llm/optimization_and_quantization/`](../../llm/optimization_and_quantization/optimization_and_quantization.md) for quantization (GPTQ/AWQ) built on top of the FP8/INT8 paths introduced here; [`../../ml/model_compression_and_efficiency/`](../../ml/model_compression_and_efficiency/model_compression_and_efficiency.md) for the training-time mixed-precision recipes this module's kernel-level view underlies; [`../case_studies/cross_cutting/numerical_precision_and_determinism.md`](../case_studies/cross_cutting/numerical_precision_and_determinism.md) for the broader numerical-determinism concerns (FMA reordering, atomics) that compound with mixed precision; [`../case_studies/optimize_matrix_multiplication_kernel.md`](../case_studies/optimize_matrix_multiplication_kernel.md) for the full GEMM optimization ladder ending at the Tensor Core rung covered here.
