...
[memory_management_and_data_transfer](../memory_management_and_data_transfer/memory_management_and_data_transfer.md) — pinned
...
[cuda_graphs](../cuda_graphs/cuda_graphs.md), which captures a stream's operation sequence once and replays
...
- **Pinned (page-locked) host memory is mandatory for real overlap.** `cudaMemcpyAsync` from
  ...
  had. See [memory_management_and_data_transfer](../memory_management_and_data_transfer/memory_management_and_data_transfer.md)
...
- **Graph-captured stream sequences**: record a stream's exact operation sequence once via
  ...
  repeatedly with a single, cheap launch call — see [cuda_graphs](../cuda_graphs/cuda_graphs.md) for the
...
| CUDA graphs over captured streams | Same runtime concurrency, near-zero *launch* overhead | Highest upfront (capture/instantiate lifecycle) | Same operation sequence replayed thousands of times — see [cuda_graphs](../cuda_graphs/cuda_graphs.md) |
...
- The workload is a **single, one-shot, tiny transfer+kernel** — the fixed pipeline
  ...
  [gpu_computing_foundations](../gpu_computing_foundations/gpu_computing_foundations.md) §14 for the underlying
...
- The kernel itself is **not yet optimized** — overlapping transfer with a slow, unoptimized
  ...
  kernel first (see [occupancy_and_launch_configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md),
...
- You are about to **replay the exact same operation sequence thousands of times** — at that
  ...
  [cuda_graphs](../cuda_graphs/cuda_graphs.md) is the better tool.
- You need concurrency **across GPUs**, not within one — that's the domain of
  [multi_gpu_programming_and_nccl](../multi_gpu_programming_and_nccl/multi_gpu_programming_and_nccl.md), where streams are
...
8. **Consider CUDA graphs once the operation sequence stabilizes and repeats.** If the exact
   ...
   removes the remaining per-launch CPU dispatch cost — see [cuda_graphs](../cuda_graphs/cuda_graphs.md).
...
**See also**: [memory_management_and_data_transfer](../memory_management_and_data_transfer/memory_management_and_data_transfer.md)
for the pinned-vs-pageable and unified-memory mechanics this module assumes as a
prerequisite; [cuda_graphs](../cuda_graphs/cuda_graphs.md) for capturing a stable stream sequence to
eliminate remaining per-launch CPU overhead; and
[multi_gpu_programming_and_nccl](../multi_gpu_programming_and_nccl/multi_gpu_programming_and_nccl.md) for extending streams
