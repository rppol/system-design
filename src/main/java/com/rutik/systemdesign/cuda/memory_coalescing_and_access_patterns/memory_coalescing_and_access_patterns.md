...
<!-- study-paths
senior: memory_coalescing_and_access_patterns.md
...
**Prerequisites**: the SIMT/warp execution model ([warps_and_simt_execution](../warps_and_simt_execution/warps_and_simt_execution.md)) and the CUDA memory hierarchy ([cuda_memory_model_and_hierarchy](../cuda_memory_model_and_hierarchy/cuda_memory_model_and_hierarchy.md)) — coalescing is a property of how a *warp* touches *global* memory, so both must already be second nature.
...
*SoA does not change how much data the algorithm needs — it changes which bytes are adjacent to which, so the warp's natural "same field, all lanes" access becomes the coalesced case instead of the worst case. See [shared_memory_and_bank_conflicts](../shared_memory_and_bank_conflicts/shared_memory_and_bank_conflicts.md) for the analogous banking story once this same data is staged on-chip.*
...
- The kernel is compute-bound (arithmetic intensity far above the roofline's ridge point) — reshaping memory access won't move a kernel that's ALU-limited, not bandwidth-limited; check with [profiling_and_performance_analysis](../profiling_and_performance_analysis/profiling_and_performance_analysis.md) first.
...
**Short:** Coalescing merges a warp's 32 memory requests into the fewest transactions when addresses are consecutive and aligned, hitting 4 sectors for a fully coalesced load versus up to 32.
...
**Short:** The hardware still fetches whole 32-byte sectors, so a stride-2 warp access doubles the sectors fetched to 8 for the same 128 bytes of useful data, wasting exactly half.
...
**Short:** When no two lanes share a 32-byte sector, the hardware issues one sector fetch per lane, moving up to 1024 bytes to deliver 128 useful bytes, an 8x waste.
...
**Short:** Yes — a unit-stride warp access starting at an address not aligned to 128 bytes straddles two cache lines and costs two transactions instead of one.
...
**Short:** Reading one field per thread in an AoS layout separates consecutive lanes' copies by sizeof(struct) bytes, turning a unit-stride load into a 16-byte-stride load at roughly 25% efficiency.
...
**Short:** SoA stores each field in its own contiguous array so reading one field across the warp becomes a plain unit-stride load again, restoring 100% bus efficiency.
...
**Short:** A float4 load moves 16 bytes per thread in one instruction instead of four separate 4-byte loads, cutting instruction count roughly 4x while staying fully coalesced.
...
**Short:** A transpose must read row-major and write column-major or vice versa, so swapping loop order just moves the strided cost; only staging through shared memory decouples the two.
...
**Short:** Check the Memory Workload Analysis L1/TEX Cache table's Sectors/Req against the ideal of 4 for a fully coalesced 32-bit warp load; a much higher number means wasted transactions.
...
**Short:** No — coalescing governs a warp's global-memory addresses mapping to DRAM/L2 sectors, while bank conflicts govern shared-memory addresses mapping to the 32 on-chip banks.
...
**Short:** No — __ldg and const __restrict__ route reads through the read-only cache, reducing repeated-fetch cost, but leave the first-touch sector math unchanged for a strided pattern.
...
**Short:** No — occupancy hides latency by keeping more warps in flight, but a poorly coalesced access wastes bandwidth, so more warps just multiply the wasted traffic.
...
**Short:** Pre-Fermi required strict half-warp alignment to one segment with no partial credit, while Fermi-onward coalesce at sector granularity and are far more forgiving of misalignment.
...
**Short:** A row width not a multiple of 128/256 bytes drifts each successive row out of alignment, so cudaMallocPitch pads rows to a hardware-friendly stride the kernel must use for indexing.
...
**Short:** Generally no — the compiler can't prove aliasing and alignment safety across arbitrary pointer-indexed code, so float4 vectorization is almost always written explicitly.
...
**Short:** Coalescing is preserved as long as the per-iteration index still maps consecutive threadIdx.x to consecutive addresses within each iteration, since only which elements are visited changes over time.
...
**Short:** AoSoA groups threads into warp-sized SoA chunks preserving per-field coalescing while keeping a chunk's fields physically close, improving cache locality when several fields are read together.
...
**Short:** A sector (32 bytes) is the smallest fetchable unit, while a transaction bundles up to four sectors (128 bytes) together to service one warp instruction.
...
**Short:** It's worth a quick check but not deep investment — if the roofline chart places the kernel near the compute ceiling, even a perfect coalescing fix won't move wall-clock time.
...
**Short:** The naive matrix transpose's uncoalesced writes cap it at roughly 10-15% of peak HBM bandwidth, while a tiled version making both read and write coalesced reaches 70-90%, a 6-8x speedup.
...
**Short:** A 1D array has no next row to drift, but a 2D array's row-to-row stride equals its width, so an unpadded non-multiple-of-128 width misaligns every subsequent row.
...
**Short:** A kernel reading several fields of the same element pays a cache-locality cost under pure SoA since fields live far apart, so AoSoA keeps a chunk's fields close while staying coalesced within it.
...
- **Cross-check occupancy separately from coalescing** — a memory-bound kernel with wasted bandwidth needs a layout fix; an under-occupied kernel needs launch-configuration tuning (see [occupancy_and_launch_configuration](../occupancy_and_launch_configuration/occupancy_and_launch_configuration.md)); conflating the two wastes debugging time.
...
- How would you adapt this tiled pattern for a matrix too large to have both `in` and `out` resident in device memory simultaneously — what streaming/tiling strategy at the grid level would you add on top of the shared-memory tile? (See [streams_events_and_concurrency](../streams_events_and_concurrency/streams_events_and_concurrency.md).)
...
- [shared_memory_and_bank_conflicts](../shared_memory_and_bank_conflicts/shared_memory_and_bank_conflicts.md) — the on-chip analogue of this module's global-memory story; the tiled transpose in §14 depends directly on the padding technique explained there.
- [warps_and_simt_execution](../warps_and_simt_execution/warps_and_simt_execution.md) — the warp execution model that makes "coalescing is a per-warp property" true in the first place.
- [profiling_and_performance_analysis](../profiling_and_performance_analysis/profiling_and_performance_analysis.md) — how to read Nsight Compute's Memory Workload Analysis and roofline chart to confirm a coalescing fix actually worked.
