...
| Implementation language | Both: hand-written CUDA C++ for the reference/production kernel; Triton for portability and rapid iteration | CUTLASS-templated C++ only | CUTLASS/hand-written CUDA gets the last few percent of peak FLOP/s and full control over warp-specialization; Triton reaches ~90-95% of hand-tuned performance with 5-10x less code and compiles to multiple GPU vendors — see [`../triton_and_kernel_dsls/`](../triton_and_kernel_dsls/triton_and_kernel_dsls.md) for the full tradeoff |
...
**FlashAttention-4 (Dao et al., 2026)** is the Blackwell-generation continuation of the same lineage, and it is written entirely in **CuTe-DSL** — CUTLASS's Python kernel DSL — rather than C++ templates. The online-softmax recurrence in Section 4c is unchanged; what changes is the pipelining around it. The forward pass ping-pongs query tiles across two dedicated softmax warpgroups (so they do not contend for the same exponential unit) with a third warpgroup doing nothing but the `alpha` rescale, and it emulates `exp` in software by splitting `2^x` across `MUFU.EX2` and FMA (Cody-Waite range reduction plus a polynomial) to raise effective exponential throughput past what the SFU alone delivers. The backward pass keeps intermediates in Blackwell's tensor memory and uses the 2-CTA MMA mode to roughly halve shared-memory traffic, and the grid scheduler uses a longest-processing-time-first heuristic to absorb the load imbalance causal masking and variable sequence lengths create. Reported result on a B200 in BF16: up to ~1,605 TFLOP/s (about 71% utilization), 1.1-1.3x over cuDNN 9.13 and 2.1-2.7x over Triton on the forward pass. Read it as the fourth answer to this file's recurring question — once `S`/`P` never touch HBM, what is the next bottleneck — where the answer has moved from HBM traffic (FA-1) to parallelism (FA-2) to load/compute overlap (FA-3) to the non-matmul units and the scheduler themselves.
