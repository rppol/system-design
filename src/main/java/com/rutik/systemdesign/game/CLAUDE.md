...
- **`STUDY_PATHS` in `app.js` is `let STUDY_PATHS = {}` — a runtime cache, NOT a
  ...
  content: a `<!-- study-paths -->` block at the top of each module's `README.md`, one
  line per tier, naming every file that tier takes (`senior: README.md,
  singleton/README.md, …`). Listing a tier joins it; omitting it opts out; `README.md`
  must be listed and every named file must exist — both fatal under `--strict`. Case
  studies are declared the same way in `<section>/case_studies/README.md`. The shape
...
- **Order is never declared — `STUDY_ORDER` imposes it during derivation**, so every
  ...
  `STUDY_ORDER`, `<tier>Files` keys in-tier with real files including `README.md`, and
