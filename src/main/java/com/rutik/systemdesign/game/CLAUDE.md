...
- **`tech_index.json` is GENERATED and gitignored, like every other bank artifact.**
  ...
  walks by exact path (`TECH_BANK_DIR`), and a `## NN.` template heading anywhere under
  it fails `--strict`. `scripts/build_banks.sh` guards existence, 18 tiers, non-empty
  tools, and the **round-trip**: every record must still carry `k`/`r`/`l`/`s` and no key
  beyond those plus the optional authored description `d`. The file lives at the game
  ROOT, not in `questions/`, so the APK smoke test's `BANKS == SEC + 3` is unaffected.
- **The Technologies screen's per-render derivations are PRECOMPUTED into
  `questions/tech.json`.** Same philosophy as `build_diagrams.mjs` pre-rendering Mermaid
  to static SVG: `h` is the lowercase, pre-joined search haystack per tool (name + every
  citing module's title and blurb + the bank's authored short line and description — the
  short line was never searchable before), `s` is the row's section list, and top-level
  `secs`/`deep` replace a section sort and a whole-index pass over ~5,800 use-pairs. The
  `trade` array arrives sorted by section then `STUDY_ORDER` position, so the runtime sort
  is a stable no-op. **Deliberately left at runtime:** the tradeoff tab's own haystack
  (+53.5 KB gzip to save ~1.7 ms) and every facet count — `tierCounts`/`roleCounts` depend
  on the *other* active facets and the live query, so they genuinely cannot be precomputed.
- **The pinned deep-dive row is AUTHORED, in `build_tech.py`'s `PRODUCT_DEEP_DIVES` /
  `CONCEPT_DEEP_DIVES`** — `deep` in `tech.json` is where it lands. It used to be
  `m.m.startswith("technologies/")`, right only while `technologies/` was the one place a
  product page could live. The placement rule ("a product lives in the section owning its
  DOMAIN; `technologies/` is for the homeless ones") put Kafka and RabbitMQ in `backend/`,
  Redis, Elasticsearch, Postgres and MySQL in `database/`, PyTorch and MLflow in `ml/`,
  FAISS and vLLM in `llm/`, Pydantic in `fastapi/`, the JVM in `java/` and OpenTelemetry in
  `devops/` — thirteen product deep dives the screen linked none of. **The filename is not
  a safe rule either**: ten pages match `*_deep_dive` / `*_internals` and teach a CONCEPT
  (caching strategies, TCP/IP, character encoding, indexing, the asyncio event loop), and
  filing those as tools would be worse than the omission. Hence two explicit sets keyed on
  `"<module>/<source file>"`, so a deep-dive SUB-FILE addresses precisely and anything
  under `technologies/` is pinned with no entry. **A page matching the pattern and listed
  in NEITHER set is FATAL under `--strict`** (`_deep_dive_pins` → `deepErrs` → `bank_errs`);
  without it the next product page is silently absent from the screen on a green build. The
  guard earned itself on its first run, catching
  `python/asyncio_and_event_loop/event_loop_internals.md`, an `*_internals` sub-file the
  author's own enumeration missed. **`app.js` has no browser-side fallback here, on
  purpose** — product-vs-concept is a judgement call that cannot be derived at runtime, so
  `idx.deep || []` and an empty row is the honest degradation. Cards carry their SECTION's
  accent and name that section, so a reader can see RabbitMQ lives in Backend.
- **`STUDY_PATHS` in `app.js` is `let STUDY_PATHS = {}` — a runtime cache, NOT a
  ...
  `questions/paths.json`, generated by `extract.py` and gitignored exactly like the
  question banks. **There is no array here to edit.** Membership — and the whole file
  inventory — is declared ONCE per section, in the `<!-- study-paths -->` block in
  `<section>/README.md`. The block lists every module, every file that module owns
  (module page AND deep-dive sub-files) and every case study, each tagged with the tiers
  it belongs to or `-` for Full path only:
  ...
  A module line is at column 0, its files indented. **Content files carry NO structural
  metadata** — a module page or deep-dive sub-file holds its topic and nothing else. This
  moved out of the content files so one place shows a section's whole shape; before that
  it was a genuine dual source (a README table plus an array in `app.js`, hand-synced).
  `_section_inventory()` parses the block and `build_paths_from_markers()` derives
  `{senior, principal, seniorFiles, principalFiles, cases:{senior,principal}}` from it.
  Case studies use the pseudo-module `case_studies` in the same block, and drive the
  Level filter on the Case Studies tab.

  **The inventory is enforced in BOTH directions and is FATAL under `--strict`:** a file on
  disk missing from the block, a listed file that does not exist, a `STUDY_ORDER` module
  the block omits, or a block entry `STUDY_ORDER` does not know. That guard is the whole
  point — the old failure was silent, a new sub-file either invisible to every tier or
  dragged into all of them, with a green build either way.
...
- **Order is never declared — `STUDY_ORDER` imposes module order during derivation**, so
  every tier is an ordered subset by construction and the old "order deviates" warning is
  structurally impossible. WITHIN a module, file order is the order the README block lists
  them (module page first), and **both tiers share that one order — a tier is a filter over
  it, not its own sequence.** What `check_wiring()` still enforces: 2-segment module ids,
  tier members present in `STUDY_ORDER`, `<tier>Files` keys in-tier with real files
  including the module page, and case-study paths that resolve outside excluded dirs.
