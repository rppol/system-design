...
CATEGORY_HEADS = {"category", "concern", "dimension", "capability", "layer", "area",
                  ...
                  # "step" and "component" were missing, and the cost was not symmetric:
                  # the phantom label got indexed AND the row's real products were thrown
                  # away. `| Step | Tools | Notes |` in ml/ml_system_design/design_framework
                  # and `| Component | Options | Notes |` in llm/.../tool_selection_at_scale
                  # are textbook grouped tables -- Feast, Tecton, FAISS, Triton, Qdrant,
                  # Weaviate, DistilBERT and 9 others were being discarded while
                  # "Feature store", "Orchestration" and "Vector index" were indexed as
                  # technologies. Any word added here must be a word no PRODUCT is called.
                  "step", "component"}
...
# A math DECODER block -- `| Symbol | What it is |` -- explaining the variables in a formula.
# The repo asks authors to write these (see the decoder-block convention), and when one sits
# inside §11 its first column is symbols: `c x N`, `m * T`, `fail_under = 80`, `omit`. All
# get indexed as technologies, and two of them ended up with bank records opening "Not a
# tool". The markdown is CORRECT here, so the fix belongs in the reader of it, not in three
# content files. Two columns only: a third column means it is probably a real tool table.
SYMBOL_HEADS = {"symbol", "term", "variable", "notation", "abbreviation", "acronym"}
...
TECH_ALIASES = {                       # written this way -> indexed as this
    ...
    # groups were kept apart on that basis (Redis/redis-cli, Terraform/Terraform Cloud,
    # Kafka/Spring Kafka, OpenTelemetry API vs SDK, MCP CLI vs mcp-cli, and the rest).
    # Each entry maps a written form onto the canonical key; TECH_LABELS below then
    # forces the display name, because the surviving vote is often the ambiguous form.
    "amazon aurora": "aurora",
    "amazon dynamodb": "dynamodb",
    "amazon sqs": "sqs",
    "anthropic computer use api": "anthropic computer use",
    "anthropic message batches api": "anthropic message batches",
    "anthropic sdk": "anthropic",
    "antlr 4": "antlr",
    "api gateway": "aws api gateway",
    "argo cd": "argocd",
    "autovalue": "google autovalue",
    "aws bedrock guardrails": "bedrock guardrails",
    "aws cdk": "cdk",
    ...
    "graalvm reachability metadata repository": "reachability metadata repository",
    "grafana k6 cloud": "k6 cloud",
    "hashicorp raft library": "hashicorp/raft",
    "helm 3": "helm",
    "httpx asyncclient": "httpx.asyncclient",
    "hugging face peft": "peft",
    "hugging face trainer": "huggingface trainer",
    ...
    "nvidia cudnn": "cudnn",
    "nvidia nsight compute": "nsight compute",
    "nvidia nsight systems": "nsight systems",
    "nvidia transformer engine": "transformer engine",
    ...
    "rabbitmq_stomp plugin": "rabbitmq stomp plugin",
    "rank-bm25": "rank_bm25",
    ...
    "slf4j api": "slf4j",
    "spring @async": "@async",
    "spring @httpexchange": "@httpexchange",
    "spring @scheduled": "@scheduled",
    "spring @scope": "@scope",
    "spring applicationeventpublisher": "applicationeventpublisher",
    ...
    # 87 further merges from 207 candidates generated by patterns the first pass did
    # not use: an invocation whose first word is a record (curl / curl -v), a bare name
    # vs its fully-qualified twin (TreeMap / java.util.TreeMap), a version suffix, and a
    # punctuation-insensitive collision. 119 groups were KEPT apart, including two the
    # "strong merge" patterns would have fused wrongly: `Proxy` is the GoF pattern, not
    # java.lang.reflect.Proxy, and `DataLoader` is GraphQL's N+1 batcher, not PyTorch's.
    "### cache": "functools.cache",
    "### functools.cache": "functools.cache",
    "### python functools.cache": "functools.cache",
    ...
    "chars": "string.chars",
    "cilium ebpf": "cilium",
    "claude 3.5 api": "claude 3.5 sonnet",
    "claude thinking": "claude extended thinking",
    "cohere rerank 3.5": "cohere rerank",
    "compute-sanitizer racecheck": "compute-sanitizer --tool racecheck",
    "concurrent.futures.processpoolexecutor": "processpoolexecutor",
    "consul service mesh": "consul connect",
    "cuobjdump --dump-sass": "cuobjdump",
    "cuobjdump --resource-usage <binary>": "cuobjdump",
    "curl --http3": "curl",
    "curl -n --no-buffer": "curl",
    "curl -v": "curl",
    "curl -v --http2": "curl",
    "cursor ide": "cursor",
    "dataclasses [3.7]": "dataclasses",
    "datadog llm": "datadog llm obs",
    "dcgm exporter pairing": "dcgm exporter",
    "dcgm visibility caveat": "dcgm",
    "envoy as edge proxy": "envoy",
    "executors": "java.util.concurrent.executors",
    "explain analyze": "explain",
    "fastapi 0.140+": "fastapi",
    "fiddler ai": "fiddler",
    "filterchain": "jakarta.servlet.filterchain",
    "flashattention and flashinfer": "flashattention",
    "flyway community 10.x": "flyway",
    "gaia benchmark": "gaia",
    "gguf format": "gguf",
    "graalvm ce": "graalvm",
    "h2 database": "h2",
    "iperf3 -c server": "iperf3",
    "java.util.arraydeque": "arraydeque",
    "java.util.concurrent.locks.reentrantlock": "reentrantlock",
    "java.util.stream.collectors": "collectors",
    "java.util.treemap": "treemap",
    "javac --module-path": "javac",
    "javac -g": "javac",
    "javac -xdiags:verbose": "javac",
    "javac -xlint:all": "javac",
    "jcmd <pid> vm.native_memory summary.diff": "jcmd <pid> vm.native_memory",
    "jdbc java.sql": "jdbc",
    "jfr event jdk.virtualthreadpinned": "jfr jdk.virtualthreadpinned",
    "jfr jdk.virtualthreadpinned event": "jfr jdk.virtualthreadpinned",
    "jmap -clstats <pid>": "jmap",
    "jmap -dump:format=b,file=heap.dmp": "jmap",
    "jstack <pid>": "jstack",
    "jstat -gcutil": "jstat",
    "jstat -gcutil <pid> 1s": "jstat",
    "kong api gateway": "kong",
    "lightgbm 4.0+": "lightgbm",
    "liquibase oss 4.x": "liquibase",
    "lru_cache": "functools.lru_cache",
    "lsof -i": "lsof",
    "mcp python sdk": "mcp",
    "mlflow 2.10+": "mlflow",
    "mockito and other test doubles": "mockito",
    "mysql 8+": "mysql",
    "netstat -s": "netstat",
    "nmap --scan-flags syn": "nmap",
    "nvcc --ptxas-options=-v": "nvcc",
    "nvcc -rdc=true": "nvcc",
    "openai python sdk": "openai",
    "optuna 3.3+": "optuna",
    "perf stat -e dtlb-load-misses": "perf",
    "pg_dump --schema-only": "pg_dump",
    "priorityqueue": "java.util.priorityqueue",
    "prometheus and grafana": "prometheus",
    "py-spy record --gil": "py-spy",
    "pydantic-settings 2.x": "pydantic-settings",
    "python functools.cache": "functools.cache",
    "python functools.lru_cache": "functools.lru_cache",
    "pytorch 2.x": "pytorch",
    "re module": "re",
    "redis-py 6+": "redis-py",
    ...
    "sqlalchemy 2.x": "sqlalchemy",
    "ss -tan": "ss",
    "ss -tn": "ss",
    "struct module": "struct",
    "tcpdump -i eth0 port 5432": "tcpdump",
    "terraform cli": "terraform",
    "testcontainers 1.20.x": "testcontainers",
    "tf": "tensorflow",
...
TECH_HOMONYMS = {
    ...
    # Python's bisect module vs `git bisect`. The bank record papered over it in prose.
    "bisect": {"devops": "git bisect"},
    # Flux CD vs FLUX the text-to-image model. That record literally opens
    # "Name collision:", which is the tell that a homonym entry was missing.
    "flux": {"llm": "FLUX (text-to-image)"},
}
...
TECH_LABELS = {
    ...
    "anthropic message batches": "Anthropic Message Batches API",
    "antlr": "ANTLR",
    ...
    "git lfs": "Git LFS",
    "google autovalue": "Google AutoValue",
    "gpt-4o": "GPT-4o",
    ...
    "rabbitmq stomp plugin": "RabbitMQ STOMP plugin",
    "rank_bm25": "rank_bm25",
    "re2": "RE2",
    "reachability metadata repository": "GraalVM Reachability Metadata Repository",
    "reactor-test": "Reactor Test",
    ...
    "claude extended thinking": "Claude extended thinking",
    "collectors": "Collectors",
    "compute-sanitizer --tool racecheck": "compute-sanitizer --tool racecheck",
    "consul connect": "Consul Connect",
    "cuobjdump": "cuobjdump",
    "curl": "curl",
    "cursor": "Cursor IDE",
    "dataclasses": "dataclasses",
    "datadog llm obs": "Datadog LLM Obs",
    "dcgm": "DCGM",
    ...
    "h2": "H2 Database",
    "iperf3": "iperf3",
    "jakarta.servlet.filterchain": "jakarta.servlet.FilterChain",
    ...
    "javac": "javac",
    "jdbc": "JDBC",
    "jfr jdk.virtualthreadpinned": "JFR jdk.VirtualThreadPinned",
    "jmap": "jmap",
    "jstack": "jstack",
    "jstat": "jstat",
    "kong": "Kong",
    ...
    "shap": "SHAP",
    "sockjs": "SockJS",
    "sortedcontainers.sorteddict": "sortedcontainers.SortedDict",
            ...
            if i11 is not None:
                ...
                for hdr, rows in md_tables(body):
                    ...
                    if not grouped and h0 in COMPARISON_HEADS and len(hdr) >= 4:
                        continue
                    if h0 in SYMBOL_HEADS and len(hdr) == 2:
                        continue
                    for row in rows:
