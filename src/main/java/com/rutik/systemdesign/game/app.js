...
const STUDY_ORDER = {
  ...
  database: [
    ...
    "database/document_databases","database/key_value_stores","database/redis_internals","database/wide_column_databases","database/search_engines","database/elasticsearch_internals","database/graph_databases","database/time_series_databases",
  ...
  llm: [
    "llm/foundations_and_architecture","llm/tokenization_and_embeddings","llm/embeddings_and_similarity_search","llm/faiss_deep_dive",
...
// [TIERS] Curated Senior/Principal paths are DERIVED, not declared here: extract.py reads
// each file's `<!-- tiers: ... -->` marker and emits questions/paths.json, which boot()
// assigns below. Membership therefore lives with the content it describes -- the drift
// this used to have with the section READMEs is structurally impossible now.
// Shape: { <section>: { senior:[], principal:[], seniorFiles:{}, principalFiles:{},
...
//
// Generated by extract.py from `technologies/tech_bank/*.md` and gitignored, exactly like
// the question banks -- the JUDGEMENT lives in authored markdown, the artifact is derived.
// (A 96-pattern keyword attempt at deriving roles automatically reached 44.6% coverage
// with bad precision -- Prometheus drew 6 spurious roles of 10 while MongoDB got none --
// which is why a human writes them and a build reads them.) Fetched relatively like the
// rest, so no new APK seam.
//
...
// [TIERS] Case-study level filter, stored separately from the module path so the two are
// independent choices. "all" is the default and always available.
function getCaseTier(section) {
  try { return (JSON.parse(localStorage.getItem("sd_case_tier") || "{}")[section]) || "all"; }
  catch { return "all"; }
}
function setCaseTier(section, tier) {
  let m = {};
  try { m = JSON.parse(localStorage.getItem("sd_case_tier") || "{}"); } catch { }
  m[section] = tier;
  safeSet("sd_case_tier", JSON.stringify(m));
}
...
async function renderTech() {
  ...
  // Rows arrive READY: extract.py emits `s` (this tool's sections) and `h` (the lowercase
  // pre-joined haystack — name + module titles + usage blurbs + the bank's short line and
  // description). The browser used to rebuild 994,746 characters of that on EVERY render
  // to support a substring scan; none of it depends on user state, so it belongs in the
  // build. Falls back to computing it here when the artifact predates the change.
  const techRows = idx.tech[0] && idx.tech[0].h ? idx.tech : idx.tech.map((t) => {
    const sum = (bankTools && bankTools[t.n] && bankTools[t.n].s) || "";
    return { n: t.n, u: t.u, s: [...new Set(t.u.map((u) => secOf(u[0])))],
      h: (t.n + " " + sum + " " + t.u.map((u) => techName(mods[u[0]]) + " " + (u[1] || "")).join(" ")).toLowerCase() };
  });
  ...
  const tradeRows = idx.trade.map((t) => {
    const m = mods[t.i];
    // `h` here to match the tech rows' field name -- the tradeoff haystack stays a
    // RUNTIME build on purpose: emitting it costs +53 KB gzip to save ~1.7 ms on 607
    // rows of a non-default tab, and gzip cannot dedupe it against the cells it
    // lowercases a copy of.
    return { t, m, sec: m.m.split("/")[0], so: soIndex(m.m),
      h: (techName(m) + " " + t.h.join(" ") + " " + t.r.map((r) => r.join(" ")).join(" ")).toLowerCase() };
  ...
  function matchRank(name, q) {
    ...
    return 4;                       // matched only via r.h (a module title, a blurb, or the bank text)
  }
  ...
  function paint() {
    ...
    if (tab === "tech") {
      let rows = techRows;
      if (secPick !== "all") rows = rows.filter((r) => r.s.includes(secPick));
      if (q) rows = rows.filter((r) => r.h.includes(q));
      ...
      listEl.innerHTML = scoped.length ? slice.map(({ r, uses, b, w }) => {
        ...
        return `${head}<details class="tx-item${b ? " tx-item2" : ""}" data-n="${esc(r.n)}"${openRows.has(r.n) ? " open" : ""}>
          ...
          ${b && b.d ? `<div class="tx-desc">${b.d.split("\n\n").map((para) =>
            `<p>${esc(para).replace(/`([^`]+)`/g, "<code>$1</code>")}</p>`).join("")}</div>` : ""}
...
async function openStudySection(sectionPath) {
  ...
  // [TIERS] Case studies are tiered too: senior gets the canonical builds you are asked to
  // design end to end, principal the ones carrying hard tradeoffs, migration, cost or
  // failure-domain reasoning. `caseTier` persists independently of the module path, so you
  // can browse Senior case studies without leaving the Full module tree.
  const caseTiers = (_sp.cases || { senior: [], principal: [] });
  const caseTier = getCaseTier(section);                // "all" | "senior" | "principal"
  const casesList = (caseTier !== "all" && (caseTiers[caseTier] || []).length)
    ? allCases.filter((c) => caseTiers[caseTier].includes(c.file))
    : allCases;
  ...
  // [TIERS] Level filter, shown only inside the Case Studies tab and only when the section
  // has actually been tiered. Counts are on the chips so the cut is legible before you click.
  const caseTierOpts = casesMode ? [
    { v: "all", label: `All ${allCases.length}` },
    (caseTiers.senior || []).length ? { v: "senior", label: `Senior ${caseTiers.senior.length}` } : null,
    (caseTiers.principal || []).length ? { v: "principal", label: `Principal ${caseTiers.principal.length}` } : null,
  ].filter(Boolean) : [];
  const caseTierHtml = caseTierOpts.length >= 2 ? `
      <div class="pathswitch" role="radiogroup" aria-label="Case study level">
        <span class="pathswitch-label">Level</span>
        ${caseTierOpts.map((o) => `<button class="pathopt casetier${caseTier === o.v ? " on" : ""}" role="radio" aria-checked="${caseTier === o.v}" data-casetier="${o.v}">${o.label}</button>`).join("")}
      </div>` : "";
  ...
  document.querySelectorAll(".pathopt").forEach((b) => b.addEventListener("click", () => {
    ...
    if (b.dataset.casetier) {                          // [TIERS] case-study level filter
      setCaseTier(section, b.dataset.casetier);
      announce(b.dataset.casetier === "all" ? "All case studies"
        : b.dataset.casetier === "senior" ? "Senior case studies" : "Principal case studies");
      openStudySection(section);
      return;
    }
    setStudyPath(section, b.dataset.path);
    announce(b.dataset.path === "senior" ? "Senior path" : b.dataset.path === "principal" ? "Principal path"
      : b.dataset.path === "cases" ? "Case Studies — read-only" : "Full path");
...
let _shortPrefPath = "";        // the reader page applyShortPref last ran for
function applyShortPref(path) {
  if (path !== undefined) _shortPrefPath = path || "";
  const on = localStorage.getItem("sd_reader_short") === "1";
  const panel = el("#reader");
  if (!panel) return;
  // The Summaries toggle hides `**Short:**` lines because in a Q&A that line duplicates
  // the answer directly beneath it. Under technologies/tech_bank/ the SAME marker means
  // the opposite: it is the one sentence saying what the tool IS, and the record is
  // headless without it. So those pages always show it, whatever the toggle says.
  const isBank = _shortPrefPath.includes("technologies/tech_bank/");
  panel.classList.toggle("short-on", on || isBank);
}
...
async function openReaderPath(path, title, navCtx, frag) {
  ...
  try {
    ...
    applyShortPref(path);                          // [SH] hide authored MCQ summaries unless opted in
