...
**Q15: What does a `@model_validator(mode="before")` receive that `mode="after"` does not?**
A `@model_validator(mode="before")` runs before Pydantic has parsed or coerced any field, so it receives the raw input — typically a `dict` — exactly as submitted, letting you rename keys, merge aliases, or reject a malformed shape before individual field validators ever run. A `@model_validator(mode="after")` runs once every field has already been parsed and coerced into its declared type, so it receives a fully-constructed model instance and is the right place for cross-field checks like `end_date > start_date`, as shown in Section 4.5. Use `mode="before"` for input normalization that must happen ahead of field-level parsing (for example, accepting both `"user_id"` and `"userId"` from different API versions); use `mode="after"` for validation that depends on multiple already-typed fields together.
