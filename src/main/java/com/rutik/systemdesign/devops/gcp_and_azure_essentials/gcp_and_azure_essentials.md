...
See [cloud_fundamentals_and_aws](../cloud_fundamentals_and_aws/cloud_fundamentals_and_aws.md) for the AWS baseline this module maps from.
...
**Pitfall 4 — Forgetting Cloud Run/Functions cold starts when scaling to zero.** `min-instances 0` saves cost but adds latency on the first request. FIX: set `min-instances` > 0 for latency-sensitive endpoints — see [serverless_and_faas](../serverless_and_faas/serverless_and_faas.md).
...
| Secret Manager / Key Vault | Secrets ([secrets_management](../secrets_management/secrets_management.md)) |
...
**Short:** EC2, S3, EKS, Lambda, RDS, and IAM map cleanly to GCP and Azure equivalents like Compute Engine/VMs and GCS/Blob Storage, since the underlying concepts are identical.
...
**Short:** AWS nests Accounts in Organizations, GCP nests Projects under Organization and Folders, and Azure nests Resource Groups under Subscriptions, with IAM/RBAC inheriting downward in each.
...
**Short:** A GCP VPC is a global resource with regional subnets, unlike AWS/Azure's regional VPCs, so instances across GCP regions communicate over Google's backbone without peering.
A GCP VPC is a global resource with regional subnets, whereas AWS VPCs and Azure VNets are regional. The practical consequence is that instances in different regions of the same GCP VPC can communicate over Google's private backbone without VPC peering, simplifying global architectures. On AWS/Azure you'd create regional VPCs/VNets and connect them with peering or Transit Gateway — see [cloud_networking_and_cdn](../cloud_networking_and_cdn/cloud_networking_and_cdn.md).

**Q4: What is Cloud Run and how does it compare to AWS Fargate and Lambda?**
**Short:** Cloud Run is a fully managed serverless container platform scaling to zero with per-instance concurrency, accepting any container unlike Lambda and needing no cluster unlike Fargate.
...
**Short:** AWS IAM attaches policies per request with explicit deny precedence, GCP IAM binds roles that inherit down the resource hierarchy, and Azure RBAC scopes role assignments at any management level.
...
**Short:** GCP's Workload Identity and Azure's federated Workload Identity both bind a Kubernetes service account to a cloud identity via the cluster's OIDC issuer, just like AWS IRSA.
...
**Short:** Choose Azure when the organization is heavily invested in Microsoft identity and licensing, since Entra ID integrates natively and Azure Hybrid Benefit lowers cost for existing Windows/AD estates.
...
**Short:** BigQuery is GCP's standout service, a serverless data warehouse separating storage and compute that scales query capacity automatically with no cluster to manage.
...
**Short:** Standardize on portable layers like Kubernetes and Terraform so workloads move with minimal change, and keep one cloud primary with the other as DR or best-of-breed services.
...
**Short:** AWS PrivateLink maps to GCP Private Service Connect and Azure Private Link, while CloudFront maps to Cloud CDN and Azure Front Door.
...
**Short:** The most common mistake is scoping the trust binding to an entire namespace or cluster instead of one exact service account, letting an unrelated pod assume the wrong permissions.
...
**Q12: What's the concrete cost/latency tradeoff between `--min-instances 0` and `--min-instances 1` on Cloud Run?**
**Short:** `min-instances 0` scales fully to zero for free idle time but pays a cold start on the next request, while `min-instances 1` keeps a container warm at an always-on cost.
`min-instances 0` means Cloud Run can scale fully down to zero containers, so idle periods cost nothing. The next request after an idle gap then pays a cold start — pulling the container and initializing the runtime — before it gets served, which is the latency this module's Pitfall 4 warns about. Setting `min-instances` to 1 or higher keeps that many containers warm continuously, eliminating cold-start latency for every request but billing for that reserved capacity around the clock, the same always-on cost profile as a small VM regardless of traffic. Background jobs and low-traffic internal tools tolerate scale-to-zero fine; user-facing, latency-sensitive endpoints need `min-instances` set above zero to avoid a slow first request after any idle period.

**Q13: If a broad role is granted at a GCP Folder and a narrower one is granted directly on a Project beneath it, which wins?**
**Short:** Neither overrides the other — GCP IAM is additive, so a broad role granted at a Folder still applies to every Project beneath it regardless of narrower grants below.
Neither one overrides the other — GCP IAM is additive, so the effective permissions are the union of every binding at every ancestor level. A broad `Editor` role granted at the Folder still applies to the Project underneath even if that Project also carries a narrower custom role, because a lower-level grant can only add permissions, never revoke or shadow what's inherited from above. This differs from a mental model borrowed from network ACLs or from Azure's deny assignments, where a more specific rule can override a broader one — plain GCP IAM has no such override without explicitly adding IAM Conditions or a separate deny policy. The practical implication: you cannot fix an over-broad Folder-level grant by tightening permissions at the Project below it, only by fixing the grant at the level where it was made.

**Q14: Bicep vs Terraform for Azure infrastructure — how do you choose?**
**Short:** Bicep is Azure-native with no separate state file and day-one support for new resource types, while Terraform is cloud-agnostic across providers at the cost of managing explicit state.
Bicep is Azure-native, compiling directly to ARM JSON with no separate state file to manage since Azure Resource Manager tracks deployment state itself. It also gets day-one support for brand-new Azure resource types, since it's maintained by the same team that ships the Azure APIs. Terraform is cloud-agnostic instead — the same tool and workflow provision AWS, GCP, and Azure resources through separate providers, which is what a real multi-cloud organization needs, at the cost of an explicit state file to manage and occasionally lagging support until the `azurerm` provider catches up to the newest Azure features. Choose Bicep when you're Azure-only and want the fastest access to new capabilities; choose Terraform when you're multi-cloud or already standardized on it elsewhere, as this module's best practices recommend.

**Q15: Structurally, how does an Azure RBAC role assignment differ from a GCP IAM binding, and what can Azure do that plain GCP IAM can't?**
**Short:** A GCP IAM binding attaches a role to members on a resource's policy, while an Azure RBAC role assignment is its own auditable object, and Azure also supports explicit deny assignments GCP IAM lacks.
A GCP IAM binding attaches one role to a list of members directly on a resource's policy object, so the policy is organized role-first, with members nested underneath each role. An Azure RBAC role assignment is its own separate object per grant — one principal, one role definition, one scope — which makes each individual grant independently visible, auditable, and removable without touching any other assignment at that scope. Azure also supports explicit deny assignments, a separate mechanism (typically applied by platform teams or Azure Blueprints) that blocks a specific action at a scope even for principals who'd otherwise be allowed by their role — something classic, purely-additive GCP IAM has no equivalent for without adding IAM Conditions or a newer deny-policy feature. In practice this means Azure can lock a resource down against even its own Owners in a way plain GCP IAM bindings cannot.

**Q16: What's the cross-cloud egress-pricing gotcha teams miss when designing a multi-cloud or cross-cloud DR architecture?**
**Short:** All three clouds charge egress for data leaving their network at roughly similar per-GB rates, so continuous cross-cloud replication compounds that cost far more than staying inside one cloud.
All three clouds charge for data leaving their network, while inbound transfer is typically free. That means any architecture moving data between providers pays egress on every transfer, not just once, and standard egress pricing across AWS, GCP, and Azure sits in a broadly similar range — commonly around $0.08-$0.09 per GB in the first tier, dropping at higher monthly volume. That sounds small per request but compounds fast under continuous replication, like the cross-cloud DR pattern in this module's §7 that mirrors S3 objects into GCS. Same-region, same-cloud traffic is often free or far cheaper, and private interconnects (Direct Connect, Cloud Interconnect, ExpressRoute) reduce sustained high-volume egress cost, but neither eliminates the fact that leaving a cloud always costs more than staying inside it — architects who price compute and storage carefully often forget that the data-transfer line item can dominate the bill once replication volume grows.
...
- **Use Terraform** for cloud-agnostic IaC ([infrastructure_as_code_terraform](../infrastructure_as_code_terraform/infrastructure_as_code_terraform.md)); keep provider-specific modules isolated.
...
- **Set `min-instances` > 0** for latency-sensitive Cloud Run/Functions endpoints ([serverless_and_faas](../serverless_and_faas/serverless_and_faas.md)).
