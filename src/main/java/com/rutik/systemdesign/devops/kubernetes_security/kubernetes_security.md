...
**Network & data:**
- **NetworkPolicy** — segment Pod-to-Pod traffic (default-allow until a policy applies — see [kubernetes_networking](../kubernetes_networking/kubernetes_networking.md)).
- **Secrets** — base64-encoded by default; require encryption-at-rest and tight RBAC (see [secrets_management](../secrets_management/secrets_management.md)).
- **Supply chain** — image scanning, signing, admission gating (see [devsecops_and_supply_chain_security](../devsecops_and_supply_chain_security/devsecops_and_supply_chain_security.md)).
...
- **Restricted PSS + OPA Gatekeeper/Kyverno** in regulated environments reject any Pod that runs as root, lacks resource limits, or pulls an unsigned image — enforced at admission (see [policy_as_code_and_compliance](../policy_as_code_and_compliance/policy_as_code_and_compliance.md)).
...
- **External Secrets Operator + Vault** keeps real secrets out of etcd, syncing them just-in-time with rotation (see [secrets_management](../secrets_management/secrets_management.md)).
...
3. How would signed-image admission + image scanning (see [devsecops_and_supply_chain_security](../devsecops_and_supply_chain_security/devsecops_and_supply_chain_security.md)) have prevented the malicious dependency from running at all?

---

**Cross-references:** [kubernetes_networking](../kubernetes_networking/kubernetes_networking.md) (NetworkPolicy segmentation), [secrets_management](../secrets_management/secrets_management.md) (Vault, ESO, encryption), [policy_as_code_and_compliance](../policy_as_code_and_compliance/policy_as_code_and_compliance.md) (Gatekeeper/Kyverno admission), [devsecops_and_supply_chain_security](../devsecops_and_supply_chain_security/devsecops_and_supply_chain_security.md) (image scanning/signing), [container_runtimes_and_oci](../container_runtimes_and_oci/container_runtimes_and_oci.md) (sandboxed runtimes), [linux_and_os_fundamentals](../linux_and_os_fundamentals/linux_and_os_fundamentals.md) (capabilities, non-root).
