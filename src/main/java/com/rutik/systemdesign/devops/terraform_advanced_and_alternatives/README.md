...
A `dependency "vpc" { config_path = "../vpc" }` block tells Terragrunt to run the VPC state's `terraform output` and expose the results as `dependency.vpc.outputs.*` inside the current module's inputs, and it also adds an implicit ordering constraint so `run-all apply` provisions the VPC before anything that depends on it. This replaces manually copying a VPC ID between environments (which drifts the moment the VPC is recreated) with a live lookup that always reflects the dependency's current state, while still keeping each environment as an independent Terraform state rather than one giant monolithic configuration. It's the mechanism that lets Terragrunt decompose infrastructure into many small, independently-applicable states without losing the wiring between them.
