...
**Short:** GPUs are exposed as non-overcommittable integer extended resources with no kernel-level fractional throttling, so fractional sharing is faked via MIG or time-slicing.
...
**Short:** It installs the driver, container toolkit, device plugin, DCGM exporter, and MIG manager in strict dependency order, since each layer needs the one before it.
...
**Short:** MIG gives hardware-enforced isolation by physically partitioning the GPU, while time-slicing oversubscribes one GPU with no isolation, round-robin time-multiplexed.
...
**Short:** Three Pods grabbed GPUs while the fourth has none left, so all four idle waiting for synchronized all-reduce — fixed by an all-or-nothing gang scheduler like Volcano or Kueue.
...
**Short:** Allocation isn't utilization — check DCGM's SM-busy percentage, since low utilization despite full allocation usually means the data pipeline is starving the GPU.
...
**Short:** Karpenter provisions the cheapest instance type satisfying a Pod's GPU requirements, and training is protected with `consolidationPolicy: WhenEmpty` plus a do-not-disrupt annotation.
...
**Short:** Training and serving have opposite profiles — bursty and Spot-friendly versus steady and latency-sensitive — so sharing a pool starves or destabilizes one of them.
...
**Short:** Spot GPUs can be reclaimed with only a 2-minute warning, so training mitigates this by checkpointing state regularly and resuming after interruption.
...
**Short:** Namespace-per-team ResourceQuotas cap GPU counts, Kueue enforces fair-share queuing, and cost tags let a tool attribute GPU-hours per team for chargeback.
...
**Short:** At large scale, etcd, the API server, the scheduler, and DNS hit their own limits from Pod and status churn well before raw GPU count becomes the constraint.
...
**Short:** NVLink is a much faster GPU-to-GPU interconnect than PCIe, so a topology-unaware scheduler scattering a multi-GPU job across non-NVLink links bottlenecks gradient exchange.
...
**Short:** KServe is a Kubernetes-native serving CRD with autoscaling and canary rollout, while Triton is NVIDIA's inference server maximizing GPU throughput via batching, and they often compose.
...
**Short:** Scale-to-zero removes idle GPU replicas to stop paying for them, but the catch is a cold start that can take tens of seconds to minutes to reschedule and reload.
...
**Short:** DCGM's ECC and XID error signals flag a degrading card, and the response is to cordon the node, drain its Pods, and replace the instance.
...
**Short:** MPS lets trusted processes share a GPU context with weak isolation for better throughput than time-slicing, while MIG gives hardware-enforced isolation for untrusted multi-tenancy.
...
**Short:** DCGM GPU utilization is the headline exec metric because it exposes the allocation-versus-utilization gap that determines dollar efficiency.
...
**See also:** [kubernetes_scheduling_and_autoscaling](../kubernetes_scheduling_and_autoscaling/kubernetes_scheduling_and_autoscaling.md) · [kubernetes_operators_and_crds](../kubernetes_operators_and_crds/kubernetes_operators_and_crds.md) · [cloud_cost_optimization_finops](../cloud_cost_optimization_finops/cloud_cost_optimization_finops.md) · [case_studies/cross_cutting/gpu_node_lifecycle.md](../case_studies/cross_cutting/gpu_node_lifecycle.md) · [case_studies/design_ml_platform_infrastructure.md](../case_studies/design_ml_platform_infrastructure.md) · [`../../llm/case_studies/design_gpu_inference_platform.md`](../../llm/case_studies/design_gpu_inference_platform.md) · [`../../ml/distributed_training/`](../../ml/distributed_training/distributed_training.md)
