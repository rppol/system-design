...
Peak ingest times the retention window computes cumulative bytes written over that period, not bytes actually stored on disk at any instant. At 600 MB/s peak over a 7-day retention that arithmetic yields roughly 363 TB, drastically overstating real needs because peak traffic isn't sustained for the whole window. The correct formula is average ingest times retention times replication factor: at a 200 MB/s average with RF 3, stored bytes come out to about 363 TB replicated, or roughly 121 TB per broker across 3 brokers — still far past a 2 TB disk, which is exactly why tiered storage exists. Always size disk from measured average ingest, not the peak figure used for partition-count sizing.
