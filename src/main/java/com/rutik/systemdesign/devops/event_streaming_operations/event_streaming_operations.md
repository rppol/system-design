...
**Short:** RF=3, `min.insync.replicas=2`, and `acks=all` must align so an acked message lives on at least two brokers, blocking writes rather than silently losing data.
...
**Short:** Take the max of throughput needs (peak rate over per-partition write limit) and parallelism needs (partitions at least equal to max active consumers), rounding up modestly.
...
**Short:** Consumer lag is log-end-offset minus committed-offset summed across partitions, the key health signal because rising lag risks data loss once messages age past the retention window.
...
**Short:** A rebalance triggers on any group membership change, and with the default eager protocol it revokes all partition assignments from all consumers at once, pausing consumption entirely.
...
**Short:** Rack awareness places a partition's replicas across different AZs, so without it Kafka can put all replicas in one AZ and an AZ failure can take the whole partition offline despite RF=3.
...
**Short:** Reassigning partitions to a new broker needs a replication throttle so the data-copy traffic doesn't saturate NICs and starve client traffic, typically automated with Cruise Control.
...
**Short:** Kafka's read performance comes from the OS page cache, not the JVM heap, so an oversized heap starves the cache, forces disk reads, and creates long GC pauses.
...
**Short:** KRaft replaces the external ZooKeeper ensemble with an internal Raft-based controller quorum, giving faster failover and a much higher partition ceiling, and is the only mode from Kafka 4.0.
...
**Short:** Tiered storage keeps only recent hot segments on local broker disk and offloads older segments to object storage, decoupling retention cost from broker disk at the cost of slower cold reads.
...
**Short:** A lag spike across all partitions at once points to a group-wide event like a rebalance or a downstream dependency slowing down, while a single-partition spike suggests a hot key or stuck consumer.
...
**Short:** Roll brokers one at a time waiting for full ISR recovery between each, since restarting two brokers of an RF=3/min-ISR=2 topic simultaneously drops below the minimum and blocks writes.
...
**Short:** Choose managed Kafka when the operational burden of running a stateful cluster outweighs the cost premium, and self-managed Strimzi for fine-grained tuning, GitOps topics, or data-residency needs.
...
**Short:** A Schema Registry enforces compatibility rules as producers evolve their data, and without one a producer adding or renaming a field can silently break every consumer's deserialization cluster-wide.
...
**Short:** Peak ingest times retention overstates capacity by computing cumulative bytes at an unsustained rate; the correct formula uses average ingest times retention times replication factor.
...
**Short:** `compact` keeps only the latest value per key forever for changelog and CDC-style topics representing current state, while `delete` suits pure event streams within a bounded time window.
...
**Short:** Redpanda is a Kafka-API-compatible, JVM-free, thread-per-core broker with no ZooKeeper/KRaft dependency, giving lower tail latency as a drop-in swap for existing Kafka clients.
