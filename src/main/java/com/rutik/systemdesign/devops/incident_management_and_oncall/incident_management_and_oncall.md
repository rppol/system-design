...
1. **Detection** — alerts (see [visualization_and_alerting](../visualization_and_alerting/visualization_and_alerting.md)) page on-call when a user-impacting symptom crosses a burn-rate threshold. MTTD measures how long from problem-starts to someone-knows.
...
Every alert annotation should link to its runbook (see the `runbook:` field in [visualization_and_alerting](../visualization_and_alerting/visualization_and_alerting.md)). Runbooks turn a 3am scramble into a checklist.
...
## Impact
  - 38 min, ~4.2% of checkout attempts failed; ~$310k delayed/lost revenue; SLO budget 88% burned.
...
- **Measure MTTD/MTTR/MTBF** and connect incident outcomes to the error budget (see [sre_principles_and_slos](../sre_principles_and_slos/sre_principles_and_slos.md)).
...
**Cross-references:** [visualization_and_alerting](../visualization_and_alerting/visualization_and_alerting.md) (the alerts/escalation that page on-call and start the process), [sre_principles_and_slos](../sre_principles_and_slos/sre_principles_and_slos.md) (error budget consumed by incidents; blameless culture; MTTD/MTTR), [observability_tracing_and_otel](../observability_tracing_and_otel/observability_tracing_and_otel.md) and [observability_logging](../observability_logging/observability_logging.md) (diagnosis via metric→trace→log), [gitops_argocd_flux](../gitops_argocd_flux/gitops_argocd_flux.md) (rollback via `git revert` as mitigation), [disaster_recovery_and_resilience](../disaster_recovery_and_resilience/disaster_recovery_and_resilience.md) (failover as mitigation for regional incidents), [../../backend/chaos_engineering](../../backend/chaos_engineering/chaos_engineering.md) (practice incident response via game days).
