...
**Q13: What happens if the primary on-call engineer doesn't acknowledge a page within the escalation window?**
The escalation policy automatically bumps the page to the next tier so an unacknowledged alert is never silently dropped. In the module's PagerDuty-style policy, if primary doesn't ack within 10 minutes the page escalates to secondary, and if secondary also misses its own 10-minute window it escalates again to the engineering manager. This is enforced declaratively (`escalation_delay_in_minutes: 10` chained across schedule targets) so the tool — not a human noticing a miss — guarantees someone is eventually paged. Tune the ack window to be short enough to catch a genuinely unavailable responder but long enough not to escalate on someone who is already actively working the page.

**Q14: When is a full ICS activation (dedicated IC, Scribe, and Comms Lead) overkill?**
A full ICS activation is overkill for a SEV4 or any incident a single on-call engineer can resolve alone in minutes — just fix it and log it. Spinning up separate IC/Scribe/Comms roles for a cosmetic bug wastes coordination overhead and, per the module's guidance, erodes the meaning of severity so a future real SEV1 gets taken less seriously. The opposite mistake is just as damaging: winging a genuine multi-team outage with no IC because "everyone's already in the loop" recreates the exact duplicate-work chaos ICS exists to prevent. Match process weight to actual severity rather than applying one fixed process — or no process — to every incident.
