...
**Short:** Idempotency means re-running the same configuration produces the same end state with zero changes after the first run, achieved by declaring desired state instead of imperative commands.
...
**Short:** Push tools like Ansible connect out on demand over SSH with no agent, ideal for orchestration and bootstrap, while pull tools like Puppet poll from an agent for continuous drift correction.
...
**Short:** Immutable infrastructure bakes a versioned image with everything pre-installed and replaces instances rather than reconfiguring them, using CM as the image-build provisioner.
...
**Short:** Packer builds a versioned, immutable image by booting a base, running CM provisioners, and snapshotting the result, which Terraform then references to launch instances.
...
**Short:** Ansible suits pragmatic agentless orchestration, Puppet suits large fleets needing continuous pull-based compliance, Chef gives procedural programmability, and Salt excels at high fan-out real-time operations.
...
**Short:** Never commit plaintext secrets — encrypt them at rest, or better, fetch them at runtime from a dedicated store so the secret of record lives outside the CM repo.
...
**Short:** Procedural tools like Ansible execute tasks in the order written, while declarative tools like Puppet build a dependency graph and resolve ordering automatically; both can still be idempotent.
...
**Short:** Lint and dry-run the change, verify a role in isolation, then roll out gradually to a canary with health checks gating the rest instead of a big-bang run across the whole fleet.
...
**Short:** Ad-hoc SSH changes on long-lived hosts accumulate undocumented differences until no two servers are identical or reproducible, prevented by routing every change through reviewed CM or immutable images.
...
**Short:** Skip host-level CM when containers make the image itself the configuration, when provisioning infrastructure is Terraform's job, or on fully managed platforms with no host to configure.
When you run everything in containers, the container image *is* the configuration — you build it with a Dockerfile and deploy immutable images, so in-host CM is redundant (see [containers_and_docker](../containers_and_docker/containers_and_docker.md)). You also skip CM for provisioning the infrastructure itself (that's Terraform's job) and for fully managed/serverless platforms where there's no host to configure. CM remains valuable for image-baking provisioners, on-prem fleets, bootstrap, and compliance enforcement on long-lived VMs.

**Q11: End-to-end, what happens when a Puppet agent "pulls" its configuration?**
**Short:** The agent gathers local facts, sends them to the master which compiles a node-specific catalog, then the agent applies that catalog locally and reports the result back on each polling interval.
...
**Short:** Idempotency only guarantees one process converges to its own desired state, so a manual SSH hot-fix and a cron-driven CM re-apply can each be internally consistent yet silently overwrite each other.
...
**Short:** Reusing one playbook keeps the baked image and any live-configured host converging to one definition of correct instead of two definitions that can quietly drift apart.
...
**Short:** A lookup plugin fetches the plaintext secret from an external store at runtime and never writes even ciphertext into the repo, avoiding the vault-password-distribution problem Vault still has.
...
**Short:** `--check` can't accurately simulate raw `command` or `shell` tasks, so a playbook can report no changes in dry-run and still break production when actually run.
...
**Short:** Dynamic inventory plugins query the cloud provider's API at run time to build the target list from live tags, so a playbook always runs against what actually exists instead of a stale manual list.
...
- **Never store plaintext secrets** — encrypt at rest and prefer runtime fetch from [secrets_management](../secrets_management/secrets_management.md).
...
**Cross-references:** [infrastructure_as_code_terraform](../infrastructure_as_code_terraform/infrastructure_as_code_terraform.md) (provisions the hosts CM configures), [containers_and_docker](../containers_and_docker/containers_and_docker.md) (the image-is-config alternative to in-host CM), [secrets_management](../secrets_management/secrets_management.md) (keeping secrets out of playbooks), [linux_and_os_fundamentals](../linux_and_os_fundamentals/linux_and_os_fundamentals.md) (the host layer CM operates on), [ci_cd_fundamentals](../ci_cd_fundamentals/ci_cd_fundamentals.md) (running playbooks/Packer builds in pipelines), [disaster_recovery_and_resilience](../disaster_recovery_and_resilience/disaster_recovery_and_resilience.md) (reproducible hosts aid recovery).
