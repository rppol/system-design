...
Implementation is the `promote.yml` snippet in §4.2 + the pinned `cosign verify` in §4.3. The gate runs in CI and is double-enforced by the admission controller at the cluster — defense in depth, so a bypassed CI gate still can't run an unsigned image. See [`../devsecops_and_supply_chain_security/devsecops_and_supply_chain_security.md`](../devsecops_and_supply_chain_security/devsecops_and_supply_chain_security.md).
