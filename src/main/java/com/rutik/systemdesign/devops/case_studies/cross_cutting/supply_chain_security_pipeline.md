...
**SLSA Build track levels.** SLSA v1.0 reorganized the spec into independent *tracks*. The Build track runs **L0 to L3 — there is no Build L4**:
- **L0** — no guarantees. The floor for any artifact shipping without provenance at all.
...
- **L3** — Provenance is generated by a hardened, isolated builder where build steps cannot influence or forge it (e.g. GitHub's SLSA generator running in an isolated reusable workflow). Provenance is **non-falsifiable**. This is the top of the Build track and the practical target for production.

Two things people still look for as "L4" are not Build-track levels at all. Two-party review belongs to the separate **Source track** (its own L1-L4, graded on how the source revision was produced and reviewed), and **hermetic and reproducible builds were deferred out of the levels entirely** in v1.0 — they remain among the strongest anti-tamper practices available, but claiming a SLSA level for them is claiming something the spec does not define.
...
jobs:
  build-sign-attest:
    ...
    steps:
      ...
      # Third-party actions get pinned to a full commit SHA — a mutable ref like
      # @master is a supply-chain hole in the very pipeline meant to close one.
...
# Note the indentation: ctlog is a field of the AUTHORITY, a sibling of keyless.
# Nested one level deeper (under keyless) it is not part of the schema at all.
authorities:
  - keyless:
      url: https://fulcio.sigstore.dev
      identities:
...
|---|---|---|---|
| **syft** | SBOM generation | CycloneDX (also standardized as ECMA-424), SPDX | Fast filesystem + package introspection; pairs with grype |
...
authorities:
  - keyless:
    ...
    ctlog:
      url: https://rekor.sigstore.dev
    # ctlog and attestations are fields of the AUTHORITY — siblings of keyless,
    # not children of it. And require the provenance attestation on the same digest:
    attestations:
