...
**Why does a freshly provisioned GPU node report `nvidia.com/gpu: 0` for a minute or two?**
...
**What is a startup taint and why is it essential for GPU nodes?**
...
**How do you handle a Spot interruption on a training node without losing the run?**
...
**Why isn't "node is Ready" sufficient to know a GPU node is healthy?**
...
**Walk through what the GPU Operator does during node bring-up and how it sequences the steps.**
...
**How do you drain a GPU serving node without causing an availability incident?**
...
**What's the right checkpoint frequency for Spot training, and what's the tradeoff?**
...
**How does Karpenter decide to remove a GPU node, and how do you stop it removing one mid-training?**
...
**Why keep a warm pool of GPU nodes, and what does it cost?**
...
**A GPU node is stuck in NotReady with Pods Pending against it. How do you debug?**
...
**How do you upgrade GPU node drivers across a fleet safely?**
...
**What signals tell you a GPU is degrading and should be fenced?**
...
**How does MIG affect the node lifecycle?**
...
**Why separate training and serving GPU node pools at the lifecycle level (not just for pricing)?**
...
**What's the failure mode if you don't cap a GPU NodePool's size?**
...
**How do you validate that a newly-ready GPU node is actually usable, beyond the device plugin advertising capacity?**
