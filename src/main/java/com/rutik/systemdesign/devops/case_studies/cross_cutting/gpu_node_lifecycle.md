...
Read this alongside [`../../ml_platform_and_gpu_infrastructure/`](../../ml_platform_and_gpu_infrastructure/ml_platform_and_gpu_infrastructure.md) (the module) and [`../design_ml_platform_infrastructure.md`](../design_ml_platform_infrastructure.md) (the case study). For Kubernetes node basics and scheduling cross-reference [`../../kubernetes_scheduling_and_autoscaling/`](../../kubernetes_scheduling_and_autoscaling/kubernetes_scheduling_and_autoscaling.md) and [`../../kubernetes_architecture/`](../../kubernetes_architecture/kubernetes_architecture.md).
...
**Pitfall 5: Idle GPU nodes never scale down.** A misconfigured consolidation policy (or none) leaves emptied GPU nodes running for hours — at $3.43/hr per idle A100 80GB, an 8-GPU p4de.24xlarge burns ~$27.45/hr doing nothing. Set `consolidationPolicy: WhenEmpty` + a short `consolidateAfter`, with a hard `limits` cap as a backstop.
...
A runaway condition — a buggy job that keeps creating Pods, a misconfigured HPA, a retry storm, or a malicious/accidental burst — makes Karpenter keep launching GPU instances to satisfy pending Pods, and since each p4de.24xlarge costs ~$27.45/hour (8 x A100 80GB at $3.43/GPU-hr), the bill can reach thousands of dollars per hour within minutes before anyone notices. Setting `limits: { nvidia.com/gpu: N }` on the NodePool is a hard backstop: once the cap is hit, further Pods stay Pending (a visible, alertable condition) instead of silently provisioning more hardware. It's the GPU equivalent of a spending circuit breaker, and it's cheap insurance against the most expensive class of cloud accident.
...
**See also:** [`../../ml_platform_and_gpu_infrastructure/`](../../ml_platform_and_gpu_infrastructure/ml_platform_and_gpu_infrastructure.md) · [`../design_ml_platform_infrastructure.md`](../design_ml_platform_infrastructure.md) · [`../../kubernetes_scheduling_and_autoscaling/`](../../kubernetes_scheduling_and_autoscaling/kubernetes_scheduling_and_autoscaling.md) · [`../../kubernetes_architecture/`](../../kubernetes_architecture/kubernetes_architecture.md) · [`../../cloud_cost_optimization_finops/`](../../cloud_cost_optimization_finops/cloud_cost_optimization_finops.md) · [`../cross_cutting/kubernetes_production_hardening.md`](./kubernetes_production_hardening.md)
