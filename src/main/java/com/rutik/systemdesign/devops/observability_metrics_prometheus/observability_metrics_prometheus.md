...
<!-- study-paths
senior: observability_metrics_prometheus.md
...
| Strategy | When | Notes |
|----------|------|-------|
| Direct instrumentation | Your own service | Client library exposes `/metrics` (see [../../backend/observability_and_monitoring](../../backend/observability_and_monitoring/observability_and_monitoring.md)) |
...
Here SLO = 99.9% so the budget is `1 - 0.999 = 0.001`. Multi-window pairing (short + long) avoids both flapping (short-only) and slow detection (long-only). See burn-rate math in [sre_principles_and_slos](../sre_principles_and_slos/sre_principles_and_slos.md) and routing in [visualization_and_alerting](../visualization_and_alerting/visualization_and_alerting.md).
...
(Per-request identifiers belong in traces/logs, not metrics — see [observability_tracing_and_otel](../observability_tracing_and_otel/observability_tracing_and_otel.md).)
...
| Alertmanager | Dedupe/group/route alerts (see [visualization_and_alerting](../visualization_and_alerting/visualization_and_alerting.md)) |
...
- **Adopt SLO-based, multi-window burn-rate alerts** instead of static thresholds (see [sre_principles_and_slos](../sre_principles_and_slos/sre_principles_and_slos.md)).
...
- **Lint rules** with `promtool`/`pint` in CI; instrument apps via standard client libraries (see [../../backend/observability_and_monitoring](../../backend/observability_and_monitoring/observability_and_monitoring.md)).
...
**Cross-references:** [visualization_and_alerting](../visualization_and_alerting/visualization_and_alerting.md) (Grafana dashboards + Alertmanager routing over these metrics), [sre_principles_and_slos](../sre_principles_and_slos/sre_principles_and_slos.md) (SLIs/SLOs and burn-rate math expressed in PromQL), [observability_logging](../observability_logging/observability_logging.md) and [observability_tracing_and_otel](../observability_tracing_and_otel/observability_tracing_and_otel.md) (the other two pillars; where high-cardinality per-event data belongs), [../../backend/observability_and_monitoring](../../backend/observability_and_monitoring/observability_and_monitoring.md) (application-level instrumentation with client libraries — owned there), [kubernetes_architecture](../kubernetes_architecture/kubernetes_architecture.md) (controller/reconcile model and the cluster Prometheus monitors), [cross_cutting/prometheus_cardinality_and_scale](../case_studies/cross_cutting/prometheus_cardinality_and_scale.md) (deep dive on cardinality and scaling).
