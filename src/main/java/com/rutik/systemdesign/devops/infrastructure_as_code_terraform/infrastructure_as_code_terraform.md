...
<!-- study-paths
senior: infrastructure_as_code_terraform.md
...
Terraform's lineage matters for naming: HashiCorp relicensed Terraform from MPL 2.0 to the BUSL in August 2023, which forked the community-driven **OpenTofu** (covered in [terraform_advanced_and_alternatives](../terraform_advanced_and_alternatives/terraform_advanced_and_alternatives.md)). The core concepts here apply to both.
...
| Terragrunt | DRY wrapper generating backends per env | Less duplication (see [terraform_advanced_and_alternatives](../terraform_advanced_and_alternatives/terraform_advanced_and_alternatives.md)) |
...
**Reconsider when:** configuring the *inside* of servers (package installs, files, services) — that's [configuration_management](../configuration_management/configuration_management.md)'s job (Ansible/Packer), though Terraform can trigger it. Avoid Terraform for fast-changing application deploys where a GitOps/Kubernetes controller fits better (see [gitops_argocd_flux](../gitops_argocd_flux/gitops_argocd_flux.md)). For a single resource you'll touch once, the console may be faster — but anything you'll reproduce or audit belongs in IaC. Heavy imperative logic (loops with side effects, complex branching) signals you may want Pulumi instead (see [terraform_advanced_and_alternatives](../terraform_advanced_and_alternatives/terraform_advanced_and_alternatives.md)).
...
**Cross-references:** [terraform_advanced_and_alternatives](../terraform_advanced_and_alternatives/terraform_advanced_and_alternatives.md) (Terragrunt, Pulumi, OpenTofu, policy), [configuration_management](../configuration_management/configuration_management.md) (server config after provisioning), [secrets_management](../secrets_management/secrets_management.md) (keeping secrets out of state), [gitops_argocd_flux](../gitops_argocd_flux/gitops_argocd_flux.md) (declarative reconciliation for Kubernetes), [policy_as_code_and_compliance](../policy_as_code_and_compliance/policy_as_code_and_compliance.md) (Sentinel/OPA gating plans), [disaster_recovery_and_resilience](../disaster_recovery_and_resilience/disaster_recovery_and_resilience.md) (rebuild a region via apply), [cloud_fundamentals_and_aws](../cloud_fundamentals_and_aws/cloud_fundamentals_and_aws.md) (the resources Terraform provisions).
