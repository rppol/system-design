...
**Short:** FinOps is a cross-functional model cycling through Inform (visibility/allocation), Optimize (rightsizing, waste, discounts), and Operate (governance, automation) to maximize cloud value.
...
**Short:** On-Demand costs the most with no commitment, Spot saves up to 90% but can be reclaimed with 2 minutes' notice, RIs commit to a family for 1-3 years, and Savings Plans commit to $/hour more flexibly.
...
**Short:** Untagged resources pool into an unowned bucket, making cost attribution, rightsizing, and chargeback impossible until every resource carries team, product, and cost-center tags.
...
**Short:** Showback reports cost to teams for visibility without moving money, while chargeback actually bills those costs back to a team's budget, creating harder accountability but more friction.
...
**Short:** Put fault-tolerant, interruptible, stateless workloads like batch jobs and CI runners on Spot, never stateful services or latency-critical databases, since reclamation causes outages for the latter.
...
**Short:** Commit only to the stable 70-90% baseline, prefer flexible Compute Savings Plans over family-specific RIs, and stagger shorter terms so a later refactor can't strand the commitment.
...
**Short:** Over-provisioned instances, idle 24/7 non-prod, orphaned volumes and IPs, old snapshots, and NAT-routed traffic that could use free VPC endpoints are the biggest waste, all fixable with no reliability tradeoff.
...
**Short:** Rightsizing compares sustained p95 utilization against provisioned size and recommends a smaller instance when utilization stays low, commonly cutting 20-40% per resource.
...
**Short:** Cost anomaly detection alerts on unusual spend within hours, and per-team budgets with 80% threshold alerts turn a month-end surprise into a near-real-time signal.
...
**Short:** Tools like Kubecost or OpenCost allocate cost by namespace or label using pod resource requests, enabling per-team showback plus request rightsizing and Spot-backed autoscaling.
A shared cluster mixes many teams' workloads on the same nodes, so node-level billing doesn't map to teams; tools like Kubecost or OpenCost allocate cost by namespace, label, or workload using pod resource requests/usage to produce per-team showback. Optimize by right-sizing pod requests/limits (over-requesting wastes node capacity), using cluster autoscaling/Karpenter with Spot for bin-packed efficiency, and scaling non-prod namespaces down off-hours. This brings the same tag-driven allocation and rightsizing discipline inside the cluster — see [kubernetes_architecture](../kubernetes_architecture/kubernetes_architecture.md) and [cloud_fundamentals_and_aws](../cloud_fundamentals_and_aws/cloud_fundamentals_and_aws.md).

**Q11: What's the difference between enforcing tags with an SCP versus an AWS Organizations Tag Policy?**
**Short:** An SCP can hard-block untagged resource creation outright via a `Deny` condition, while a Tag Policy only standardizes allowed tag values and flags non-compliant resources without blocking them.
...
**Short:** Average CPU hides bursty peaks, so an instance averaging 25% but spiking to 90% would be wrongly downsized on the mean, while p95 still captures near-peak behavior.
...
**Short:** AWS delivers the interruption warning about two minutes ahead via the Instance Metadata Service, and tooling like the Node Termination Handler cordons and drains the node before reclaim.
...
**Short:** Coverage measures what percentage of usage a commitment covers, while utilization measures what percentage of the purchased commitment is used; low coverage wastes discount, low utilization wastes paid capacity.
...
**Short:** Unit economics ties spend to a business metric like cost per request or per customer, so a bill that grew because the customer base tripled can still show the platform got more efficient.
...
**Short:** A 12-hours-a-day, weekdays-only schedule leaves an instance on 60 of 168 weekly hours for roughly 64% savings, while narrowing to 8 hours a day pushes it to roughly 76%.
...
- **Automate governance**: budgets with 80% alerts, anomaly detection, policy-as-code ([policy_as_code_and_compliance](../policy_as_code_and_compliance/policy_as_code_and_compliance.md)).
- **Estimate cost in CI** (Infracost on Terraform PRs) so cost is visible before merge ([infrastructure_as_code_terraform](../infrastructure_as_code_terraform/infrastructure_as_code_terraform.md)).
...
**Cross-references:** [cloud_fundamentals_and_aws](../cloud_fundamentals_and_aws/cloud_fundamentals_and_aws.md) (pricing models, instance families, Graviton, VPC endpoints), [cloud_networking_and_cdn](../cloud_networking_and_cdn/cloud_networking_and_cdn.md) (data-transfer/egress cost), [serverless_and_faas](../serverless_and_faas/serverless_and_faas.md) (per-invocation vs always-on cost), [kubernetes_architecture](../kubernetes_architecture/kubernetes_architecture.md) (Kubecost, cluster autoscaling), [infrastructure_as_code_terraform](../infrastructure_as_code_terraform/infrastructure_as_code_terraform.md) (Infracost, tagging via IaC), [policy_as_code_and_compliance](../policy_as_code_and_compliance/policy_as_code_and_compliance.md) (tag/cost governance policies), [gcp_and_azure_essentials](../gcp_and_azure_essentials/gcp_and_azure_essentials.md) (CUDs, Spot VMs equivalents).
