...
**Q16: What's the difference between a traffic-weighted canary and a replica-ratio canary?**
A traffic-weighted canary sends an exact traffic percentage via a mesh or ingress regardless of pod count, while a replica-ratio canary only approximates that percentage through pod count. With weighted routing, one canary pod can hold steady at 1% of traffic indefinitely; with replica-ratio, exposure is coupled to replica count (1 of 20 pods is roughly 5%, and only if requests balance evenly across pods), which makes fine steps like 1% impractical. Weighted routing is what lets Argo Rollouts or Flagger drive precise `setWeight: 5` steps through Istio, Linkerd, or an ingress controller, fully decoupling exposure from scale. Prefer weighted traffic-splitting over replica-ratio whenever your mesh or ingress supports it, especially for precise, low-percentage initial canary steps.
