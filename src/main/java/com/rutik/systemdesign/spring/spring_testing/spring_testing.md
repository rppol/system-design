...
<!-- study-paths
senior: spring_testing.md
principal: spring_testing.md
...
**Short:** @Mock creates a plain Mockito mock with no Spring context; @MockitoBean also registers it into the context.
...
**Short:** Spring caches ApplicationContext instances keyed by config; identical keys let test classes reuse one context.
...
**Short:** MOCK loads a mock servlet environment with no real socket; RANDOM_PORT starts a real embedded Tomcat.
...
**Short:** @WebMvcTest loads only the MVC layer and Security, leaving services, repositories, and unrelated auto-config unloaded.
...
**Short:** @DataJpaTest auto-configures an embedded H2 database and rolls back each test's transaction for isolation.
...
**Short:** @MockitoSpyBean wraps a real bean so only stubbed methods are intercepted while the rest delegate to the original.
...
**Short:** Remove @Transactional from the test (or add @Commit) since the annotation otherwise rolls back every change.
...
**Short:** A static @DynamicPropertySource method registers container-derived properties before the context starts.
...
**Short:** A base class's @MockitoBean is shared by all subclasses, maximizing cache hits but risking overly broad mocking.
...
**Short:** Block on the async result with Awaitility or a CountDownLatch instead of Thread.sleep, which makes tests brittle.
...
**Short:** jsonPath() applies XPath-like expressions to JSON via the Jayway JsonPath library to assert response fields.
...
**Short:** @Sql declaratively runs a classpath SQL script at a given phase, while @BeforeEach with JdbcTemplate is imperative.
...
**Short:** @WebMvcTest tests the server side of requests; @RestClientTest tests outbound calls via a MockRestServiceServer.
...
**Short:** A static @Container field starts once per class and is shared; a non-static field restarts per test method.
...
**Short:** @JsonTest loads only Jackson (or Gson/Jsonb) auto-configuration and @JsonComponent beans for JSON assertions.
...
**Short:** @WithMockUser injects a preset authenticated principal; @WithSecurityContext builds a custom SecurityContext.
...
- [Spring Boot Auto-Configuration](../spring_boot_autoconfiguration/spring_boot_autoconfiguration.md) — test slices
...
- [Backend Testing Strategies](../../backend/backend_testing_strategies/backend_testing_strategies.md) — test pyramid theory, contract testing, flaky-test triage outside the Spring-specific tooling
