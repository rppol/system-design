...
6. **Error handling, retries, and circuit breaking are opt-in layers.** Spring's built-in error handler only tells success apart from 4xx/5xx; anything richer is composed on top, typically with Resilience4j (see [Spring Cloud Patterns](../spring_cloud_patterns/spring_cloud_patterns.md)).
...
Calling `.block()` here is safe because a Spring MVC controller method runs on an ordinary Tomcat worker thread, not a Netty event-loop thread — see [Spring WebFlux](../spring_webflux/spring_webflux.md) for why the same call inside a `@Controller` in a WebFlux application deadlocks.
...
Both shapes are the Chain of Responsibility pattern (see [Filters and Interceptors](../filters_and_interceptors/filters_and_interceptors.md)): each interceptor/filter decides whether, and how, to forward to the next link, and every registered one runs in registration order.
...
Neither `RestTemplate` nor `RestClient` retries anything on its own; wrap the call with Resilience4j (see [Spring Cloud Patterns](../spring_cloud_patterns/spring_cloud_patterns.md)) or Spring Retry:
...
**Netflix Feign and its Spring lineage.** Feign originated at Netflix as a declarative HTTP client generator and was later open-sourced and adopted by Spring Cloud as OpenFeign (see [Spring Cloud Patterns](../spring_cloud_patterns/spring_cloud_patterns.md)). `@HttpExchange` is Spring's own, dependency-light answer to the same idea: define an interface, get a working client, without adding a third-party annotation processor.
...
| WireMock | Yes — real embedded HTTP server | Contract-style testing with richer stub matching/scenario support (see [Spring Cloud Patterns](../spring_cloud_patterns/spring_cloud_patterns.md)) |
...
**Fixed:** generate an idempotency key per logical operation and have the retried call carry the same key so the server can deduplicate — the full pattern (including the server-side dedup table) is covered in [Spring Cloud Patterns](../spring_cloud_patterns/spring_cloud_patterns.md).
```java
@Retry(name = "orderCreate")
...
```
See [Spring HATEOAS & REST Maturity](../spring_hateoas_rest_maturity/spring_hateoas_rest_maturity.md) for the full `ProblemDetail` (RFC 9457) contract this pattern assumes on the server side.
...
**Short:** Its default request factory leaves connect and read timeouts unset, so they default to infinite.
...
**Short:** No, RestClient is built on RestTemplate's synchronous transport, not on Reactor at all.
...
**Short:** They bound pool-acquisition wait, TCP/TLS handshake time, and time waiting for response bytes.
...
**Short:** It blindly re-executes the call, risking a duplicate side effect like a second created order.
...
**Short:** Any 4xx or 5xx status is an error, thrown as HttpClientErrorException or HttpServerErrorException.
...
**Short:** HttpServiceProxyFactory reads the annotations and generates a JDK dynamic proxy that delegates calls.
...
**Short:** They determine which underlying client executes @HttpExchange calls and whether those calls block.
...
**Short:** Size the pool from the dependency's observed p99 latency times the caller's required throughput.
...
**Short:** Every additional caller queues indefinitely for a free connection since the wait is unbounded.
...
**Short:** Bind a MockRestServiceServer to the client and register request/response expectations up front.
...
**Short:** Use MockWebServer when the real wire protocol, sockets, and headers must actually be exercised.
...
**Short:** Register one ClientHttpRequestInterceptor on the builder so every call adds the header automatically.
...
**Short:** retryWhen re-subscribes reactively to the Mono, while @Retry is an AOP proxy re-invoking a blocking call.
...
**Short:** The JDK client needs no extra dependency; Apache HttpClient 5 exposes finer pool and timeout control.
...
**Short:** It relies on the JDK's small built-in keep-alive cache instead of implementing real application pooling.
...
- [Spring WebFlux and Project Reactor](../spring_webflux/spring_webflux.md) — `WebClient`'s reactive engine, `.block()` deadlock mechanics, Reactor Netty internals in depth
- [Spring HATEOAS & REST Maturity](../spring_hateoas_rest_maturity/spring_hateoas_rest_maturity.md) — `ProblemDetail` (RFC 9457) error contracts and the hypermedia-consuming angle of `RestClient`/`@HttpExchange`
- [Spring Cloud Patterns](../spring_cloud_patterns/spring_cloud_patterns.md) — Resilience4j retry/circuit-breaker/bulkhead, OpenFeign, and idempotency-key patterns layered on top of these clients
- [Networking & HTTP Client (Java)](../../java/networking_and_http_client/networking_and_http_client.md) — the plain JDK `HttpClient`/NIO/connection-pooling fundamentals underneath every Spring HTTP client
