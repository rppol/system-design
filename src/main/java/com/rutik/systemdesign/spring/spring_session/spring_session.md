...
**Short:** Sessions live in one instance's heap, so a request routed to a different node can't see them.
...
**Short:** Sticky sessions only postpone the failure, since a node crash or redeploy still wipes its pinned sessions.
...
**Short:** SessionRepositoryFilter wraps the request and overrides getSession() to resolve against an external store.
...
**Short:** Spring Session uses a SESSION cookie with a Base64 id instead of the container's JSESSIONID.
...
**Short:** RedisIndexedSessionRepository's expiration events depend on Redis keyspace notifications, disabled by default.
...
**Short:** RedisSessionRepository relies on plain TTL; RedisIndexedSessionRepository adds indexing and expiration events.
...
**Short:** changeSessionId() rotates the id in the external store, copying attributes and deleting the old key.
...
**Short:** Back concurrent-session control with SpringSessionBackedSessionRegistry so limits apply across the cluster.
...
**Short:** Enumerate a user's session ids via findByPrincipalName and delete each from the shared store.
...
**Short:** JWT scales statelessly but is hard to revoke; Spring Session revokes instantly at the cost of a shared store.
...
**Short:** flush-mode controls when session changes write to Redis, buffering by default until the request completes.
...
**Short:** Register HeaderHttpSessionIdResolver.xAuthToken() to carry the session id in a header instead of a cookie.
...
**Short:** Default JDK serialization can raise InvalidClassException when a deployed class shape changes mid-rollout.
...
**Short:** HttpSessionCsrfTokenRepository stores the CSRF token inside the shared session, so it survives failover.
...
**Short:** WebFlux uses a non-blocking WebSession backed by a ReactiveSessionRepository instead of HttpSession.
...
**Short:** Security reads SecurityContext from the session, so SessionRepositoryFilter must swap it in first.
...
**Short:** The main session key keeps a 5-minute TTL buffer so data is still readable when the expiration event fires.
...
**Short:** Yes — apps sharing the same Redis namespace read the same sessions, enabling lightweight shared-session SSO.
...
- [Spring Security Architecture](../spring_security_architecture/spring_security_architecture.md) — filter chain, `SecurityContext`, session management policies
- [Spring Security JWT/OAuth](../spring_security_jwt_oauth/spring_security_jwt_oauth.md) — the stateless alternative to shared sessions
- [Auth & Authorization Systems](../../backend/auth_and_authorization_systems/auth_and_authorization_systems.md) — session vs token auth, revocation, OIDC
- [Key-Value Stores](../../database/key_value_stores/key_value_stores.md) — Redis internals, TTL, keyspace notifications
- [Security & Auth (HLD)](../../hld/security_and_auth/security_and_auth.md) — session management at system-design scale
