...
**Short:** Config Server centralizes configuration in a versioned Git repository instead of baking it into each artifact.
...
**Short:** The client declares spring.config.import=configserver:... so ConfigDataLocationResolver fetches remote properties.
...
**Short:** Bootstrap context is a separate pre-main phase in Boot 2.x; spring.config.import folds that into one phase in Boot 3.x.
...
**Short:** @RefreshScope proxies a bean so it is destroyed and re-created on refresh; never apply it to a DataSource.
...
**Short:** Spring Cloud Bus broadcasts a refresh event over a shared message broker so one call refreshes the whole cluster.
...
**Short:** Config Server encrypts values via POST /encrypt, storing ciphertext prefixed with {cipher} in the config repo.
...
**Short:** Config Server remote properties take highest priority, overriding OS env vars and local application.yml files.
...
**Short:** Config Server fetches short-lived credentials from Vault and renews the lease before it expires.
...
**Short:** With fail-fast=true the service throws at startup and retries with backoff instead of running misconfigured.
...
**Short:** Deploy multiple stateless Config Server instances behind a load balancer, each cloning the same Git repository.
...
**Short:** spring.cloud.config.label selects the Git branch, tag, or commit the Config Server clones configuration from.
...
**Short:** Refreshing a DataSource under @RefreshScope destroys live connections and bursts new connections to the database.
...
**Short:** Make the server optional for tests with spring.config.import=optional:configserver: and inject test properties.
...
**Short:** The native profile serves configuration from the local filesystem or classpath instead of a Git repository.
...
**Short:** Config Server layers application.yml, profile files, and service-specific files, with later files overriding earlier ones.
...
- [Secrets Management](../../devops/secrets_management/secrets_management.md) — Vault dynamic secrets, rotation, and encryption-at-rest patterns behind this module's Vault backend
