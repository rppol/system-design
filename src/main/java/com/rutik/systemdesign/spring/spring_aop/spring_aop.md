...
**Short:** Spring AOP is proxy-based and limited to Spring beans; AspectJ weaves any object at compile or load time.
...
**Short:** Spring AOP offers Before, AfterReturning, AfterThrowing, After, and Around advice types.
...
**Short:** The execution pointcut designator with a package wildcard matches all public methods in that package tree.
...
**Short:** JoinPoint exposes the target, proxy, method arguments, and signature to non-Around advice.
...
**Short:** TransactionInterceptor is applied as Around advice through a proxy wrapped around @Transactional beans.
...
**Short:** Annotating aspect classes with @Order controls nesting, with lower values wrapping further outside.
...
**Short:** @Before cannot block execution, but the pre-proceed code in @Around can skip or alter the call.
...
**Short:** Spring AOP proxies only apply within the single ApplicationContext that created them.
...
**Short:** Skipping proceed() silently prevents the target method from ever executing.
...
**Short:** An uncaught exception from proceed() propagates normally and still triggers AfterThrowing advice.
...
**Short:** within matches by package or class name, while @within matches classes carrying a given annotation.
...
**Short:** Binding the annotation as a matching-named advice parameter exposes its attribute values.
...
**Short:** Spring AOP proxy dispatch costs only a few nanoseconds per call, not a real-world bottleneck.
...
**Short:** An @Around aspect matching a custom annotation can log every call with zero business-logic changes.
...
**Short:** AspectJ is needed for non-Spring objects, field or constructor interception, or private methods.
...
**Short:** It registers the AnnotationAwareAspectJAutoProxyCreator post-processor that builds aspect proxies.
...
**Short:** Self-invocation bypasses AOP because internal calls run through the raw target, not the proxy.
...
**Short:** Lower @Order values wrap aspects further outside, nesting them like stacked try-finally blocks.
...
**Short:** CGLIB cannot proxy final classes or final, private, or static methods, so advice on those never applies.
...
**Short:** A join point is one execution point; a pointcut is the expression that selects a set of join points.
...
- [Spring Proxies](../spring_proxies/spring_proxies.md) — JDK proxy vs CGLIB
- [Spring Transactions](../spring_transactions/spring_transactions.md) — @Transactional is AOP
- [Filters & Interceptors](../filters_and_interceptors/filters_and_interceptors.md) — filter vs AOP
- [Proxy Pattern](../../lld/structural/proxy/proxy.md) — the GoF pattern AOP proxies (JDK/CGLIB) are built on
- [Bytecode & Class-File Format](../../java/bytecode_and_classfile/bytecode_and_classfile.md) — how CGLIB generates a proxy subclass's bytecode at runtime
