...
- Overhead is roughly 10-20% versus single-document operations (same ballpark as the
  storage-engine numbers in [Document Databases](../../database/document_databases/document_databases.md)).
...
[Spring WebFlux](../spring_webflux/spring_webflux.md) for the full non-blocking picture, including
...
count (see [Key-Value Stores](../../database/key_value_stores/key_value_stores.md) for the full
skiplist mechanics).
...
[Document Databases](../../database/document_databases/document_databases.md)) feed the same events to
a search index.
...
**Q: Why does calling `mongoTemplate.findAll(Order.class)` on a large collection risk an OutOfMemoryError?**
...
**Q: Why does a MongoDB field annotated `@Indexed` still get a COLLSCAN in production?**
...
**Q: Why does `RedisTemplate<String, Object>` store values as unreadable binary blobs by default?**
...
**Q: Why can't a `@RedisHash` repository query a field unless it's annotated `@Indexed`?**
...
**Q: Why does wrapping two MongoDB writes in `@Transactional` throw at runtime against a fresh single-node `mongod`?**
...
**Q: What's the difference between `StringRedisTemplate` and `RedisTemplate<String, Object>`?**
...
**Q: When should you drop from a `MongoRepository` down to `MongoTemplate` directly?**
...
**Q: How does the MongoDB aggregation framework differ from a derived query or `@Query` method?**
...
**Q: What are the hard constraints on MongoDB multi-document transactions (4.0+)?**
...
**Q: How does `@Version` optimistic locking work in Spring Data MongoDB, and how does it differ from JPA?**
...
**Q: How does Spring Data Redis implement a secondary index for a `@RedisHash` entity?**
...
**Q: What is `@Reference` in Spring Data Redis and what does it cost?**
...
**Q: How do Redis pub/sub and Redis Streams differ as messaging mechanisms for a Spring application?**
...
**Q: Does `ReactiveMongoRepository` give you the same non-blocking guarantee as `ReactiveCrudRepository` over R2DBC?**
...
**Q: Does Spring Data Redis offer reactive `@RedisHash` repositories the same way MongoDB does?**
...
**Q: When should you choose MongoDB or Redis over Spring Data JPA on a relational database?**
...
**Q: When should Redis be a `@Cacheable` cache versus a Spring Data Redis-backed primary store?**
...
[Spring Caching](../spring_caching/spring_caching.md)'s `RedisCacheManager`. Use Spring Data Redis
...
**Q: How do you manage MongoDB indexes safely across multiple application instances in production?**
...
**Q: What happens when two concurrent writers both update the same `@Version`-tracked MongoDB document?**
...
**Q: Why can't you page through a 50-million-document MongoDB collection with `skip()`/`limit()`?**
...
**Q: Can Spring Data's derived query method names (`findByXAndY`) express the same things across JPA, MongoDB, and Redis?**
...
**Q: What happens to data already stored with the default JDK serializer if you switch a Redis app to JSON serialization?**
...
- [Spring Data JPA](../spring_data_jpa/spring_data_jpa.md) — the relational sibling: same repository
  contract, different consistency and query model
- [Spring Caching](../spring_caching/spring_caching.md) — `@Cacheable`/`RedisCacheManager`, the
  right layer when Redis merely accelerates data owned elsewhere
- [Document Databases](../../database/document_databases/document_databases.md) — MongoDB internals:
  WiredTiger, sharding, change streams, embedding vs referencing
- [Key-Value Stores](../../database/key_value_stores/key_value_stores.md) — Redis internals: data
