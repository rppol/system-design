...
<!-- study-paths
senior: spring_performance.md
principal: spring_performance.md
...
**Short:** HikariCP sizes to (database core count * 2) + effective spindle count, not the application host's core count.
...
**Short:** It defers bean creation to first access instead of startup, cutting startup time at the cost of late-fail errors.
...
**Short:** @Cacheable works via an AOP proxy; self-invocation bypasses that proxy so the cache is never consulted.
...
**Short:** Virtual threads keep the synchronous programming model, while WebFlux requires a non-blocking reactive rewrite.
...
**Short:** readOnly=true only tunes Hibernate flushing and the JDBC flag; a RoutingDataSource is needed to reach a replica.
...
**Short:** maxLifetime retires a pooled connection after a set duration to avoid handing out stale or firewall-dropped connections.
...
**Short:** spring-context-indexer generates a compile-time component list so Spring skips full classpath scanning at startup.
...
**Short:** GraalVM's closed-world AOT analysis needs explicit hints for reflection, dynamic loading, and JDK proxies to work.
...
**Short:** Pool saturation shows up as rising pending connections and acquire latency, remediated by sizing, then query, then DB fixes.
...
**Short:** Only a blocking JNI or Foreign Function & Memory call still pins a virtual thread to its carrier thread.
...
**Short:** TTL-based caching risks stale reads proportional to TTL length; @CacheEvict on writes restores strong consistency.
...
**Short:** server.tomcat.threads.max caps concurrent in-flight requests, since a blocked I/O thread still occupies its slot.
...
**Short:** Enable JFR to record GC pause events with microsecond resolution and zero production overhead for correlation with latency.
...
**Short:** GraalVM native image cuts startup to ~50ms and memory to tens of MB, trading away reflection and peak JIT throughput.
...
**Short:** Lazy initialisation defers bean creation to first use, risking deferred startup failures and a slow first request.
...
- [Spring Boot Actuator](../spring_boot_actuator/spring_boot_actuator.md) — metrics, GraalVM native
- [Observability & Tracing](../observability_and_tracing/observability_and_tracing.md) — tracing overhead
- [Structured Concurrency & Loom (Java)](../../java/structured_concurrency_and_loom/structured_concurrency_and_loom.md) — virtual threads, pinning, carrier threads
- [Performance Profiling](../../backend/performance_profiling/performance_profiling.md) — JFR, async-profiler, flame graphs, and profiling methodology behind the JVM tuning in this module
