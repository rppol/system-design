...
<!-- study-paths
principal: spring_hateoas_rest_maturity.md
...
**Short:** It adds hypermedia links so the client discovers actions instead of hardcoding URLs.
...
**Short:** A concatenated URL bakes in the host and path, breaking behind load balancers or routing changes.
...
**Short:** They use correct verbs and status codes but return no hypermedia links, which is a reasonable target.
...
**Short:** EntityModel wraps one resource, CollectionModel wraps a list, and PagedModel adds page metadata and links.
...
**Short:** It records a dummy method invocation to read routing metadata, so the method body never actually runs.
...
**Short:** HAL describes where you can go via links; HAL-FORMS adds templates describing how to invoke an action.
...
**Short:** It centralizes converting a domain object into a linked EntityModel, keeping link logic out of controllers.
...
**Short:** It is Spring's RFC 9457 standardized JSON error body, replacing every team's custom error shape.
...
**Short:** URI versioning is most visible and cache-friendly, header versioning keeps URLs clean, media-type is purest REST.
...
**Short:** An assembler adds an action link only when the resource's state and caller's permissions allow it.
...
**Short:** Clients navigate by relation name, so renaming a rel breaks every client that looked it up.
...
**Short:** Yes, and you pay HATEOAS's full cost in payload and complexity while getting none of its decoupling benefit.
...
**Short:** It is built on Spring HATEOAS and auto-exposes JPA or Mongo repositories as a full HAL API.
...
**Short:** It is Spring HATEOAS's client for navigating a HAL API by following relation names, not URLs.
...
**Short:** No, HATEOAS sits on top of Level 2 and only adds links without changing HTTP semantics.
...
- [Request Handling](../request_handling/request_handling.md) — `@RequestMapping`, `ResponseEntity`, `ProblemDetail`
- [Spring MVC Architecture](../spring_mvc_architecture/spring_mvc_architecture.md) — DispatcherServlet + message converters that emit HAL
- [REST API Design](../../backend/rest_api_design/rest_api_design.md) — resource modeling, verbs, status codes at Level 2
- [GraphQL](../../backend/graphql/graphql.md) — REST vs GraphQL: schema-driven graph vs hypermedia links
- [API Design (HLD)](../../hld/api_design/api_design.md) — API style tradeoffs at the system-design level
