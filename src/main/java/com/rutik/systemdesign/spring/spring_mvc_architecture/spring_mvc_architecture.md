...
<!-- study-paths
senior: spring_mvc_architecture.md
...
**Short:** It is the Front Controller that orchestrates every HTTP request through the Spring MVC pipeline.
...
**Short:** HandlerMapping resolves a request to a handler; HandlerAdapter knows how to actually invoke that handler.
...
**Short:** They convert request bodies into Java objects and Java objects into response bodies based on content type.
...
**Short:** A root ApplicationContext holds services while a child WebApplicationContext holds MVC-specific beans.
...
**Short:** DeferredResult, Callable, and SseEmitter all release the servlet thread while a response is prepared.
...
**Short:** It picks the response format from the Accept header, an optional parameter, or a configured default.
...
**Short:** Implement supportsParameter and resolveArgument, then register it via WebMvcConfigurer to bind custom arguments.
...
**Short:** A Filter runs in the servlet container before DispatcherServlet ever sees the request.
...
**Short:** DispatcherServlet raises NoHandlerFoundException, rendered as a 404 through the exception resolver chain.
...
**Short:** It disables all of Boot's MVC auto-configuration, including Jackson, Actuator, and static resources.
...
**Short:** A MultipartResolver parses multipart/form-data requests into parts bound as MultipartFile parameters.
...
**Short:** Each request occupies one Tomcat thread for its full duration, capping concurrency at the pool size.
...
**Short:** @RestController is @Controller plus @ResponseBody, serializing every return value straight to the body.
...
**Short:** The produces attribute narrows a handler to only match requests whose Accept header requests that media type.
...
**Short:** ExceptionHandlerExceptionResolver runs first, then ResponseStatusExceptionResolver, then the default resolver.
...
- [Request Handling](../request_handling/request_handling.md) — @RequestMapping, argument resolvers
- [Filters & Interceptors](../filters_and_interceptors/filters_and_interceptors.md) — filter vs interceptor
- [Spring WebFlux](../spring_webflux/spring_webflux.md) — reactive alternative
- [REST API Design](../../backend/rest_api_design/rest_api_design.md) — resource modeling, versioning, pagination for the APIs DispatcherServlet serves
