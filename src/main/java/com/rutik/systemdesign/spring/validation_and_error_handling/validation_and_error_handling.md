...
<!-- study-paths
senior: validation_and_error_handling.md
...
Key components:
- **Jakarta Validation 3.1** — constraint annotations (`@NotNull`, `@Size`, `@Pattern`, custom `@ConstraintValidator`)
- **`@Valid`** — triggers Jakarta Validation on a method parameter (Spring MVC, WebFlux)
...
- **`ProblemDetail`** — RFC 9457-compliant error response body (`org.springframework.http.ProblemDetail`)
- **`spring.mvc.problemdetails.enabled=true`** — auto-enables ProblemDetail for standard Spring exceptions

`ProblemDetail` is the standard error response format for Spring's built-in exceptions (`MethodArgumentNotValidException`, `HttpMessageNotReadableException`, and the rest), emitted as `application/problem+json`.
...
2. **`@Valid` vs `@Validated`**: `@Valid` = standard Jakarta Validation without group support; `@Validated` = Spring extension with groups + method-level AOP validation on any Spring bean.
...
5. **`ProblemDetail` (RFC 9457)**: Spring returns structured JSON error bodies (`type`, `title`, `status`, `detail`, `instance`) for its built-in exceptions once `spring.mvc.problemdetails.enabled=true`.
...
| Feature | `@Valid` (Jakarta Validation) | `@Validated` (Spring) |
|---|---|---|
| Standard | Jakarta Validation 3.1 | Spring Framework extension |
...
// Spring Boot auto-configures this; an explicit bean is only needed for customization.
...
### 6.5 @ControllerAdvice with ProblemDetail

```java
// Enable ProblemDetail for Spring's built-in exceptions
// application.yaml:
//   spring:
//     mvc:
//       problemdetails:
...
Spring MVC and WebFlux implement all of these natively rather than leaving you to hand-roll them. Pick the resolver once in `WebMvcConfigurer.configureApiVersion(...)` — `useRequestHeader("API-Version")`, `usePathSegment(n)`, `useQueryParam("v")` or `useMediaTypeParameter(...)` — then declare the version on the mapping itself: `@GetMapping(value = "/orders/{id}", version = "1.0")` for an exact match and `version = "1.2+"` for "1.2 and above". Versions are parsed semantically (`1.2` becomes major 1, minor 2, patch 0). A request naming a version no handler declares fails with `InvalidApiVersionException` and a request missing the version fails with `MissingApiVersionException` — both surface as 400, so an unknown version is a client error rather than a silent fall-through to the wrong handler. `StandardApiVersionDeprecationHandler` emits `Deprecation` (RFC 9745), `Sunset` (RFC 8594) and `Link` headers so clients learn a version is retiring from the response itself. `RestClient`, `WebClient`, HTTP service clients, MockMvc and `WebTestClient` all understand the same version, so the contract is testable end to end.
...
```
Spring's `ProblemDetail` follows the same pattern but with the standardised RFC 9457 fields (`type`, `title`, `status`, `detail`, `instance`) plus custom extension fields via `setProperty()`.
...
- Cross-field validation with complex interdependencies — consider a dedicated `Validator` class (Spring's `org.springframework.validation.Validator` interface, not a Jakarta constraint)
...
// FIX: ensure MethodValidationPostProcessor is registered. Spring Boot auto-configures it
// whenever a jakarta.validation provider is on the classpath (spring-boot-starter-validation);
// in plain Spring you must declare the MethodValidationPostProcessor bean yourself.
```
...
If a `@Validated` service method is called via `@Async`, the validation AOP proxy runs on the async thread. Spring binds transactions to the thread, so the caller's transaction is not available there. If the validator needs a DB lookup (like `UniqueEmailValidator`), it must open its own — annotate the validator's repository method `@Transactional(readOnly = true)`, which uses the default `REQUIRED` propagation and therefore starts a transaction when none is bound. `SUPPORTS` would run non-transactionally instead, which is exactly the failure you are trying to avoid.
...
|---|---|---|
| Jakarta Validation 3.1 | Jakarta EE 11 | Constraint annotation specification |
| Hibernate Validator 9.x | Spring Boot 4.x default (4.1 manages 9.1.0.Final) | Jakarta Validation reference implementation |
| `@Valid` | Jakarta Validation 3.1 | Cascade validation trigger |
...
| `spring.mvc.problemdetails.enabled=true` | Spring Boot | Auto-enables ProblemDetail for built-in exceptions |
...
**Q5: What is `ProblemDetail` and what problem does it solve?**
...
```
Verify `Content-Type: application/problem+json` — the RFC 9457 media type. `ProblemDetail` serialises to this content type.
...
Jakarta Validation uses `ValidationMessages.properties` in the classpath root for constraint message templates (e.g., `{jakarta.validation.constraints.NotBlank.message}`). Override messages by creating `src/main/resources/ValidationMessages.properties`:
```properties
...
5. **Enable `spring.mvc.problemdetails.enabled=true`** — auto-applies RFC 9457 to Spring's standard exceptions.
...
10. **Use `spring-boot-starter-validation`** explicitly in `pom.xml` / `build.gradle` — Hibernate Validator is not on the classpath by default; the starter adds it.
...
**Cross-links:**
- [Request Handling](../request_handling/request_handling.md) — `@RequestMapping`, `@ControllerAdvice`, MVC exception handling
- [Spring MVC Architecture](../spring_mvc_architecture/spring_mvc_architecture.md) — `HandlerExceptionResolver` chain
- [Spring Boot Autoconfiguration](../spring_boot_autoconfiguration/spring_boot_autoconfiguration.md) — how `spring-boot-starter-validation` auto-configures `MethodValidationPostProcessor`

---

## Related / See Also

- [Request Handling](../request_handling/request_handling.md) — @ControllerAdvice, ProblemDetail
- [Spring MVC Architecture](../spring_mvc_architecture/spring_mvc_architecture.md) — HandlerExceptionResolver
- [Case Study: Multi-Tenant API](../case_studies/design_multitenant_api.md) — tenant-aware error responses
- [REST API Design](../../backend/rest_api_design/rest_api_design.md) — error response contracts, RFC 9457 in the broader REST API design context
