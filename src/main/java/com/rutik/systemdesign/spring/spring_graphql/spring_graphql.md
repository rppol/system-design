...
- [request_handling](../request_handling/request_handling.md) — `@RequestMapping` and argument
  binding, the MVC analogue of the GraphQL controller model.
- [spring_data_jpa](../spring_data_jpa/spring_data_jpa.md) — the N+1 problem at the persistence
  layer and `Window`/`ScrollPosition` pagination GraphQL builds on.
- [spring_webflux](../spring_webflux/spring_webflux.md) — `Mono`/`Flux` resolvers and the
  reactive transport behind subscriptions.
- [GraphQL (backend)](../../backend/graphql/graphql.md) — GraphQL at the architecture
  level (federation, schema design, query cost).
- [REST API Design](../../backend/rest_api_design/rest_api_design.md) — REST vs GraphQL API
  design tradeoffs.
