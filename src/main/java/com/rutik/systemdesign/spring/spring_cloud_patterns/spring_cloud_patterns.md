...
<!-- study-paths
senior: spring_cloud_patterns.md, gateway_server_webmvc.md
principal: spring_cloud_patterns.md, gateway_server_webmvc.md
...
**Short:** Spring Cloud Gateway is a programmatic, reactive gateway on WebFlux, unlike Nginx's static declarative config.
...
**Short:** A GlobalFilter runs on every route; a GatewayFilter applies only to routes that explicitly declare it.
...
**Short:** HALF_OPEN lets a limited number of test calls through after OPEN to probe recovery before fully closing.
...
**Short:** Spring Cloud LoadBalancer replaced Netflix Ribbon, resolving lb:// URIs via a cached service instance list.
...
**Short:** Set spring.cloud.openfeign.circuitbreaker.enabled=true, which wraps every Feign method in a circuit breaker.
...
**Short:** Eureka favors Availability and Partition Tolerance, serving stale registry data rather than no data at all.
...
**Short:** Micrometer Tracing replaced Spring Cloud Sleuth from Boot 3.0 onward, adding vendor-neutral Brave/OTel support.
...
**Short:** B3 propagates trace context via X-B3-TraceId, SpanId, ParentSpanId, and Sampled headers across HTTP calls.
...
**Short:** Define two routes sharing a Weight predicate group so Gateway splits traffic by percentage between them.
...
**Short:** A Bulkhead proactively limits concurrent calls to a dependency, while a circuit breaker reacts after failures.
...
**Short:** RequestRateLimiter uses a Redis-backed token bucket, checked atomically via Lua, returning 429 when exceeded.
...
**Short:** FeignException isn't retried by default; a custom ErrorDecoder must map specific statuses to RetryableException.
...
**Short:** Implement a custom KeyResolver bean that extracts the authenticated user id instead of the client IP.
...
**Short:** Use WebTestClient with WireMock stubbing the downstream service to test a route without a real dependency.
...
**Short:** Resilience4j can count calls exceeding a duration threshold as failures even when they return 200 OK.
...
**Short:** The semaphore bulkhead is cheap but callers can still wait; the thread-pool bulkhead fully isolates callers.
...
**Short:** Blind retries of a non-idempotent POST can duplicate side effects; use an idempotency key to make it safe.
...
- [Spring Messaging](../spring_messaging/spring_messaging.md) — Kafka + Spring Cloud Stream
...
- [Fault Tolerance Patterns](../../backend/fault_tolerance_patterns/fault_tolerance_patterns.md) — circuit breaker, retry, bulkhead theory outside the Spring wrapper
- [Resilience Patterns (HLD)](../../hld/resilience_patterns/resilience_patterns.md) — cascading failure and blast-radius containment at the system level
- [Service Mesh & Service Discovery](../../backend/service_mesh_and_service_discovery/service_mesh_and_service_discovery.md) — sidecar-based alternative to Eureka + client-side LoadBalancer
