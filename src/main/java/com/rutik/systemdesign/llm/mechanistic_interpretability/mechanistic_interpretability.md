...
This module is the causal/structural counterpart to [Evaluation & Benchmarks](../evaluation_and_benchmarks/evaluation_and_benchmarks.md) (which measures *what* a model does) and [Safety & Alignment](../safety_and_alignment/safety_and_alignment.md) (which reasons about *whether* what it does is safe). Mech interp answers *why* and *how* — at the level of weights, activations, and circuits — and increasingly underpins production debugging, auditing, and targeted model editing.
...
- **Building monitors for properties that aren't visible in outputs** — e.g., a "is the model's internal state consistent with an honest answer" probe/steering-vector-based lie detector, as a complement to output-level [guardrails](../guardrails_and_content_safety/guardrails_and_content_safety.md).
...
**Short:** Probe accuracy shows a direction is linearly decodable, not that the model causally uses it — only ablation or patching proves causation.
...
**Short:** Superposition packs more concepts than neurons into overlapping directions, making individual neurons polysemantic and unreadable one at a time.
...
**Short:** The linear representation hypothesis says concepts are linear, additive directions in activation space — the assumption underlying logit lens, SAEs, and steering.
...
**Short:** An SAE reconstructs activations through a sparse, wider dictionary (`d_sae` 8x-64x `d_model`) so each learned direction becomes a near-monosemantic feature.
...
**Short:** A vanilla SAE's L1 penalty causes shrinkage by biasing active features toward zero, while TopK keeps the `k` largest activations at full, unshrunk strength.
...
**Short:** Activation patching overwrites one activation with its clean-run value to test causation, whereas zero-ablation sets it out-of-distribution and can break unrelated computation.
...
**Short:** The IOI circuit chains duplicate-token, S-inhibition, and name-mover heads to copy the non-repeated name, with every edge validated by path patching.
...
**Short:** Attribution patching uses one backward pass as a linear approximation to rank components fast, but it misjudges heads operating in a saturated, non-linear regime.
...
**Short:** Tuned lens learns a per-layer affine map into the final layer's basis, fixing logit lens's inaccurate early-layer readings caused by representation drift.
...
**Short:** CAA averages activation differences across many contrastive prompt pairs into a steering vector; pushed too high, it dominates the residual stream and degrades output into repetitive, incoherent text.
...
**Short:** ROME's rank-one edits to one MLP layer interfere when stacked past a few facts, while MEMIT distributes updates across many layers to edit thousands at once.
...
**Short:** A cross-layer transcoder lets one feature write to multiple downstream layers, avoiding the mismatched per-layer dictionaries that made stitching SAEs into attribution graphs intractable.
...
**Short:** Different random seeds yield similar but non-identical SAE features, so feature IDs aren't stable ground truth and must be pinned per checkpoint and causally validated.
...
**Short:** Comparing an attribution graph's actual causal path against the stated chain-of-thought text can reveal the answer was determined before the claimed reasoning step occurred.
...
**Short:** Patching a known-entity prompt against the unknown-product prompt isolates a misfiring 'familiar entity' SAE feature responsible for the confident hallucination.
...
**Short:** Training one SAE is cheap, single-digit GPU-days, but evaluating and labeling the resulting tens of thousands of features is the real, often underestimated cost.
...
**Short:** Unexplained variance is the fraction of model behavior an attribution graph's edges fail to account for, since every SAE/CLT in it has imperfect reconstruction fidelity.
...
**Short:** Mech interp today is limited by a scale gap between SAE sizes and true concept counts, non-identifiable features, and the lack of formal causal guarantees.
