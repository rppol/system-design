...
> Cross-reference: [`../multimodal_models/`](../multimodal_models/multimodal_models.md) covers the broader multimodal landscape including audio and video modalities. This module focuses specifically on the **vision-language intersection**: CLIP contrastive training, dual encoder architecture, LLaVA-style adapters, visual grounding, and VQA benchmarks.
...
- `FAISS` (Meta): ANN index for CLIP embedding search at scale — index selection and PQ tradeoffs in [Embeddings & Similarity Search](../embeddings_and_similarity_search/embeddings_and_similarity_search.md)
...
**Cross-reference**: [`../multimodal_models/`](../multimodal_models/multimodal_models.md) — covers the broader multimodal landscape including audio modalities, video-specific architectures (VideoLLaMA, Video-LLaVA), and speech-vision integration not covered here.
...
**Short:** CLIP maps images and text into a shared embedding space to score similarity, but has no decoder at all and cannot generate a caption or any free-form text.
...
**Short:** CLIP pools the entire image into one CLS token discarding spatial structure, while LLaVA passes all 196 patch tokens to the LLM, preserving position information.
...
**Short:** CLIP's contrastive loss treats every other pair in the batch as a negative, so a large batch size like 32,768 creates far harder training signal than a small one.
...
**Short:** Visual grounding predicts bounding box coordinates for a text query, which standard VQA models can't do natively since their language head only outputs free-form text.
...
**Short:** Fine-tuning LLaVA for medical imaging freezes the CLIP ViT encoder since its features transfer well, training only the projection and LLM unless 100K-plus domain images exist.
...
**Short:** A 224x224 image with 16x16 patches produces 196 patches, and with the prepended CLS token that's 197 ViT outputs, of which LLaVA keeps only the 196 patch tokens.
...
**Short:** VQAv2 measures general open-ended visual QA, MMMU tests college-level reasoning, and POPE specifically measures hallucination on nonexistent-object questions.
...
**Short:** Classifying 10 million images a day costs about $2 with cached CLIP embeddings versus $20,000 with GPT-4V, making CLIP roughly 10,000x cheaper for tasks it can handle.
...
**Short:** SigLIP replaces CLIP's global softmax contrastive loss with an independent sigmoid loss per pair, removing batch-wide gradient coupling and improving stability at small batches.
...
**Short:** LLaVA-1.5 crushes every image to 336x336 losing small text, while LLaVA-1.6 tiles high-resolution images into multiple tiles plus a thumbnail, roughly quadrupling resolution.
...
**Short:** The Q-Former uses fixed learnable query tokens to cross-attend into ViT patches, compressing 196 variable image tokens into a fixed-length prefix bridging a frozen ViT and LLM.
...
**Short:** InternVL2 tends to win on college-level MMMU reasoning and multilingual document understanding, while LLaVA offers a larger fine-tuning ecosystem and architectural consistency.
...
**Short:** Florence-2 handles captioning, detection, and OCR in one compact encoder-decoder under a billion parameters, unlike CLIP's retrieval-only design or LLaVA's 7B LLM backbone.
...
**Short:** VLM OCR emerges from training on document images paired with their text, but a dedicated tool wins on pure extraction accuracy, bounding boxes, and cost at scale.
...
**Short:** GPT-4V processes each image independently and interleaves its tokens with text, costing a fixed 85 tokens at low detail or up to 765 tokens per image at high detail.
...
**Short:** Captioning produces a fixed-style description with a narrow output space, while VQA must condition on an arbitrary question, which is why instruction tuning is essential for it.
