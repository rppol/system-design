...
**GGUF / llama.cpp:** Universal cross-platform format. CPU-first with optional GPU offload. Supports Q2 through Q8 quantization. Runs on Mac, Windows, Linux, Raspberry Pi, and Android (via Termux). The broader serving-engine landscape (vLLM, TensorRT-LLM, SGLang) is compared in [Inference Engines](../inference_engines/inference_engines.md).
...
**Short:** Phi models showed textbook-quality synthetic training data lets a 1.3B model trained on 6B tokens beat many 13B models trained on over 1T raw web tokens.
...
**Short:** Fine-tuning teaches a model the correct answer via labeled examples, while distillation teaches it the teacher's full probability distribution, transferring dark knowledge.
...
**Short:** Q4_K_M gives about 4x size reduction at 1-3% perplexity loss for RAM-constrained mobile deployment, while Q8_0 gives 2x reduction at under 0.5% loss.
...
**Short:** The on-device-versus-cloud decision hinges on privacy, offline connectivity, sub-100ms latency, task complexity, and context length, split by a hybrid router.
...
**Short:** A 3B INT4 model needs about 2.2-2.5GB working memory including KV cache and activations, requiring a minimum 4GB-RAM device and 6GB for comfort.
...
**Short:** The Apple Neural Engine delivers roughly 10x the energy efficiency of CPU inference at about 38 TOPS, but requires static tensor shapes and Core-ML compilation.
...
**Short:** Flagship phones sustain only about 5W for AI workloads before thermal throttling within 60-120 seconds, dropping CPU inference to roughly 15-20 tok/s sustained.
...
**Short:** A hybrid mobile architecture uses a lightweight local classifier to route privacy-sensitive or simple requests on-device and complex ones to the cloud.
...
**Short:** On-device inference only guarantees that data doesn't leave the device for inference, not protection against a malicious app, memory access, or prompt injection.
...
**Short:** Evaluating a quantized SLM requires a task-specific eval set across quantization levels benchmarked on the actual lowest-end target device, not a dev machine.
...
**Short:** GQA shares KV heads across groups of query heads, cutting KV cache memory 4x, like Phi-3 Mini's cache dropping from 1.6GB to 400MB at 4K context.
...
**Short:** Ollama wraps llama.cpp with model management and an OpenAI-compatible server for easy local deployment, while raw llama.cpp gives finer inference control.
...
**Short:** On-device fine-tuning uses low-rank LoRA adapters to keep memory near 4GB for a 3B model, but training runs 10-100x slower than GPU training.
...
**Short:** Mobile NPUs vary widely in TOPS, supported operations, and framework compatibility, so deployment should target the most constrained device as the baseline.
...
**Short:** Decode is memory-bandwidth-bound, not compute-bound, so a phone's memory bus streaming model weights caps tokens/second far below what raw TOPS would suggest.
