...
**PagedAttention** (vLLM innovation — see [vLLM Deep Dive](../vllm_deep_dive/vllm_deep_dive.md) for scheduler and block-manager internals):
```
...
→ Full mechanics and the online-softmax derivation: [Attention Mechanisms](../foundations_and_architecture/attention_mechanisms.md). Kernel-level and quantization context: [Optimization & Quantization](../optimization_and_quantization/optimization_and_quantization.md).
...
Semantic caching avoids redundant LLM inference by detecting when a new prompt is semantically equivalent to a previously cached prompt and returning the cached response directly. The full caching stack (exact, semantic, KV/prefix) is covered in [LLM Caching](../llm_caching/llm_caching.md).
...
**Short:** LLM decode is memory-bandwidth-bound because loading a 70B model's weights takes 70ms while computing on them takes under 1ms, leaving compute idle 99%+ of the time.
...
**Short:** The KV cache stores past keys and values so decoding needs only O(n) work instead of O(n^2), at the cost of hundreds of kilobytes of GPU memory per token.
...
**Short:** TTFT is prefill time dominated by input length, while TPOT is per-token decode time dominated by batch size and memory bandwidth, each needing separate optimization.
...
**Short:** Chunked prefill splits a long prefill into small chunks interleaved with other requests' decode steps, keeping TPOT stable instead of letting one request monopolize the GPU.
...
**Short:** Prefill processes all input tokens in parallel and is compute-bound, while decode generates one token at a time and is memory-bandwidth-bound.
...
**Short:** Temperature scales logits before softmax, so T=0 gives deterministic greedy output for tasks with one right answer, and 0.7-1.0 adds useful diversity for creative tasks.
...
**Short:** Min-p scales its cutoff relative to the top token's probability, handling both peaked and flat distributions better than top-k's fixed count or top-p's cumulative threshold.
...
**Short:** Continuous batching fixes GPU idle time between requests, while PagedAttention fixes KV cache fragmentation by managing it in fixed-size pages like OS virtual memory.
...
**Short:** Speculative decoding accepts each draft token with probability min(1, p_target/p_draft) and samples a correction otherwise, provably matching the target's exact output distribution.
...
**Short:** Speculative decoding needs a draft model sharing the target's tokenizer with a high acceptance rate, giving 2-3x speedup on predictable text like code.
...
**Short:** Anthropic prompt caching requires explicit cache-breakpoint annotation billing cached tokens at 0.1x price, while vLLM's prefix caching is automatic via a radix tree.
...
**Short:** High P99 with fine median points to long prefill blocking short requests, KV cache pressure causing preemption, or HBM bandwidth saturation at high batch sizes.
...
**Short:** LLaMA 3 70B needs about 320KB of KV cache per token, so 100 users at 8K context alone require roughly 256GB, more than the model weights.
...
**Short:** Beam search needs k times the KV cache and compute of greedy decoding and is incompatible with continuous batching, so production favors sampling instead.
...
**Short:** Disaggregation relocates prefill off the decode GPU to stop inter-token latency spikes but adds a KV transfer, which is why it helps tail latency, not throughput.
...
**Short:** Continuous batching adds new requests as soon as any finishes instead of waiting for the whole batch, raising GPU utilization from about 30% to 90%.
