...
**Short:** Llama-3-70B's KV cache costs about 320KB per token, so 32K context for 10 users needs roughly 102GB, already exceeding one H100's 80GB before model weights.
...
**Short:** Model weights are a fixed cost regardless of user count, but KV cache scales with context length times concurrency, capping how many users a GPU can serve.
...
**Short:** GQA models advertise query head count, not the smaller KV head count the memory formula needs, so using the wrong one can overestimate KV cache by 8x.
...
**Short:** LLMs dump disproportionate attention onto the first few tokens as a softmax artifact, so evicting position 0 in a sliding window causes a sudden perplexity cliff.
...
**Short:** H2O continuously re-ranks and evicts tokens for shifting long agentic tasks, while SnapKV prunes once early with near-zero ongoing overhead for document-QA workloads.
...
**Short:** Scissorhands assumes a token important now stays important soon, so a cheap recent-top-K counter predicts importance nearly as well as H2O's full cumulative bookkeeping.
...
**Short:** GQA, KV quantization, and eviction reduce independent terms of the memory formula, so combining all three can multiply to roughly 48x reduction versus a naive baseline.
...
**Short:** PagedAttention is lossless, eliminating allocation fragmentation, while eviction is lossy and actually discards cached tokens, so deployments typically need both.
...
**Short:** Cross-layer sharing like YOCO cuts the layers term of the memory formula by up to about 80x but must be baked in at pretraining, unlike GQA which every engine supports.
...
**Short:** RAG quality degrading after static eviction usually means SnapKV's early observation window pruned context that a later follow-up question actually needed.
...
**Short:** A benchmark's under-1% average loss hides that a query whose answer sits in an evicted token fails completely, which matters most for needle-in-a-haystack workloads.
...
**Short:** The lever order for KV pressure is check fragmentation versus genuine content pressure, then KV quantization, then right-sizing max context, then eviction last.
...
**Short:** A low-concurrency single-tenant deployment that already fits the raw KV memory should skip eviction, since it adds complexity and quality risk with no real constraint.
...
**Short:** Offloading trades PCIe transfer latency for capacity by keeping exact KV tensors in host RAM, breaking even against recomputation for long shared prefixes.
