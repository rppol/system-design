...
- **OpenAI Structured Outputs** (`response_format={"type": "json_schema", "strict": true}`) — server-side schema→grammar compilation with per-schema artifact caching; first request with a new schema pays added latency (OpenAI's docs put typical schemas under 10s and note complex ones can take up to about a minute), subsequent requests hit the cache. Supports a constrained JSON Schema subset: all fields `required` (optionality via union with `null`), `additionalProperties: false` mandatory, no `oneOf` originally, limits on nesting depth (~5 levels per launch docs) and enum sizes. Function calling with `strict: true` runs the same machinery over tool parameter schemas.
