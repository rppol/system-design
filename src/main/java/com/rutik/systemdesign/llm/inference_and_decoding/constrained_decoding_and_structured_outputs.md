...
- **OpenAI Structured Outputs** (`response_format={"type": "json_schema", "strict": true}`) — server-side schema→grammar compilation with per-schema artifact caching; first request with a new schema pays added latency (OpenAI's docs put typical schemas under 10s and note complex ones can take up to about a minute), subsequent requests hit the cache. Supports a constrained JSON Schema subset. Current published limits: all fields `required` (optionality via union with `null`), `additionalProperties: false` mandatory on every object, up to **5,000 object properties and 10 levels of nesting**, up to **1,000 enum values** across all enum properties (a single enum over 250 values additionally caps the combined enum string length at 15,000 chars), and 120,000 chars total across all property/definition/enum/const names. `anyOf` **is** supported now (each branch must itself be in the subset; the root object may not be an `anyOf`), while `allOf`, `not`, `dependentRequired` and `if`/`then`/`else` are not — and fine-tuned models additionally drop `minLength`/`maxLength`/`pattern`/`minimum`/`maximum`. The launch-era numbers (5 levels, 100 properties) are long superseded; re-read the limits table before designing around them. Function calling with `strict: true` runs the same machinery over tool parameter schemas.
- **vLLM** — guided decoding with selectable backends: `xgrammar` (default for JSON schema), `guidance` (llguidance), with Outlines lineage for regex/choice constraints. Configured via `structured_outputs` / `StructuredOutputsParams(json=..., regex=..., choice=..., grammar=...)`; the older `guided_json` / `guided_regex` / `guided_choice` / `guided_grammar` / `guided_decoding_backend` fields were **removed in vLLM v0.12.0**.
...
- **Instructor** (Jason Liu, 2023) — the dominant *retry-based* alternative: Pydantic model → tool schema → parse → on `ValidationError`, re-prompt with the error message, up to N retries. No engine support needed; works against any API. See [structured_outputs_and_instructor.md](../agentic_frameworks/structured_outputs_and_instructor.md).
...
**Short:** Constrained decoding masks every invalid next token to -inf before sampling using a precomputed grammar-state lookup, so JSON syntax is guaranteed with no retry.
...
**Short:** JSON mode only guarantees syntactically valid JSON of any shape, while structured outputs compile your exact schema into a grammar that masks against it.
JSON mode guarantees the output is *some* syntactically valid JSON — any keys, any shape; you still parse-and-pray on structure. Structured outputs (OpenAI `json_schema` with `strict: true`, vLLM `structured_outputs={"json": ...}`) compiles your specific schema to a grammar and masks against it, guaranteeing the exact shape: required keys, types, enums. Practical gotchas worth volunteering: structured outputs supports a JSON Schema *subset* (all fields required, `additionalProperties: false`, no `allOf`/`not`/`if`-`then`-`else`, and hard caps at 5,000 properties / 10 nesting levels / 1,000 enum values), and the first request per new schema pays compilation latency that is cached afterwards.

**Q3: Why can't you handle JSON schemas with just regular expressions?**
**Short:** Regex can't count unbounded nesting depth since it has no stack, so true CFG engines like XGrammar use a pushdown automaton instead of a depth-capped regex.
...
**Short:** Grammars operate on characters while models emit multi-character BPE tokens, so forced text can land off a canonical token boundary and silently degrade quality there.
...
**Short:** Jump-forward decoding appends a grammar's only legal continuation with zero forward passes for 2-3x throughput, but risks the same token-boundary quality hit if unhealed.
...
**Short:** Constrained generation degrades quality mainly when the prompt doesn't describe the enforced format or forces the answer before reasoning, not from masking itself.
...
**Short:** Putting an answer field before a reasoning field forces the model to commit before generating any chain-of-thought tokens, structurally disabling reasoning.
...
**Short:** Retry loops work against any black-box API with semantic validation but pay extra calls and cap below 100%, while constrained decoding gives 100% syntactic validity cheaply.
...
**Short:** XGrammar precomputes a context-independent token mask cache at compile time and overlaps the small runtime PDA check with the GPU forward pass, hiding the cost.
...
**Short:** Strict function calling is structured outputs applied to the tool's argument schema, so it inherits the same schema-subset limits, while tool choice stays unconstrained.
...
**Short:** Optionality, oneOf unions, patternProperties, format annotations, and numeric min/max often silently pass despite violating the real schema under a constrained engine.
Optionality (engines often require all fields, modeling optional as `union[T, null]`), `oneOf` discriminated unions (unsupported by OpenAI strict mode; `anyOf` is supported but only with an object at the root, and `allOf`/`not`/`if`-`then`-`else` are rejected outright), `patternProperties` and open-ended `additionalProperties`, semantic `format` annotations (`date-time`, `email` — usually ignored by the grammar, so invalid dates pass), numeric `minimum`/`maximum` (hard to express as token grammars — often unenforced), and depth limits (OpenAI strict mode caps at 10 levels of nesting and 5,000 total properties today — the widely-quoted 5-level figure is the superseded launch constraint). The senior-level point: the *silent* degradations are dangerous — output validates against the engine's subset while violating your real schema, so run full-schema validation post-hoc in CI and at runtime.

**Q12: How do constrained decoding and streaming interact?**
**Short:** Constrained decoding streams naturally since masking is per-token, but partial fields are still semantically incomplete and jump-forward emits token bursts unevenly.
...
**Short:** Unmasked speculative drafts waste their budget on grammar-invalid tokens and collapse acceptance rate, so both drafting and verification must share the same grammar mask.
...
**Short:** A new schema's slow first request is grammar compilation to a token-mask cache, fixed by pre-warming a dummy request per schema to avoid a cold-cache miss.
That's compilation: schema → regex/DFA index (Outlines) or grammar → token mask cache (XGrammar), or the provider building and caching its server-side artifact (OpenAI documents added latency on first strict-mode use of a schema — typically under 10s, up to about a minute for complex ones). Mitigations: pre-warm at deploy time by issuing a dummy request per schema; key caches by schema hash and pin them in memory across workers; keep schemas stable (every prompt-templated dynamic enum creates a new hash → cache miss); for inherently dynamic schemas, prefer a compile-free backend (llguidance) over an indexing one.

**Q15: How would you constrain a model to emit only valid SQL for your warehouse?**
**Short:** Valid SQL is enforced with a CFG-guided EBNF grammar restricting to allowed statements and a function whitelist, blocking DROP or DELETE syntactically that prompting cannot.
Use CFG-guided decoding with an EBNF grammar of the allowed SQL dialect subset — vLLM's `structured_outputs={"grammar": ...}` or llguidance with a Lark grammar — restricting to `SELECT` statements, your function whitelist, and optionally table/column names injected into the grammar as enums from the catalog. This guarantees parseability and blocks `DROP`/`DELETE` *syntactically*, which prompt instructions cannot. Then layer semantics: run `EXPLAIN` against a read replica to catch invalid references, and execution-sandbox with row limits. Caveats to mention: full SQL grammars are large (compile cost — cache it), and column-level enums explode grammar size, so many teams constrain structure but validate identifiers post-hoc.

**Q16: How do you evaluate a structured-output pipeline beyond "it parses"?**
**Short:** Structured-output pipelines must be evaluated on schema-semantic field accuracy and task accuracy, since those often run 20-40 points below the parse rate constraints already guarantee.
