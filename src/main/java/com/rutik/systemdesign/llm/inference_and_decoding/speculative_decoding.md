...
**Short:** Rejection sampling accepts a draft token with probability min(1, p_target/p_draft) and samples a correction otherwise, so the draft's distribution cancels out entirely.
...
**Short:** EAGLE predicts the target's next hidden-state vector instead of a token, reusing the target's own LM head, which is why it reaches 0.75-0.9+ acceptance.
...
**Short:** Medusa adds parallel prediction heads to the target model itself, avoiding a separate model, but the heads must be trained and reach lower acceptance than EAGLE.
...
**Short:** Tree verification proposes multiple branches per depth so a wrong guess on one branch doesn't waste a correct alternative branch's deeper guesses, unlike a linear chain.
...
**Short:** Lookahead decoding solves all future positions in parallel via Jacobi iteration with no separate model, trading spare decode FLOPs for speedup instead of draft accuracy.
...
**Short:** Prompt-lookup decoding drafts by copying n-grams straight from the prompt, working near-free when input and output overlap heavily, like RAG quoting or code editing.
...
**Short:** Repetition penalty's stateful history diverges between draft and target after the first rejection, breaking the rejection-sampling proof and degrading quality.
...
**Short:** DeepSeek-V3's multi-token prediction modules are trained jointly with the main model during pretraining, giving a well-calibrated draft with about 0.85-0.9 acceptance.
...
**Short:** Rejection sampling compares probabilities for the same token ID across models, so a differing tokenizer makes that comparison meaningless between draft and target.
...
**Short:** K should adapt to a rolling acceptance-rate window, increasing when acceptance is high and shrinking or disabling speculation when it drops too low.
...
**Short:** Debugging worse P99 under speculation starts with checking acceptance rate against the break-even threshold, then logit processors, then batch size, then output length.
...
**Short:** Self-speculative decoding shares the target's own early layers as the draft with zero extra deployment footprint, but requires the target be trained for early exit.
...
**Short:** Mixing high- and low-acceptance requests in one batch drags the effective speedup toward the lower end, which vLLM addresses with a per-batch-size speculation schedule.
...
**Short:** A/B testing EAGLE means measuring acceptance rate by route, end-to-end P50/P99 TPOT on versus off, and the extra GPU memory its layer and buffers add.
...
**Short:** Speculative decoding only helps TPOT during decode, not TTFT, since the first token still requires a full target forward pass over the prompt.
