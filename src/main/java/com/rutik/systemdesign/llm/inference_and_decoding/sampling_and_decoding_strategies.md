...
- **Anthropic API** — the current generation exposes **no sampler knobs at all**: setting `temperature`, `top_p` or `top_k` to any non-default value on Claude Opus 4.7 or later (including Opus 5, Fable 5, Mythos 5) returns a 400, and the guidance is to omit the fields entirely and steer behaviour by prompting. Older Claude models still accept `temperature` (0-1, default 1.0), `top_p` and `top_k` — a deliberately smaller surface than OpenAI's — but on anything current, sampler tuning is simply not a lever.
- **llama.cpp** — the most feature-complete open sampler chain: `top_k` (default 40), `top_p` (0.95), `min_p` (0.05), `typical_p` (1.0 = off), `top_n_sigma`, `mirostat` (v1/v2 with `mirostat_tau`/`mirostat_eta`), DRY (`dry_multiplier`, `dry_base` 1.75, `dry_allowed_length` 2), and XTC (`xtc_probability`, `xtc_threshold` 0.1) — plus an explicit, documented sampler *order* configuration (`--samplers`). Note the shipped default order is `penalties;dry;top_n_sigma;top_k;typ_p;top_p;min_p;xtc;temperature` — temperature runs **last**, i.e. after truncation, which is exactly Order B in Section 6.3.
- **vLLM `SamplingParams`** — `temperature`, `top_p`, `top_k`, `min_p`, `repetition_penalty`, `presence_penalty`, `frequency_penalty`, `seed`, plus `structured_outputs` / `StructuredOutputsParams` for constrained decoding (composed per Section 6.9).
...
| Tool | Notes |
|------|-------|
| llama.cpp | Most complete open sampler chain — min-p, typical, top-n-sigma, Mirostat v1/v2, DRY, XTC, with configurable order |
...
| Anthropic API | No sampler surface on current models — non-default temperature/top_p/top_k returns a 400; steer by prompting |
...
**Short:** Temperature=0 is implemented as a direct argmax rather than dividing by zero, matching the limiting case of softmax as T approaches zero.
...
**Short:** Min-p sets its cutoff relative to the top token's own probability, so the same parameter value tightens on peaked distributions and widens on flat ones.
...
**Short:** Applying temperature before top-p reshapes which tokens the nucleus considers, while applying it after top-p can only redistribute mass among tokens already selected.
...
**Short:** Frequency penalty scales with how many times a token already appeared, presence penalty is a flat one-time subtraction, and repetition penalty multiplies logits.
...
**Short:** no_repeat_ngram_size hard-bans any token that would recreate a seen n-gram, so it can block a legitimate repeated pattern and force an awkward alternative.
...
**Short:** DRY penalizes exponentially by the length of a repeated substring, targeting structural loops directly instead of frequency penalty's uniform per-token discouragement.
...
**Short:** Contrastive search penalizes tokens whose hidden-state representation resembles recent history, catching semantic rephrasing loops that token-ID penalties never see.
...
**Short:** Contrastive search uses one model contrasting probability against hidden-state similarity, while contrastive decoding contrasts an expert's and an amateur's logits.
...
**Short:** Mirostat is a feedback loop that adjusts its sampling window each step to hold generation at a constant target surprise, unlike a fixed top-p cutoff.
...
**Short:** XTC probabilistically removes tokens above a probability threshold instead of trimming the tail, forcing the model off its generic default continuation.
...
**Short:** Beam search needs k times the KV cache, breaks continuous batching's iteration scheduling, and produces more generic text than sampling for open-ended generation.
...
**Short:** Bit-exact reproducibility fails even at temperature=0 because GPU floating-point summation order depends on batch composition, which varies with production traffic.
...
**Short:** A grammar mask sets disallowed logits to -inf, which always dominates logit_bias's bounded offset, so logit_bias can never force back a grammar-invalid token.
...
**Short:** Speculative decoding's draft and target must use identical sampler configuration, or a mismatch biases their probability ratio and collapses acceptance rate.
...
**Short:** A robotic model at top_p=1.0 has no nucleus truncation at all, fixed by adding min-p around 0.05 plus a modest frequency penalty rather than raising temperature.
...
**Short:** Typical sampling keeps tokens whose own surprise is closest to the distribution's entropy rather than highest probability, deliberately excluding an overly predictable token.
...
**Short:** Sampler defaults should be per-route, not global, since near-zero temperature suits code correctness while 0.8-1.0 with frequency penalty suits creative writing.
