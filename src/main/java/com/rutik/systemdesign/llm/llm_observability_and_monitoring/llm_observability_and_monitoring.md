...
<!-- study-paths
senior: llm_observability_and_monitoring.md
principal: llm_observability_and_monitoring.md
...
- **Prompt versioning is observability**: Every prompt template change is a deployment. Track which prompt version produced which responses, and correlate prompt versions with quality metrics. Without this, debugging a quality regression requires guessing which change caused it. See [Prompt Management and PromptOps](../prompt_management_and_promptops/prompt_management_and_promptops.md) for the registry and promotion machinery.
...
Quality metrics that cannot be derived from infrastructure telemetry alone (evaluation
methodology in [Evaluation and Benchmarks](../evaluation_and_benchmarks/evaluation_and_benchmarks.md)):
...
Framework-specific instrumentation (LangChain callbacks, LlamaIndex handlers, cost tracking
hooks) is covered in [Framework Observability](../agentic_frameworks/framework_observability.md).
...
**Short:** Cost attribution requires every request to carry a team and feature tag at the gateway, aggregated daily into dashboards with budget alerts at 80% and throttling at 100%.
...
**Short:** Full prompt logging turns the observability store into a second PII database under GDPR, mitigated by redaction, sampled text logging, and short retention windows.
...
**Short:** Hallucination trend detection scores sampled responses for groundedness against retrieved context, then alerts when the daily rate exceeds two standard deviations.
...
**Short:** LLM traces carry token counts, cost, and prompt text as a branching tree of many calls, making them 10-100x larger per trace than a linear microservice trace.
...
**Short:** Tiered sampling across a free safety classifier, a mid-cost judge on a small sample, and an expensive rubric on a tiny sample cuts evaluation cost roughly 5x.
Three strategies cut evaluation cost by roughly 5x: tiered sampling, batch pricing, and a self-hosted judge. First, tiered sampling: evaluate 100% of responses with a free/cheap safety classifier (self-hosted Llama Guard, ~$0.00002/call), 10% with a mid-cost judge model (GPT-4o-mini, ~$0.0003/call), and 1% with an expensive comprehensive rubric (GPT-4o, ~$0.003/call). At 100K calls/day, this costs ~$63/day vs. ~$300/day for 100% GPT-4o-mini scoring. Second, batch API pricing: providers like OpenAI offer 50% discounts for batch API calls -- run your daily batch evaluation through the batch API. Third, self-host a small judge model: fine-tune a 7B model on your evaluation rubric and run it on a single GPU. After initial fine-tuning cost (~$200), ongoing cost is ~$0.00005/eval -- 6x cheaper than GPT-4o-mini with 85-90% agreement on your specific evaluation criteria.

**Q: How do you validate that your LLM-as-judge scores are trustworthy -- who watches the watcher?**
**Short:** An LLM judge is trustworthy only after calibrating its scores against human labels above about 80-85% agreement, with the judge version pinned to avoid silent shifts.
Calibrate the judge against human labels before trusting it: score 100-200 responses with both the judge and human reviewers and require agreement above roughly 80-85% before wiring the judge into alerts. Pin the judge model version -- a silent judge upgrade shifts every score and fires false regression alerts; when you deliberately upgrade the judge, re-baseline all thresholds on a fixed reference set. Guard against known judge biases: position bias (randomize order in pairwise comparisons), verbosity bias (longer answers score higher), and self-preference (a judge favoring outputs from its own model family). Re-run the human-agreement audit quarterly on fresh production samples, because judge reliability drifts as your traffic changes.

**Q: How do you turn user feedback signals (thumbs up/down) into observability value?**
**Short:** A thumbs-down should trigger immediate full quality evaluation of that trace regardless of the sampling rate, since explicit feedback is a free high-precision signal.
Treat explicit feedback as a free, high-precision sampling trigger. A thumbs-down should immediately enqueue that trace for full quality evaluation (rubric scoring, groundedness check) regardless of the normal 10% sample rate, with the feedback event linked to the trace ID so the complete execution tree is one click away. Aggregate the ratio as a daily satisfaction metric -- it integrates quality, relevance, and helpfulness into one signal -- and alert on sustained shifts (e.g., thumbs-down rate above 20% against a 12% baseline). Mind the biases: only 1-5% of users leave feedback and unhappy users are over-represented, so use it for triage and trends, not as an absolute quality estimate. Feed thumbs-down traces into your eval dataset -- they are the cheapest source of hard examples for regression suites.

**Q: What alerting thresholds would you set for an LLM system?**
**Short:** Alerts should be prioritized by blast radius, from immediate paging on error-rate spikes to weekly digests on cost trends, and every alert needs a documented runbook.
...
**Short:** Debugging a bad response walks the trace span by span from query understanding through retrieval and generation, since retrieval quality is usually the largest failure bucket.
...
**Short:** Prompt drift has a template dimension tracked by versioning and a query dimension tracked by embedding-centroid distance, both needing significance testing to confirm.
...
**Short:** Synchronous scoring can block a bad response before the user sees it but doubles cost and latency, while async scoring has zero latency impact but reacts after the fact.
...
**Short:** LLM A/B testing needs deterministic hash-based variant assignment, blind quality scoring, and bootstrap confidence intervals since quality distributions are often non-normal.
...
**Q: How do you detect and stop a runaway agent loop using observability data?**
**Short:** Runaway agents are caught by instrumenting per-trace iteration and cost as span attributes, enforced by a hard iteration cap and a gateway-level cost circuit breaker.
...
**Short:** TTFT is reduced with streaming endpoints, shorter input prompts, prompt caching that skips prefill on repeated prefixes, and smaller or geographically closer models.
