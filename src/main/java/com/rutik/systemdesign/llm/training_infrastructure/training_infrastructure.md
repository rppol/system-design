...
**Short:** Tensor parallelism splits each layer's matrix ops across GPUs needing an all-reduce every layer, while pipeline parallelism assigns whole layers per GPU needing only point-to-point transfers.
...
**Short:** ZeRO shards optimizer states, gradients, and parameters across data-parallel GPUs instead of replicating all 16 bytes/param on every GPU, cutting per-GPU memory to roughly total/N.
...
**Short:** Multi-week training checkpoints state every 30-60 minutes to a parallel filesystem so a failed node can be replaced and the job resumed, since failures at scale are frequent.
...
**Short:** Gradient checkpointing recomputes forward-pass activations during backward instead of storing them, cutting activation memory to about O(sqrt(layers)) for roughly 33% more compute.
...
**Short:** Training cost is estimated with the 6N rule, about 6 FLOPs per parameter per token, and using the 2N inference-only figure instead makes a training timeline three times too optimistic.
...
**Short:** A synchronous dataloader reading from network storage stalls GPUs waiting for batches, fixed by staging shards on local NVMe with prefetching and pinned memory.
...
**Short:** ZeRO-3 shards parameters, gradients, and optimizer states so a 7B model's 112GB per-GPU baseline drops to about 14GB across 8 GPUs, at the cost of an all-gather before each pass.
...
**Short:** Data parallelism suits a model that fits on one GPU, tensor parallelism suits fast NVLink within a node, and pipeline parallelism suits spanning nodes over slower interconnects.
...
**Short:** FSDP is PyTorch's native ZeRO-3 implementation favored for models up to about 30B, while DeepSpeed adds ZeRO-Infinity offloading and better MoE support for 70B-plus models.
...
**Short:** The pipeline bubble wastes (stages-1)/(microbatches+stages-1) of the schedule, so raising micro-batch count toward roughly 4x the stage count shrinks the idle fraction.
...
**Short:** Sequence parallelism splits one sequence's tokens across GPUs so activation memory scales with seq_len/N, needed once long-context activations exceed what checkpointing alone can contain.
...
**Short:** Expert parallelism adds two all-to-all exchanges per MoE layer to route tokens to their experts and back, and router imbalance toward one hot expert creates stragglers.
...
**Short:** Mixed-precision pitfalls include FP16 overflow needing loss scaling and BF16's lower mantissa precision in accumulation, largely avoided by using BF16 on Ampere-and-later GPUs.
...
**Short:** Checkpointing every k layers instead of storing all activations can cut a 7B model's activation memory from about 30GB to 8GB for roughly one extra forward pass per segment.
...
**Short:** Distributed training communication overhead is minimized by overlapping all-reduce with compute, gradient accumulation to reduce sync frequency, and gradient compression under tight bandwidth.
...
## See Also
- [Distributed Training (ML)](../../ml/distributed_training/distributed_training.md) — DDP, FSDP, DeepSpeed ZeRO, gradient accumulation — the ML foundations of LLM-scale distributed training
- [GPU & Hardware Optimization (ML)](../../ml/gpu_and_hardware_optimization/gpu_and_hardware_optimization.md) — CUDA, tensor cores, gradient checkpointing
