...
- **Model improvement cycles** — periodic or continuous fine-tuning on curated production data (see [Fine-Tuning](../fine_tuning/fine_tuning.md) for LoRA/full-FT mechanics)
...
Production feedback can be repurposed as preference data for RLHF or DPO (see [Alignment & RLHF](../alignment_and_rlhf/alignment_and_rlhf.md) for the training mechanics):
...
| Experiment tracking | MLflow | Open-source experiment tracking and model registry | Promote versions with registry aliases (`@champion`) and tags, not stage transitions |
...
**Q: What is a data flywheel, and why is it considered a competitive moat for LLM applications?**
**Short:** A data flywheel is a self-reinforcing loop where production usage improves the model, and it moats competitors because years of feedback can't be copied.
...
**Q: What is the difference between implicit and explicit feedback, and when would you rely on each?**
**Short:** Implicit feedback like regeneration clicks is high-volume but noisy, while explicit feedback like thumbs-down is lower-volume but higher-signal.
...
**Q: How does active learning reduce annotation cost in a data flywheel?**
**Short:** Active learning prioritizes uncertain or failed examples for annotation, cutting the labeling budget needed for a given quality gain by 50-80%.
...
**Q: How would you detect distribution shift in production LLM traffic?**
**Short:** Distribution shift is detected by tracking Population Stability Index on embedded query clusters, where PSI above 0.2 signals a major shift.
...
**Q: Walk me through how you would design an A/B test to validate a new fine-tuned model.**
**Short:** Validating a fine-tuned model needs a power-analysis sample size, session-hashed traffic splitting, outcome metrics, and a canary with rollback triggers.
...
**Q: What are the main risks of training on production data, and how do you mitigate them?**
**Short:** Training on production data risks PII leakage, feedback bias toward power users, label noise, and garbage amplification without quality gates.
Five risks dominate: PII leakage, feedback bias, label noise, garbage amplification, and label distribution shift. Concretely: PII leakage (users input sensitive data; the model memorizes it), feedback bias (only engaged users provide ratings; the model overfits to power users), label noise (incorrect corrections from users, inconsistent annotators), garbage amplification (training on negative examples without filtering), and distribution shift in labels (what "correct" means changes as policies or products change). Mitigate with: mandatory PII scrubbing before any data enters the pipeline; use outcome-based implicit signals to balance explicit rating bias; implement inter-annotator agreement checks and quality filters; require explicit quality gates before training; and include a model of historical curated data in every fine-tuning mix to stabilize behavior.

**Q: What is the cold start problem in a data flywheel, and how do you address it?**
**Short:** The cold start problem is having no production data to seed the flywheel, addressed by synthetic seed data and collecting outcomes from day one.
The cold start problem is that a new application has no production data to seed the flywheel. Until enough feedback accumulates, there is neither the volume nor the quality of signal needed to improve the first model. Address it by: starting with the best available base model or a model fine-tuned on publicly available domain data; seeding the annotation dataset with synthetic data generated via Self-Instruct or domain expert demonstrations; collecting outcome data (resolved/escalated) from day one even before fine-tuning is feasible; and setting a data accumulation target (e.g., 500 curated examples) before the first fine-tuning cycle, rather than attempting to train on day one with 20 examples.

**Q: How do you prevent model regression after a fine-tuning update?**
**Short:** Regression is prevented by a 200-500 example golden test suite with a no-more-than-2%-regression gate and automated canary rollback triggers.
...
**Q: How do you handle annotation consistency and quality when scaling the annotation team?**
**Short:** Annotation consistency at scale is maintained with inter-annotator agreement metrics like Cohen's kappa and recurring calibration batches.
...
**Q: How do you estimate the ROI of a data flywheel investment for a business stakeholder?**
**Short:** Flywheel ROI is framed as the human-agent cost avoided per percentage point of autonomous resolution rate gain, often paying back within months.
...
**Q: What is the difference between online learning and periodic batch fine-tuning for LLMs, and which would you recommend?**
**Short:** Periodic batch fine-tuning beats online learning for generative LLMs because continuous weight updates cause catastrophic forgetting.
...
**Q: How do you bootstrap a data flywheel when you have zero production data?**
**Short:** Bootstrapping with zero data combines synthetic seed pairs with a soft launch that routes low-confidence queries to humans whose fixes become training data.
...
**Q: How do you make each flywheel iteration reproducible and revertible?**
**Short:** Reproducible iterations version the dataset snapshot, model, and eval suite together, and reverting is a routing flip back to a prior deployed adapter.
Version everything an iteration consumed and produced, all recorded in one model registry entry. That means an immutable dataset snapshot (content-hashed, with the curation pipeline version that built it), the base model and adapter identifiers, the training config, and the pinned eval-suite version. Reproducible means anyone can re-run iteration N from its snapshot and get the same artifact; revertible means rollback is a routing change, not a retraining — keep the last 2-3 adapters deployable behind the serving layer so a regression discovered post-deploy is a one-line traffic flip. The operational gotcha is versioning the dataset but not the curation code: a silent change in the PII scrubber or dedup threshold makes "the same data" unreproducible one quarter later. Rule: a fine-tune whose exact inputs cannot be reconstructed should not ship.
