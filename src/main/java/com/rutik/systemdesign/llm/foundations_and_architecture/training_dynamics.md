...
| `torch.amp` | Autocast for BF16/FP16 | `torch.amp.autocast("cuda", dtype=torch.bfloat16)`; `torch.amp.GradScaler("cuda")` for FP16 only |
...
**Short:** Random Q/K/V projections at init make attention patterns collapse under large early updates, while CNN filters are statistically independent and lack this failure mode.
...
**Short:** QK-norm bounds query and key magnitudes before the dot product, preventing unbounded attention-logit growth that saturates softmax and stalls gradients.
...
**Short:** Above the critical batch size the gradient estimate is already nearly noise-free, so larger batches add compute cost without improving convergence per step.
...
**Short:** BF16 matches FP32's exponent range so it cannot overflow where FP32 would not, while FP16's narrow range forces risky dynamic loss scaling.

BF16 (brain floating point 16) and FP16 both use 16 bits but allocate exponent bits differently. BF16: 1 sign + 8 exponent + 7 mantissa bits. FP16: 1 sign + 5 exponent + 10 mantissa bits. The exponent determines the maximum representable value: FP16 max ≈ 65,504; BF16 max ≈ 3.39×10^38 (identical to FP32). LLM training produces numerical values that can approach or exceed FP16's range in several places: (1) unscaled dot-product accumulations grow with d_k and with activation magnitude; (2) gradient AllReduce across 1024+ GPUs accumulates values that can exceed 65K; (3) some model weights and activations grow during training beyond FP16 range. Note the softmax itself is not one of these places — every correct implementation subtracts the row max before exponentiating, so `exp()` never sees a large logit. BF16 never overflows for any value that FP32 would handle. The cost: BF16 has 7 mantissa bits vs FP16's 10 — lower precision in the significant digits. In practice, this precision difference does not matter for training because the important quantity (gradient direction) is determined by the exponent, not the mantissa. FP16 also requires dynamic loss scaling (multiplying loss by 128-512 before backward, dividing gradients after) to prevent underflow — this adds complexity and can fail silently. BF16 requires no loss scaling.

**Q: What is muP and how does it enable hyperparameter transfer across model scales?**
**Short:** muP rescales initialization and learning rate by width so updates stay the same magnitude at any scale, letting hyperparameters transfer zero-shot from a small proxy.
...
**Short:** Doubling the batch size should double the learning rate to preserve convergence, but this breaks down near or above the critical batch size.
...
**Short:** Raising the code fraction of pretraining data reliably lifts coding and structured-reasoning benchmarks while displacing world-knowledge text.
...
**Short:** BF16's 7 mantissa bits round many small gradients to the same value during AllReduce across many GPUs, so gradients should be upcast to FP32 first.
...
**Short:** Layer real-time loss and gradient-norm alerts with periodic validation PPL checks, data-distribution monitoring, and hardware health tracking.
...
**Short:** WSD keeps the learning rate identical throughout the stable phase, so new data absorbs the same way at any point, unlike cosine's continuously falling rate.

Cosine decay continuously decreases the learning rate from peak to minimum over the full training duration. At step T/2 (halfway), the LR is at ~50% of peak. Adding new data at step T/2 means the model learns it at a reduced LR — not ideal for learning new information. At step 9T/10, LR is near minimum — practically not learning. The training run is "committed" to its endpoint. WSD's stable phase maintains constant peak LR indefinitely. If new data is added at any point during the stable phase, the model learns it at maximum LR. The final decay (10-20% of total steps) then consolidates all learned representations at a rapidly decreasing LR, locking in stability. The structural consequence — and this is the argument, not a measured result to quote: because the LR is identical at every point of the stable phase, when you inject a new data domain does not change the rate at which the model can absorb it, whereas under cosine the same injection at step 100K and at step 500K meets very different learning rates. MiniCPM (2024) is the reference for the schedule itself; verify any absorption-rate claim on your own run.

**Q: What is the "critical batch size" and how does it relate to the training compute budget?**
**Short:** The critical batch size is where gradient noise scale equals one, so batches larger than it waste compute on redundant gradient signal.
...
**Short:** muP has a validated rule for width transfer but none for depth, so practitioners keep depth fixed between proxy and target and only scale width.
...
**Short:** Binary-encoded content, encoding corruption, and repeated sequences each spike loss, and are caught by entropy, unicode, and n-gram repetition filters.
