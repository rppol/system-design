...
**Short:** Summing d_k independent unit-variance products gives score variance d_k, so dividing by sqrt(d_k) restores variance to 1 and keeps softmax learnable.
...
**Short:** Flash Attention tiles K/V in SRAM and uses online softmax to track a running max and denominator, avoiding ever materializing the full N×N score matrix.
...
**Short:** GQA stores only G shared key-value heads instead of H, cutting cache by H/G with a small quality loss shown by Ainslie et al.'s near-identical GQA-8 benchmark scores.
...
**Short:** The first 4-8 tokens absorb disproportionate attention as aggregate storage, so StreamingLLM keeps them permanently while sliding the rest of the KV cache.
...
**Short:** MQA gives maximum cache savings at the largest quality cost, GQA offers a tunable middle ground, and MLA compresses KV into a latent vector for roughly 57x reduction.
...
**Short:** Flash Attention recomputes the attention matrix from stored softmax statistics during backward instead of storing the full O(N²) matrix from the forward pass.
...
**Short:** Longformer and BigBird restrict attention to local windows plus global or random tokens, but Flash Attention made dense O(N²) attention cheap enough to make them unnecessary.
...
**Short:** NSA and DeepSeek's DSA learn sparsity end-to-end and share block selection across a GQA group, turning sparsity into real bandwidth savings rather than a fixed hand-designed pattern.
...
**Short:** KV cache grows linearly with sequence length, so production systems combine GQA, INT8/FP8 quantization, PagedAttention, prefix caching, and sink-preserving eviction to control it.
...
**Short:** FA-2 reaches 50-73% of peak A100 FLOPs versus FA-1's 25-40%, giving roughly 2x kernel speedup that grows with sequence length as memory traffic dominates.
...
**Short:** Local tokens use a fixed sliding window while global tokens attend to and are attended by the entire sequence, letting task-critical positions aggregate information globally.
...
**Short:** MLA compresses K and V into a jointly-learned low-dimensional latent vector, giving DeepSeek-V2 about a 57x cache reduction while matching or beating MHA quality.
...
**Short:** The softmax bottleneck limits the LM head's output rank via the embedding dimension, a separate issue from attention's per-row softmax which instead bounds the rank of V's convex combination.
...
**Short:** Critics argue attention's O(N²) cost is fundamentally limiting at extreme context lengths, while GQA only shrinks KV cache memory without changing that underlying complexity.
...
**Short:** scaled_dot_product_attention automatically picks Flash Attention, memory-efficient attention, or a naive math fallback based on device, dtype, and mask, with sdpa_kernel forcing a specific backend.

`torch.nn.functional.scaled_dot_product_attention` is a composite operation that automatically selects the best implementation based on input characteristics: (1) Flash Attention 2 kernel: when inputs are on CUDA, dtype is FP16 or BF16, d_head <= 256, and no custom attention mask (or is_causal=True); (2) Memory-efficient attention (xformers-style): when FA-2 conditions are not fully met but CUDA is available; (3) Math (naive): CPU or custom conditions. The dispatch is automatic — no code change needed. Check which backend is used: `torch.backends.cuda.flash_sdp_enabled()`, `torch.backends.cuda.mem_efficient_sdp_enabled()`. To force a specific backend: `from torch.nn.attention import sdpa_kernel, SDPBackend` then `with sdpa_kernel(backends=[SDPBackend.FLASH_ATTENTION]): ...`. Newer Flash kernels are pluggable rather than hard-wired — a provider library registers itself via `register_flash_attention_impl`, and you opt in explicitly with `activate_flash_attention_impl("FA3")` (or `"FA4"`); `list_flash_attention_impls()` reports what is installed and `restore_flash_attention_impl()` returns to the built-in FA-2. Practical implication: always use SDPA in production code; it uses FA-2 by default and lets you swap in a newer kernel without touching the model code.

**Q: Walk through the memory math for serving 100 concurrent users at 32K context on LLaMA 3 8B.**
**Short:** LLaMA 3 8B's KV cache reaches about 128KB per token, making 32K-context serving for 100 users cost roughly 400GB, which forces context limits, quantization, or PagedAttention sharing.
...
1. Always use `F.scaled_dot_product_attention(is_causal=True)` instead of manually implementing attention — automatic Flash Attention dispatch, with newer kernels opted into via `activate_flash_attention_impl("FA3")` and no model-code changes.
