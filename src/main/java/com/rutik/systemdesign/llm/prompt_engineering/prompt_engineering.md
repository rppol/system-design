...
**Short:** Chain-of-thought works by giving the model more intermediate context and allocating more compute tokens to a hard problem before it commits to a final answer.
...
**Short:** Few-shot includes demonstration pairs and wins when the output format is unusual or the task is subtle, while zero-shot works when the model already covers the task.
...
**Short:** Prompt injection overrides intended behavior via malicious input text, defended with clear delimiters, duplicated safety instructions, and an injection-detection classifier.
...
**Short:** Self-consistency samples multiple reasoning chains and takes the majority vote, worth its 5-10x cost only for high-stakes, verifiable problems like math.
...
**Short:** LLMs attend less to information in the middle of a long context than at the start or end, so critical instructions should sit at the end or be duplicated at both ends.
...
**Short:** Temperature=0 makes sampling greedy but floating-point non-associativity in batched GPU matmuls and silent provider weight updates still make outputs vary.
...
**Short:** Negative instructions still inject the forbidden concept into context, so rephrasing as a positive action-specifying instruction gets far better compliance.
...
**Short:** Few-shot accuracy typically plateaus and can regress past about 3-8 examples, since extra examples add cost, majority-label bias, and lost-in-the-middle risk.
...
**Short:** CoT fails through unfaithful post-hoc reasoning, early-error propagation, overthinking simple problems, and sensitivity to the exact trigger phrase used.
...
**Short:** Effective few-shot selection retrieves semantically similar examples from a curated bank, which consistently beats random selection more than example ordering does.
...
**Short:** Securing a system prompt needs defense in depth across instruction hierarchy, input sanitization, output filtering, and canary tokens, since no single technique is foolproof.
...
**Short:** Reliable structured output combines schema-in-prompt techniques with a validation and retry loop, since only constrained decoding is schema-valid by construction.
...
**Short:** ReAct interleaves reasoning with tool-calling actions and observations, grounding answers in external information, unlike CoT which reasons only from parametric knowledge.
...
**Short:** Temperature reshapes the logit distribution before sampling while top_p truncates its tail afterward, and standard advice is to fix one and tune only the other.
...
**Short:** Prompt engineering is the cheapest lever to try first since it costs no training, reserving fine-tuning for a consistent format or a quality bar prompting can't reach.
A: Prompt engineering first — it is zero training cost and iterates in minutes, so it is the cheapest lever to try. How much of the quality gap it closes is task-specific and has to be measured, not assumed. Prefer fine-tuning only when: (1) you have exhausted prompting and still miss a quality bar; (2) you have 500+ labeled examples of the desired behavior; (3) the task needs a consistent format or style that few-shot examples eat too many tokens to specify; or (4) you want to shrink prompts (and cost) by baking the instructions into weights. A useful rule: a good system prompt plus few-shot examples usually beats a small fine-tune, and only reaches for fine-tuning when the marginal quality is worth the annotation and MLOps burden. Retrieval ([RAG](../rag_fundamentals/rag_fundamentals.md)) is the right lever when the gap is missing knowledge, not missing behavior.

**Q: What is automatic prompt optimization (e.g., DSPy) and when is it worth using?**
**Short:** Automatic prompt optimization like DSPy searches instructions and example selections against a scored metric and labeled dev set, worth it at high volume.
...
**Short:** Meta-prompting is a single LLM call rewriting a prompt with generic hygiene and no metric, while automatic prompt optimization is a scored search needing a dev set.
