...
**OpenAI Deep Research**: A GPT-5.2-backed reasoning agent inside ChatGPT, gated by subscription tier with a monthly task quota. It applies chain-of-thought reasoning before each retrieval decision — effectively the model plans what to search next based on what it has already found — and runs 5-30 minutes depending on task complexity, producing reports with inline citations. Architecture is not public, but OpenAI's blog describes a specialized browser tool that the reasoning model calls to navigate the web: the model decides which URLs to fetch from its reasoning traces rather than executing a predetermined crawl plan. The launch evaluation (Feb 2025) reported roughly 26% on Humanity's Last Exam, well above the single-pass baseline of the day, which is the quantified case for multi-step iterative retrieval over single-pass generation.
...
**Google Gemini Deep Research (launched Dec 2024)**: Distinctive UX: generates and shows the research plan to the user for approval before executing. The user sees the list of sub-topics Gemini intends to research and can add, remove, or reorder them before execution begins. This is a significant UX advantage — users feel in control and the final report better matches their intent. It rides the current Gemini Pro-tier model, Gemini 3.1 Pro, whose 1M-token context window means Gemini can hold all retrieved sources in a single context rather than chunking synthesis. Google Search integration gives breadth coverage not available to OpenAI or Perplexity. Available in Google AI Pro, the consumer subscription tier.
...
| Grounded-search LLM endpoint (Gemini grounding, Azure AI Agents) | Hours | Moderate | Billed as model tokens | One call returns cited answers; no separate crawl budget |
