...
**Why it matters**: A 2-hour SWE agent run on a mid-tier frontier model costs roughly $4–8 in LLM tokens plus any tool API costs — at Claude Sonnet 5's July 2026 list price of $3/M input and $15/M output, that is on the order of a million cached-and-reread input tokens plus a few hundred thousand output tokens across 50 turns. A non-durable agent that fails at step 48 of 50 throws away nearly all of that and starts over. With durability, the same failure costs one checkpoint-save latency (5ms) and resumes in under 1 second.
...
sequenceDiagram
    ...
    Note over C,W: CRASH RECOVERY — Worker restarts — Temporal replays the event history<br/>deterministically from memory with no re-execution of activities — Worker resumes at next pending activity
```

Every state transition — workflow start, activity completion, signal receipt — is persisted to the immutable event history before execution proceeds, which is what makes deterministic replay possible after a worker crash. Replay re-runs only the workflow function, substituting stored results for completed activities, so it is bounded by history length rather than by the original wall-clock duration — a multi-hour workflow typically replays in well under a second.
...
- Temporal workflow replay: re-executes only the workflow function with stored activity results, so recovery time scales with **history length, not original run duration** (benchmark it on your own histories rather than assuming a fixed steps/sec figure)
...
| Versioning support | No | No | Yes (`workflow.patched()` in the Python SDK; `Workflow.getVersion()` in Java/Go) | No |
...
A code analysis agent stored the full content of every file it read in the messages list (as tool call results). After 200 tool calls reading large source files, the checkpoint blob grew to 500MB — half of Postgres's 1 GB hard ceiling on a single field value, well past any sane TOAST-compressed size, and taking 800ms to serialize/deserialize. The symptom was progressive slowdown: checkpoint saves that started at 5ms grew to 800ms as the agent accumulated more file reads. The fix: store only tool call inputs and summary outputs in the checkpoint; reconstruct the full message list (including raw file contents) from the checkpoint on resume by re-reading files as needed. Checkpoint size target: under 100KB for a 50-tool-call agent.
...
| Tool | Category | Notes |
|------|----------|-------|
| Temporal (Go/Python SDK) | Full workflow durability | Python SDK: `temporalio`; deterministic event-history replay on worker restart; requires a Temporal server (self-hosted or Temporal Cloud) |
...
Temporal re-executes the workflow code against its stored event history; a database checkpoint restores state without re-executing anything. Replay returns pre-stored results for completed activities, so its cost scales with history length rather than the original run's wall-clock duration. A database checkpoint-based resume loads the serialized state at the last checkpoint and continues forward — it does not replay code, only restores state. The difference matters in three cases: (1) versioning — Temporal's patch API (`workflow.patched("my-change-id")` in the Python SDK, `Workflow.getVersion()` in Java and Go) handles code changes between checkpoint and resume; a DB checkpoint approach provides no protection against state-schema mismatches after code deployment; (2) audit — Temporal's event history is immutable and queryable; DB checkpoints are mutable blobs; (3) debugging — Temporal's Web UI shows the full event history including which activities ran, how long they took, and what they returned.
...
Use a `schema_version` field in the checkpoint JSON. On load, check the version and apply migration logic if needed: `if state.schema_version < CURRENT_VERSION: state = migrate(state)`. For backward compatibility: add new fields with default values in `from_json` using `d.get("new_field", default)` — this handles checkpoints written before the field existed. For breaking changes (field renamed, type changed): increment `schema_version` and write a migration function. In Temporal, guard the changed branch with the patch API — `if workflow.patched("add-retry-budget"): ... else: ...` in the Python SDK, or `Workflow.getVersion()` in Java and Go — which lets in-flight workflows continue on the old code path while new workflows take the new one, with the decision recorded in the event history for deterministic replay. Once no old-version workflows remain, swap `patched` for `deprecate_patch` and then delete the branch.
