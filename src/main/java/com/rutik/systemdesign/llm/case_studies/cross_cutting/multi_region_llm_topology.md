...
GPU scarcity distorts geography. Accelerator supply is not uniform across regions: no cloud publishes per-region GPU inventory, but instance-type availability tables and quota experience consistently show the newest accelerator generations landing first and deepest in a handful of US regions, with EU and APAC regions offering a narrower selection and tighter quotas. The planning consequence is what matters: if your APAC region can absorb only a fraction of global traffic before exhausting its quota, naive "nearest region" routing is dangerous. Measure your own obtainable quota per region before sizing anything; do not assume a fixed split.
...
class BrokenRouter:
    ...
    def route(self, request: dict) -> str:
        ...
        #   Cost impact = (recomputed prefix tokens/day) / (prefill tokens per
        #   GPU-hour) x GPU hourly rate. Compute it from your own prefill
        #   throughput rather than quoting a headline figure.
```
...
**OpenAI**: OpenAI's compute has been publicly tied to Microsoft Azure, but it does not publish a region list, a routing design, or per-incident root causes at the level of "which service was single-region." The widely retold story that the November 2023 ChatGPT outages were caused by single-region session-token validation is not something the public incident history supports — OpenAI attributed the most prominent November 2023 disruption to a DDoS-pattern traffic surge. Treat the single-region-auth failure mode as a real and instructive pattern (see Section 10, Pitfall 1) rather than as a reported OpenAI post-mortem, and do not attribute a specific `conversation_id`-to-cluster header scheme to them.

**Anthropic**: the first-party Claude API is documented as **global by default** — Anthropic routes inference dynamically rather than exposing a region to the caller — and data residency is an explicit, separately-priced product feature rather than a side effect of where you connect from. Claude 4.6 and later accept an `inference_geo` request parameter; setting it to `"us"` pins inference to the United States and applies a 1.1x multiplier on all token pricing categories, while the default `"global"` uses standard pricing. On the partner clouds the same idea appears as endpoint types: Bedrock offers global and regional endpoints, Google Cloud adds a multi-region tier, and regional/multi-region endpoints carry a 10% premium over global. There is no documented `claude-region` response header; if you need to know where a request ran, the mechanism is the residency feature, not a debug header.

**Open-model inference providers** (Together AI, Fireworks, Baseten and similar) run multi-region GPU fleets across a mix of hyperscaler and neocloud capacity, and expose stateless completion APIs where the whole conversation is resent on every call. That statelessness is the architecturally interesting part: with no server-side conversation, there is no KV-cache locality to preserve, so the routing layer is free to optimise purely for queue depth and available capacity rather than session affinity. Fleet composition and routing internals are not published by these vendors — the design point that transfers is that statelessness buys you routing freedom.

**Cloudflare Workers AI**: Workers AI runs inference on GPUs in Cloudflare's own network, which spans 337 cities. Routing is anycast BGP: the user's connection lands at the nearest PoP that can serve it, with no DNS-based region selection in the path. Cloudflare does not publish a per-city model residency map, so do not design around an assumption that a given model is available in every location — smaller models are inherently cheaper to place widely than large ones, but treat that as a tendency, not a documented tier list. KV-cache stickiness is not part of the model: Workers AI serves stateless inference, so multi-turn conversation state belongs in the application layer, with AI Gateway's caching used for exact-match prompt reuse rather than as a KV cache.
...
This pattern is illustrative rather than a citation of any published post-mortem — it is often attributed to OpenAI's November 2023 disruption, which OpenAI actually attributed to a DDoS-pattern traffic surge. The shape: GPU serving infrastructure is distributed across regions. The JWT validation service that every request had to call before reaching inference was deployed in a single region. When that region experienced elevated latency (not even an outage — just 200 ms extra), all inference globally stalled behind authentication timeouts. Fix: replicate stateless auth services to every region where inference runs; never make inference depend on a single-region RPC call in the hot path. Validate JWTs locally using public key caching (JWKS endpoint cached with 1-hour TTL).

**Pitfall 2: KV cache stickiness ignored — 10× GPU waste (illustrative)**
...
**Pitfall 3: GDPR violation via failover routing (illustrative)**
...
|----------|--------------|---------------------|-------------------|------|
| BGP Anycast (Global Accelerator) | 10–30 s | No | No (RST + retry) | $0.025 per accelerator-hour + a DT-Premium data charge that varies by region pair (e.g. $0.015/GB US/Canada to Europe; $0.007–$0.105/GB overall) |
...
Accelerator availability is regionally lopsided, so equal-sized regional clusters are almost always the wrong plan. No cloud publishes per-region GPU inventory, but the newest generations reliably land first and deepest in a few US regions, with narrower selection and tighter quotas elsewhere — so an active-active topology built on identical clusters over-provisions the capacity-constrained regions relative to their demand while under-provisioning the dense ones. Size each region from its own traffic share plus failover headroom, and derive the headroom explicitly: a region carrying 15% of normal traffic that must absorb 33% when the other two fail needs 33/15 = 2.2x its steady-state capacity. Then check that number against the quota you can actually obtain in that region — the answer often requires a capacity reservation or a multi-month commitment, and that constraint, not the arithmetic, is what decides the topology.
...
Redis cross-region replication is billed as AWS inter-region data transfer — a single per-GB charge, not an inter-region fee stacked on top of an internet-egress fee. Do not price it at the ~$0.09/GB internet rate; inter-region transfer is materially cheaper, and the rate depends on the region pair (check the current EC2 data-transfer table). The volume is small either way: for a 50-turn conversation with 200 tokens per turn (≈800 bytes per turn after serialization) total session state is ≈40 KB, so 100,000 active sessions replicated across 3 regions is 40 KB × 100,000 × 2 = 8 GB/day — well under $1/day in transfer fees, which is negligible. The actual cost driver is the Redis cluster itself (3 shards × 3 regions × $200/month/shard = $1,800/month). Minimize Redis state size by storing only the last N turns (N=10 is typically sufficient for KV cache recomputation) rather than the full conversation history.

**Q: How does Cloudflare Workers AI achieve low-latency global LLM serving?**
Workers AI runs inference on GPUs inside Cloudflare's own network, which spans 337 cities, so the first hop is a PoP hop rather than a trip to a distant origin region. BGP anycast delivers each user's TCP connection to the nearest PoP that can serve the request — there is no DNS-based region selection in the path, which is why failover happens at BGP convergence speed instead of DNS TTL speed. The economics of weight placement mean small models are cheaper to put in many locations than large ones, but Cloudflare does not publish a per-city model residency map, so do not design around an assumption that any specific model is present everywhere. KV cache stickiness is not part of the model at all: Workers AI serves stateless inference, so multi-turn conversation state belongs to the application layer.
...
Use S3 Cross-Region Replication with Replication Time Control (RTC) which guarantees 99.99% of objects replicate within 15 minutes — too slow for a 5-minute SLA. Instead, use parallel direct uploads: when a new adapter is committed, trigger simultaneous S3 PutObject calls to all regional buckets from a central coordination service. At 5 Gbps a 300 MB adapter is 2.4 Gbit of payload, so the transfer itself is well under a second; budget a few seconds end to end for TLS setup, multipart overhead and checksum verification. The coordination service waits for all regional PutObject responses before publishing a "adapter ready" event to each region's inference pods via SNS. Each region's pod pool loads the adapter from local S3 (no cross-region read required post-upload). Total latency: upload + propagation + pod hot-load = typically 30–60 seconds. This is the pattern used by multi-tenant fine-tune serving platforms to provide rapid adapter deployment without model restarts.
...
OpenAI does not publish its routing architecture, so the topology below is the design the workload forces rather than a description of theirs. A conversation must carry a stable identifier from creation — ChatGPT's URLs expose a conversation id, which is the natural affinity key and is exactly the session_id pattern described in this document. A global anycast or edge-routing tier picks the nearest healthy region; within the region, a proxy layer consistent-hashes on that conversation id to select the specific inference pod holding its KV cache. Do not attribute a specific vendor product (Azure Front Door, Envoy) or a specific incident root cause to OpenAI on the strength of this section — the distributed-auth lesson in Section 10's Pitfall 1 stands on its own merits as a failure mode, not as a reported post-mortem.
