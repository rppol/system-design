...
**Anthropic Claude Computer Use**: runs inside a Docker container provided by the user via the `anthropics/claude-quickstarts` repository (`computer-use-demo`). Uses screenshot-only grounding — no accessibility tree integration — and drives one model for every step, with no cost-tiered routing. Actions executed via xdotool (X11) and Python Pillow for screenshot capture. The published reference implementation completes a loop in approximately 1.5-2 seconds per action. Fragile on dynamic UIs where elements shift between screenshot and click; Anthropic's own documentation labels the system "experimental" and warns explicitly about irreversible actions. Cost per task at 30 uncropped 1080p actions: approximately $0.57 at Claude Opus 5 pricing ($5/$25 per MTok).

**OpenAI ChatGPT agent**: browser-first scope, not full desktop, bundled into paid ChatGPT tiers rather than billed per task. The architectural point that carries forward is its grounding: it drives Chrome DevTools Protocol for DOM-level element interaction, so coordinates derive from DOM element IDs rather than pixel positions. That hybrid visual-plus-DOM approach is what eliminates coordinate drift, and it is the same reasoning behind this design's AXTree-primary choice in §5. It runs in a hardened Chromium instance on OpenAI infrastructure with explicit confirmation gates before payments and other consequential actions. OpenAI's Computer-Using Agent line reported 58.1% on WebArena, 87% on WebVoyager, and 38.1% on OSWorld.

**Adept ACT-1** (the category's first mover, absorbed into Amazon's AGI group in 2024): focused on enterprise SaaS workflows — Salesforce CRM data entry, Workday payroll processing, Zendesk ticket management. It fine-tuned a VLM on enterprise UI screenshot corpora rather than using a general-purpose model, and that is the transferable result: accuracy on the known enterprise UIs went up, generalization to unfamiliar apps went down. Fine-tuning the grounding model to your app inventory is a real lever, but it buys a narrower agent, not a better one.

**MultiOn**: consumer browser agent whose differentiating design is worth copying — the agent explicitly hands control back to the user when it is stuck or uncertain, rather than retrying indefinitely. This "graceful handoff" pattern reduces cost overrun and user frustration compared to agents that loop until timeout, and it is the same idea as §4.4's confidence-gated confirmation. It publishes neither task-completion rates nor list pricing, so treat any circulating completion-rate figure as unsourced.
...
**Q: How do you prevent prompt injection through webpage content?**
...
**Q: What is the coordinate drift problem and how do you mitigate it?**
...
**Q: How do you build the audit trail to be tamper-proof?**
...
**Q: How do you enforce a cost ceiling mid-task without abandoning the user?**
...
**Q: Why is WebArena success rate not representative of real-world task success?**
...
**Q: How do you handle session resume when a user disconnects mid-task?**
...
**Q: How do you measure and improve task completion rate over time?**
