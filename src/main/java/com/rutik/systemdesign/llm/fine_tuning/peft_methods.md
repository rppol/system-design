...
**Short:** PEFT trains under 1% of parameters, cutting a 70B model's fine-tuning memory from over 420GB to under 40GB at a 1-3% quality cost.
...
**Short:** Prompt tuning fails under 1B parameters because the model lacks capacity to interpret soft prompts, but matches full fine-tuning above 10B.
...
**Short:** PEFT methods can be combined, such as stacking a language adapter with a task adapter, but independently trained adapters risk interfering when combined.
...
**Short:** PEFT is optional for BERT-scale models where full fine-tuning is routine, but essential for LLM-scale models where it enables training on limited hardware at all.
...
**Short:** LoftQ initializes the LoRA adapter from the base model's quantization error instead of randomly, closing the gap QLoRA otherwise wastes early steps recovering.
...
**Short:** PEFT adapters are used at inference for multi-tenant personalization, style switching, and A/B testing, all sharing one base model loaded once on GPU.
...
**Short:** Choose LoRA to change how the model reasons or formats output, and choose prefix tuning to condition its generation style without changing its knowledge.
...
**Short:** IA3 learns element-wise scaling vectors instead of low-rank matrices, using roughly 32x fewer parameters than LoRA but with less expressive updates.
...
**Short:** AdapterFusion learns an attention-weighted combination of task adapters but pays a linear-in-tasks inference cost, so production favors per-request routing instead.
...
**Short:** Zero-initializing B makes a QLoRA adapter start as a no-op on the quantized weights, wasting steps recovering the residual that LoftQ or PiSSA init avoids.
