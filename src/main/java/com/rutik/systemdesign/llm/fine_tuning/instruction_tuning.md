...
**Short:** Pre-training learns language and world knowledge from raw text at massive scale, while instruction tuning uses a small curated dataset to teach conversational behavior.
...
**Short:** Label masking sets instruction token labels to -100 so loss is computed only on response tokens, preventing the model from learning to recite prompts.
...
**Short:** Synthetic GPT-4 data scales cheaply to millions of examples, but blending in even under 5% human-annotated examples measurably improves real-world quality.
...
**Short:** Sequence packing fills one training sequence with several short examples using a masked attention boundary, raising GPU utilization from under 30% to over 90%.
...
**Short:** Curating instruction data means diversifying sources, filtering the lowest-scoring 20-30% of responses, deduplicating near-identical instructions, and reviewing for safety.
...
**Short:** Instruction format overfitting is the model memorizing training phrasing instead of generalizing, fixed with over 100 distinct instruction types and paraphrase augmentation.
...
**Short:** Instruction-tuned models are evaluated on verifiable task accuracy, format compliance, instruction-following rate, LLM-as-judge scores, and general benchmark regression.
...
**Short:** Multi-turn training formats a full conversation with masking on user turns, teaching the model to track context across turns that single-turn data can't.
...
**Short:** Training must include diverse system prompts covering personas, constraints, and formats, or the model will ignore system prompts inconsistently at inference.
...
**Short:** Overlong sequences are handled by filtering examples past the context limit or chunking long documents, rather than truncating and losing the end of a response.
...
**Short:** LIMA showed 1,000 carefully curated examples matching 52,000 unfiltered ones, since noisy examples past a few thousand mostly add gradient noise, not new signal.
...
**Short:** Without format diversity across JSON, prose, and multi-turn examples, a model rigidly reproduces only the formats it saw during training regardless of the instruction.
...
**Short:** FLAN found instruction-tuning quality scales with task count only up to about 282 tasks, after which adding hundreds more tasks yields barely any gain.
...
**Short:** The unfiltered Alpaca dataset had hallucinated and empty outputs, with roughly 80% of its math answers estimated incorrect, from unfiltered text-davinci-003 generation.
...
**Short:** Generalization is evaluated on held-out instruction categories deliberately excluded from training, scored by LLM-as-judge and human preference win rate.
...
**Short:** A model that keeps generating a fake next turn was never trained with the turn terminator inside its loss window, so it never learned to predict a stop token.
