...
**So why does anyone leave adapters unmerged?** Because that sub-1% arithmetic cost is not the real cost. Unmerged serving adds two extra kernel launches per adapted layer — 256 of them at `r=16` across 128 projections — and small skinny matmuls run far below peak GPU utilization, so the measured latency overhead is an order of magnitude above the FLOP arithmetic. Fireworks, who serve both forms, publish it as "tens of milliseconds" of weight-load overhead plus **about 10-30% on prompt processing time** for an unmerged adapter, rising with request concurrency, against "latency matches the base model" once merged. Treat even that as kernel-quality-dependent rather than physical: vLLM's own multi-LoRA TTFT was **10x the base model's** in 0.11.1rc3 and 87% lower by 0.15.0 on the same GPT-OSS 20B / rank-32 / 8-adapter workload, purely from kernel work. Benchmark your engine version; do not carry a number across engines. What you buy for it is the multi-adapter economics from the diagram above: one 14 GB base model in memory and N adapters at ~34 MB each, versus N merged 14 GB checkpoints. At `N = 4` adapters that is 14.1 GB against 56 GB. Merge when you ship exactly one task; stay unmerged the moment there are two.
...
**Short:** LoRA adds trainable low-rank matrices to frozen weights, cutting a 4096x4096 layer's trainable parameters 128x at rank 16 versus full fine-tuning.
...
**Short:** Full fine-tuning updates every parameter at full pre-training memory cost, while LoRA trains 0.1-1% of parameters and fits in a single 16GB GPU.
...
**Short:** The intrinsic rank hypothesis says a fine-tuning update lives mostly in a low-rank subspace, which is why LoRA's rank-r parameterization can match full FT.
...
**Short:** Start LoRA at r=16 with alpha=32 for most production tasks, moving to r=32-64 only for domain adaptation or complex reasoning.
...
**Short:** Alpha scales the adapter's output as alpha/r, and the alpha=2r convention keeps that effective scale stable when you change rank.
...
**Short:** Merge a LoRA adapter for a single high-volume deployment, but keep adapters separate and unmerged when serving many tenants from one base model.
...
**Short:** LoRA prevents forgetting because the frozen base weights W are never updated, so only the additive adapter output can shift behavior.
...
**Short:** Q and V projections capture most task behavior changes, while adding FFN modules is what's needed to inject new domain knowledge.
...
**Short:** Houlsby adapters add always-on inference overhead as inserted bottleneck layers, while LoRA modifies weights in place and merges to zero overhead.
...
**Short:** LoRA applies to vision transformers and multimodal models the same way as language models, though vision encoders often need a lower rank like 4-8.
...
**Short:** vLLM's multi-LoRA serving keeps one base model resident and swaps tens-of-megabyte adapters per request, serving 50-plus adapters from one instance.
...
**Short:** Rank insufficiency shows as validation loss plateauing while training loss keeps falling, plus failures specifically on the task's harder examples.
...
**Short:** Linearly combining two LoRA adapters at equal weight keeps roughly 80-90% of each task's performance, but quality drops sharply past two adapters.
...
**Short:** Adding MLP layers to LoRA more than doubles trainable parameters but helps most on knowledge-heavy tasks, since FFN weights store factual knowledge.
...
**Short:** Mixing 10-20% general instruction data and using a lower learning rate keeps a heavily domain-focused LoRA adapter from suppressing general capability.
...
**Short:** rsLoRA scales the adapter by alpha/sqrt(r) instead of alpha/r, fixing gradient collapse at high rank so ranks above 64 actually train usefully.
