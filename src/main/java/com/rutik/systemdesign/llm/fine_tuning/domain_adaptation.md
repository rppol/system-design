...
**Short:** Catastrophic forgetting during domain adaptation is prevented mainly by mixing in 15-20% general text, using LoRA, and a lower learning rate than initial pre-training.
...
**Short:** Continued pre-training is warranted when domain vocabulary and reasoning are poorly represented in the base model, not when the gap is purely behavioral or RAG-solvable.
...
**Short:** Sequential domain-then-instruct training outperforms doing both at once because CPT and SFT need different learning rates, masking, and data formats.
...
**Short:** A domain CPT corpus is built by sourcing authoritative documents, deduplicating with MinHash, quality-filtering, decontaminating against eval sets, and mixing in general text.
...
**Short:** Forgetting shows up as over 10% MMLU or GSM8K regression, broken instruction-template following, and degraded code or reasoning ability versus the base model.
...
**Short:** Most 7B-13B models hit the elbow of diminishing CPT returns around 1B-5B domain tokens, with minimal gain and rising forgetting risk past 10B.
...
**Short:** Confidential domain data like patient records is handled via synthetic generation, federated on-premise training, or strict de-identification under a data use agreement.
...
**Short:** Poor tokenizer coverage splits domain terms into many subword tokens, wasting context and blurring the semantic representation of common jargon.
...
**Short:** Domain adaptation success is measured with domain knowledge exams, downstream production task accuracy, and human expert review, always held out from training.
...
**Short:** Domain adaptation broadens the model's knowledge of an entire field over weeks of CPT, while task-specific fine-tuning teaches one behavior in hours of SFT.
...
**Short:** The practical CPT sweet spot is 80-90% domain data to 10-20% general data, since even 5% general data meaningfully reduces catastrophic forgetting.
...
**Short:** Forgetting is tracked by evaluating MMLU, HumanEval, and MT-Bench every 500-1,000 steps and halting training on a pre-set regression threshold.
...
**Short:** Temperature-based sampling upsamples smaller datasets relative to raw size and is the most robust CPT mixing strategy across varied domain corpus sizes.
...
**Short:** Domain adaptation success requires two parallel eval suites, passing only if domain metrics improve and general metrics stay within about 5% of baseline.
...
**Short:** A replay buffer mixes a fixed set of general examples into 5-15% of each CPT training batch, continuously preserving general capability without a full general corpus.
...
**Short:** Holding the learning rate constant during CPT forgets the least but barely adapts; re-warming and cosine re-decaying it adapts better for the same forgetting budget.
...
**Full CPT vs. LoRA CPT tradeoff.** Full CPT consumed ~53,000 A100-hours. LoRA CPT at r=128 is cheaper per step but not dramatically so — freezing the base removes the weight-gradient matmul, taking training from roughly `6ND` to roughly `4ND`, about a third less compute, not a multiple — and it reached only 81% clause extraction F1 against 88% for full CPT. Those two F1 numbers are part of this composite, not a measurement — but the *direction* is a published result: Biderman et al., "LoRA Learns Less and Forgets Less" (TMLR 2024, arXiv 2405.09673) find LoRA substantially underperforms full finetuning in both continued pretraining and instruction tuning, and attribute it to full finetuning learning perturbations of 10-100x higher rank than a typical LoRA config. The same paper names the compensation: LoRA better preserves out-of-domain performance than weight decay or dropout do. So the real LoRA CPT wins are memory (no fp32 optimizer state for 8B parameters), which is what lets a smaller team run it on far fewer GPUs, and less forgetting; the compute saving alone would not justify the F1 loss.
