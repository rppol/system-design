...
**Why it matters**: Software engineering is the #1 commercial application of LLMs today. The market is moving from "AI helps write code one line at a time" ([Copilot-style completion](../code_generation/code_generation.md)) toward "AI completes whole tasks autonomously" (Devin, Claude Code). The pivot creates new architectural challenges: navigation at scale, test-driven loops, mistake recovery, multi-file coherence, human collaboration patterns.
