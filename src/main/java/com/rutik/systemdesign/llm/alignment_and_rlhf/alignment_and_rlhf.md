...
**The idea behind it.** "Make the good answer more likely and the bad answer less likely — but score both *relative to the frozen original model*, so you are rewarding improvement, not raw confidence."
...
|--------|---------------------|
| `π_θ` | The model you are training. `π` = policy (RL's word for "the model"), `θ` = its weights |
| `π_ref` | The frozen SFT model. The yardstick you measure improvement against |
| `π(y|x)` | "probability of y given x" | How likely that model thinks response `y` is, for prompt `x` |
| `β` | Leash length. Small β = free to wander; large β = stay near the reference. Typical 0.01–0.5 |
| `σ` | Squashes any number onto 0–1. Big positive in → near 1; big negative in → near 0 |
| `log[a/b]` | Turns "how many times bigger" into "how many steps better". Ratio 1 → 0; ratio >1 → positive; ratio <1 → negative |
...
**Stated plainly.** "Do ordinary fine-tuning on the good answer, and bolt on a second term that actively penalizes the bad one. Two jobs, one loss, no reference model."

| Piece | What it does |
|-------|--------------|
| `-log P(chosen)` | The plain SFT loss. "Learn to produce the good answer" |
| `odds_ratio` | Gambling odds. How much better are the odds of the chosen answer vs the rejected one |
| `λ` | Mixing weight. How hard to push down the bad answer relative to learning the good one |
...
Generating preference pairs at scale (best-of-N ranking, strong-vs-weak model pairs, self-critique revision) is covered in [Synthetic Data Generation](../synthetic_data_generation/synthetic_data_generation.md).
...
**What the formula is telling you.** "Score each response by its average per-token confidence, require the good one to beat the bad one by a set margin, and drop the reference model entirely."

| Symbol | What it is |
|--------|------------|
| `y_w` | The chosen response (`w` = winning) |
...
| Piece | What it does |
|-------|--------------|
| `r_θ(prompt, response)` | The reward model's scalar score. Higher = humans liked it more |
| `r_chosen - r_rejected` | Gap between the two scores. Positive = correct ordering |
| `σ(margin)` | Converts the gap into "probability we ranked this pair right" |
| `-log σ(...)` | Loss. Zero when certain and correct, large when confident and wrong |
...
**In plain terms.** "Take the improvement you earned — but if this single update moves the model more than 20% away from where it started, stop counting the extra."
...
| Symbol | What it is |
|--------|------------|
| `r_t(θ)` | New probability ÷ old probability of the same action. `1.0` = unchanged, `1.2` = 20% more likely now |
| `A_t` | Was this action better (+) or worse (−) than the baseline expected? Sign is what matters |
| `ε` | Trust-region width, `0.2`. Allows the ratio to roam in `[0.8, 1.2]` |
| `clip(r, 1-ε, 1+ε)` | Clamp: anything below `0.8` becomes `0.8`, above `1.2` becomes `1.2` |
| `E[...]` | Plain average over the batch |
| `min(a, b)` | Take the smaller — always choose the *more pessimistic* of the two estimates |
...
**Read it like this.** "You never needed a separate reward model. The gap between your trained model and the frozen one *is* the reward, already sitting there in the log-probabilities."
...
| Symbol | What it is |
|--------|------------|
| `r*(x, y)` | The *optimal* reward. The star means "the best possible one", not a specific trained network |
| `π*` | The optimal policy — the model RLHF would have converged to |
| `log[π*/π_ref]` | How much more likely the tuned model finds this response than the frozen one |
| `Z(x)` | Partition function — a normalizing constant that makes probabilities sum to 1 |
...
**Short:** RLHF trains a reward model on human preference comparisons, then uses PPO to optimize the LLM's outputs against that reward model.
...
**Short:** DPO directly optimizes a closed-form loss on chosen/rejected preference pairs, skipping the separate reward model and PPO stage that RLHF uses.
...
**Short:** Reward hacking is the policy exploiting the reward model's proxy signal, such as length, to score well without truly improving human-judged quality.
...
**Short:** Constitutional AI aligns a model by having it critique its own outputs against a written constitution, then trains on AI-generated rather than human preferences.
...
**Short:** Sycophancy is agreeing with a user's stated belief even when it's false, because RLHF raters tend to rate belief-confirming responses higher.
...
**Short:** DPO trades RLHF's three-model pipeline and reward-hacking risk for a single stable training run, at some cost to capturing complex, multi-dimensional preferences.
...
**Short:** Reward hacking is detected by rising response length or reward-vs-human-eval divergence, and prevented with a KL penalty, reward-model ensembles, and length normalization.
...
**Short:** CAI is preferred when alignment must scale without proportional human-annotator growth, using an explicit, auditable constitution the model uses to self-critique and revise.
...
**Short:** KTO aligns models from independent binary thumbs-up/down signals rather than paired preferences, weighting bad outputs more heavily per prospect theory.
...
**Short:** A reward model is a Bradley-Terry classifier trained on 50K-500K preference pairs and evaluated by holding out 10% and measuring agreement with human labels.
...
**Short:** SimPO drops the frozen reference model entirely, saving GPU memory by using average per-token log-probability as an implicit reward against a tuned target margin.
...
**Short:** GRPO replaces PPO's learned critic with a group-relative baseline from the mean and std of G sampled rollouts per prompt, roughly halving trainer memory.
...
**Short:** Online methods like GRPO avoid distribution shift by scoring on-policy outputs but cost 2-5x more wall-clock time than offline methods like DPO trained on a fixed dataset.
...
**Short:** Verifiable rewards use objective, execution-based signals like test pass/fail or math correctness, eliminating reward-model noise but failing on tasks with no ground truth.
...
**Short:** The KL penalty anchors the policy to the SFT reference so it can't drift into regions where the reward model's scores become meaningless extrapolations.
