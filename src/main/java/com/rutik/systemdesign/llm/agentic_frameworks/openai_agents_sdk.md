...
**Short:** A tool call returns a result to the same agent; a handoff transfers control so the target agent runs to completion instead.
...
**Short:** It loops until the active agent returns output with no tool calls or handoff, or `max_turns` is exceeded, validating `output_type` if set.
...
**Short:** A typed wrapper passed to every tool and guardrail whose generic parameter gives static type checking across context, tools, and agents.
...
**Short:** Input guardrails screen the request before the LLM call; output guardrails validate the final response after it; both can abort the run.
...
**Short:** The full history travels with the handoff by default, so the target agent inherits full context unless filtered via `input_filter`.
...
**Short:** Swarm was a synchronous educational reference; the Agents SDK is async-first with built-in tracing, guardrails, and retries for production.
...
**Short:** Yes, via a LiteLLM or direct Anthropic model adapter, though OpenAI-native features like the tracing dashboard are then lost.
...
**Short:** `run_streamed()` emits raw token events for token-by-token display and higher-level run-item events for tool calls and handoff switches.
...
**Short:** It forces the model into a Pydantic schema via structured outputs, validated after the call and retried up to three times on failure.
...
**Short:** The `@function_tool` decorator turns an async function's signature into the tool's JSON schema and its docstring into the description.
...
**Short:** Identical API cost -- the SDK's value is far less orchestration code plus built-in tracing, guardrails, and retries.
...
**Short:** Mock the model with `FakeModel` returning canned responses, assert on tool calls and final output, then test cheaply against real models.
...
**Short:** Every run auto-emits a full execution tree of agent invocations, model and tool calls, handoffs, and timings for debugging.
...
**Short:** Gate the real call behind a preview step that returns a proposed action for user approval before it actually executes.
...
**Short:** The SDK is stateless, so you persist conversation history and long-lived memory yourself and pass it back in as input.
...
**Short:** The underlying OpenAI SDK retries 429s with exponential backoff by default, configurable via `max_retries` for stricter throughput needs.
