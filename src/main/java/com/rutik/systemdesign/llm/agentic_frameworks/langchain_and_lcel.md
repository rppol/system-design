...
**Short:** LCEL composes stateless Runnables with the pipe operator and is streaming/async-native; legacy chain classes moved to `langchain-classic`.
...
**Short:** Any object implementing `invoke`/`stream`/`batch` plus async variants, so components compose uniformly and swap without changing callers.
...
**Short:** `RunnableParallel` runs multiple Runnables concurrently and returns a dict, e.g. fetching retrieved context alongside the raw question.
...
**Short:** Wrap the chain in `RunnableWithMessageHistory` with a session-keyed history store, backed by Redis or DynamoDB in production.
...
**Short:** It returns its input unchanged, preserving the original question inside a `RunnableParallel` branch that would otherwise consume it.
...
**Short:** The model yields token chunks that flow through a streaming-compatible output parser, which requires `.stream()` instead of `.invoke()`.
...
**Short:** `.with_fallbacks([alt])` retries with a substitute Runnable when the primary raises, such as falling back to a cheaper model on a 429.
...
**Short:** Tool-calling parses structured JSON from native function calling; ReAct regex-parses free-text Thought/Action blocks and fails more often.
...
**Short:** Trace it in LangSmith, print the chain graph with `get_graph().print_ascii()`, or invoke each component individually to isolate the failure.
...
**Short:** A custom callback handler accumulates token usage per session and raises before invocation once a budget threshold is hit.
...
**Short:** `langchain-core` holds the Runnable protocol dependency-free, `langchain` adds `create_agent`, and `langchain-classic` keeps the old chains.
...
**Short:** Unit test components in isolation, record/replay API calls for integration tests, and score chains with LangSmith datasets and an LLM judge.
...
**Short:** Fighting LangChain's abstractions more than solving the actual problem is the sign -- a short direct API call is often clearer.
...
**Short:** `ChatOpenAI` retries with exponential backoff via `tenacity` by default, and `RunnableRetry` or a custom limiter can extend this.
...
**Short:** `langchain-core` is the stable dependency-free foundation, while `langchain-community` bundles 300+ integrations of mixed maintenance quality.
...
**Short:** LangSmith is LangChain's tracing and evaluation platform, capturing every LLM/tool call so debugging isn't limited to guessing from I/O.
...
**Short:** Anthropic cache reads cost 10% of standard input price while writes cost more upfront, paying back once a cached prefix is reused.
...
**Short:** `with_structured_output` yields progressively filled Pydantic objects, while `JsonOutputParser` yields a raw dict growing at every token.
...
**Short:** Run both chain variants against the same LangSmith dataset with `evaluate()` and compare scores before promoting the winning variant.
