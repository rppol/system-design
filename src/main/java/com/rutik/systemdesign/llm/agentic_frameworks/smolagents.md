...
**Short:** CodeAgent has the LLM write Python code as its action, chaining multiple tool calls per response, unlike one-call-at-a-time ToolCallingAgent.
...
**Short:** Code-as-action matches the model's native training distribution far better than JSON tool-call sequences, boosting GAIA success significantly.
...
**Short:** `LocalPythonExecutor` is a lightweight in-process AST filter, while `E2BExecutor` runs code in an isolated cloud microVM for real security.
...
**Short:** It only blocks import statements, not attribute access to already-loaded objects, so untrusted code still needs VM isolation like E2B.
...
**Short:** The run ends only when generated code calls `final_answer`, so a model that never calls it burns steps until `max_steps` is hit.
...
**Short:** Decorate a Python function with `@tool`; its type hints and Google-style docstring become the JSON schema and parameter descriptions.
...
**Short:** Yes -- `LiteLLMModel` proxies to any provider via LiteLLM, alongside native classes for direct OpenAI access.
...
**Short:** `ToolCollection.from_mcp()` connects to an MCP server and exposes its tools as Python callables inside a context manager.
...
**Short:** `max_steps` caps agent iterations, while `additional_authorized_imports` whitelists which modules the generated code may import.
...
**Short:** `ManagedAgent` wraps a specialist agent as a callable tool that an orchestrator CodeAgent can invoke like any other tool.
...
**Short:** Yes -- the executor's Python namespace stays alive within one run, so large objects stay in the sandbox instead of entering the prompt.
...
**Short:** Inspect the logged generated code and execution output per step, watching for whitelist violations and bad argument types.
...
**Short:** Minimal -- the code-as-action format adds little prompt overhead and often needs fewer LLM calls than ToolCallingAgent.
...
**Short:** Smolagents has no built-in tool retrieval, so apply tool RAG or classifier routing at the application layer before invoking it.
...
**Short:** Yes for narrow, properly sandboxed tasks, though it still lacks built-in tracing and durable execution for high-throughput use.
