...
**Short:** Workflow agents make zero model calls and apply a fixed sequence/parallel/loop pattern; `LlmAgent` decides its action via a model call.
...
**Short:** Nothing -- `SequentialAgent` has no `instruction` parameter and only ever runs its `sub_agents` in list order.
...
**Short:** Bare keys are session-scoped, `user:` persists per-user across sessions, `app:` is global, and `temp:` never survives the current invocation.
...
**Short:** Each branch writes to its own state fork to avoid races; forks merge on completion, so distinct `output_key`s per branch are essential.
...
**Short:** Setting `actions.escalate = True` exits the loop right after the current iteration finishes; otherwise it runs the full `max_iterations`.
...
**Short:** `transfer_to_agent` is a one-way handoff that never returns control; `AgentTool` calls the sub-agent as a tool and returns its output.
...
**Short:** Cloud Run's multi-instance scaling and scale-to-zero pin session state to one instance, so it gets lost or unreachable across requests.
...
**Short:** First check whether the critic sub-agent actually sets `escalate` on success, then check whether its approval bar is miscalibrated too high.
...
**Short:** Whether the system is mostly fixed orchestration, where ADK's zero-LLM-call workflow agents have no Strands equivalent, or dynamic reasoning.
...
**Short:** It lets agent logic deploy as just another JVM service inside existing Spring Boot infrastructure, with no new Python dependency.
...
**Short:** Use `RemoteA2aAgent` when the partner side is itself a multi-step reasoning process, not a single deterministic call a `FunctionTool` can wrap.
...
**Short:** Check the trace for a state-key mismatch between where a tool wrote and what the synthesizing agent's instruction reads, or a stale `ParallelAgent` branch.
