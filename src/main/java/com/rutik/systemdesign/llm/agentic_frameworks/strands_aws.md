...
- Need complex stateful workflows (LangGraph fits better)
- Heavy reliance on non-Bedrock providers (use LiteLLM directly)
...
Yes via `MCPClient` from `strands.tools.mcp`. Connect to MCP server, get tools, pass as agent tools alongside native `@tool` functions.
