...
Two mechanisms exist for structured outputs: (1) native structured output APIs (OpenAI `response_format={"type": "json_schema", ...}`, Anthropic tool use), which constrain the model at the token sampling level; and (2) prompt-based parsing (ask the model to output JSON, parse the response), which is less reliable.
