...
**Short:** It captures full prompts, completions, per-step token counts and cost, and semantic quality -- not just calls and status codes.
...
**Short:** A trace is the full tree of spans for one agent execution, recording start/end time, input/output, and attributes for each step.
...
**Short:** Log model name, full input/output messages, token counts, and latency and finish reason -- but never secrets or unmasked PII.
...
**Short:** Log input/output tokens per span, price them against a model rate map, then aggregate cost by user, team, or model for dashboards.
...
**Short:** A separate LLM scores sampled production traces against quality criteria, calibrated by checking agreement with human-labeled examples.
...
**Short:** OpenTelemetry propagates trace context via `contextvars` automatically, but the trace client must be initialized before async tasks start.
...
**Short:** Attach identifiers like `user_id` or `ticket_id` to every trace, then join trace data with warehouse business data on that key.
...
**Short:** They are standard attribute names like `gen_ai.usage.input_tokens` that let dashboards and queries work across different observability tools.
...
**Short:** Walk the trace tree to isolate whether retrieval, prompt construction, or the model's own output caused the incorrect answer.
...
**Short:** It fetches versioned prompt templates from a registry at runtime, so you can A/B test or roll back a prompt without a code deploy.
...
**Short:** Compute rolling-window quality metrics like faithfulness or error rate and page the on-call engineer when they cross a threshold.
...
**Short:** OpenTelemetry injects the W3C `traceparent` header into outgoing requests so an LLM span nests under its calling HTTP handler span.
...
**Short:** Never log PII, secrets, financial data, or confidential system prompts, since a leaked trace can hand them to a prompt-injection attacker.
...
**Short:** Use Langfuse's `@observe()` decorator, OpenLLMetry auto-instrumentation, or an HTTP proxy like Helicone to trace calls with no framework.
...
**Short:** Intercept LLM calls at a gateway to price each one, then alert or throttle at per-request, hourly, and daily cost thresholds.
