...
Covered in depth in [Guardrails & Content Safety](../guardrails_and_content_safety/guardrails_and_content_safety.md); for automated/algorithmic jailbreak generation (GCG, AutoDAN, TAP) see [automated_jailbreak_algorithms.md](automated_jailbreak_algorithms.md). Key additional concern:
...
| Symbol | What it is |
|--------|------------|
| `s` | Severity of one finding, 1 (mild) to 5 (catastrophic) |
| `c_s` | Count of findings at severity `s` |
| weighted risk | `sum over s of (c_s x w_s)`. One number for the whole red-team cycle |
| `w_s` | The weight attached to severity `s`. Linear (`w = s`) or exponential (`w = 10^(s-1)`) |
| blind evaluation | Graders do not know which model produced the output. Removes the pull toward scoring your own model kindly |
...
| Symbol | What it is |
|--------|------------|
| `ASR` | `complied_harmful / total_harmful`. Fraction of harmful prompts the model answered. Lower is better |
...
[AI Regulations & Compliance](../ai_regulations_and_compliance/ai_regulations_and_compliance.md), whose closest analogue is
...
- External red teaming including domain expertise, described in Anthropic's Responsible Scaling
  Policy and in the per-model system cards. The RSP commits to "partner with a diverse range of
  external red team and penetration testing experts", and from v3.4 (July 2026) requires that "all
  parts of the unredacted report are evaluated by at least one external reviewer"
- Biosecurity expertise is specifically part of it. The concrete, dated figure Anthropic has
  published is from *Frontier Threats Red Teaming for AI Safety*: "over the past six months, we
  spent more than 150 hours with top biosecurity experts red teaming and evaluating our model's
  ability to output harmful biological information." Treat "before every release" as the RSP's
  general commitment rather than a per-version claim you can cite
- Findings are published per model in the system cards (Claude Opus 4, 4.6, 4.8 and so on), which
  document the RSP evaluations and the ASL determination
...
**Short:** Hallucination is confident but factually incorrect output, mitigated with RAG grounding, uncertainty prompting, multi-sample consistency checks, and required citations.
...
**Short:** Jailbreaking is a user deliberately bypassing safety training, while prompt injection is malicious instructions hidden in data the model processes, more dangerous for agents.
...
**Short:** Safety prevents near-term harmful outputs from a deployed model, while alignment is the broader problem of ensuring an AI pursues goals humans actually intend.
...
**Short:** Red teaming is adversarial pre-deployment testing by domain experts to find jailbreaks and misuse that safety training missed, feeding a mitigation-then-retest loop.
...
**Short:** The alignment tax is safety training degrading helpfulness by over-refusing benign requests, measured by tracking false-refusal rate alongside attack success rate.
The alignment tax is the capability degradation that safety training imposes — a model tuned hard to refuse anything risky also starts refusing benign requests, giving worse answers, and hedging excessively. The trap is optimizing a single "harmlessness" metric: push refusal rate up and you silently drive over-refusal on legitimate prompts up too (a cooking question mentioning "knife," an educational "how do viruses work"). Measure both axes simultaneously — attack success rate on AdvBench AND false-refusal rate on a benign-but-scary eval set (target: refuse <1% of benign requests) — and treat any regression in either as a failure. Techniques that reduce the tax: [Constitutional AI / RLAIF](../constitutional_ai/constitutional_ai.md) that trains calibrated refusals with explanations rather than blanket denials, and keeping a held-out capability benchmark (MMLU, MT-Bench) as a guardrail during [RLHF](../alignment_and_rlhf/alignment_and_rlhf.md) so you catch capability loss before shipping.

**Q: What is sycophancy and why is it a safety problem, not just an annoyance?**
**Short:** Sycophancy is the model agreeing with a user's false premise because reward models prefer agreeable responses, amplifying misinformation and manipulation risk.
...
**Short:** Multi-turn jailbreaks like crescendo escalate gradually across benign-looking messages, defended by scoring risk cumulatively across the conversation rather than per message.
...
**Short:** Prompt injection makes a model follow attacker-controlled instructions from processed data, categorized as direct, indirect, stored, or cross-context, indirect being most dangerous.
...
**Short:** A model that refuses everything scores a perfect 0% attack success rate while being useless, so safety needs a joint low-ASR-and-low-false-refusal operating point.
...
**Short:** A capability threshold is a pre-defined dangerous-capability level that triggers mandatory safeguards before release, used by Anthropic, OpenAI, and DeepMind's safety frameworks.
