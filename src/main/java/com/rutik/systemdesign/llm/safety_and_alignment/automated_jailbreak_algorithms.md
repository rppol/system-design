...
**Short:** Hand-crafted jailbreaks are individually iterated prompts, while automated algorithms like GCG frame jailbreaking as an optimization problem yielding reproducible metrics at scale.
...
**Short:** GCG uses the embedding gradient only to narrow each suffix position to top candidate token swaps, then picks the best by actual forward-pass loss, since gradients can't update discrete tokens directly.
...
**Short:** AutoDAN avoids gibberish structurally, not through its fitness function, by seeding its population with fluent handcrafted prompts and mutating via LLM paraphrasing.
...
**Short:** TAP's judge prunes tree branches that drift off-topic or fail to progress toward compliance, reaching PAIR-level attack success with far fewer target-model queries.
...
**Short:** Transferability reveals jailbreak vulnerabilities are structural rather than model-specific, letting a team run GCG against an open-weight proxy as a cheap signal for a closed model.
...
**Short:** SmoothLLM perturbs the input randomly and majority-votes across several queries, exploiting that finely optimized suffixes like GCG's are fragile to small character changes.
...
**Short:** A defense validated only against 2023-era attack outputs doesn't generalize to newer algorithms' different outputs, favoring defenses targeting the underlying mechanism instead.
...
**Short:** An affirmative-prefix target exploits the model continuing consistently with its own prior tokens, making an early monitor for a forming compliance prefix a cheap defense point.
...
**Short:** With budget for only one algorithm, a closed-model team should choose TAP for fluent black-box coverage while explicitly naming what remains untested, like transfer signal.
...
**Short:** Against a tool-using agent, a harmful jailbreak outcome means the model calling a destructive tool rather than generating text, requiring action-level harm classification.
...
**Short:** A single-turn attack-success-rate number can't represent a multi-turn attack like Crescendo, since each individual turn is benign and only the full conversation carries harmful intent.
