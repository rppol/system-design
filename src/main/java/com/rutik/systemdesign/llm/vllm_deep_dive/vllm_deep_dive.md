...
SGLang, TensorRT-LLM, and the rest of the engine landscape are compared in [Inference Engines](../inference_engines/inference_engines.md).
...
[Inference Engines](../inference_engines/inference_engines.md); what follows is vLLM's control surface.
...
**Short:** PagedAttention allocates KV cache in fixed-size, non-contiguous pages to eliminate fragmentation and enable prefix sharing.
...
**Short:** Static batching waits for the whole batch to finish; continuous batching inserts a new request the moment a slot frees up.
...
**Short:** Chunked prefill splits a long prompt's processing into small chunks interleaved with decode steps to keep TTFT low for other users.
...
**Short:** Automatic prefix caching hashes KV blocks so requests sharing a prefix reuse cached pages instead of recomputing them.
...
**Short:** Tensor parallelism splits each weight matrix across GPUs within a node, while pipeline parallelism splits layers across nodes.
...
**Short:** Speculative decoding verifies a draft model's tokens against the target model via rejection sampling, preserving the target's exact distribution.
...
**Short:** Raising gpu_memory_utilization grows the whole VRAM cap, not just the KV pool, so it can starve CUDA graph and NCCL buffers.
...
**Short:** LoRA serving suits many small adapters sharing one base model; separate deployments suit differing configs, traffic, or isolation needs.
...
**Short:** KV cache saturation shows up as high kv_cache_usage_perc, rising preemptions, and a growing waiting-request queue together.
...
**Short:** Multi-LoRA serving uses --enable-lora with --max-loras and --max-cpu-loras to keep hot adapters on GPU and cold ones on CPU.
...
**Short:** Prefix caching gives zero benefit when every prompt is unique, but its hashing overhead costs under 1% throughput even then.
...
**Short:** vLLM V1 moved scheduling and execution into a separate EngineCore process, and V0 was fully removed in v0.11.0.
...
**Short:** vLLM preemption always discards and recomputes a request's KV blocks -- it never swaps them out to CPU.
...
**Short:** TP=4 splits LLaMA 70B's ~140GB of FP16 weights into ~35GB per A100, leaving roughly 35GB of headroom for KV cache.
...
**Short:** Both interleave prefill and decode across requests, and TensorRT-LLM also pages its KV cache by default, unlike the common assumption.
...
**Short:** Chunked prefill shares one GPU pool for prefill and decode, while PD disaggregation runs them on separate pools linked by KV transfer.
...
**Short:** --enforce-eager disables torch.compile and CUDA-graph capture, trading decode throughput for faster startup and clearer stack traces.
...
**Short:** The default infinite request rate fires every request at once, so the reported latency measures saturation, not service under real load.
