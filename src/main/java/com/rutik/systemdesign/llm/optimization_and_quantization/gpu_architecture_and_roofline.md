...
3. **Batching converts memory-bound to compute-bound.** B sequences decoding together share each weight read: intensity ≈ 2B/bytes_per_param FLOPs per byte for the GEMMs. On H100 that crossover is B≈296 at BF16 and B≈148 at INT8 (§6.2) — which is exactly why continuous batching exists.
...
**Two formats, two jobs** (introduced in §6.4): **E4M3** (4 exponent bits, 3 mantissa bits, max magnitude 448) carries forward activations and weights, where a 12.5% step between representable values (2^-3 mantissa, so at most 6.25% rounding error per element) is tolerable because normalization layers keep these tensors' magnitudes roughly stable. **E5M2** (5 exponent bits, 2 mantissa bits, max magnitude 57,344) carries gradients, which — especially early in training and in layers far from the loss — can span several orders of magnitude more dynamic range than activations; E5M2 trades mantissa precision for that range.
...
**Transformer Engine** (NVIDIA) is the library most teams use rather than hand-rolling the above: `te.Linear`, `te.LayerNormLinear`, etc. are drop-in replacements for their PyTorch equivalents that manage the scaling state internally. Its `DelayedScaling` recipe — the default on Hopper and earlier — is `Format.HYBRID` (E4M3 forward / E5M2 backward) with `amax_history_len=1024` and `margin=0`; on Blackwell the default recipe is now `MXFP8BlockScaling` (block scaling in hardware, no amax history). The mechanics above describe what runs inside those modules.
...
- **Groq LPU** — weights in SRAM across hundreds of chips; ~275-330 tok/s single-stream on 70B-class models (Artificial Analysis benchmarked Llama 3.3 70B at 276 tok/s, the fastest provider measured) by buying the top of the memory pyramid; the cost structure (chips scale with model size, not traffic) shows the same roofline from the opposite side.
...
**Short:** Decode has arithmetic intensity near 1-2 FLOPs/byte, far below an H100's ~295 ridge point, while prefill processes many tokens per weight load and sits against the compute roof.
...
**Short:** A 70B model's single-stream decode ceiling is roughly bandwidth over model bytes, giving about 48 tok/s on two H100s at FP16 or 96 tok/s at INT4 on one.
...
**Short:** The ridge point is peak FLOPS divided by peak bandwidth, about 295 FLOPs/byte at BF16 on H100, below which a kernel is memory-bound and above which compute-bound.
...
**Short:** MFU measures achieved-over-peak FLOPS for compute-bound phases like prefill, while MBU measures bandwidth utilization and is the honest metric for memory-bound decode.
MFU = achieved/peak FLOPS: meaningful for compute-bound phases — training (healthy: ~40–60%) and prefill. MBU = achieved/peak memory bandwidth: the honest metric for decode, where 60–80% is good and MFU of 2–5% is *expected physics*, not inefficiency. The interview trap is being shown "our inference MFU is 4%, help us fix it": the senior answer asks for the phase split and MBU first — if decode MBU is already 75%, the fix is batching/quantization/economics, not kernels. Using one number for both phases is how teams chase phantom optimizations.

**Q5: Size the KV cache for Llama-3-70B and explain why GQA exists.**
**Short:** Llama-3-70B's KV cache costs about 320KB per token, and GQA cuts that 8x by sharing one KV head across eight query heads instead of giving each its own.
...
**Short:** Batching raises decode's arithmetic intensity roughly linearly with batch size until it hits the ridge point, runs out of KV cache capacity, or breaches a latency SLO.
Batched decode shares each weight read across B sequences: GEMM intensity rises from ~1 to ~B FLOPs per byte at BF16 (2B/bytes_per_param in general), sliding the workload up the bandwidth roof — throughput grows almost linearly in B while per-step time stays flat (you were idle on compute anyway). Three things end the party: (1) intensity reaches the ridge (B≈296 on H100 BF16, ≈148 at INT8) and you go compute-bound; (2) KV capacity — each sequence carries its cache (Q5), and HBM runs out long before the ridge at long contexts; (3) latency SLOs — batching trades per-token latency for throughput. Continuous batching (vLLM) exists to hold effective B high despite ragged sequence lengths; attention over per-sequence KV remains memory-bound even when the GEMMs aren't, which is why long-context batched decode still hungers for bandwidth.

**Q7: Why does INT4 quantization speed up decode ~3–4× but barely move prefill?**
**Short:** INT4 quantization cuts the weight bytes decode must stream, directly raising its bandwidth-bound ceiling, but prefill's FLOPs are unchanged since GEMMs dequantize back to BF16.
...
**Short:** Tensor parallelism needs an all-reduce inside every layer, requiring NVLink bandwidth within a node, while pipeline parallelism communicates only once per stage boundary.
...
**Short:** FlashAttention tiles Q, K, V into SRAM and keeps a running softmax so the score matrix never touches HBM, trading slightly more FLOPs for far fewer bytes moved.
...
**Short:** H200 keeps H100's FLOPS but nearly doubles bandwidth and capacity for decode-heavy workloads, while B200 raises both FLOPS and bandwidth outright at higher platform cost.
...
**Short:** MoE models must keep every expert resident in HBM even though only a small active fraction streams per token, shifting the binding constraint to total capacity.
...
**Short:** Speculative decoding spends decode's idle compute by verifying several drafted tokens in one forward pass, amortizing one weight streaming over multiple accepted tokens.
...
**Short:** High reported GPU utilization can mean only that a kernel was resident while stalled on memory, so real diagnosis starts with checking MBU, not the utilization number.
...
**Short:** Groq and Cerebras get an order of magnitude more decode bandwidth by keeping weights in SRAM instead of HBM, needing hundreds of chips per model regardless of traffic.
They relocate weights to the top of the memory pyramid: SRAM at tens of TB/s instead of HBM at ~3–8 TB/s. Decode's ceiling (BW ÷ model bytes) jumps an order of magnitude — hence ~275-330 tok/s single-stream on 70B-class models, several times the ~48-96 tok/s a GPU pair reaches. The catch is capacity economics: SRAM is ~hundreds of MB per chip (Groq: 230 MB), so the model shards across hundreds of chips that must all be lit regardless of traffic — cost scales with *model size*, while GPU fleets scale with *traffic* and amortize via batching. So SRAM-first wins where single-stream latency is the product (interactive agents, voice) and loses on bulk $/token at scale; it's the same roofline, with capital allocated to the bandwidth term instead of the capacity term.

**Q15: Walk me through sizing a deployment: 50M tokens/hour generated, p50 inter-token latency <30ms, Llama-3.1-70B.**
**Short:** Sizing a deployment works through capacity, then bandwidth, then compute ceilings per replica before dividing total demand by per-replica throughput to get replica count.
...
**Short:** KV cache bytes are added per sequence rather than shared like weight bytes, so attention stays memory-bound at every batch size no matter how much batching helps the GEMMs.
...
**Short:** Evaluating a chip swap means computing the same capacity, bandwidth, and FLOPS numbers plus the ridge point, then discounting hard for the vendor's software maturity.
...
1. Re-layout: TP=4 *within* each node (70 GB of FP8 weights sharded 4 ways = 17.5 GB/GPU after quantizing weights — eval gate passed, MT-Bench delta −0.1), two independent replicas. Decode MBU 31% → 68%; inter-token p50 85ms → 24ms.
...
3. Capacity plan rewritten with the three-gate template and pinned to the dashboard: per-replica ceilings (single-stream = aggregate bandwidth over total model bytes, 3.35e12×4/70e9 ≈ 190 tok/s ideal; measured 71% of that), prefill pool sized separately.
