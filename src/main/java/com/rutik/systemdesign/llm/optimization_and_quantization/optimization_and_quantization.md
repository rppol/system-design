...
(Full module: [Mixture of Experts](../mixture_of_experts/mixture_of_experts.md) — routing math, load balancing, expert parallelism.)
...
Traditional KV cache suffers from **fragmentation** — pre-allocated contiguous memory per sequence wastes space (full internals: [vLLM Deep Dive](../vllm_deep_dive/vllm_deep_dive.md)):
...
Train a small student model to mimic a large teacher (deep dive: [Knowledge Distillation & Model Merging](../knowledge_distillation_and_model_merging/knowledge_distillation_and_model_merging.md), which also covers the SLERP/TIES/DARE merging methods below):
...
(Full mechanics covered in [Inference & Decoding](../inference_and_decoding/inference_and_decoding.md); summary here for completeness.)
...
**Short:** GPTQ uses Hessian-based error compensation column by column, while AWQ scales important weight channels found from activation magnitudes, giving AWQ the edge at 3-bit.
...
**Short:** INT8 gives a safe 2x memory cut under 0.5% quality loss, while calibrated INT4 gives 4x but risks 5-10% domain-specific loss depending on calibration data.
...
**Short:** An INT4 model that fails your task despite passing MMLU usually has a calibration-data mismatch, fixed by re-quantizing with samples from your actual domain.
...
**Short:** QAT simulates quantization noise during training so weights adapt to it, worth its training cost only when calibrated PTQ quality is unacceptable for production.
...
**Short:** Flash Attention tiles Q, K, V into SRAM-sized blocks and computes an online softmax so the full attention matrix never touches HBM, cutting memory to O(n).
...
**Short:** Tensor parallelism shards every layer across NVLink-connected GPUs for low latency, while pipeline parallelism splits layers vertically across nodes with idle bubbles.
...
**Short:** GQA shares each KV head across multiple query heads, shrinking KV cache size proportionally, like LLaMA 3 70B's 8 KV heads giving an 8x smaller cache than MHA.
...
**Short:** 2:4 structured sparsity keeps exactly two nonzero values per four elements so Sparse Tensor Cores execute in one pass, unlike unstructured pruning which gets no GPU speedup.
...
**Short:** KV cache quantization matters most at long context or high concurrency, where cache memory can dwarf weight memory, with INT8 alone halving it.
...
**Short:** Quantization is fast and needs no retraining for a 2-4x cut, while distillation needs full training infrastructure but can reach 10-100x compression with better quality.
...
**Short:** FP8's floating-point exponent handles activation outliers natively where INT8's fixed grid needs careful per-channel scaling, giving FP8 near-BF16 quality on H100s.
...
**Short:** GPTQ redistributes each column's quantization error onto remaining unquantized columns via the inverse Hessian, far outperforming round-to-nearest especially at 3-bit.
...
**Short:** Flash Attention improves speed as much as memory because avoiding HBM round-trips for the full attention matrix removes the actual bottleneck, not just its footprint.
...
**Short:** MoE activates only a small fraction of a much larger parameter count per token via routing, beating a comparably sized dense model's quality at similar inference compute.
...
**Short:** Distillation trains a smaller model on a teacher's soft probability outputs for a permanent deployment, while fine-tuning adapts an existing model to a new domain.
...
**Short:** W4A16 only saves memory bandwidth by unpacking to FP16 before the GEMM, while W8A8 quantizes activations too and buys actual compute throughput at high batch size.
...
**Short:** NVFP4's smaller 16-element blocks and finer FP8 scale make it more accurate than MXFP4's 32-element power-of-two scale, but both need Blackwell-generation Tensor Cores.
