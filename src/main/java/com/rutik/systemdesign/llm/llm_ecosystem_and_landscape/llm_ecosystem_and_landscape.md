...
<!-- study-paths
principal: llm_ecosystem_and_landscape.md
...
**Short:** Benchmark contamination inflates public scores through training-data leakage, so a private domain evaluation set is the only trustworthy signal for model selection.
...
**Short:** DeepSeek-V3 reaching near-GPT-4 quality for $5.5M showed algorithmic gains can substitute for compute budget, undermining any assumed cost moat around a model tier.
...
**Short:** Open weights means downloadable parameters under a possibly restrictive license, while true open source like Apache 2.0 permits any use, including training competitors.
...
**Short:** A complexity-based router scores each query and sends easy ones to a cheap model and hard ones to an expensive one, but misrouting a fifth of traffic erases the savings.
...
**Short:** Self-hosting breaks even when API cost at your volume exceeds GPU rental plus engineering overhead combined, and omitting engineering overhead is a common costing mistake.
...
**Short:** A new model release warrants migration only after shadow-mode testing against your domain eval set shows a meaningful improvement, not just a headline benchmark score.
...
**Short:** Chinchilla's compute-optimal ratio balances tokens and parameters, but LLaMA over-trains a smaller model on far more tokens because inference cost dominates its lifetime.
...
**Short:** A multi-model abstraction layer centers on a model registry mapping logical names to providers, so no application code ever references a provider or model ID directly.
...
**Short:** Single-provider risk spans pricing swings, availability deprecation, capability obsolescence, and policy changes, mitigated by an abstraction layer and quarterly audits.
Single-provider risk falls into four categories: pricing, availability, capability and policy. (1) Pricing risk — providers have changed pricing 3-5x in either direction; a 2x price increase on your primary model can materially affect unit economics; (2) Availability risk — individual models are deprecated with 3-6 months notice; provider outages affect your SLA; (3) Capability risk — the model you selected may fall behind a competitor; rebuilding on a different provider is expensive if you are tightly coupled; (4) Policy risk — providers update usage policies; use cases that were permitted (certain categories of content generation) may be restricted. The mitigation is the abstraction layer pattern above, combined with quarterly model evaluation audits comparing your primary model to alternatives.

**Q: How do you handle model deprecation when a provider sunsets an API endpoint?**
**Short:** Handling deprecation means tracking model IDs centrally and shadow-evaluating the replacement well before sunset, not discovering it from a production 404.
...
**Short:** This distinction matters commercially because only true open-source licenses like Apache 2.0 or MIT permit training competing models from the released weights.
...
**Short:** API pricing is variable with no commitment, cloud self-hosting is semi-fixed via reserved instances, and on-premise is capital-intensive but cheapest per token at scale.
...
**Short:** Public benchmarks miss your production distribution, so skipping domain-specific evaluation routinely means paying 5-10x more for a model that performs worse on your task.
...
**Short:** Prompt caching skips reprocessing a stable prefix on repeated calls, cutting cached-token cost by roughly 90% with Anthropic, so stable content must sit at the prompt's front.
