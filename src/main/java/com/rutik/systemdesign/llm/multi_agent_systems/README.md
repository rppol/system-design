...
<!-- study-paths
senior: README.md, orchestrator_worker_pattern.md, multi_agent_security.md
principal: README.md, orchestrator_worker_pattern.md, agent_to_agent_protocols.md, multi_agent_security.md
...
**Short:** A central orchestrator decomposes the task, dispatches subtasks to specialized workers, and assembles their results.
...
**Short:** Multi-agent systems multiply LLM calls and duplicate context across workers, so summarize inter-agent messages to bound cost.
...
**Short:** Prefer a single agent until you hit real context overflow, parallelism needs, or measured quality gains from critique.
...
**Short:** Agents argue or critique each other's output over rounds, improving reasoning quality at the cost of extra latency.
...
**Short:** ChatDev simulates a software company with CEO, CTO, programmer, reviewer, and tester agents working through structured phases.
...
**Short:** Combine per-agent timeouts, one retry, checkpointing, and fallback agents so a single agent failure degrades gracefully.
...
**Short:** A handoff passes control and context to a more specialized agent, creating routing without a central orchestrator.
...
**Short:** Insert validation checkpoints between pipeline stages so one agent's hallucination can't silently propagate into the next.
...
**Short:** Message passing gives explicit, loggable inputs and outputs; shared memory is simpler but hides dependencies between agents.
...
**Short:** Resolve parallel-agent conflicts with majority voting, a dedicated judge agent, or a domain-specific merge rule set in advance.
...
**Short:** Pick LangGraph for stateful production workflows, AutoGen for conversational code-execution loops, and CrewAI for quick role-based crews.
...
**Short:** Log every inter-agent message with an agent ID and replay each agent in isolation to localize where an error entered.
...
**Short:** An orchestrator's accumulated context underweights middle sections, so hierarchical summarization keeps every input recallable.
...
**Short:** Route all agents through a shared token-bucket queue with backoff and per-agent quotas instead of calling the API directly.
...
**Short:** Choose horizontal fan-out for independent subtasks, vertical chains for dependent steps, and hierarchical for large open-ended goals.
...
**Short:** Record and replay inter-agent messages, isolate each agent, and inject synthetic failures to test how the orchestrator degrades.
