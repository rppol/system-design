...
**Short:** Intent, Cart, and Payment Mandates separate advance authorization, the specific transaction, and agent-risk signaling into distinct signed steps.
...
**Short:** Enforce a spend cap as a structural code check on the Cart Mandate, since a prompt instruction is only advisory to the model.
...
**Short:** Draw the human-present line on transaction value, category predictability, and how reversible the settlement rail is.
...
**Short:** A human-not-present dispute turns on whether the Cart Mandate genuinely fit the signed Intent Mandate's bounds, not just the transaction record.
...
**Short:** An orchestrating agent can settle an internal sub-task via x402 micropayments while the user-facing Cart Mandate total stays accurate.
...
**Short:** Tighten category and merchant bounds for predictable recurring purchases, and rely on price ceiling plus short expiry for one-off tasks.
