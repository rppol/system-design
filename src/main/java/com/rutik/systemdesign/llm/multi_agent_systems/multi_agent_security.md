...
**Short:** An output's trustworthiness depends on what data flowed into it, not who wrote the agent's code that produced it.
...
**Short:** Cross-agent injection adds a second hop where a downstream agent trusts a compromised agent's output as clean information.
...
**Short:** Prompt-infection makes the compromised agent itself reproduce the payload to every agent it talks to, spreading like a worm.
...
**Short:** A controller consumes the quarantined LLM's output only as inert data, so an injected instruction has nowhere to be parsed as an action.
...
**Short:** Signing proves who sent a message, not that its content is safe to act on, so injected content can still arrive validly signed.
...
**Short:** Two individually-correct agents optimizing stated objectives can jointly violate an unstated constraint neither one was ever given.
...
**Short:** A Byzantine agent is one genuinely independent participant compromised; a Sybil attack is one attacker posing as several voters.
...
**Short:** A schema-restricted summary field still lets injected text inside its string content influence the downstream agent's model.
...
**Short:** Transitive trust means an agent's effective trust equals the lowest trust of anything that fed it, so the comms graph is the compromise-propagation graph.
...
**Short:** Pattern-based output scanning flags legitimate security discussions as injections too, so route ambiguous hits to human review rather than drop or forward.
...
**Short:** A privileged zone with any path back to untrusted data makes the boundary cosmetic, so data flow must be strictly one-directional.
...
**Short:** Every external A2A agent sits in its own trust zone, so its output deserves at least as much suspicion as raw untrusted web content.
...
**Short:** Cross-agent tracing is often the only way to trace a propagated injection back to its original entry point after an incident.
...
**Short:** A wildcard recipient allowlist makes that capability check always pass, structurally equivalent to having no check at all.
...
**Short:** A single-model jailbreak's harmful text becomes another agent's input, letting a confused-deputy action follow from what was just text.
...
**Short:** Ask for the full trust graph per agent, the actual configured bounds of every capability check, and the worst downstream action possible.
...
**Short:** A poisoned shared store outlives any single run and is read by unrelated future sessions, unlike an injection that dies with its pipeline.
