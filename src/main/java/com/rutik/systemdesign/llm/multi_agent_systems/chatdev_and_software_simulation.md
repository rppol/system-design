...
**Short:** The chat chain sequences two-agent dialogues per phase, forcing requirements before design before code before QA.
...
**Short:** MetaGPT validates each phase's output against a Pydantic schema; ChatDev relies on open-ended dialogue evaluation instead.
...
**Short:** A hallucinated file references functions or modules that don't exist, caught by parsing imports or running the code in a sandbox.
...
**Short:** A shared repository lets agents read only relevant files, cutting per-call tokens 60-80 percent versus full-codebase injection.
...
**Short:** SOPs force each agent's output into a Pydantic schema with concrete required fields, leaving no room for vague prose.
...
**Short:** An unbounded QA repair loop can oscillate forever if a fix introduces a new bug, so cap iterations and degrade gracefully.
...
**Short:** A simple CRUD API typically costs 50,000-200,000 tokens in ChatDev, rising sharply with QA repair iterations and complexity.
...
**Short:** Role collapse happens when a bloated context makes an agent ignore its role prompt, prevented by injecting only relevant files.
...
**Short:** ChatDev's dialogue is synchronous turn-taking between two agents; MetaGPT's bus lets subscribed agents run asynchronously in parallel.
...
**Short:** Add an execution-verification step that runs generated code in a sandbox and feeds real tracebacks back as bug reports.
...
**Short:** The Architect makes system-design decisions and the Project Manager makes task-sequencing decisions, kept separate for quality.
...
**Short:** A schema validation failure triggers one retry with the error details, then raises an exception rather than proceeding on bad data.
...
**Short:** Agents read a fresh repository snapshot each phase instead of trusting conversation history, so no one edits a stale file.
...
**Short:** ChatDev's paper reports 86.66 percent executability, meaning the app launches without crashing, not that it's functionally correct.
...
**Short:** Seed the repository with existing code, retarget the programmer's prompt to modify not create, and add a diff-review phase.
