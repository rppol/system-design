...
<!-- study-paths
senior: deployment_and_mlops.md
principal: deployment_and_mlops.md
...
Route queries to the appropriate model based on complexity (full treatment: [LLM Routing & Model Selection](../llm_routing_and_model_selection/llm_routing_and_model_selection.md)):
...
Cache LLM responses by semantic similarity of queries (deep dive: [LLM Caching](../llm_caching/llm_caching.md)):
...
GPU memory is the most constrained resource in LLM serving. A single OOM crash kills all in-flight requests on that node. Serving-engine-level memory mechanics (PagedAttention, KV block management, preemption) are covered in [vLLM Deep Dive](../vllm_deep_dive/vllm_deep_dive.md).
...
**Short:** The LLM data flywheel turns production feedback into better fine-tuning data, which improves the model and attracts more users generating more data.
...
**Short:** Shadow mode runs a candidate model on live traffic without serving its output, catching distribution-specific failures offline benchmarks miss.
...
**Short:** Semantic caching returns a stored response when a new query embeds above a similarity threshold, cutting cost 40-50% at a 50% hit rate.
...
**Short:** A routing classifier sends most simple queries to a cheap model and the rest to an expensive one, saving 50-70% cost for only 2-5% quality loss.
...
**Short:** GPU serving cost is estimated from tokens-per-second throughput per dollar, with self-hosting beating API pricing only above a break-even volume.
...
**Short:** Autoscaling should trigger on KV-cache occupancy and queue depth, because GPU utilization looks near-100% even when memory-bound decode has spare room.
...
**Short:** OOM prevention caps per-request context length, alerts at 75-85% GPU memory, and degrades gracefully by shrinking batch size before returning 503s.
...
**Short:** Cost attribution meters requests at the gateway by team metadata, starting with showback dashboards before graduating to chargeback budget debits.
...
**Short:** LLM observability tracks both service metrics like TTFT and quality signals like LLM-as-judge scores, hallucination rate, and regeneration rate.
...
**Short:** Graceful degradation chains primary, secondary, and self-hosted fallback models behind a circuit breaker that trips after consecutive failures.
