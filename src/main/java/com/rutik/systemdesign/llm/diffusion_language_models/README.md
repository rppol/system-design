...
**1. What's the fundamental difference in generation order between diffusion LMs and autoregressive LMs — and why doesn't "parallel" mean "instant"?**
...
**2. Why can't you just plug a diffusion-LM into an existing AR serving stack and reuse the KV-cache for a free speedup?**
...
**3. Isn't masked diffusion training just BERT's masked language modeling (MLM) — what's actually different?**
...
**4. What does it mean that the masked-diffusion objective is an "ELBO," and why does that matter practically?**
...
**5. Walk through what happens in one denoising step, concretely.**
...
**6. How does Mercury achieve >1,000 tokens/sec — what's the architectural reason, not just "it's diffusion"?**
...
**7. What's the relationship between D3PM, SEDD, and LLaDA — is LLaDA "just SEDD at scale"?**
...
**8. Why is fill-in-the-middle (infilling) "native" to diffusion but a special-case for AR models?**
...
**9. What problem does Block Diffusion (BD3-LM) solve, and what does it give up?**
...
**10. How would you apply classifier-free guidance (CFG) to a text diffusion model, and what's the risk of setting the guidance scale too high?**
...
**11. Why might a diffusion-LM be a poor fit for a chain-of-thought reasoning task, even if it matches AR on knowledge benchmarks like MMLU?**
...
**12. How do you choose the number of denoising steps `T` for a production deployment?**
...
**13. What's the practical difference between discrete and continuous text diffusion, and why did discrete win for LLM-scale deployment?**
...
**14. As of 2026, what specifically is "missing" from inference engines like vLLM that prevents diffusion-LMs from being served the same way as AR models?**
...
**15. If you wanted to benchmark a diffusion-LM's quality without relying on AR-style perplexity, what would you measure?**
...
**16. How does the "any-order autoregressive" framing change how you'd think about a reversal-curse-style failure (e.g., "A is B" doesn't imply the model knows "B is A")?**
...
**17. What's the latency profile difference between diffusion and AR for a very short completion (e.g., 8 tokens) vs. a long one (e.g., 2,000 tokens)?**
...
**18. A colleague says "diffusion LMs will replace autoregressive LMs within a few years." How would you push back or refine that claim in an interview setting?**
