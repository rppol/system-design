...
> [Multimodal Models §4.2](../multimodal_models/multimodal_models.md).
...
- **Editing and infilling are first-class, not a hack.** AR models generate left-to-right, so
  ...
  training (see [Code Generation](../code_generation/code_generation.md)). A diffusion-LM can mask *any*
...
- Your **serving infrastructure is built around AR primitives** (vLLM's PagedAttention,
  ...
  [vLLM Deep Dive](../vllm_deep_dive/vllm_deep_dive.md) and Pitfall 10.7).
...
Reasoning models (see [Reasoning Models](../reasoning_models/reasoning_models.md)) rely heavily on
...
**Short:** Diffusion LMs predict all positions every pass but still need T sequential passes to resolve masked tokens, so parallel means fewer steps, not one step.
...
**Short:** AR KV-caches rely on causal, append-only attention; diffusion's bidirectional attention makes cached keys/values for resolved positions stale once other positions change.
...
**Short:** Masked diffusion trains on a random mask ratio spanning 0-100% with a 1/t-weighted ELBO loss, unlike BERT's fixed 15% ratio built for a non-generative task.
...
**Short:** An ELBO is only a provable lower bound on log-likelihood, not an exact value, so diffusion loss numbers can't be compared directly to AR perplexity.
...
**Short:** Each denoising step runs one bidirectional forward pass, then commits the highest-confidence masked predictions while leaving the rest masked for the next step.
...
**Short:** Mercury's speed comes from needing only 16-32 full-sequence passes versus AR's 256 sequential decodes, since the step-count reduction outweighs each pass's higher O(L^2) cost.
...
**Short:** D3PM defined the discrete-diffusion ELBO, SEDD's score-entropy objective closed much of the perplexity gap, and LLaDA proved that objective scales to 8B parameters.
...
**Short:** Infilling needs no special training in diffusion because prefix and suffix tokens simply stay unmasked from the start, unlike AR's rearranged FIM training format.

An AR model generates strictly left-to-right; to produce "the missing middle of a function given its start and end," it must be explicitly trained with a reformatted input — e.g., FIM training rearranges `prefix [SUFFIX] suffix [MIDDLE]` so the model still generates left-to-right but learns to treat the rearranged sequence correctly (see [Code Generation](../code_generation/code_generation.md)). A diffusion model has no such restriction: the prefix and suffix tokens are simply *never masked* (they're given as conditioning context from `t=1`), and only the middle positions start as `[MASK]`. The same model, same training, same sampling procedure handles "generate the whole function," "fill in this one line," or "fill in three scattered lines" — the only difference is which positions start masked.

**Q9: What problem does Block Diffusion (BD3-LM) solve, and what does it give up?**
**Short:** Block diffusion generates fixed-size blocks autoregressively while diffusing within each block, restoring cross-block KV-caching at the cost of block-granularity infilling.
...
**Short:** CFG extrapolates conditional and unconditional logits by a guidance scale; pushing it too high makes diffusion text repetitive and lose fluency, like over-saturated CFG images.
...
**Short:** Discrete diffusion operates directly on token IDs with cross-entropy loss, while continuous diffusion needs a lossy rounding step that has capped it below 1B-parameter scale.
...
**Short:** vLLM's PagedAttention, continuous batching, and speculative decoding all assume growing causal AR sequences, none of which hold for diffusion's bidirectional resolving process.
...
**Short:** Benchmark diffusion LMs with task-level output metrics like exact-match, pass@1, and BLEU, and only compare ELBOs against other diffusion models, never against AR perplexity.

Use task-level, output-based metrics that don't depend on the generative model's internal factorization: exact-match or F1 on QA datasets, pass@1/pass@10 on code-generation benchmarks (HumanEval, MBPP) by actually executing generated code, BLEU/ROUGE/BERTScore for translation/summarization against references, and human or LLM-as-judge pairwise preference (see [Evaluation & Benchmarks](../evaluation_and_benchmarks/evaluation_and_benchmarks.md)). If a likelihood-style number is needed for model comparison, report the ELBO explicitly labeled as an ELBO (an upper bound), and only compare ELBOs *between diffusion models*, never directly against AR exact perplexity.
...
**Short:** Training over all mask orderings lets a diffusion model learn both 'A is B' and 'B is A' directions, giving it an empirical edge on reversal-curse-style failures.
...
**Short:** AR's per-step overhead dominates short completions while diffusion offers little edge there, but for long sequences diffusion's near-constant step count beats AR until quadratic attention cost catches up.
...
**Short:** Diffusion LMs suit short, structured, latency-critical generation like code autocomplete, but lag AR on long-form generation, chain-of-thought parity, and serving maturity.
...
- [Vision-Language-Action Models & Robotics Foundation Models](../vla_and_robotics_foundation_models/vla_and_robotics_foundation_models.md) — the other major Phase 6 "new generation paradigm" module added alongside this one
...
- [Multimodal Models §4.2](../multimodal_models/multimodal_models.md) — continuous/image diffusion, the architecture family this work adapts from
- [Inference and Decoding](../inference_and_decoding/inference_and_decoding.md) — AR decoding strategies (sampling, KV-cache, speculative decoding) that diffusion-LM serving must reimagine
- [Reasoning Models](../reasoning_models/reasoning_models.md) — chain-of-thought reasoning, contrasted in Q11/Pitfall 10.6
- [Code Generation](../code_generation/code_generation.md) — fill-in-the-middle training, contrasted with diffusion's native infilling in Q8
