...
Concerns: OpenAI ToS prohibits using outputs to train competing models. Use carefully. Logit-level distillation and the full teacher-student tradeoff space are covered in [Knowledge Distillation & Model Merging](../knowledge_distillation_and_model_merging/knowledge_distillation_and_model_merging.md).
...
Quality matters more than quantity: 10K high-quality preference pairs with clear quality margins can outperform 100K noisy pairs where chosen and rejected are barely distinguishable. Production pattern: generate 5-10 responses per prompt and use the best/worst for maximum margin. How these pairs are consumed by DPO/RLHF training is covered in [Alignment & RLHF](../alignment_and_rlhf/alignment_and_rlhf.md).
...
**Short:** Self-Instruct bootstraps from about 175 human seed instructions, having an LLM generate and filter new instructions for diversity before generating responses.
...
**Short:** LIMA showed 1,000 carefully curated examples matched models trained on 52K+, since pre-training supplies knowledge and instruction tuning only teaches format.
...
**Short:** Evol-Instruct systematically increases instruction difficulty through operators like adding constraints or deepening, unlike Self-Instruct which generates from scratch.
...
**Short:** Greedy decoding at temperature=0 makes similar prompts produce near-identical responses, collapsing a dataset onto a handful of templates that look healthy by raw count.
...
**Short:** Code and math synthetic data can be executed and checked for correctness, filtering hallucinated solutions to reach near-100% accuracy versus 80-90% unverified.
...
**Short:** A synthetic data quality pipeline stages format validation, deduplication, difficulty filtering, factual verification, and a small human spot-check before training.
...
**Short:** Backtranslation starts from trusted expert outputs and generates the instruction that would produce each one, guaranteeing response quality since only the instruction is synthetic.
...
**Short:** Evol-Instruct applies operators like adding constraints, deepening, and increasing reasoning steps across several rounds, creating a natural difficulty curriculum.
...
**Short:** Assigning a different persona per generation batch shifts vocabulary, complexity, and topic selection at the distribution level, beyond what raising temperature alone can.
...
**Short:** Model collapse is a model trained on its own or similar outputs progressively losing diversity across generations, mitigated by keeping at least 30% human data in the mix.
...
**Short:** LIMA fine-tuned on 1,000 curated pairs was preferred over Alpaca's 52K examples 78% of the time, showing alignment is mostly about format, not new knowledge.
...
**Short:** Generating training data compliantly means using a permissively licensed teacher like Llama or Mistral, or generating with the same provider you'll fine-tune within.
...
**Short:** Dataset diversity is measured via embedding-cluster balance, topic coverage against a taxonomy, vocabulary and n-gram counts, and a deduplication rate above 10%.
...
**Short:** DPO preference pairs are best generated by scoring several sampled responses per prompt and picking the highest and lowest, since a large margin speeds convergence.
...
**Short:** Model collapse is detected by tracking distinct n-gram counts and topic entropy across training generations, flagging a drop of more than 10% in any diversity metric.
