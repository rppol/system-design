...
Multimodal RAG extends the standard text-based RAG pipeline (see [RAG Fundamentals](../rag_fundamentals/rag_fundamentals.md)) to handle documents that contain non-text content: images, charts, tables, diagrams, equations, and slides. Enterprise documents are rarely purely textual — PDFs contain embedded charts, research papers include figures, presentations are image-heavy, and technical documentation mixes diagrams with prose.
...
Retrieved context must include both text and images; the generation LLM must be vision-capable (architecture details in [Multimodal Models](../multimodal_models/multimodal_models.md)):
...
**Short:** Vision-to-text description embedded as text handles complex charts best, while direct CLIP/SigLIP image embedding is cheaper and enables true cross-modal search.
...
**Short:** CLIP contrastively trains paired text and image encoders into one shared vector space, letting a text query embedding directly nearest-neighbor search an image collection.
...
**Short:** Try lattice extraction for bordered tables and stream extraction for unbordered ones first, then fall back to a vision LLM for complex layouts they can't parse.
...
**Short:** A chart alone often omits its subject, units, or time period, so including roughly 200 tokens of surrounding text yields descriptions matching real user query vocabulary.
...
**Short:** CLIP aligns poorly with scientific diagrams, financial charts, and medical imaging since these are rare in its general web-image training data.
...
**Short:** Skip tiny icons, deduplicate repeated images, use lower detail for simple content, and cache descriptions, since vision calls dominate indexing cost at scale.
...
**Short:** Cap retrieved images to 2-3 per query, prefer detail="low" unless resolution matters, and pass the stored text description before falling back to the raw image.
...
**Short:** Score vision descriptions against human annotations for data accuracy, measure cross-modal retrieval recall@K, and verify chart-based answers cite the correct source image.
...
**Short:** Scanned PDFs needing OCR, vector-graphic schematics, multi-column papers, and tables with merged cells are the hardest formats to extract correctly.
...
**Short:** "fast" does direct text extraction with no layout awareness, "hi_res" runs layout analysis on rendered page images for best table accuracy, and "ocr_only" forces OCR everywhere.
...
**Short:** Embed simple lookup tables as markdown text, but route tables needing aggregation or filtering to text-to-SQL since embedding cannot support exact computation.
...
**Short:** Poor descriptions missing data values or axis labels cause retrieval mismatches, so measure numeric accuracy and semantic coverage against human-verified ground truth.
...
**Short:** At GPT-4o's roughly 1,105 tokens per high-detail page, 10,000 pages with 3 images each cost about $83 to index, and switching to a mini tier can raise cost, not lower it.
...
**Short:** Measure Recall@K on labeled (text query, expected image) pairs separately for the CLIP path and the vision-description path, then combine and rerank both.
...
**Short:** Chunk each extracted element (text, table, figure) as its own unit with linked surrounding context, never splitting a table or separating a figure from its caption.
...
**Short:** ColPali indexes page-image patch embeddings directly and scores with ColBERT-style MaxSim, skipping OCR and layout parsing while beating text-pipeline retrieval on ViDoRe.
