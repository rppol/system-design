...
Limitation: requires token-level probability access, which not every hosted API exposes.
  Self-hosted open-weight models: always available.
  OpenAI Chat Completions: available (logprobs / top_logprobs request parameters).
  Anthropic Messages API: not exposed — FLARE's confidence trigger cannot be
    implemented directly there; approximate it with a self-rated confidence
...
- Iterative web browsing and search driven by a reasoning model (it launched in Feb 2025 on a version of o3; the backing model has been rolled forward with OpenAI's frontier line since)
...
**Short:** Choose it for multi-hop queries where each retrieval depends on the prior result, accepting 5-10x latency for the ability to chain retrievals.
...
**Short:** Enforce a hard max-iteration limit in code, exclude already-retrieved document IDs from new searches, and add a wall-clock timeout independent of iteration count.
...
**Short:** FLARE retrieves mid-generation at low-token-probability points instead of at discrete checkpoints, but needs logprob access that Anthropic's API doesn't expose.
A: Standard agentic RAG retrieves at discrete checkpoints (before generation, or after deciding context is insufficient). FLARE retrieves mid-generation, triggered when the LLM encounters a low token-probability prediction — indicating a knowledge gap at that specific point in the text. FLARE is more precise: it retrieves exactly when and for what the model is uncertain. The limitation is that FLARE requires token-level probability access, which not every hosted API exposes. It is straightforward with self-hosted open-weight models (Llama, Mistral, Qwen) and works on OpenAI's Chat Completions API, which exposes `logprobs` and `top_logprobs`; Anthropic's Messages API does not return log-probabilities, so on Claude you have to approximate the trigger with a self-rated confidence prompt or an external uncertainty estimator.

**Q: How do you evaluate an agentic RAG system?**
**Short:** Measure per-iteration retrieval precision/recall, final-answer faithfulness and relevance, and efficiency metrics like average iterations and max-iteration hit rate.
...
**Short:** It's an LLM judgment on whether accumulated context answers the query; miscalibration in either direction causes max-iteration loops or premature incomplete answers.
...
**Short:** Compress accumulated context into summaries, keep only top-K ranked chunks across iterations, or maintain a hierarchical "knowledge so far" summary alongside recent context.
A: Three strategies. First, compression: after each iteration, summarize the accumulated context using an LLM to extract key facts, reducing from full chunks to a compact knowledge summary. Second, selection: instead of accumulating all retrieved chunks, maintain a ranked list and keep only the top-K most relevant across all iterations. Third, hierarchical context: keep a compressed "knowledge so far" summary alongside the full most-recent-iteration context, providing both to the LLM for sufficiency checking and generation. For very long agentic loops (10+ iterations), streaming partial answers or moving to a long-context model is an option — the current frontier tiers from Anthropic, OpenAI and Google all ship 1M-token context windows, an order of magnitude past the 128K assumed in Pitfall 2.

**Q: What makes an agentic RAG loop fail silently?**
**Short:** A permissive sufficiency check or a generation model that doesn't flag gaps lets the loop finish with a confident answer built on incomplete context.
...
**Short:** Classify queries by comparative, referential, or aggregation language and route roughly 20% flagged as multi-hop to agentic RAG, the rest to standard RAG.
...
**Short:** Trace every iteration's query, retrieved IDs, and sufficiency result, and alert when average iterations per query creeps toward the 5-iteration cap.
...
**Short:** Agentic RAG is a ReAct instantiation where the tool action is specifically retrieval and the reasoning centers on judging information sufficiency.
...
**Short:** Track seen document IDs, filter them out of new searches at the vector DB level, and terminate early if two consecutive iterations return an identical set.
...
**Short:** Write precise per-tool descriptions with usage examples, measure accuracy on labeled (query, expected_tool) pairs, and consolidate tools if accuracy falls below 80%.
...
**Short:** Log every sub-query, retrieved document and sufficiency verdict per iteration, then scan traces for circular retrieval, premature termination, or maxed-out loops.
...
**Short:** Independent sub-queries in a joined (non-chained) multi-hop question can dispatch concurrently as a DAG, cutting latency to the DAG's depth, not its node count.
...
**Short:** Track iteration efficiency, multi-hop accuracy, unnecessary-iteration rate on single-hop queries, context utilization, and cost-per-correct-answer versus standard RAG.
