...
**Short:** CRAG scores retrieved-document relevance before generation and falls back to web search on low-quality retrieval, preventing hallucinations from irrelevant context.
...
**Short:** A relevance evaluator scores each document; all above threshold use KB directly, mixed scores trigger sentence-level refinement, and all below threshold trigger web search.
...
**Short:** Build a 200-300 labeled (query, document, relevance) set, pick the F1-maximizing threshold biased toward recall, then tune it from the live web-search trigger rate.
...
**Short:** Unvetted sources can be biased or wrong, raw queries can leak sensitive business data, and regulated industries may contractually prohibit citing external sources at all.
...
**Short:** Measure the evaluator's precision/recall on labeled document pairs, its trigger-rate accuracy on should-use-web labels, and end-to-end hallucination reduction on out-of-KB queries.
...
**Short:** For "Ambiguous" documents, CRAG re-scores at sentence granularity and keeps only sentences above threshold instead of discarding or using the whole document.
...
**Short:** A lightweight cross-encoder evaluator adds under 100ms per batch, while a web search fallback adds 500ms-3s and needs a strict timeout with a KB-context fallback.
...
**Short:** Start from a pre-trained cross-encoder like ms-marco-MiniLM-L-6-v2, then fine-tune on 500-1000 domain-labeled (query, document) pairs for specialized corpora.
...
**Short:** Evaluation adds 50-300ms and web fallback 500ms-3s; it's worth it when hallucination on out-of-KB queries drops more than 20% and wrong answers carry real user harm.
...
**Short:** A cross-encoder jointly evaluates query and document pairs and cuts false-positive relevance judgments by 40-60% versus thresholding raw bi-encoder cosine similarity.
...
**Short:** Refine ambiguous documents at sentence level, optionally combine them with web search results, or treat the top ambiguous score as incorrect to force a web fallback.
...
**Short:** Insert the relevance evaluator as a gate after retrieval and before generation, roll it out in a logging-only monitoring mode first, then enable the correction behavior.
