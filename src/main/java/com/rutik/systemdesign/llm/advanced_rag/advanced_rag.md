...
- **Diminishing returns on context**: Adding more retrieved documents doesn't always help; too much irrelevant context actively degrades generation quality (see [Context Engineering](../context_engineering/context_engineering.md)).
...
**Short:** Score retrieval (context recall/precision) and generation (faithfulness/answer relevance) separately using a golden test set and RAGAS or TruLens.
...
**Short:** Start with standard RAG and only add complexity like agentic or Graph RAG once evaluation shows it falls below your accuracy target.
...
**Short:** Context recall measures retrieval completeness, context precision measures retrieval focus, faithfulness measures hallucination, and answer relevance measures response quality.
...
**Short:** Multi-query/HyDE add 1.5-2x latency for modest gains, agentic RAG adds 5-30x latency, and Graph RAG adds 100x indexing cost for global-query quality.
...
**Short:** Extra chunks lower context precision faster than they raise recall, and "lost in the middle" attention makes buried evidence get ignored or misused.
...
**Short:** HyDE hurts on factual, entity-specific queries where the generated hypothetical answer invents wrong facts, pulling retrieval toward matching documents.
...
**Short:** Use Graph RAG when queries need multi-hop reasoning over entity relationships or corpus-wide summarization that flat vector similarity cannot capture.
...
**Short:** CRAG scores each retrieved document as Correct, Incorrect, or Ambiguous and triggers web search on bad retrievals to stop hallucination from irrelevant chunks.
...
**Short:** Self-RAG fine-tunes a 7B+ model on ~150K reflection-token-annotated examples, though CRAG often gets 80% of the benefit without any training cost.
...
**Short:** Combine an iteration cap, a confidence threshold, information-gain checks, and a token budget, stopping at whichever condition triggers first.
...
**Short:** It rewrites the user query via HyDE, decomposition, step-back prompting, or expansion to bridge the vocabulary gap between question and document language.
...
**Short:** Parse text, tables, and images with layout-aware tools, embed each modality with context or descriptions, and generate answers with a multimodal LLM.
...
**Short:** FLARE monitors token-level generation confidence and triggers retrieval mid-answer whenever the model's predicted continuation probability drops low.
...
**Short:** It embeds individual sentences for precise matching, then expands each hit to a surrounding window, avoiding the embedding dilution of large fixed chunks.
...
**Short:** Global queries synthesize answers from pre-computed community summaries, while local queries traverse an entity's neighborhood subgraph directly.
...
**Short:** It prepends an LLM-generated preamble naming the document, entity, and period to each chunk, fixing self-containment rather than diluting embeddings with size.
