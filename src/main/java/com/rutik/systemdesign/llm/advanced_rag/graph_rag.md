...
Graph RAG (Microsoft, 2024) extends [standard RAG](../rag_fundamentals/rag_fundamentals.md) by extracting a knowledge graph from documents — entities as nodes, relationships as edges — and using that graph structure for retrieval. Documents are not just indexed as text chunks; they are analyzed for their entities, relationships, and the communities those entities form.
...
Fix: Apply entity normalization before graph construction. Use a combination of string matching, [embedding similarity](../embeddings_and_similarity_search/embeddings_and_similarity_search.md), and LLM-assisted deduplication. Poor deduplication fragments the graph and degrades community detection.
...
**Short:** It builds a knowledge graph, clusters it into communities via Leiden, and summarizes each so global corpus-wide theme queries get answers no single chunk contains.
...
**Short:** Leiden partitions entities into hierarchical, modularity-maximizing communities with no disconnected subsets, unlike Louvain, giving Graph RAG coherent multi-level summaries.
...
**Short:** Global queries map-reduce over community summaries, while local queries extract the subgraph around identified entities and generate from its linked source chunks.
...
**Short:** Entity extraction (one LLM call per chunk), community summarization (one call per community), and embedding, totaling roughly $8-9 per million source tokens indexed.
...
**Short:** Treating name variants like "Microsoft" and "MSFT" as separate nodes fragments the graph, splits connections, and produces incoherent community summaries.
...
**Short:** Extract entities from new documents only, re-run community detection just on the affected sub-graph, and regenerate summaries only for communities that changed.
...
**Short:** LazyGraphRAG builds the graph with classical NLP at no LLM cost and defers all LLM calls to query time, cutting global-query cost over 700x at comparable quality.
...
**Short:** Thematic, relationship, and trend queries benefit most, while specific factual lookups, very recent data, and code documentation queries fare better with standard RAG.
...
**Short:** Fine-grained levels give narrow but detailed answers and coarse levels give broad thematic coverage, so the best level must be chosen per query distribution, not fixed.
...
**Short:** Judge global-query community summaries with LLM-as-judge on comprehensiveness, diversity and empowerment, and score local queries with standard recall and faithfulness.
...
**Short:** Traditional KGs are human-curated with high precision and low recall for structured queries, while Graph RAG's LLM-built graph trades some precision for automated recall.
...
**Short:** Sweep several resolution values, inspect community coherence and query coverage on labeled queries, and pick the value producing coherent, comprehensive results.
...
**Short:** Extraction and deduplication errors compound multiplicatively into community summaries, so a 10% extraction error rate can yield 30-40% unreliable summaries.
...
**Short:** Leiden clusters densely connected entities into communities summarized at multiple resolution levels, with L1 or L2 typically balancing detail against thematic coverage best.
...
**Short:** Move to a distributed graph database like Neo4j, batch or parallelize Leiden and community summarization, and switch to a cheaper model once community counts exceed 1000.
...
**Short:** Extract entity subgraphs, pull relevant community summaries, and run standard vector search in parallel, then let a reranker combine all three before generation.
...
**Short:** High hallucinated-entity rates, unresolved duplicate variants, and incoherent communities signal a poor knowledge graph that misleads the LLM more than no graph at all.
