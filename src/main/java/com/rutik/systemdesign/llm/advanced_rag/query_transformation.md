...
**Short:** HyDE embeds an LLM-generated hypothetical answer instead of the query, bridging the terse-query-to-verbose-document gap in domains like academic or legal text.
...
**Short:** A hallucinated hypothesis, a style or register mismatch with real documents, and a length mismatch can each steer HyDE's embedding away from the right documents.
...
**Short:** Generating 3-5 alternate phrasings catches documents worded differently than the original query, at the cost of N times retrieval plus an essential dedup-and-rerank step.
...
**Short:** Step-back adds a more general query alongside the specific one for background context, while decomposition splits a multi-hop question into sequential sub-questions.
...
**Short:** Skip it for already-precise technical queries, latency budgets under 200ms, or when the rewriter hallucinates assumed context that silently misdirects retrieval.
...
**Short:** Compare context recall@K with and without each transformation on a 100-200 pair golden set, and only keep techniques that lift recall by more than 5%.
...
**Short:** Dedup by document ID before reranking, keeping the highest score across all query variants, so results and context aren't skewed by repeated chunks.
...
**Short:** It fuses multiple query variants' rankings via `1/(k+rank)` Reciprocal Rank Fusion with k=60, so documents ranked highly across variants outscore a simple merge-and-dedup.
...
**Short:** Match the technique to the query distribution: rewriting for terse conversational queries, HyDE for vocabulary mismatch, multi-query for broad queries, decomposition for multi-hop.
...
**Short:** Transformations only affect semantic similarity, so the original query's metadata filters must still be applied to every generated variant to preserve intended scope.
...
**Short:** A single cheap-model rewrite call costs well under a cent per query even at a million queries a day, dwarfed by retrieval and reranking cost at that scale.
...
**Short:** Hallucinated hypotheses, distributional mismatch, and recency blindness on rapidly changing topics; detect by comparing HyDE recall to direct-embedding recall per query type.
...
**Short:** Generate one sub-query per logical condition, answer each independently, then apply the explicit AND/OR/comparison join as a final synthesis step.
A: Generate one sub-query per condition, answer each independently, then apply the logical join (intersection or comparison) as an explicit final step. A question like "Which of our products had both above-average margin AND declining customer satisfaction in Q3?" needs at least three sub-queries: (1) "What is average product margin?", (2) "Which products had above-average margin in Q3?", (3) "Which products had declining customer satisfaction in Q3?" — then the intersection. The decomposition strategy is: identify the logical structure (AND, OR, greater-than, trend comparisons); generate one sub-query per condition; retrieve and answer each independently; then synthesize with the LLM by providing all sub-answers and asking for the final intersection or comparison. The decomposition LLM must produce executable sub-questions, not vague paraphrases. Validate decomposition quality by checking that answering all sub-questions in sequence is logically sufficient to answer the original question.
...
**Short:** Track retrieval recall, context precision, final answer accuracy, and faithfulness together, since better recall doesn't always translate into better answers.
...
**Short:** The system rewrites the turn into a standalone question using chat history before embedding it, since a bare pronoun carries no retrievable signal on its own.
...
def transform_and_retrieve(ticket_text: str, query_type: str) -> list[Document]:
    ...
    if query_type == "precise_technical":
        ...
        # Retrievers are Runnables — invoke them.
