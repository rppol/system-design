...
- Single index holding dense + sparse vectors is the recommended pattern (fused server-side);
  separate dense and sparse indexes with client-side RRF is the alternative
- Sparse side uses BM25-style sparse weights or Pinecone's `pinecone-sparse-english-v0`;
  neither is normalized to the dense score range, so alpha weighting is required
...
**Short:** Dense and BM25 fail in opposite ways, semantic paraphrase versus exact identifiers, so fusing them via RRF beats either alone whenever the query mix contains both types.
...
**Short:** Summing 1/(k+rank) across each retriever's ranked list is scale-invariant and needs no weight tuning, unlike linear score combination which requires normalizing incomparable scales.
...
**Short:** A multi-layer proximity graph lets search start at a coarse top layer and descend to fine-grained neighbors at the bottom, giving O(log N) search time and 95-99% recall.
...
**Short:** Pre-filtering restricts the ANN search itself to matching vectors, while post-filtering retrieves an unconstrained top-K first and can starve results under a selective filter.
...
**Short:** Preserve identifiers like "E-2045" as single tokens instead of letting the tokenizer split them into common substrings, and use phrase matching so BM25 requires the exact sequence.
...
**Short:** SPLADE trades a larger sparse index and a self-hosted fine-tuned model for roughly 4 points of BEIR nDCG@10 over BM25, while keeping inverted-index-level search speed.
A: Traditional BM25 uses raw term frequencies and IDF; it's non-learned and requires exact keyword matches. SPLADE (Sparse Lexical and Expansion model) is a learned sparse retrieval method that expands both query and document with related terms (a document about "cardiac arrest" gets "heart attack" added to its sparse representation), enabling semantic matching while maintaining the efficiency of inverted index retrieval. SPLADE vs. BM25: published BEIR comparisons put SPLADEv2 around 0.47 nDCG@10 against a BM25 baseline in the 0.43 range — roughly 4 points absolute, or under 10% relative — while maintaining BM25-level speed; it requires a fine-tuned SPLADE model and an index storing expanded sparse vectors (larger than a BM25 inverted index). In practice: for RAG systems, SPLADE + dense hybrid often outperforms BM25 + dense hybrid, at the cost of a learned sparse model.

**Q: How does metadata filtering interact with hybrid retrieval?**
**Short:** Pre-filter by metadata first to shrink the candidate space, then run dense and sparse retrieval within that smaller scope, since filtering often reduces millions of vectors to thousands.
...
**Short:** Skip hybrid when queries are entirely semantic or entirely exact-match, the corpus is small enough for brute-force search, or the BM25 index itself is poorly tokenized and maintained.
...
**Short:** The default k=60 is validated across many TREC tasks and only needs tuning if your own eval shows a strong signal; smaller k amplifies top-rank dominance, larger k flattens it.
...
**Short:** Short queries favor BM25 because there is too little context for a meaningful embedding, while long queries favor dense retrieval since it captures intent BM25 dilutes with stopwords.
...
**Short:** Apply Maximal Marginal Relevance to pick each next chunk by relevance minus its similarity to chunks already selected, since independent ranking never penalizes near-duplicate results.
