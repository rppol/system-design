...
**Short:** Filter candidates by MTEB Retrieval score, validate on a 50-200 pair domain eval set, weigh API vs self-hosted constraints, and check context window against your chunk size.
...
**Short:** MTEB spans 58 tasks in 8 categories, but only the Retrieval subset, built from BEIR datasets like MSMARCO and Natural Questions, actually predicts RAG performance.
...
**Short:** STS models score similar-length, similar-style sentence pairs, while retrieval models train on asymmetric short-query-to-long-passage pairs, so only retrieval models suit RAG.
...
**Short:** MRL trains embeddings so the first N dimensions alone are meaningful, letting a 3072-dim vector truncate to 512 dims with only about 2-5% quality loss.
...
**Short:** Continuing training on 5K-50K domain (query, positive, negative) triples via MultipleNegativesRankingLoss is worth it only when domain recall@10 falls under 70%.
...
**Short:** Swapping embedding models can leave vectors the same shape but in a different semantic space with no error, so store model_name in index metadata and enforce it at query time.
...
**Short:** Chunk to fit the model's context window as the default, or reach for a long-context model like bge-m3 or text-embedding-3-large, both 8192 tokens, when boundary loss dominates.
...
**Short:** Compare recall@10, MRR@10 and NDCG@10 for the base and fine-tuned model on a 100-200 labeled eval set, and require over 5% recall@10 gain to justify the added maintenance.
...
**Short:** At 10M chunks of 400 tokens each, self-hosting bge-base on one GPU costs about $8 versus $80 for text-embedding-3-small and $520 for 3-large in API fees.
...
**Short:** BGE prepends a query-only instruction prefix, "Represent this sentence for searching relevant passages:", so the model encodes queries into retrieval mode distinct from documents.
...
**Short:** Dense embeddings win on paraphrased conceptual queries while sparse BM25 wins on exact keywords like SKUs and regulation IDs, so production systems combine both via hybrid fusion.
...
**Short:** Mine hard negatives by retrieving top-50 BM25 or base-model candidates and keeping the high-scoring non-relevant ones, since random negatives are too easy and training plateaus fast.
...
**Short:** Quality degrades gradually because MRL front-loads information into leading dimensions; OpenAI's own anchor shows 3-large at 256 dims still beating full-size ada-002 at 1536.
...
**Short:** Low-resource languages get weaker embeddings from sparser training data, and cross-lingual retrieval accuracy typically drops 10-20% versus monolingual retrieval even for strong pairs.
...
**Short:** Measure recall@10 across MRL-truncated sizes like 256, 384, 512 and 768 on your domain eval set and stop increasing dimensions once the gain per doubling falls under about 1%.
