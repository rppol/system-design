...
**Short:** Start at 300-500 tokens, then measure recall@10 on a labeled (query, expected_chunk) eval set at several sizes (100, 300, 500, 1000) and pick whichever wins on your corpus.
...
**Short:** Splitting at drops in adjacent-sentence embedding similarity produces topic-coherent chunks whose embeddings match queries more precisely than arbitrary character-count cuts.
A: Fixed-size chunking cuts text at arbitrary character/token positions, frequently splitting mid-sentence or mid-argument. The resulting chunk's embedding represents a partial idea, which matches queries for that idea poorly. Semantic chunking detects topic shifts by measuring embedding similarity between adjacent sentences — when similarity drops below a threshold (topic changes), a chunk boundary is inserted. The resulting chunks each represent a coherent topic, producing embeddings that precisely capture that topic's meaning. Size the expected win carefully: Chroma's 2024 chunking evaluation found the whole spread between the best and worst chunking strategy was about 9 points of recall, and its semantic chunkers were only marginally ahead of well-configured recursive splitting. The cost is the need to embed sentences during indexing.

**Q: What is hierarchical chunking and what problem does it solve?**
**Short:** It embeds small child chunks for precise retrieval while storing larger parent sections that are returned as LLM context, resolving the precision-vs-completeness tradeoff.
...
**Short:** Repeating the last N tokens of one chunk at the start of the next keeps boundary-straddling facts retrievable in at least one chunk, at the cost of 10-15% more index size.
...
**Short:** Code chunks at function or class boundaries via an AST parser like tree-sitter, since sentence or paragraph splitting cuts mid-function and corrupts the resulting embedding.
...
**Short:** At minimum document_id, chunk_id, source_path, section_title, page_number, and document_date, since metadata filtering often beats embedding quality for precision-critical queries.
...
**Short:** Chunks under 50 tokens are dominated by function words while chunks over 1000 tokens average multiple ideas into one vector, leaving 100-500 tokens as the precise range.
...
**Short:** It sets which adjacent-sentence similarity drops count as boundaries, so the default of 95 turns only the least-similar 5% of sentence transitions into chunk splits.
...
**Short:** Build 100-200 labeled (query, expected_chunk) pairs, measure recall@10 per candidate chunking strategy, and inspect 20 retrieved chunks for mid-sentence truncation.
...
**Short:** A 512-token model like bge-small needs chunks under 400-450 tokens with headroom, while an 8192-token model like text-embedding-3-large can take 1000-2000-token chunks.
...
**Short:** Advancing a fixed window by less than its full width, such as stride 250 on a 500-token window, gives 50% overlap and roughly doubles chunk count for dense narrow-fact text.
...
**Short:** Split at the highest-level headers first, then subdivide, storing parent_section_id and depth_level as metadata so a retrieved child chunk can expand to its parent section.
...
**Short:** Chroma's 2024 evaluation measured only about 9 points of recall separating the best and worst chunker, with the gain largest on heterogeneous corpora and smallest on uniform ones.
A: Smaller than most people assume — Chroma's 2024 chunking evaluation found the spread between the best and worst chunking strategy was about 9 points of recall, with semantic chunkers only marginally ahead. In that study the LLM-driven and cluster-based semantic chunkers reached roughly 0.91-0.92 recall, close enough to a well-configured recursive splitter that the ranking is corpus-dependent rather than a law. The direction of the effect is real — chunks aligned to topic boundaries produce sharper, more distinctive embeddings — and it is largest on heterogeneous collections where sections shift meaning frequently, smallest on homogeneous ones (all structured data sheets, all consistent news articles) where fixed-size boundaries accidentally coincide with semantic ones. The cost is materially longer indexing time because each sentence must be embedded to detect topic shifts. Practical guidance: measure the gap on a sample of your specific corpus before committing to the higher indexing compute; there is no transferable percentage.

**Q: How does late chunking work and when should you use it?**
**Short:** Embedding the whole document first with a long-context encoder, then splitting the resulting token embeddings into windows, lets every chunk retain full-document context.
A: Late chunking embeds the entire document first using a long-context encoder, producing token-level embeddings for the full document, then splits the resulting embedding sequence into chunk-sized windows — each chunk embedding retains global document context because the attention mechanism saw the full document before the split. This differs from standard chunking where each chunk is embedded in isolation without awareness of the surrounding document. Use late chunking when: document coherence is critical (each chunk must be understood in the context of the whole document), your embedding model supports long sequences (jina-embeddings-v2 supports 8192 tokens), and chunks from isolated embedding produce coherence artifacts. The gain is real but highly dataset-dependent: Günther et al. (2024, arXiv 2409.04701) report late chunking never losing to naive chunking on BEIR with jina-embeddings-v2-small at 256-token chunks, but the size of the win ranges from nothing (Quora, 87.19 nDCG@10 either way) to +6.5 points (NFCorpus, 23.46 to 29.98).

**Q: What overlap strategy works best for legal and regulatory documents?**
**Short:** Legal and regulatory chunks need 15-25% overlap versus the standard 10%, plus the governing definition block attached as structured metadata since terms are cited paragraphs away.
...
**Short:** Hold the embedding model and retriever fixed, vary only the chunking strategy, and measure recall@5/@10 against a labeled (question, gold_passage) set of 100-200 pairs.
...
**Short:** Its separator list treats `##` headings and pipe tables as ordinary text, so it can split a table's header row away from its data rows or ignore heading hierarchy entirely.
