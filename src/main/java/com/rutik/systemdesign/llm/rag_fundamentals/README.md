...
<!-- study-paths
senior: README.md, chunking_strategies.md, embedding_models.md, retrieval_methods.md, reranking.md
principal: README.md, embedding_models.md, retrieval_methods.md, reranking.md
...
**Short:** RAG retrieves and injects fresh, private documents into the prompt at query time, avoiding fine-tuning's retraining cycle and adding automatic source citations.
...
**Short:** Decompose the question into sub-questions, retrieve and answer each independently, then combine the sub-answers, or map-reduce across all retrieved documents.
...
**Short:** Retrieval failure (doc never found), ungrounded context (doc lacks the answer), and generation failure (LLM hallucinates despite good context) each need a separate metric to diagnose.
...
**Short:** Give the model an exact refusal string for missing context, and skip generation entirely when the top retrieval score falls below a calibrated similarity threshold.
...
**Short:** Stuffing a full corpus into a 1M-token window costs far more per query, cannot fit a multi-million-document corpus at all, and still suffers lost-in-the-middle attention decay.
...
**Short:** A production baseline needs a parser, sentence-boundary chunker, an embedding model, a vector DB, hybrid dense+BM25 retrieval, a reranker, and a grounded generation prompt.
...
**Short:** Smaller chunks (128-256 tokens) sharpen retrieval precision while larger chunks (512-1024) preserve more surrounding context, with 256-512 tokens the empirical sweet spot.
...
**Short:** Alpha blends dense and sparse scores from 0 (pure BM25) to 1 (pure dense) and must be tuned per query type, while RRF fuses ranked lists via 1/(k+rank) with no tuning needed.
...
**Short:** Rewriting the query before retrieval, via expansion, HyDE's hypothetical-answer embedding, or decomposition into sub-queries, helps most when the raw query is short or vague.
...
**Short:** A bi-encoder embeds query and document separately for fast, pre-indexable ANN search, while a cross-encoder scores the concatenated pair jointly but must run per candidate.
...
**Short:** A small cross-encoder like ms-marco-MiniLM-L-12-v2 adds roughly 30ms for 30 pairs, while a 0.6B multilingual reranker runs an order of magnitude slower on the same batch.
...
**Short:** Pick Chroma or pgvector under 1M vectors, Qdrant, Weaviate or Pinecone from 1M-100M, and Milvus or managed Pinecone for distributed scale beyond 100M vectors.
...
**Short:** LLMs attend more to the start and end of context and under-attend the middle, so mitigate by ordering the most relevant chunks first and last and keeping the chunk count small.
...
**Short:** Pre-filtering restricts the ANN search itself so a full top-K of eligible chunks returns, while post-filtering discards non-matches afterward and can starve the context.
...
**Short:** A multilingual embedding model like BGE-M3 places same-meaning text from different languages near each other in vector space, while BM25 still catches exact identifiers.
...
**Short:** Measure retrieval metrics (Recall@K, MRR, Precision@K) and generation metrics (faithfulness, relevancy, correctness) independently, typically via an LLM-judge framework like RAGAS.
...
**Short:** Chunking splits whatever string the parser produced, so a scrambled multi-column PDF, an empty scanned page, or a flattened table poisons every downstream chunk regardless of size.
...
```
  ...
  │  Latency: ~30ms for 30 pairs on a V100 (model card:      │
  │            960 docs/sec); faster on newer GPUs           │
  └──────────────────────────────────────────────────────────┘
             |
             v
  ┌──────────────────────────────────────────────────────────┐
...
The cross-encoder reranker takes (query, document) pairs and computes a joint relevance score — unlike bi-encoders which compute embeddings independently, the cross-encoder can model the interaction between query terms and document terms explicitly. It is significantly more accurate — in this case study the reranker moved NDCG@5 from 0.71 to 0.79 — but also slower: the ms-marco-MiniLM-L-12-v2 reranker here costs about 30ms for 30 pairs on top of a 24ms p99 HNSW search — that is the model card's own throughput figure, 960 docs/sec on a V100, and newer accelerators do better, but the case study names no GPU so read it as the card's number rather than a measurement of this deployment — and a larger 0.6B reranker would cost an order of magnitude more. The standard practice is "retrieve many, rerank fewer": HNSW retrieves the top-30 candidates cheaply, the cross-encoder reranks them to the top-5 that are actually sent to the LLM. Use a cross-encoder when the quality of top-5 context matters more than throughput — almost always true for enterprise RAG where hallucinations from irrelevant context are costly.
