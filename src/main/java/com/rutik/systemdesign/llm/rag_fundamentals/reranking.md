...
**Short:** A cross-encoder scores query and candidate jointly via full self-attention as a second stage, capturing interactions a bi-encoder's separate vectors miss, at added per-candidate latency.
...
**Short:** It needs a full forward pass per (query, document) pair since the joint representation depends on the query, making it O(N) per query versus a bi-encoder's precomputed O(1).
...
**Short:** ColBERT precomputes one embedding per document token like a bi-encoder, then scores via MaxSim, giving richer interaction than a single pooled vector without a per-query forward pass.
...
**Short:** 50-100 candidates is the standard range, since a reranker barely improves on a top-10 pool but shows its full benefit by top-100, at a roughly linear latency cost.
...
**Short:** Collect (query, relevant_doc, irrelevant_doc) triples from expert annotation, LLM generation, or click logs, then apply contrastive loss over 2-5 epochs at a low learning rate.
...
**Short:** Compare MRR@5 and NDCG@5 before and after reranking on a labeled (query, document_pool, relevance) test set, and expect at least a 10-15% NDCG@5 gain to call it working.
...
**Short:** The reranker's output count is sized to the LLM's context window and chunk size, typically 3-10 chunks, since more chunks fill the window but irrelevant ones still degrade generation.
...
**Short:** A weak embedding model paired with a strong reranker can beat a strong embedding model alone, so evaluation must score the retriever-plus-reranker pipeline together, not retrieval alone.
...
**Short:** An LLM reranker is worth it only for explainable scores or an undertrained domain cross-encoder, since it costs far more per query and runs slower at scale than a cross-encoder.
...
**Short:** Expanding the candidate pool shows diminishing returns, roughly 3-5% more recall@5 from top-20 to top-50 but only 1-2% more from top-50 to top-100, so retriever recall@20 guides the cutoff.
...
**Short:** Pointwise scores each document alone in one forward pass, pairwise compares documents two at a time at O(n^2) cost, and listwise scores the whole candidate list jointly for the best ranking.
...
8. **Decide how the reranked order is laid out in the prompt** — the reranker hands you a ranking, not a prompt; where those chunks sit in the context window is a separate decision governed by the "lost in the middle" effect, covered in [context engineering](../context_engineering/context_engineering.md). If the top-5 still contain near-duplicates, apply [MMR](retrieval_methods.md) over the reranked pool before the layout step.
...
3. [Hybrid retrieval](../advanced_rag/advanced_rag.md) for support-specific queries: support queries mix semantic ("my account keeps getting locked") and exact-match ("error code 403 when logging in") queries. BM25 handles error code matching precisely — dense retrieval alone missed 28% of error-code queries.
