...
**Short:** The most common LLMOps mistake is adding observability and eval tooling reactively after an incident, when a golden dataset should exist before the first deployment.
...
**Short:** Eval-gated deployment stores prompts in Git and blocks any PR whose new version drops the primary quality metric more than 2% against the golden baseline.
...
**Short:** LLM costs are calculated from per-call token counts tagged by customer and feature, alerting when a single request costs 5x the median or spend outpaces budget.
...
**Short:** RAG pipelines are tracked on faithfulness, answer relevancy, context precision and recall, plus retrieval and generation latency, alerting hardest on faithfulness drops.
...
**Short:** Comparing prompt versions at scale uses an LLM judge scoring both on a golden dataset, or a small live traffic split analyzed for statistical significance.
...
**Short:** A W&B Sweep's Bayesian optimization fits a surrogate model to prior trials and picks the next promising hyperparameter combination, needing far fewer runs than grid search.
...
**Short:** Prompt injection detection scores production traces for suspicious patterns like ignore-previous-instructions or unexpected tool calls, and re-runs known attacks weekly.
...
**Short:** Simple structured logging suffices under roughly 1,000 calls a day with one engineer and no paying customers, while LLMOps becomes necessary once stakes rise.
...
**Short:** Offline evaluation gates deployments with a fixed golden dataset, while online evaluation samples real production traffic to catch distribution shifts after deployment.
...
**Short:** LangSmith exports traces in OpenTelemetry format via OTLP, though its token and cost attributes need custom OTel attributes since they aren't part of the standard schema.
...
**Short:** The no-baseline problem is deploying a prompt without first building a golden dataset, leaving no objective way to tell whether a later revision actually improved anything.
...
**Short:** Handling PII in traces means sampling instead of logging everything, scrubbing named entities before storage, capping retention, and self-hosting for regulated industries.
...
**Short:** Eval-gated deployment runs every prompt-file change through the golden dataset in CI and blocks the PR if any quality metric drops more than 2% from baseline.
...
**Short:** Cost attribution tags every call with customer and feature identifiers, then a daily aggregation query rolls per-tag spend into billing and optimization reports.
