...
**Short:** ReAct interleaves reasoning, action, and observation, reducing hallucination and impulsive tool choices versus direct calls.
...
**Short:** Reflexion adds a post-failure self-critique stored in memory, letting later attempts learn from a verifiable failure signal.
...
**Short:** Tree of Thoughts searches multiple candidate reasoning branches and prunes weak ones, worth it only when early choices matter most.
...
**Short:** Self-consistency samples several reasoning chains at temperature above zero and takes the majority final answer.
...
**Short:** A scratchpad is free-form scratch space for hidden reasoning; ReAct's Thought field is structured and drives the next action.
...
**Short:** Loops happen when the agent repeats a tool call after an unhelpful result instead of updating its reasoning.
...
**Short:** Default to ReAct, add Reflexion when success is verifiable, and reserve Tree of Thoughts for large planning searches.
...
**Short:** Chain-of-thought is pure in-context reasoning; ReAct is chain-of-thought interleaved with real tool actions and observations.
...
**Short:** Native thinking is model-level reasoning in an encrypted scratch space, replacing a prompted ReAct Thought field.
...
**Short:** Dropping a signed thinking block before a tool result gets the request rejected, since the API verifies its signature.
...
**Short:** Reasoning before acting lets the model's Thought tokens condition the Action's arguments, improving tool selection.
...
**Short:** ReAct adapts step by step but drifts on long tasks; Plan-and-Execute holds strategic coherence but needs replanning.
...
**Short:** Detect exact-duplicate tool-and-argument pairs and inject a message forcing the model to try a different approach.
...
**Short:** Cut ReAct latency by trimming Thought verbosity, using a cheaper model per step, and parallelizing independent calls.
A: ReAct's latency is N_steps × LLM_call_latency, where each step adds 1-3 seconds for a frontier model. Reduction strategies: (1) Thought verbosity control — add to the system prompt "Keep your Thought concise — 1-2 sentences maximum. Do not explain your reasoning at length."; verbose Thoughts use more output tokens and slow generation by 30-50%; (2) Smaller model for executor — if the ReAct loop uses a frontier model for every step, switch to a smaller model (gpt-5.4-nano, Claude Haiku 4.5) for standard tool calls; reserve the frontier model only for the final synthesis step; (3) Parallel tool calls — emit multiple independent tool calls in one response to eliminate one full LLM round-trip per parallel call; (4) Early termination — after each step check if the accumulated information already suffices to answer; inject a "can you answer now without more tool calls?" gate to short-circuit loops. A well-optimized ReAct agent can achieve 40-60% latency reduction vs. naive implementation while maintaining most quality.

**Q: How do you use ReAct with structured tool outputs to improve observation parsing?**
**Short:** Structured JSON observations let the model reference exact fields instead of parsing ambiguous prose.
...
**Short:** Test faithfulness by predicting the Action from the Thought alone, or by ablating Thoughts and checking success rate.
