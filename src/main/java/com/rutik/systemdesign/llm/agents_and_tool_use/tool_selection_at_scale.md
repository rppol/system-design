...
### 4.6 Provider-native deferred loading

Sections 4.2–4.5 all assume you own the filtering layer. Both major providers now
ship one server-side, which for many catalogues removes the need to build it.

On the Claude API you send **every** tool definition on every request and mark the
ones that should not enter the context window:
...
tools = [
    ...
    {"name": "github_create_pr", "description": "...", "input_schema": {...},
     "defer_loading": True},
    # ... up to 10,000 further deferred tools
]
```

Deferred definitions are excluded from the system-prompt prefix. When the model
needs one it calls the search tool; the API returns `tool_reference` blocks (5 per
search by default) and expands them into full definitions inline. Two variants
exist: `tool_search_tool_regex_20251119`, where the model writes Python
`re.search()` patterns (200-character limit), and `tool_search_tool_bm25_20251119`,
where it writes natural-language queries (500-character limit). Both search names,
descriptions, argument names and argument descriptions — the same four fields
`embedding_text()` concatenates in 6.2.

Three constraints are worth memorising, because each is a 400 or a silent
regression. At least one tool must stay non-deferred, so deferring everything
including the search tool is rejected. A deferred tool cannot also carry
`cache_control`. And because deferral touches only the inline conversation and
never the cached prefix, **prompt caching survives** — which is exactly what a
client-side registry that rewrites the `tools` array every turn destroys, a cost
the naive-versus-RAG table below does not price. Anthropic reports a typical
five-server setup (GitHub, Slack, Sentry, Grafana, Splunk) consuming ~55K tokens
of definitions, cut by over 85% with search enabled, and places the selection
accuracy cliff at 30–50 available tools. OpenAI's Responses API exposes the same
idea as a hosted `tool_search` tool.

The catalogue-quality work is unchanged either way: namespacing (Principle 3),
description quality (Principle 2) and version pinning (Principle 4) decide whether
a *server-side* search finds the right tool exactly as they decide whether your
FAISS index does. Build the client-side pipeline when you need a retrieval method
the built-ins lack — embeddings rather than lexical matching, or a routing
classifier — which the API still accommodates by letting a custom tool return
`tool_reference` blocks of its own.
