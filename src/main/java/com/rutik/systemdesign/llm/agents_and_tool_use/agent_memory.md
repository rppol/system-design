...
**Short:** Agent memory splits into working, episodic, semantic, and procedural, each retrieved for a different purpose.
...
**Short:** MemGPT treats context like OS virtual memory, letting the model page facts between core, recall, and archival storage.
...
**Short:** Sliding window drops old messages for speed; summarize-and-replace compresses them with an LLM call to preserve content.
...
**Short:** Extract preferences per conversation into a vector store, then inject the top-K matches into the next session's prompt.
...
**Short:** Token cost forces selective top-K retrieval, pre-storage summarization, and tiered memory rather than injecting everything.
...
**Short:** Mem0 automates fact extraction, deduplication, and scoped retrieval that you would otherwise hand-build on a raw vector store.
...
**Short:** A file-based memory tool replaces similarity ranking with model-driven file edits, shifting the burden to path safety and organization.
...
**Short:** For single-session bounded tasks under the context limit, just extend the context window instead of adding retrieval.
...
**Short:** Measure recall accuracy on planted facts, retrieval precision, and how much injected memory the response actually used.
...
**Short:** Working memory is the fast, temporary context window; long-term memory is the slower, persistent external store feeding it.
...
**Short:** Score candidate memories with a weighted blend of similarity, exponential recency decay, and a pre-assigned importance value.
...
**Short:** Pin early turns with a sliding window, or summarize the middle into hierarchical per-turn and per-phase digests.
...
**Short:** Scope every memory by user_id, extract and write at session end, version superseded facts, and honor deletion requests.
...
**Short:** Guard against auto-storing PII, cross-user retrieval leakage, and GDPR erasure deadlines with row-level scoping and a review gate.
