...
**Short:** Function calling emits structured tool_calls in a dedicated field; prompted JSON risks unparseable free text.
...
**Short:** parallel_tool_calls lets the model emit multiple independent tool requests in a single response.
...
**Short:** Strict mode constrains decoding so tool arguments always exactly match the declared JSON schema.
...
**Short:** State the tool's mechanism, trigger condition, exclusions, and a worked example in its description.
...
**Short:** Append the assistant's tool_calls message before the tool result message, or the API rejects the context.
...
**Short:** Inject a structured error result and retry only transient failures with exponential backoff before giving up.
...
**Short:** Return flat structured JSON with a status field and a summary, not deeply nested or multi-thousand-token prose.
...
**Short:** Tool chaining feeds one call's output into the next call's input; parallel tools run independently at once.
...
**Short:** Ship the new tool version alongside the old one, add a deprecation notice, and remove the old tool once selection logs show zero use.
...
**Short:** tool_choice="required" forces at least one tool call, useful for guaranteeing structured extraction output.
...
**Short:** Code-agent tools return execution feedback like stdout and exit codes; research tools return ranked snippets.
...
**Short:** Add disambiguation and trigger phrases per tool, and route through a candidate-selection layer past 20 tools.
...
**Short:** Catch every tool exception and convert it to a structured error message instead of aborting the agent loop.
...
**Short:** Parallelize calls with no data dependency; keep calls sequential when one call's result feeds the next.
...
**Short:** Combine a hard step limit, exact-duplicate call detection, and periodic LLM progress scoring.
...
**Short:** OpenAI nests strict mode under function with string tool_choice; Anthropic nests it top-level with object tool_choice.
...
**Short:** Version only breaking changes with a suffixed name; non-breaking optional additions extend the existing tool in place.
