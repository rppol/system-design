...
**Short:** ToT lets the model branch, evaluate, and backtrack across a tree of thoughts, unlike CoT's single irreversible linear path.
...
**Short:** On the 24-game benchmark, GPT-4 solved just 4 percent with chain-of-thought but 74 percent with ToT plus breadth-5 search.
...
**Short:** ToT scores candidate thoughts independently with a numeric rating or by voting across siblings to pick the best one.
...
**Short:** BFS expands every node level by level for the shallowest solution; DFS commits to one branch and backtracks on failure.
...
**Short:** Beam search keeps only the top B candidates per level, making cost linear in depth instead of exponential like plain BFS.
...
**Short:** MCTS balances exploration and exploitation across rollouts via UCB1, while beam search is purely greedy and never revisits branches.
...
**Short:** UCB1 adds an exploration bonus scaled by the log of parent visits over node visits to a node's average value.
...
**Short:** A PRM replaces the LLM evaluator with a small trained classifier that scores intermediate steps for a fraction of the cost.
...
**Short:** Beam search with width 3 needs roughly a third the LLM calls of naive branching-4 BFS at the same depth.
...
**Short:** Skip ToT when one obvious next step exists, latency is tight, or no value function can distinguish candidates.
...
**Short:** Test-time compute scaling spends more inference on a query, and ToT is one explicit mechanism for doing that via branching search.
...
**Short:** A good ToT thought is one complete, independently evaluable agent action — typically one tool call or one implementation step.
...
**Short:** Give the evaluator an adversarial prompt to hunt for flaws, and use a different model or a deterministic verifier instead of the generator's peer.
...
**Short:** Score each candidate patch by the fraction of unit tests it passes, replacing subjective LLM scoring with a deterministic verifier.
...
**Short:** ToT applies classical heuristic search directly to LLM reasoning steps, swapping only the branching source and the heuristic function.
...
**Short:** Start with beam search for shallow, latency-sensitive tasks and escalate to MCTS only when deep exploration quality falls short.
