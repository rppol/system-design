...
**Short:** Agent reliability tracks task completion despite partial failures; service reliability tracks uptime and latency SLA.
...
**Short:** Independent per-step success compounds multiplicatively, so retrying each step turns exponential decay into near-constant success.
...
**Short:** Retry assumes a transient failure and keeps trying; a circuit breaker assumes persistent failure and stops calling entirely.
...
**Short:** Fingerprint tool name and arguments in a rolling window and require a repeat with unchanged results before flagging a dead loop.
...
**Short:** interrupt_before always pauses at a named node; interrupt() pauses conditionally based on state computed inside the node.
...
**Short:** Concurrent agents retrying together can multiply load on a degraded service; jitter, circuit breakers, and a retry budget prevent it.
...
**Short:** Checkpoint only IDs and references, push large blobs to object storage, and compress the serialized state.
...
**Short:** Fall back to a cached or alternate tool first, then tell the model the tool is unavailable rather than raising an exception.
...
**Short:** Set a soft step counter below LangGraph's hard recursion_limit, giving room for outliers before the kill switch fires.
...
**Short:** Track task success rate, stuck rate, mean step count, per-tool error rate, and cost per task against a rolling baseline.
...
**Short:** Validate tool output against its schema, double-check high-stakes results, and trip the circuit breaker on schema-violation rate.
...
**Short:** Long-running coding agents persist the workspace, gate risky actions on confidence, and adapt their step budget to task complexity.
...
**Short:** An escalation payload needs the task, completed steps, recent actions, the stuck reason, and concrete resume options.
...
**Short:** Inject tool faults, kill calls mid-execution, and craft loop-inducing inputs to verify recovery, checkpointing, and handoff.
...
**Short:** A crash between an API mutation and its checkpoint write can replay the call, so mark actions in-flight with an idempotency key.
...
**Short:** A checkpoint restores only agent state, so external writes need a LIFO compensating action registered after the write succeeds.
