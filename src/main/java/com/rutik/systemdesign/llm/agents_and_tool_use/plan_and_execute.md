...
**Short:** Plan-and-Execute generates a full plan upfront then executes it, unlike ReAct's step-by-step interleaved decisions.
...
**Short:** A plan goes stale when a step fails or contradicts an assumption, triggering the replanner with completed context.
...
**Short:** HTN recursively decomposes a goal into sub-goals until every leaf is a single atomic executable action.
...
**Short:** ReWOO writes the whole plan with placeholder variables before any tool runs, avoiding re-billing growing observations.
...
**Short:** Steps the planner marks as data-independent execute concurrently via asyncio.gather or a thread pool.
...
**Short:** Use the strongest model for planning and route routine executor steps to a cheaper model to cut total cost.
...
**Short:** Context accumulation caps reliable Plan-and-Execute at 10-20 steps unless you summarize or nest hierarchical plans.
...
**Short:** Validate each step's output against its declared expected format, then retry, replan, or flag the gap on failure.
...
**Short:** LangGraph's interrupt_before pauses at a named node so a human can approve the plan or a risky step before it runs.
...
**Short:** Plan-and-Execute is usually cheaper than all-large-model ReAct because it routes most steps to a small executor model.
...
**Short:** Check tool availability, dependency ordering, completeness, and parallelism validity before running any executor step.
...
**Short:** The replanner needs the completed results and the specific violated assumption, or it just regenerates the same plan.
...
**Short:** Size each plan step to one executor invocation — coarse steps under-specify, and micro-steps balloon the plan.
