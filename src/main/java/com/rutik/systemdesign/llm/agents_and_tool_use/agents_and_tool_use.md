...
Two consequences shape agent design. First, "add a tool" stops meaning "edit the agent" and starts meaning "point the agent at another server", so a single agent routinely ends up facing 200+ tools it never had to implement — which is why [Tool Selection at Scale](tool_selection_at_scale.md) is a load-bearing concern rather than an optimization. Second, it moves the trust boundary: a tool result is now attacker-influenceable text arriving from a third-party server, making indirect prompt injection a protocol-level problem rather than an application-level one. See [MCP — Model Context Protocol](../mcp_model_context_protocol/mcp_model_context_protocol.md) for the protocol itself, transports, server and client construction, and the security model.
...
**Short:** An agent loops the LLM through tool calls and observations across multiple steps, unlike one input-output call.
...
**Short:** ReAct alternates Thought, Action, and Observation steps, making the model's reasoning explicit and debuggable.
...
**Short:** Combine a hard iteration cap, a timeout, repetition detection, a cost budget, and human-in-the-loop checks.
...
**Short:** Working memory is the volatile context window; long-term memory is persistent external storage retrieved on demand.
...
**Short:** Parallel tool calls fire independent requests at once, cutting latency to the slowest single call instead of the sum.
...
**Short:** A good tool description states its mechanism, trigger condition, exclusions, and a concrete example.
...
**Short:** Agent cost is input plus output tokens summed per call, driven mainly by growing history and large tool results.
...
**Short:** LangGraph's interrupt hooks pause execution before high-risk actions until a human approves via update_state.
...
**Short:** A tool call error is an execution failure; a reasoning error is the model choosing or concluding wrongly.
...
**Short:** Skip an agent for single-call tasks, fixed sequences, tight latency budgets, or unsupervised high-stakes actions.
...
**Short:** Validate tool output, retry once with formatting guidance, then fall back to an explicit error observation.
...
**Short:** Track task success rate, cost per task, P95 latency, step count, and tool error rate against a rolling baseline.
...
**Short:** Use create_agent for a plain model-tool loop; hand-write a LangGraph graph when the control flow itself varies.
...
**Short:** create_agent generates the same agent-plus-tool-node graph a hand-built loop would, just in five lines instead of thirty.
...
**Short:** Make mutating tools idempotent with a per-action key, and gate irreversible writes behind a confirm step.
...
**Short:** MCP turns tools into external servers, exploding catalogue size and making tool output an untrusted trust boundary.
