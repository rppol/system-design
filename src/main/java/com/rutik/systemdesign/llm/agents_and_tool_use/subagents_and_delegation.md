...
Subagents matter for three reasons: (1) **context isolation** — a subagent doing web research doesn't need to see the parent's 30 prior tool calls, cutting cost and improving focus; (2) **parallelism** — 5 subagents dispatched simultaneously finish in roughly the time of one, dramatically reducing wall-clock latency on multi-source tasks; (3) **security** — restricting a subagent's tools shrinks the blast radius of any single [prompt injection](../llm_security/llm_security.md) or model misbehavior.
