...
6. **Provider terms are part of your architecture.** Retention windows — and they are **not** uniform, so verify per provider rather than assuming a shared default: OpenAI and Anthropic both delete API inputs and outputs within 30 days, while Google's Vertex abuse-monitoring logging runs up to 90 days (30 for Advanced AI models) and exempts Google Cloud Master Agreement customers by default. Every one of the three offers a zero-data-retention path, but by request and approval, not as a self-serve toggle. Train-on-your-data defaults (consumer products often opt-out, enterprise APIs opt-in/never) and regional processing guarantees differ by provider and tier the same way — and together they determine what you may legally send.
...
2. **Fine-tuning on raw support tickets.** A real incident pattern: a support-bot fine-tune ingests tickets containing addresses and order details; months later the bot offers another customer "your address on file is..." — a memorized completion. Typical impact once it surfaces: full model rollback, weeks of retraining, regulator notification — an illustrative composite of this failure pattern, not an account of a specific named incident. Fix: pseudonymize before the training set is even written.
...
**Short:** You cannot reliably delete a user's data from trained weights, so personal data must live in deletable stores like RAG and logs while training sets stay pseudonymized.
...
**Short:** Duplication count, model size, and prompt context length drive memorization log-linearly, and deduplicating training data is the cheapest fix since it also improves quality.
...
**Short:** The 2023 ChatGPT divergence attack made the model emit verbatim training data including real names and emails by prompting it to repeat one word forever.
...
**Short:** Membership inference decides whether a record was in the training set via loss thresholds or shadow models, mattering for privacy audits and copyright disputes.
...
**Short:** DP-SGD clips and noises per-example gradients for a provable privacy bound, but its compute overhead and quality damage make it practical only for small sensitive fine-tunes.
...
**Short:** A support assistant's PII pipeline layers NER and validated pattern recognizers at every boundary, replacing PII with reversible per-entity pseudonyms rather than blank redaction.
...
**Short:** Full redaction destroys utility, pseudonymization preserves entity structure as the default for LLM inputs, and a tokenization vault adds authorized reversibility for real values.
...
**Short:** Vector databases are a privacy liability because embeddings are invertible to near-exact source text, requiring encryption, isolation, and a document-to-vector deletion index.
...
**Short:** RAG wins on privacy governance since data stays in an ACL-filtered, deletable store, while fine-tuning bakes data into weights with no per-request access control.
...
**Short:** A logging policy must scrub PII before the write happens, hash identifiers, set purpose-differentiated retention TTLs, and include observability in deletion-request fan-out.
...
**Short:** Provider data-handling terms determine sensitivity-based routing, sending regulated traffic to zero-data-retention or BAA-covered endpoints and generic traffic to standard tiers.
They determine what you may send where. Key variables: training defaults (enterprise API tiers typically never train on your data; consumer tiers may unless opted out — the Samsung incident is the canonical consequence), retention (OpenAI and Anthropic delete API inputs and outputs within 30 days; Google Vertex logs prompts for abuse monitoring for up to 90 days, 30 for Advanced AI models, with an exception process — so do not carry one provider's window to another), regional processing/residency endpoints (EU processing for GDPR transfers), and BAA availability for PHI. Architecturally this becomes sensitivity-based routing: classify the request, then route regulated traffic to ZDR/regional/BAA-covered endpoints (or self-hosted models) and generic traffic to cheaper tiers — with the classifier and the routing table audited like any other security control.
...
**Short:** Privacy red-teaming combines canary extraction gates, membership-inference attempts, cross-tenant retrieval probes, and a deletion-request fire drill verifying every store purged.
...
**Short:** SISA trains on disjoint data shards so deleting a record only retrains its shard, but sharding hurts LLM quality and ensemble cost too much to be practical at scale.
...
**Short:** Fine-tuning on support transcripts needs a checklist covering legal basis, pseudonymization before the dataset exists, deduplication, canary gates, and a deletion story.
...
**Short:** Confidential computing removes the datacenter operator from the trust boundary via an attested enclave, but does nothing to stop a model regurgitating a memorized secret.
