...
**Short:** Distillation trains a student on the teacher's softened output distribution via KL divergence, where higher temperature exposes more dark knowledge about other tokens.
...
**Short:** Logit-level distillation needs teacher weight access and transfers dark knowledge for maximum quality, while data-level distillation only needs teacher API outputs.
...
**Short:** A student can hallucinate more than its teacher because it inherits every teacher error as confident ground truth and loses the teacher's calibrated uncertainty.
...
**Short:** Model merging works because fine-tuned models from the same base create approximately orthogonal task vectors that can be added without destructive interference.
...
**Short:** TIES trims small deltas, elects a dominant sign per parameter, and merges only agreeing values, preventing averaging from canceling conflicting task vectors to near zero.
...
**Short:** DARE randomly zeros most of a task vector and rescales survivors to preserve its magnitude, exploiting redundancy while sharply cutting sign conflicts between models.
...
**Short:** SLERP interpolates along the hypersphere's great circle instead of a straight line, avoiding the magnitude shrinkage linear interpolation causes at intermediate points.
...
**Short:** A model soup averages same-task checkpoints from different hyperparameters or seeds, moving weights toward a flatter loss basin that generalizes better at zero extra cost.
...
**Short:** SparseGPT prunes column by column with inverse-Hessian calibration to redistribute each pruning error onto surviving weights, reaching 50% sparsity under 0.5 perplexity loss.
...
**Short:** Wanda scores weights by magnitude times input activation norm instead of magnitude alone, correctly discarding large weights connected to near-zero activations.
...
**Short:** Choose distillation for over 4x compression or an architecture change, and quantization for a simpler 2-4x reduction that preserves the original architecture exactly.
...
**Short:** FrankenMerge risks residual-stream incompatibility between stacked layers from different models, since each layer expects statistics only its own training established.
...
**Short:** Unstructured pruning's scattered zeros need specialized sparse hardware to speed up, while structured pruning removes whole neurons or heads for speedup on any hardware.
...
**Short:** 2:4 semi-structured sparsity zeros exactly two of every four weights and gets a hardware-guaranteed 2x speedup on NVIDIA Ampere-and-later Sparse Tensor Cores.
...
**Short:** A production distillation pipeline collects teacher outputs on real traffic, trains the student with combined KL and cross-entropy loss, and deploys within about 5% of teacher quality.
...
**Short:** Task arithmetic adds or subtracts a capability's weight-space delta from a shared base model, composing or removing skills without any retraining.
