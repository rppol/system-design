...
6. **Not all compression techniques are substitutes.** Distillation changes the architecture (different model). Quantization changes the precision (same model, fewer bits). Pruning changes the sparsity (same precision, fewer parameters). Merging changes the capabilities (same size, combined skills). They compose: distill -> merge -> prune -> quantize.
...
This is how Alpaca, Vicuna, and most open-source instruction-tuned models were created. Simpler than logit distillation (no need to capture teacher's full distribution) but transfers less dark knowledge.
...
**What is knowledge distillation and how does temperature scaling affect the process?**
...
**Compare logit-level distillation with data-level distillation. When would you choose each?**
...
**What is model merging and why does it work despite not involving any training?**
...
**Explain TIES merging. What problem does it solve that simple weight averaging does not?**
...
**What is SLERP and why is it preferred over linear interpolation for model merging?**
...
**How does SparseGPT achieve one-shot pruning without retraining?**
...
**What is Wanda and how does it improve upon magnitude pruning?**
...
**When would you choose distillation over quantization for model compression?**
...
**What are the risks of FrankenMerge (layer-wise merging from different models)?**
...
**How does structured pruning differ from unstructured pruning in terms of hardware requirements?**
...
**What is N:M sparsity and which hardware supports it?**
...
**How would you design a distillation pipeline for a production LLM deployment?**
...
**What is task arithmetic and how is it used to compose or remove model capabilities?**
