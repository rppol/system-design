...
> [Vision-Language Models](../vision_language_models/vision_language_models.md) — in particular its coverage of
> Grounding DINO and SAM for "pick the red mug"-style grasp-point grounding
> ([VLM README §4, §8, §12](../vision_language_models/vision_language_models.md)), which is the perception
...
forgetting, see [Fine-Tuning](../fine_tuning/fine_tuning.md)); co-fine-tuning with the original
...
- **Dual-system** (Helix, GR00T): an explicit architectural split — a **slow System 2** (large VLM,
  ...
  the broader [Agentic Workflow Patterns](../agentic_workflow_patterns/agentic_workflow_patterns.md) orchestrator/worker
...
(see [Guardrails & Content Safety](../guardrails_and_content_safety/guardrails_and_content_safety.md) for the analogous
...
**Q1: What's the core architectural difference between a VLM and a VLA — is it really "just adding an action head"?**
**Short:** A VLA adds an action-output pathway to a VLM, but the hard problem is that robot actions are continuous, high-frequency, and safety-critical in ways text tokens never are.
...
**Q2: Why does RT-2's "co-fine-tuning" on web data matter — wouldn't fine-tuning purely on robot demonstrations produce a more accurate robot policy?**
**Short:** RT-2's co-fine-tuning mixes web-scale vision-language data into the same run as robot demonstrations, preventing forgetting of general knowledge and boosting novel-skill performance 3x.
Fine-tuning purely on robot demonstrations does produce a policy that's accurate *on the demonstrated tasks*, but it tends to suffer catastrophic forgetting of the broad visual-semantic knowledge from VLM pretraining (see [Fine-Tuning](../fine_tuning/fine_tuning.md)) — the model "forgets" what an "extinct animal" looks like because robot demo data never mentions extinct animals. RT-2's co-fine-tuning mixes the original web-scale vision-language data into the *same* fine-tuning run as robot data, acting as a regularizer that preserves general knowledge while still teaching the action-token "language." The 3x improvement on emergent/novel-skill evaluations (§7) is the direct empirical payoff of this regularization — it's the mechanism, not just "more data is better."

**Q3: A robot needs 50Hz control, but your VLA's forward pass takes 150ms. Isn't that fundamentally incompatible?**
**Short:** Action chunking decouples VLM thinking rate from robot acting rate by producing many future actions per forward pass, amortizing the 150ms latency across all of them.
...
**Q8: How does Open X-Embodiment's cross-embodiment training actually produce "positive transfer" — intuitively, why would training on a different robot's data help THIS robot?**
**Short:** Cross-embodiment training helps because most of what a manipulation policy learns is embodiment-agnostic visual and task structure rather than robot-specific kinematics.
...
**Q9: If you were choosing between OpenVLA and pi-0 for a new manipulation task, what questions would you ask first?**
**Short:** Choosing between OpenVLA and pi-0 comes down to the task's precision requirement, since pi-0's flow-matching action expert avoids OpenVLA's tokenized-action discretization error.
...
**Q10: What does "the robot collaborating with another robot using the same model checkpoint" (Figure Helix) actually demonstrate technically?**
**Short:** Two robots driven by the same VLA checkpoint with no fine-tuning demonstrates the System 2 representation captures shared task state rather than one robot's specific next action.
It demonstrates that the dual-system architecture's System 2 (the VLM doing scene/task understanding) produces a representation general enough that **the same weights, with no robot-specific fine-tuning, can drive two physically distinct control contexts** (two robots with different roles in a shared task) — each robot's System 1 consumes its own proprioception/vision and the shared System 2 understanding to produce role-appropriate actions. Technically, this is a strong signal that the System 2 representation has learned something like "shared task state" rather than "this specific robot's next action," which is the generalization property the whole VLA research program is aiming for — analogous to how a single LLM can play different roles in a [multi-agent system](../multi_agent_systems/multi_agent_systems.md) via different prompts/contexts without retraining.

**Q11: Your team trained a VLA in simulation and it achieves 95% success in Isaac Sim. What do you tell leadership before they greenlight real-hardware deployment?**
**Short:** A 95% simulation success rate isn't deployment-ready evidence, since the sim-to-real gap routinely causes 30-50-plus point drops without domain randomization and real evaluation.
...
**Q12: Why is proprioception input necessary if the cameras can already see the robot's gripper?**
**Short:** Proprioception gives exact, noise-free joint and pose data that vision alone can't match due to depth ambiguity and occlusion, critical during the final grasp phase.
...
**Q13: How would you debug a VLA policy that works well on a stationary tabletop but fails when the object is moving (e.g., on a conveyor)?**
**Short:** A VLA that fails on moving objects but works on stationary ones likely suffers open-loop chunking latency, fixed by shrinking chunk size or adding a dual-system reactive layer.
...
**Q14: What's the relationship between VLA action chunking (§3.3) and the action-chunking transformer (ACT) — is ACT a specific model or a general technique?**
**Short:** ACT is a specific named model and training recipe for bimanual manipulation, while action chunking is the now-general technique of predicting many future actions per pass.
...
**Q15: Why might a VLA trained primarily on Open X-Embodiment data underperform on a brand-new robot embodiment with a very different gripper (e.g., a suction gripper vs. a parallel-jaw gripper)?**
**Short:** A VLA pretrained mostly on parallel-jaw grippers transfers scene and task understanding to a new suction-gripper robot but not the gripper-specific grasping behavior.
...
- [Vision-Language Models](../vision_language_models/vision_language_models.md) — the VLM backbones (CLIP, SigLIP, DINOv2) and visual-grounding techniques (Grounding DINO, SAM) that every VLA in this module builds on
- [Diffusion Language Models](../diffusion_language_models/diffusion_language_models.md) — the other major Phase 6 "new generation paradigm" module; pi-0's flow-matching action expert (§6.3) shares mathematical lineage with diffusion
- [Multimodal Models](../multimodal_models/multimodal_models.md) — broader multimodal architecture context
- [Small Language Models & Edge AI](../small_language_models_and_edge_ai/small_language_models_and_edge_ai.md) — on-device/on-robot inference constraints relevant to System 1 in dual-system architectures
...
- [Agentic Workflow Patterns](../agentic_workflow_patterns/agentic_workflow_patterns.md) — the orchestrator/fast-worker pattern that dual-system VLA architectures mirror
- [Guardrails & Content Safety](../guardrails_and_content_safety/guardrails_and_content_safety.md) — the text-domain analogue of the external safety-layer pattern in §10.7/§14
