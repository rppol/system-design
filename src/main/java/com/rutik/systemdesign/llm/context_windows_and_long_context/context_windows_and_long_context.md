...
<!-- study-paths
senior: context_windows_and_long_context.md
...
**Key insight**: Long context and [RAG](../rag_fundamentals/rag_fundamentals.md) are not competing approaches but complementary tools — use RAG for large corpora and cost sensitivity, use long context for holistic reasoning, and use both together (retrieve relevant chunks → put them in long context) for the best results.
...
**Cross-reference → see [Inference & Decoding](../inference_and_decoding/inference_and_decoding.md) §3.8 and §3.9 for full details on:**
...
length" ([Inference & Decoding](../inference_and_decoding/inference_and_decoding.md)) holds at chat lengths and
...
For deciding *what* goes into the window and in what order (compaction, ordering, memory), see [Context Engineering](../context_engineering/context_engineering.md).
...
**Short:** RoPE rotates query/key vectors so their inner product depends only on relative position, letting it generalize and extrapolate where absolute embeddings cannot.
...
**Short:** LLMs attend more to the start and end of context and lose accuracy on facts placed in the middle, even below their own closed-book baseline.
...
**Short:** Needle tests plant an isolated, lexically distinct fact with no distractors, while real tasks are full of semantically similar distractors that make performance collapse.
...
**Short:** Quadratic-cost prefill over 200K tokens takes 10-30 seconds; prefix/prompt caching reuses the KV cache for the stable portion and cuts TTFT to 0.5-2 seconds.
...
**Short:** Choose long context for holistic reasoning over a document small enough to fit whole, and RAG for large corpora, frequent updates, or exact fact lookup.
...
**Short:** Flash Attention tiles the attention computation in on-chip SRAM so memory scales O(n) instead of O(n-squared), avoiding the 32GB matrix a 128K sequence would otherwise need.
...
**Short:** RoPE encodes relative position via multiplicative rotation with no added parameters, extrapolates with NTK/YaRN scaling, and composes cleanly with GQA and KV caching.
...
**Short:** YaRN rescales RoPE's frequency bands to extend context with a short fine-tune, while ALiBi adds a fixed linear distance bias that needs no fine-tuning at all.
...
**Short:** Mitigate it by placing key facts at the start or end, reranking down to a handful of highly relevant chunks, and iteratively summarizing long documents.
...
**Short:** Context rot is reliability declining with total input length at a fixed position, while lost-in-the-middle is a positional effect at a fixed length; the two compound.
...
**Short:** Long context can cost roughly 80x more per query than RAG on the same document (e.g., $0.60 vs $0.0075 at 200K tokens), so reserve it for reasoning across all chunks at once.
...
**Short:** KV cache memory scales linearly with context length via num_kv_heads and head_dim, so GQA and FP8 quantization are what make long contexts fit in GPU memory.
...
**Short:** Mistral bounds each token's attention to the nearest W=4096 tokens, keeping KV cache memory constant while information still propagates indirectly across layers.
...
**Short:** Ring attention shards the sequence across GPUs and circulates KV blocks around a ring so each GPU only holds and computes roughly 1/N of the full attention.
...
**Short:** Position interpolation uniformly compresses every RoPE frequency band, degrading local resolution, which is why a short fine-tune after scaling remains necessary.
...
**Short:** Every decode step re-reads the full KV cache from HBM, so tokens-per-second falls linearly with context length once KV reads exceed the weight read.
...
**Short:** Models park heavy attention mass on the first few tokens, so evicting those sink tokens in a sliding window collapses perplexity even though they carry little content.
