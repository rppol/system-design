...
<!-- study-paths
principal: polyglot_persistence_patterns.md
...
**Short:** Syncing PostgreSQL and Elasticsearch relies on the outbox pattern with Debezium CDC, guaranteeing at-least-once delivery through idempotent upserts.
...
**Short:** Dual-write can leave the database and Elasticsearch diverged or out of order, which CDC fixes by making Elasticsearch a downstream consumer of the WAL.
...
**Short:** The outbox pattern solves dual-write by committing the message and the business data in one database transaction, then relaying it durably.
...
**Short:** Event sourcing fits when you need a full audit trail, temporal queries, or multiple projections of the same events, not simple CRUD workloads.
...
**Short:** Since events are immutable, schema evolution relies on tolerant readers, upcasting old events, or explicit event versioning rather than editing history.
...
**Short:** The Strangler Fig pattern migrates a database gradually by dual-writing to both systems and incrementally shifting read traffic before decommissioning the old one.
...
**Short:** Multi-model access designates one source of truth and streams its changes via CDC into specialized derived stores like search, analytics, and cache.
...
**Short:** Materialize maintains continuously updated SQL views over Kafka streams, replacing custom consumer code for simple CDC-driven read model projections.
...
**Short:** CDC ordering per key comes from Kafka partitioning by primary key, while idempotency comes from upserts and using the event's commit timestamp as a version.
...
**Short:** Debezium's initial snapshot reads all existing rows as CREATE events under a consistent lock point before switching to streaming WAL changes.
...
**Short:** DataLoader batches per-tick GraphQL field lookups into one call and deduplicates identical keys, eliminating the N+1 problem across polyglot stores.
...
**Short:** Rolling back a polyglot migration safely requires keeping dual-write active so read traffic can switch back to the original database instantly.
...
**Short:** Polyglot consistency is tested with continuous reconciliation queries, shadow reads, CDC-propagation integration tests, and chaos tests on the consumer.
...
**Short:** Data mesh has each domain team own and publish its own data product, while a centralized CDC pipeline routes every domain's changes through one team.
...
**Short:** CQRS gives the read side a structurally different model like Elasticsearch documents, while a read replica just copies the same schema for scaling.
...
**Short:** Each bounded context should own exactly one source-of-truth database for its entities, with every other context treating that data as a CDC-fed projection.
