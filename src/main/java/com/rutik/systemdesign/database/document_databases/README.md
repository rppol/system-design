...
**Q: What happens when a sharded MongoDB query doesn't include the shard key?**
A query that omits the shard key can't be routed to a single shard, so `mongos` fans it out to every shard in the cluster and merges the results — a pattern called scatter-gather. On a 10-shard cluster, a non-shard-key query costs roughly 10 times the work of a targeted single-shard query, because every shard independently runs the full query and mongos then has to merge all of their results. This cost scales directly with cluster size: the more you shard for scale, the more expensive every non-shard-key query becomes relative to a properly targeted one. Always include the shard key in hot query predicates, and choose the shard key around the application's dominant query pattern from the start rather than retrofitting it later.
