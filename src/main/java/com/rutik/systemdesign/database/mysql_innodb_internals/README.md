...
**Short:** InnoDB's redo log replays committed changes for crash recovery, while the undo log reverts uncommitted changes and feeds MVCC snapshots.
...
**Short:** innodb_flush_log_at_trx_commit=1 fsyncs on every commit for zero data loss, while =2 batches fsyncs once per second for far higher throughput.
...
**Short:** InnoDB's buffer pool inserts new pages into the midpoint old sublist, so a one-time sequential scan can't evict genuinely hot pages from the young sublist.
...
**Short:** A gap lock blocks inserts into an index range under REPEATABLE READ, which is what stops phantom reads from appearing on a re-executed range query.
...
**Short:** InnoDB MVCC gives each transaction a consistent snapshot via a read view and undo log chain, so readers and writers never block each other.
...
**Short:** MySQL's binary log records logical statement or row changes for replication and PITR, while InnoDB's redo log records physical page changes only for crash recovery.
...
**Short:** gh-ost avoids table locks by copying rows to a shadow table while tailing the binlog, then swapping tables with only a brief metadata lock.
...
**Short:** InnoDB's change buffer defers secondary index updates for pages not in memory, cutting random I/O for write-heavy workloads on non-unique indexes.
...
**Short:** GTID replication tags every transaction with a unique ID, letting a replica auto-position itself and simplifying failover over binlog file and position.
...
**Short:** A column type change that requires a table rebuild is done zero-downtime with gh-ost, which copies and swaps the table without blocking traffic.
...
**Short:** InnoDB's doublewrite buffer prevents torn pages on crash and can be safely disabled only on storage with power-loss-protected write caching.
...
**Short:** REPEATABLE READ adds gap and next-key locks to prevent phantom reads, while READ COMMITTED skips them for better insert concurrency.
