...
**Short:** HNSW's multi-layer graph takes large hops at top layers and fine steps at the bottom for O(log N) search.
...
**Short:** ANN search trades some recall for speed via parameters like HNSW's ef or IVF's nprobe.
...
**Short:** Vector database multi-tenancy uses separate namespaces, metadata filtering, or a hybrid depending on tenant scale.
...
**Short:** Use pgvector when vectors must join relational data transactionally; use a dedicated DB past roughly 100M vectors.
...
**Short:** Hybrid search combines dense vector similarity with sparse keyword matching because pure vector search misses exact terms.
...
**Short:** Embedding model choice affects RAG recall and precision through dimensionality, domain match, and context window.
...
**Short:** Product quantization compresses vectors into learned centroid codes, trading a recall floor for large storage savings.
...
**Short:** Recall@K, MRR, and NDCG@K measure retrieval quality on a domain-specific query-document test set.
...
**Short:** Scalar and binary quantization keep every dimension at lower precision, unlike PQ which discards dimensions entirely.
...
**Short:** Vector search eases "lost in the middle" by feeding the LLM only a few precisely relevant chunks, not whole documents.
...
**Short:** IVFFlat centroids freeze after build, so vectors inserted later just join the nearest existing cluster.
...
**Short:** ef_construction sizes the candidate list at build time; ef sizes it at query time, both trading speed for recall.
...
**Short:** Cosine similarity equals dot product for unit-normalized vectors, so normalized embeddings can use the cheaper dot product.
...
**Short:** Embedding a query with a different model than its documents makes similarity scores meaningless with no error raised.
...
**Short:** pgvector's default ivfflat.probes of 1 searches only one cluster, silently capping recall around 70%.
...
**Short:** Naive fixed-size chunking severs sentences mid-thought; semantic or overlapping chunking keeps each chunk coherent.
...
**Short:** Building an HNSW index holds every vector and its graph edges in memory at once, which can exhaust RAM.
