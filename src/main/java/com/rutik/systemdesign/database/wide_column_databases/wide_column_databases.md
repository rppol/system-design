...
<!-- study-paths
senior: wide_column_databases.md
principal: wide_column_databases.md
...
**Short:** Model time-series with a compound partition key of device_id plus a time bucket to cap partition size.
...
**Short:** Tombstones must be scanned and skipped on reads until gc_grace_seconds passes, slowing reads under heavy deletes.
...
**Short:** Cassandra achieves linearizability only for single-partition ops via Paxos-based lightweight transactions, at four round trips.
...
**Short:** Vnodes give each physical node many small token ranges, spreading rebalancing load across the whole cluster.
...
**Short:** STCS favors low write amplification, LCS favors low space amplification and reads, TWCS drops SSTables by time window.
...
**Short:** Strong consistency holds when write plus read consistency levels exceed the replication factor (W + R > RF).
...
**Short:** Cassandra's leaderless design lets any node accept writes with no election on failure, unlike a primary-replica model.
...
**Short:** ALLOW FILTERING forces a full cluster scan across every partition, fine on tiny data and disastrous at scale.
...
**Short:** Multi-DC Cassandra uses NetworkTopologyStrategy with LOCAL_QUORUM reads and writes to avoid cross-datacenter latency.
...
**Short:** A per-SSTable bloom filter skips disk I/O for SSTables that definitely lack the requested partition key.
...
**Short:** Read repair fixes a stale replica by comparing quorum read responses and pushing the latest value to it.
...
**Short:** Detect oversized partitions with nodetool tablestats and toppartitions, then re-bucket the key to shrink them.
...
**Short:** A failed node is covered by hinted handoff until it returns, then nodetool repair reconciles any remaining divergence.
...
**Short:** Cassandra secondary indexes are per-node and fan out cluster-wide, making them unsuitable for high-cardinality lookups.
...
**Short:** A large IN clause forces one coordinator into cluster-wide scatter-gather; token-aware drivers route sub-batches directly.
...
**Short:** Counter columns only support atomic increment and decrement, merging per-replica shards to avoid read-modify-write races.
