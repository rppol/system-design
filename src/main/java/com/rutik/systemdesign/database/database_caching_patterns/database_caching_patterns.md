...
- **Set maxmemory and an appropriate eviction policy** (allkeys-lfu for general caches) before production; never run without memory limits. Policy-by-policy tradeoffs and the `volatile-*` traps are in [`key_value_stores/`](../key_value_stores/key_value_stores.md) and [`in_memory_databases/`](../in_memory_databases/in_memory_databases.md).
