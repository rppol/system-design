...
**Q: How do version-based cache keys avoid the need for explicit invalidation?**
Version-based keys embed a version number in the cache key itself, so writing a new version creates a brand-new key instead of requiring the old one to be explicitly deleted. The database stores a version column per row; each write increments it, and reads build the cache key from the current version, such as `user:42:v7`; the previous key, `user:42:v6`, simply becomes unreachable once callers start requesting v7 and expires on its own via its normal TTL rather than needing a synchronous DEL. The tradeoff is an extra version lookup before the main key can be constructed, plus some wasted memory from orphaned old-version entries sitting around until their TTL passes. Use version-based keys when explicit delete-on-write is hard to guarantee across every write path, since stale versions age out safely instead of serving wrong data indefinitely.
