...
<!-- study-paths
principal: database_selection_framework.md
...
**Short:** 10M writes/day is only about 116/sec, well within a single PostgreSQL node, so Cassandra is unjustified without multi-region writes.
...
**Short:** A fraud detection system typically combines PostgreSQL for canonical storage, Redis for hot-path lookups, and Neo4j for graph-based ring detection.
...
**Short:** CockroachDB earns its complexity only once writes exceed a benchmarked PostgreSQL primary's ceiling or true multi-region write consistency is required.
...
**Short:** A high-traffic e-commerce product is evaluated component by component, pairing PostgreSQL with Elasticsearch and ClickHouse only where proven necessary.
...
**Short:** Polyglot persistence becomes a liability once cross-database sync drift, onboarding cost, and operational burden outweigh its performance gain.
...
**Short:** A constraint disqualifies a database outright regardless of score, while a preference only earns weighted points in the scorecard.
...
**Short:** Migrate off a database when a leading indicator like shrinking headroom crosses a pre-set threshold, not once a symptom already hurts.
...
**Short:** Migrating PostgreSQL to NoSQL without downtime uses dual-writes, a batched backfill, and a shadow-read comparison before shifting reads over.
...
**Short:** DynamoDB fits access purely by key at throughput beyond a single PostgreSQL node, but loses to PostgreSQL once complex queries or joins are needed.
...
**Short:** Selecting a time-series database comes down to write rate, query style, and retention, defaulting to TimescaleDB when the team knows SQL.
...
**Short:** A 100M-user global application splits data by type across PostgreSQL, Redis, Cassandra, and Elasticsearch rather than one database for everything.
...
**Short:** Benchmarking two candidates requires production-sized, skewed data, realistic indexes and concurrency, and measuring P99 over a 30+ minute run.
...
**Short:** A high-frequency trading system keeps the execution path fully in-memory with async disk writes, since disk latency breaks the sub-1ms requirement.
...
**Short:** Self-hosting only wins once the managed price premium on your fleet exceeds a loaded DBA's salary, which rarely holds at small scale.
...
**Short:** Vector search is worth adding once semantic similarity, recommendations, or deduplication are needed, starting with pgvector first.
...
**Short:** Resolving a database selection disagreement means agreeing on requirements first, then running a time-boxed proof of concept instead of debating.
...
**Short:** A team double-charged a payment because DynamoDB's default eventually consistent read missed a just-completed write, which ConsistentRead=true would have caught.
...
**Short:** Self-hosted PostgreSQL often costs more overall because engineer operational time dwarfs the managed service's hourly price premium.
