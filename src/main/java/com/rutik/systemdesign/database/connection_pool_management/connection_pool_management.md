...
<!-- study-paths
senior: connection_pool_management.md
...
**Short:** Adding connections past roughly 2x CPU cores wastes throughput on context switching instead of executing queries.
...
**Short:** Transaction mode breaks server-side prepared statements because a client's connection can change between PREPARE and EXECUTE.
...
**Short:** A per-node PgBouncer DaemonSet, staggered pod startup, or a managed proxy like RDS Proxy all absorb Kubernetes connection storms.
...
**Short:** ConcurrentBag avoids lock contention by having each thread check its own thread-local connection list before the shared pool.
...
**Short:** Pool exhaustion appears as a correlated spike in active, pending, and timeout HikariCP connection metrics.
...
**Short:** HikariCP prevents stale-connection errors with keepalive pings and a maxLifetime that proactively replaces aging connections.
...
**Short:** PostgreSQL connection setup takes 5-10ms, which can exceed a short Lambda invocation's own execution time.
...
**Short:** idle_in_transaction_session_timeout auto-terminates stalled transactions that would otherwise hold locks and block VACUUM.
...
**Short:** Pool size should be derived from QPS times average query duration plus a headroom factor, not picked arbitrarily.
...
**Short:** Odyssey's multi-threaded, event-driven design handles far more client connections per CPU than PgBouncer's single-threaded model.
...
**Short:** AWS RDS Proxy is a managed pooler that absorbs Lambda connection spikes and preserves connections through Multi-AZ failover.
...
**Short:** Tuning HikariCP for JPA means sizing the pool for real concurrency and disabling Open Session in View to stop connection exhaustion.
...
**Short:** ProxySQL adds query-aware read/write splitting on top of connection pooling, which PgBouncer alone does not do.
...
**Short:** Pooled connections held open through a primary failover go stale and must be cycled, since HikariCP does not detect it mid-transaction.
...
**Short:** SHOW POOLS counters reveal whether client demand, server pool sizing, or a connection leak is causing pool exhaustion.
...
**Short:** Multi-tenant pool isolation needs per-tenant PgBouncer pools with capped limits so one noisy tenant cannot starve the rest.
