...
<!-- study-paths
principal: newsql_and_distributed_sql.md
...
**Short:** Spanner uses TrueTime commit-wait, waiting out the clock uncertainty interval instead of running cross-datacenter 2PC coordination.
...
**Short:** TrueTime returns a bounded time interval instead of a single timestamp, letting Spanner assign globally ordered commit timestamps without a central oracle.
...
**Short:** Auto-increment keys funnel all inserts to the range holding the highest values, so CockroachDB fixes this with random UUIDs or hash-sharded indexes.
...
**Short:** CockroachDB's gateway node pushes predicates down to local ranges and either shuffles or co-locates joins before assembling the final result.
...
**Short:** CockroachDB is worth choosing once write scale needs multiple nodes or multi-region active-active writes, while single-region reads favor PostgreSQL replicas.
...
**Short:** TiDB serves OLTP from row-oriented TiKV and near-real-time analytics from a Raft-replicated columnar TiFlash copy of the same data.
...
**Short:** Spanner and CockroachDB are both CP systems, so a network partition halts writes to the affected region or range rather than risking divergence.
...
**Short:** Spanner rolls out schema changes gradually across all splits with old and new versions coexisting, which can take hours on billion-row tables.
...
**Short:** Raft splits leader election, log replication, and safety into separate, easier-to-implement modules than Paxos while giving the same quorum-based guarantees.
...
**Short:** A write intent is an uncommitted MVCC value from an in-progress transaction that readers must resolve, and contention creates intent storms that slow reads.
...
**Short:** YugabyteDB exposes both a PostgreSQL-compatible YSQL and a Cassandra-compatible YCQL API over the same Raft-replicated DocDB storage engine.
...
**Short:** Aurora's single-writer shared-log design lets many replicas read without full sync, but writes never scale horizontally like true distributed SQL.
...
**Short:** Distributed SQL retries must use exponential backoff with jitter and always restart the whole transaction, not just the failing statement.
...
**Short:** REGIONAL BY ROW pins each row's Raft leader to its owning region, turning writes and same-region reads into a single regional round trip.
...
**Short:** PlanetScale shards MySQL via Vitess without native cross-shard ACID, while Spanner supports cross-shard ACID transactions with external consistency.
