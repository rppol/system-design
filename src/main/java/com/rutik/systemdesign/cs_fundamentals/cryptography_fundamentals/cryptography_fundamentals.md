...
**Short:** SHA-256 is designed to be fast, so GPUs can try over a billion guesses per second, while a memory-hard, salted algorithm like Argon2id is intentionally slow to resist that.
...
**Short:** A timing attack measures response-time differences from a short-circuiting `==` to learn matching bytes one at a time; `hmac.compare_digest` prevents it with constant-time comparison.
...
**Short:** SHA-2's length-extension vulnerability lets an attacker forge a valid MAC for extended messages under `H(key||message)`, which HMAC's nested construction prevents.
...
**Short:** Hashing is one-way and irreversible, used for integrity and password storage, while encryption is two-way and reversible with a key, used for confidentiality.
...
**Short:** Reusing a nonce with the same key makes two ciphertexts share a keystream, so XORing them cancels it out and leaks the XOR of the two plaintexts.
...
**Short:** DH lets two parties derive a shared secret over an untrusted channel without ever transmitting it, relying on the hardness of the discrete logarithm problem.
...
**Short:** HMAC uses a shared secret so either party could have produced it, giving no non-repudiation, while a signature's private key proves authorship even to third parties.
...
**Short:** A unique per-user salt combined with the password forces an attacker to compute a fresh hash per user, making a precomputed rainbow table useless.
...
**Short:** The 16-byte tag detects any tampering with the ciphertext or associated data, causing decryption to fail rather than silently returning modified plaintext.
...
**Short:** RSA is slow (~10 ms per RSA-2048 operation) and size-limited, so hybrid encryption uses RSA only to wrap an AES key, then AES-GCM to encrypt the actual data.
...
**Short:** Forward secrecy means a stolen long-term key can't decrypt past sessions, which ephemeral ECDH provides by discarding each session's private key after use.
...
**Short:** Collisions in an n-bit hash need only about 2^(n/2) random inputs to find with 50% probability, which is why 128-bit MD5 collisions are feasible but 256-bit SHA-256 ones are not.
...
**Short:** Security-critical randomness needs a hardware-entropy-backed CSPRNG like `secrets`, not a predictable PRNG like Python's Mersenne-Twister-based `random` module.
...
**Short:** Each increment of bcrypt's cost factor doubles the 2^cost rounds of key setup, letting operators raise verification time as hardware gets faster; it also silently truncates input at 72 bytes.
...
**Short:** PBKDF2 adds computational hardness to stretch a low-entropy password, while HKDF cheaply derives multiple keys from an already high-entropy secret like an ECDH output.
...
**Short:** AEAD encrypts plaintext for confidentiality while also authenticating unencrypted metadata like headers, so tampering with either breaks the authentication tag.
...
**Short:** Symmetric crypto like AES-256 survives with its margin halved by Grover's algorithm, but Shor's algorithm breaks RSA/ECDH/ECDSA outright, replaced by NIST's ML-KEM and ML-DSA.
...
**Short:** An adversary can record encrypted traffic today and decrypt it once a quantum computer exists, so the exposure date is the recording date, not the decryption date.
...
- [networking_fundamentals](../networking_fundamentals/networking_fundamentals.md) — TLS 1.3 handshake builds directly on these crypto primitives: X25519 (ECDH), HKDF, AES-256-GCM, Ed25519/RSA certificates
- [../../backend/backend_security_owasp](../../backend/backend_security_owasp/backend_security_owasp.md) — OWASP A02 Cryptographic Failures: BCrypt cost calibration, applied crypto in web applications, common vulnerability patterns
- [../../backend/auth_and_authorization_systems](../../backend/auth_and_authorization_systems/auth_and_authorization_systems.md) — JWT internals (HMAC-SHA256 for HS256, RSA-SHA256 for RS256), mTLS certificate verification, OAuth 2.0 token security
- [../../devops/secrets_management](../../devops/secrets_management/secrets_management.md) — HashiCorp Vault, AWS KMS, key rotation strategies, HSMs, envelope encryption in production
