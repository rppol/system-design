...
<!-- study-paths
senior: memory_management_and_virtual_memory.md
...
**Short:** Physical memory is the actual DRAM in the machine; virtual memory is a per-process address space the MMU translates to physical frames via the page table.
...
**Short:** A soft fault just remaps a page already in RAM with no I/O; a hard fault must read from swap or disk, costing 100 microseconds to 10 milliseconds.
...
**Short:** The TLB caches recent VPN-to-PFN translations; a hit resolves in about 1 cycle, while a miss forces a multi-level page-table walk costing up to ~400 ns.
...
**Short:** Clock scans a circular buffer of frames with per-frame use-bits, giving a recently accessed page a second chance instead of evicting it immediately.
...
**Short:** It is more page faults after adding more frames, seen with FIFO (and its Clock derivative), but never with LRU, OPT, or LFU stack algorithms.
...
**Short:** `fork()` marks shared pages read-only in both processes, and the kernel only copies a page into a new frame when either side actually writes to it.
...
**Short:** Thrashing is when combined process working sets exceed RAM, so the OS spends more time swapping than computing; fix by admitting fewer processes or adding RAM.
...
**Short:** Overcommit lets processes reserve more virtual memory than physical RAM plus swap, risking OOM-killer terminations if usage is ever fully realized.
...
**Short:** VSZ is the total virtual address space reserved; RSS is the physical memory actually resident, and RSS is the number that matters for capacity planning.
...
**Short:** The OS evicts clean pages by simply unmapping them and dirty anonymous pages by writing to swap first, choosing victims via a Clock/LRU approximation.
...
**Short:** NUMA gives each CPU socket its own local RAM bank; local access (~100 ns) is faster than remote access (~200-300 ns) over the interconnect.
...
**Short:** File-backed pages can be discarded and re-read from disk cheaply, while anonymous pages have no backing file and must be written to swap when dirty.
...
**Short:** It only lets a process run once its referenced-page set fits in RAM, preventing thrashing by suspending any process whose working set does not fit.
...
**Short:** Paging uses fixed-size pages with only internal fragmentation; segmentation uses variable-size segments with no internal but real external fragmentation.
...
**Short:** It scores each process on RSS plus swap usage plus page-table size, adjusted by `oom_score_adj`, and kills whichever process scores highest.
...
- [`java/jvm_internals`](../../java/jvm_internals/jvm_internals.md) — JVM heap regions, G1/ZGC garbage collectors, TLAB, JVM memory flags
- [`python/cpython_memory_model`](../../python/cpython_memory_model/cpython_memory_model.md) — CPython refcounting, cyclic GC, pymalloc arenas
- [processes_threads_and_context_switching](../processes_threads_and_context_switching/processes_threads_and_context_switching.md) — virtual address space per process, fork + COW
- [computer_architecture_and_memory_hierarchy](../computer_architecture_and_memory_hierarchy/computer_architecture_and_memory_hierarchy.md) — cache hierarchy, cache lines, NUMA
- [`devops/linux_and_os_fundamentals`](../../devops/linux_and_os_fundamentals/linux_and_os_fundamentals.md) — OOM killer, cgroups memory limits, vmstat, /proc/meminfo
