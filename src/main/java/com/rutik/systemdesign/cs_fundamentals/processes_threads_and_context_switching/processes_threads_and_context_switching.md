...
<!-- study-paths
senior: processes_threads_and_context_switching.md
...
**Short:** A process has its own isolated address space and costs 0.1-2 ms to create; a thread shares its process's memory but has its own stack, costing only 10-100 microseconds.
...
**Short:** It saves the current registers to the TCB, swaps the page-table register (flushing the TLB) only when switching processes, then loads the new context's registers and resumes at its PC.
...
**Short:** A context switch costs about 1-10 microseconds for register save/restore, TLB flush, and cache cold-start, which is why high-concurrency servers avoid one OS thread per connection.
...
**Short:** User mode blocks privileged instructions so buggy or malicious code can't corrupt the OS or other processes; a syscall uses a dedicated fast instruction to enter kernel mode.
...
**Short:** A zombie is an exited process whose exit status the parent hasn't collected yet; prevent it by always reaping children with `waitpid()` or a `SIGCHLD` handler.
...
**Short:** `fork()` copy-on-write duplicates the calling process, and `exec()` then replaces the child's address space with a new program image; they stay separate so setup can happen in between.
...
**Short:** COW marks pages read-only after `fork()` and only copies one into a private frame when either process actually writes to it, making `fork()` effectively O(1).
...
**Short:** The GIL lets only one thread run Python bytecode at a time, so CPU-bound threads can't parallelize; use multiprocessing, GIL-releasing C extensions, or a free-threaded build instead.
...
**Short:** TLS gives each thread its own private copy of a variable, commonly used for per-thread DB connections, RNG state, or transaction context, and must be cleaned up to avoid leaks in pools.
...
**Short:** A daemon thread doesn't block process exit once all non-daemon threads finish, making it suited to background monitoring rather than work that must complete before shutdown.
...
**Short:** A single thread registers many sockets with `epoll` and blocks on `epoll_wait()`, which returns only the ready file descriptors, replacing one blocked OS thread per connection.
...
**Short:** Threads move through New, Ready, Running, Blocked, and Terminated, transitioning on events like `start()`, preemption, I/O calls, and completion.
...
**Short:** The thread stays Blocked indefinitely, so production code should always pair blocking I/O calls with timeouts plus a watchdog or circuit breaker.
...
**Short:** Concurrency means tasks are in progress and may interleave on one core; parallelism means they run simultaneously on multiple cores, so parallel work is always concurrent but not vice versa.
...
**Short:** Creating an OS thread costs about 10-100 microseconds versus roughly 0-1 microsecond to submit a task to a pre-created pool, so pools amortize creation cost across many tasks.
...
- [`devops/linux_and_os_fundamentals`](../../devops/linux_and_os_fundamentals/linux_and_os_fundamentals.md) — cgroups, namespaces, OOM killer, /proc/sched, perf
- [`java/concurrency`](../../java/concurrency/concurrency.md) — JVM thread model, virtual threads, happens-before, volatile, synchronized
- [`python/threading_and_multiprocessing`](../../python/threading_and_multiprocessing/threading_and_multiprocessing.md) — GIL mechanics, multiprocessing, asyncio applied depth
- [deadlocks_and_synchronization](../deadlocks_and_synchronization/deadlocks_and_synchronization.md) — mutex, semaphore, Coffman conditions
- [cpu_scheduling_algorithms](../cpu_scheduling_algorithms/cpu_scheduling_algorithms.md) — how the OS scheduler decides which thread runs next
