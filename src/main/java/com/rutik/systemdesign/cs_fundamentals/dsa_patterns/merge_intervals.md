...
**Short:** Sorting by start lets you compare each interval only against the most recently merged one, instead of rechecking every previously merged interval on each step.
...
**Short:** Merge Intervals sorts by start to combine overlapping ranges, while Non-overlapping Intervals sorts by end since the earliest-ending interval leaves the most room for future picks.
...
**Short:** Use `<=` for closed intervals that share an overlapping endpoint, and `<` for half-open intervals where a shared boundary point doesn't count as conflicting.
...
**Short:** The three-phase approach exploits that the input is already sorted and non-overlapping for one O(n) scan; append-and-resort's O(n log n) is fine when that precondition isn't emphasized.
...
**Short:** Sweep independently sorted start and end times with two pointers, incrementing rooms on an earlier start and decrementing on an earlier end, tracking the peak reached.
...
**Short:** Maintain a min-heap of occupied rooms' end times, reusing the earliest-freeing room when its end time is at or before the new interval's start, else allocating a new one.
...
**Short:** Merge all employees' intervals into one union of busy times, then the gaps between consecutive merged intervals are the free-time slots common to everyone.
...
**Short:** Record `+change` at each interval's start and `-change` at its end in a delta array, then take its prefix sum to get the net effect at every point in one pass.
...
**Short:** No — finding which intervals overlap without sorting still needs O(n^2) pairwise checks or a sweep line that itself requires sorting, so Union-Find doesn't remove that need.
...
**Short:** Tie-breaking on start doesn't matter for plain merging since either order still merges correctly, but it can matter for end-sorted greedy variants case by case.
...
**Short:** Repeated insertion into a Python list costs O(n) per insert for O(n^2) total, while a balanced structure like SortedList gives O(log n) insertion for O(n log n) overall.
