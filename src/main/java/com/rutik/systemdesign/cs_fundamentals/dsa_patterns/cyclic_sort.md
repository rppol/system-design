...
**Short:** Each element is placed into its final position at most once, bounding total swaps and pointer advances by n, so total work is O(n).
...
**Short:** The range must match the array's length so `f(v)` is always a valid index, which is what lets the array double as its own hash table.
...
**Short:** Skip swapping any value outside `[1, n]` during placement, since it can never be the answer and its slot correctly reads as missing in the second pass.
...
**Short:** Negation marking flips the sign at index `abs(nums[i])-1` to mark values seen in one O(n)/O(1) pass without swapping, but leaves the array's signs mutated.
...
**Short:** Cyclic sort mutates the array while Floyd's fast/slow pointers find the duplicate without mutating it — use fast/slow when the input must stay unmodified.
...
**Short:** A value of 0 computes `correct_index = -1`, which Python silently treats as the last element instead of erroring, corrupting results with no visible crash.
...
**Short:** No — it only works when values approximately permute the indices, though generalizing it with a separate output array yields counting sort.
...
**Short:** Termination requires every swap to strictly increase correctly-placed elements and the else branch to fire whenever `nums[i]` is already correct or duplicate.
...
**Short:** Not easily — each swap's incoming value must itself be checked next, creating a sequential dependency chain despite the O(1)-space, O(n)-time guarantee.
...
**Short:** The algorithm traces the permutation's cycle decomposition, needing exactly `n` minus the number of cycles swaps to place every element.
...
**Short:** Yes — placement still works per value, but it's the second scan for mismatches, not the placement pass, that surfaces duplicates or missing values.
