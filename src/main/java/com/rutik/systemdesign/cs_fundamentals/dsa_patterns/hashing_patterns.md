...
**Short:** A good hash function keeps O(1) elements per bucket at typical load factors, degrading toward O(n) on heavy collisions, which Java 8+ mitigates by treeifying buckets past 8 entries.
...
**Short:** Sorting destroys the value-to-original-index mapping, so returning indices needs a hashmap or (value, index) pairs, adding work over plain hashing.
...
**Short:** `Counter` builds frequency counts in one call, supports counter arithmetic, and returns 0 instead of `KeyError` for missing keys, unlike a manual `defaultdict`.
...
**Short:** A canonical key must be identical for items that should group together, different otherwise, and hashable — `sorted(word)` satisfies all three for anagrams.
...
**Short:** Only run-starting numbers trigger the inner loop, so every number is visited by exactly one inner loop across all runs, keeping total work O(n).
...
**Short:** Precompute all A+B pairwise sums into a hashmap in O(n^2), then look up `-(c+d)` for each C×D pair, cutting brute-force O(n^4) to O(n^2).
...
**Short:** A hashmap from value to array index makes finding a value's position O(1), which is what lets delete swap-with-last-and-pop in O(1) instead of O(n).
...
**Short:** Not in general — an unsorted array with indices preserved needs O(n) state, though sorting first (if indices aren't needed) allows O(1) extra space.
...
**Short:** A collision is two keys hashing to the same slot; chaining links them in a bucket while open addressing probes for the next empty slot, both O(1) amortized normally.
...
**Short:** A polynomial rolling hash updates in O(1) by removing the outgoing character's contribution and adding the incoming one, avoiding an O(k) rehash on every shift.
...
**Short:** Yes — equal objects must hash equal, and violating that (overriding `__eq__` without `__hash__`) can make membership checks silently return False.
