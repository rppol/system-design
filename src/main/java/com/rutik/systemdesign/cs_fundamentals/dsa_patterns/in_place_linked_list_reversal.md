...
- **Recursion** ([recursion_and_problem_solving_patterns](../recursion_and_problem_solving_patterns/recursion_and_problem_solving_patterns.md)) — every iterative reversal has a recursive formulation; understanding the recursive version (`reverse_list_recursive`) builds intuition for *why* `head.next.next = head` is the key relinking step, even if you'd write the iterative version in an actual interview for O(1) space.
