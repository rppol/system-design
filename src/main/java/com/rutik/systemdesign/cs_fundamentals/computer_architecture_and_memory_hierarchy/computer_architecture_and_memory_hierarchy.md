...
**Short:** Sorting groups branch outcomes contiguously so the branch predictor learns the pattern instead of mispredicting ~50% of random branches, each costing a ~15-cycle pipeline flush.
...
**Short:** False sharing is two threads writing different variables on the same cache line, causing MESI invalidation ping-pong; fix by aligning each variable to its own cache-line boundary.
...
**Short:** The three types are compulsory (cold, unavoidable), capacity (working set too big), and conflict (addresses collide in a set-associative cache), each stalling the CPU while data is fetched from a lower level.
...
**Short:** MESI keeps per-core caches coherent using four line states (Modified, Exclusive, Shared, Invalid), broadcasting invalidations whenever a core writes a Shared line.
...
**Short:** Spatial locality means nearby addresses are likely accessed soon, exploited by loading a full cache line, while temporal locality means the same address is likely reaccessed soon.
...
**Short:** A RAW data hazard, where an instruction reads a register before a prior one writes it, is resolved by forwarding the Execute stage's output directly into the next instruction's input.
...
**Short:** A misprediction flushes speculatively-executed instructions when a branch resolves the wrong way, costing roughly 15 cycles on modern Intel/AMD CPUs.
...
**Short:** A cache line is the minimum data-transfer unit between cache levels: 64 bytes on x86-64 and ARM Cortex-A, but 128 bytes on Apple Silicon.
...
**Short:** NUMA means each socket has its own local DRAM (~100 ns), while accessing another socket's memory over the interconnect costs ~200-300 ns, roughly double the latency.
...
**Short:** Linked-list traversal is a pointer chase the prefetcher cannot predict, so each node access risks an L3 or DRAM miss, making it 5-20x slower than an array despite identical O(n).
...
**Short:** Write-through updates cache and the next level on every store, while write-back marks the line dirty and defers the write until eviction; x86-64 uses write-back at every cache level by default.
...
**Short:** It detects stride patterns in miss addresses and speculatively fetches ahead, but handles pointer chasing and irregular strides poorly since the next address is unknown in advance.
...
**Short:** The BTB caches branch addresses mapped to predicted targets so the CPU can start fetching from the target before fully decoding the branch instruction.
...
**Short:** NumPy's default row-major (C-order) layout makes row-by-row iteration sequential and cache-friendly, while column-by-column iteration jumps by a full row stride and likely misses cache.
...
**Short:** Roughly registers under 1 ns, L1 ~1-4 ns, L2 ~10 ns, L3 ~40 ns, DRAM ~100 ns, NVMe SSD ~100 us, and HDD ~10 ms — each level about an order of magnitude apart.
...
**Short:** Out-of-order execution issues ready instructions regardless of program order, hiding a ~130-cycle L3-miss stall by keeping execution units busy on independent work.
...
- [processes_threads_and_context_switching](../processes_threads_and_context_switching/processes_threads_and_context_switching.md) — TLB flush on context switch; OS thread model; per-core stack allocation
- [memory_management_and_virtual_memory](../memory_management_and_virtual_memory/memory_management_and_virtual_memory.md) — TLB, page tables, physical frames, virtual address space layout; how the MMU translates virtual to physical addresses
- [../../java/concurrency](../../java/concurrency/concurrency.md) — `@Contended` annotation, false sharing in Java, memory barriers, `volatile` happens-before, `LongAdder` internals, JMM
