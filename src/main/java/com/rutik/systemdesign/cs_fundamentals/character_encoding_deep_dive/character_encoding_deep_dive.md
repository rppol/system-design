...
covered generally in [number_systems_and_bit_manipulation](../number_systems_and_bit_manipulation/number_systems_and_bit_manipulation.md).
...
**Short:** A byte is a fixed 8-bit storage unit, while a character is an abstract Unicode code point that can need 1 to 4 bytes depending on the encoding.
...
**Short:** Most languages count code points or UTF-16 units, not grapheme clusters, so one emoji built from a ZWJ sequence or skin-tone modifier can span 2 to 7 code points.
...
**Short:** Unicode allows the same visible character as two different, canonically-equivalent code point sequences, so raw `==` fails unless both strings are normalized to the same form first.
...
**Short:** Mojibake is garbled text from decoding bytes with the wrong encoding, and it stays silent because legacy 8-bit encodings treat every byte value 0-255 as valid.
...
**Short:** A surrogate pair is two 16-bit UTF-16 code units from reserved ranges that together encode one code point above U+FFFF, a retrofit for Unicode outgrowing 16 bits.
...
**Short:** Latin-1 never raises a decode error because it maps every byte 0-255 to a code point, so non-Latin-1 input silently decodes into the wrong characters instead of failing.
...
**Short:** MySQL's legacy `utf8` (now `utf8mb3`) caps at 3 bytes per character and excludes all 4-byte astral characters including emoji; `utf8mb4` is the true UTF-8 fix.
...
**Short:** UTF-8 is a byte-level superset of ASCII, self-synchronizing, and has no byte-order ambiguity, unlike UTF-16/UTF-32, which need explicit endianness.
...
**Short:** UTF-8 picks the smallest of four byte-length classes (1-4 bytes, holding 7/11/16/21 payload bits) that can fit the code point's value.
...
**Short:** A code point is just a numeric ID Unicode assigns to a symbol, whereas the perceived "character" is a grapheme cluster that can be built from several code points.
...
**Short:** A plane is a block of 65,536 code points; the BMP (Plane 0) holds nearly every living script, while astral Plane 1 holds most emoji.
...
**Short:** NFC/NFD are lossless canonical forms (composed vs. decomposed), while NFKC/NFKD additionally apply lossy compatibility folding of presentational variants.
...
**Short:** Continuation bytes always match `10xxxxxx` while lead bytes never do, so scanning back at most 3 bytes always finds a character boundary, unlike parity-dependent Shift-JIS.
...
**Short:** The BOM (U+FEFF) declares encoding and byte order for UTF-16/UTF-32, but a UTF-8 BOM is never required and can break shebangs, JSON, and CSV headers.
...
**Short:** Endianness is the byte order of a multi-byte value, which matters for 2- or 4-byte UTF-16/UTF-32 units but not for UTF-8, which is always single bytes.
...
**Short:** Reverse by extended grapheme cluster, never by raw byte or code point, since either of those splits a multi-code-point character into pieces that no longer render correctly.
...
**Short:** A homoglyph attack swaps in a visually near-identical character from another script, and Unicode's compatibility folding and confusables data (UTS #39) are used to detect it.
...
**Short:** macOS filesystems normalize filenames to NFD while Linux and Git treat NFC as canonical, so identical-looking names differ byte-for-byte across systems.
...
**Short:** Yes — every ASCII byte 0x00-0x7F is identical in UTF-8, so any pure-ASCII data is already valid UTF-8 with zero conversion, unlike in UTF-16 or UTF-32.
...
**Short:** Percent-encoding turns non-ASCII bytes into `%XX` triplets, and modern practice encodes UTF-8 bytes, so a receiver assuming the wrong source encoding decodes the wrong character.
...
- [number_systems_and_bit_manipulation](../number_systems_and_bit_manipulation/number_systems_and_bit_manipulation.md) — bit patterns, endianness, and byte-level manipulation underlying every encoding in this module
- [arrays_strings_and_hashing](../arrays_strings_and_hashing/arrays_strings_and_hashing.md) — hash table and string internals that depend on consistent, normalized encoding for correct key equality
- [`python/strings_bytes_encoding_and_regex`](../../python/strings_bytes_encoding_and_regex/strings_bytes_encoding_and_regex.md) — Python's `str`/`bytes` split, PEP 393 compact representation, `codecs`, and Unicode-aware regex in full applied depth
- [`java/strings_and_text`](../../java/strings_and_text/strings_and_text.md) — Java's Compact Strings (LATIN1/UTF16 coder), `String.codePoints()` vs `chars()`, and surrogate pairs in practice
