...
**Short:** In FCFS, one long CPU-bound process at the head of the queue blocks every process behind it, so average waiting time can grow to O(longest_burst).
...
**Short:** An exchange argument proves SJF optimal, since swapping any out-of-order adjacent pair reduces average wait, but it is impractical because burst times aren't known in advance.
...
**Short:** RR gives every process CPU within one quantum for much better response time, but slightly lower throughput than FCFS due to frequent context-switch overhead.
...
**Short:** MLFQ demotes processes that use their full quantum (assumed CPU-bound) and keeps processes that block early at high priority, approximating shortest-job-first without knowing burst times.
...
**Short:** `SCHED_NORMAL` has used EEVDF since Linux 6.6 (CFS before that), tracking virtual runtime and deadlines rather than the fixed quantum Round-Robin uses.
...
**Short:** Priority inversion lets a medium-priority thread indirectly block a high-priority one via a mutex held by a low-priority thread; priority inheritance temporarily boosts the lock holder to fix it.
...
**Short:** A short quantum gives great response time but pays hidden costs from cold caches and flushed TLBs on every switch, while a long quantum trades response time for throughput.
...
**Short:** SCHED_DEADLINE implements Earliest Deadline First, guaranteeing a task's declared runtime within its period, and suits hard real-time work like audio processing, not general scheduling.
...
**Short:** A CPU burst is pure computation and an I/O burst is time blocked on disk/network; schedulers infer interactive vs. CPU-bound processes from observed burst patterns, not declarations.
...
**Short:** Nice values -20 to +19 map to weights via a fixed table where each step differs by ~25%, so a nice-19 thread gets only about 1.4% of a nice-0 competitor's CPU share.
...
**Short:** Latency is the wait until a runnable thread gets CPU, while throughput is tasks completed per second, and lowering latency needs frequent switches that cost throughput-eating overhead.
...
**Short:** Pinning a thread to a core keeps its working set warm in that core's L1/L2 and reduces NUMA overhead, which is why databases and trading systems pin hot threads to isolated cores.
...
**Short:** An idle CPU steals tasks from the back of another CPU's deque while the owner works from the front, achieving near-optimal load balancing with rare synchronization, as in Java's ForkJoinPool.
...
**Short:** Preemptive scheduling bounds latency by letting the OS forcibly reclaim the CPU, while non-preemptive scheduling can be delayed arbitrarily by a thread that never yields.
...
**Short:** Every context switch flushes the evicted process's cache footprint, so running more threads than fit their combined working sets in shared L3 causes extra cache misses regardless of scheduler tuning.
