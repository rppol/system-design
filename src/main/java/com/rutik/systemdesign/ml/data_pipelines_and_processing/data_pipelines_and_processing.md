...
<!-- study-paths
senior: data_pipelines_and_processing.md
...
**Short:** Training/serving skew happens when features differ between training and serving pipelines, prevented by sharing one feature computation library and validating via shadow mode.
...
**Short:** Point-in-time correctness restricts each training sample to feature values known before its own timestamp, enforced via AS-OF joins to prevent future-data leakage.
...
**Short:** PySpark's lazy evaluation builds a DAG of transformations that only executes on an action, letting the Catalyst optimizer reorder steps like pushing filters before joins.
...
**Short:** Narrow transformations like map and filter need no shuffle, while wide transformations like groupBy and join move data across the network and are far more expensive.
...
**Short:** DVC hashes dataset files and tracks a dvc.yaml pipeline DAG, letting git checkout plus dvc pull and dvc repro deterministically reproduce an exact dataset and model.
...
**Short:** PSI sums (actual%-expected%)*ln(actual%/expected%) across buckets to quantify feature drift, with values above 0.2 signaling a shift serious enough to trigger retraining.
...
**Short:** Schema evolution should go through a schema registry enforcing backward compatibility, handling breaking changes via new versions, backfill migrations, and versioned output paths.
...
**Short:** Spark pipeline OOMs commonly come from cartesian joins, collect() on large DataFrames, oversized shuffle partitions, or skewed data, fixed by tuning partitions and salting.
...
**Short:** A feature store centralizes feature computation across an offline store for training and an online store for low-latency serving, eliminating duplicated logic and training/serving skew.
...
**Short:** Data quality validation layers schema checks, statistical checks like PSI and null rate, business rule assertions, and referential integrity, halting the pipeline on any failure.
...
**Short:** Avro is a row-oriented format suited to write-heavy streaming ingestion, while Parquet is column-oriented and suited to read-heavy analytical feature computation.
...
**Short:** Batch pipelines suit scheduled feature refreshes on cheap infrastructure, while streaming is justified only when serving needs features fresher than about an hour.
...
**Short:** Lambda architecture merges a separate batch and speed layer at serving time, while Kappa architecture reprocesses one streaming codebase from a replayable log.
...
**Short:** Pipeline idempotency is guaranteed by deterministic output paths keyed by partition and version combined with overwrite rather than append writes.
...
**Short:** A broadcast join ships a small table to every executor so a large table avoids shuffling, and Spark auto-triggers it below the autoBroadcastJoinThreshold, default 10 MB.
...
**Short:** The small-files problem is thousands of tiny output files from high-parallelism writes that slow reads with metadata overhead, fixed by coalescing to 128-256 MB files.
...
**Short:** Late-arriving streaming data is handled with event-time windows bounded by a watermark that decides how long to wait before finalizing and dropping stragglers.
...
**Short:** Data lineage records the inputs, code version, and parameters behind each dataset so a degraded model can be traced back to its exact upstream source.
...
**Short:** At-least-once processing risks duplicate events on retry, while exactly-once guarantees a single effect per event via idempotent writes or transactional checkpoints.
