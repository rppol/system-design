...
<!-- study-paths
principal: model_selection_and_algorithm_choice.md
...
**Netflix (content ranking):** two-phase retrieval-then-rank is the standard architecture — a cheap retrieval stage narrows the candidate set, then an expensive ranker scores the survivors — and it lets each phase optimize its own inductive bias and latency budget. Do not attach billion-item retrieval scale to Netflix: Netflix does not publish catalogue counts, but every third-party tracker puts the global library in the **tens of thousands of titles, not millions** — and the argument only needs that order of magnitude, which is safe by three orders regardless of whose counting method you prefer. Billion-scale two-tower retrieval is the regime of open user-generated catalogues such as YouTube, not of a licensed video library.
...
**Short:** LightGBM's sequential boosting has lower bias than Random Forest's bagging, but RF wins on speed, noisy labels, or tiny datasets.
...
**Short:** Pick logistic regression under about 10k rows or strict latency limits, and GBDT once data is larger with complex feature interactions.
...
**Short:** Neural networks need far more labeled data and compute than GBDT, which already reaches state-of-the-art on most tabular business problems.
...
**Short:** Inductive bias is the assumptions an algorithm encodes about the data, and matching it to the true structure cuts the data needed to learn.
...
**Short:** Run a high-accuracy scorer alongside a SHAP explanation or surrogate model, but never let a disagreeing surrogate stand in for the real reason.
...
**Short:** Survival analysis beats logistic regression for churn when the timing of the event, not just whether it happens, matters to the business.
...
**Short:** High AUC only proves good ranking, so check calibration, per-segment performance, decision-threshold metrics, and temporal drift before trusting it.
...
**Short:** Two-tower models win with rich item features, cold-start needs, and large catalogs; matrix factorization wins on small, dense catalogs.
...
**Short:** Wrong inductive bias shows up as irreducible bias on learning curves that plateau below target and stay flat even as training data grows.
...
**Short:** Model selection chooses the algorithm class, while hyperparameter tuning optimizes settings within whichever class was already chosen.
...
**Short:** Pre-compute outputs when the input space is enumerable and stable, and score online whenever request-time context carries real signal.
...
**Short:** A feature unusable at serving time should usually be dropped from training rather than left to cause silent training-serving skew.
...
**Short:** Monotonic constraints force a GBDT's output to respect required directions, like higher income meaning lower risk, satisfying regulators.
...
**Short:** SHAP values give locally additive, consistent per-instance explanations, which is why they beat gain importance for audits and stakeholders.
...
**Short:** A diverse-algorithm ensemble beats a single model only when the base models make uncorrelated errors, not when they are all similar GBDTs.
...
**Short:** Tree models cannot extrapolate beyond their training range, so growing features need a linear or ratio term, not simply a different algorithm.
...
**Short:** Severe class imbalance calls for PR-AUC or recall-at-fixed-precision as the metric, plus class-weighted GBDT over naive resampling.
...
**Short:** Match categorical encoding to the model class: ordered target encoding for GBDT, hashing for linear models, embeddings for deep networks.
...
**Short:** No Free Lunch means no algorithm dominates uniformly over all problems, but real data is structured, so matching inductive bias still wins.
