...
<!-- study-paths
senior: model_serving_and_inference.md
...
**Netflix Model Scoring Service (MSS)**: one shared inference backend behind a single gRPC interface for XGBoost, TensorFlow, PyTorch and LLMs, with NVIDIA Triton underneath handling model loading, batching and GPU scheduling, vLLM as the LLM engine, and a JVM control plane owning routing, feature retrieval, candidate generation, post-processing and logging alongside deployment, versioning, autoscaling and multi-region rollout. Netflix's own account (July 2026) is worth reading for the engine decision as much as the architecture: MSS originally ran TensorRT-LLM, and they re-benchmarked and moved to vLLM in 2025 not on raw throughput — open engines had closed that gap — but on operational fit, because vLLM loads custom architectures without a compilation step, exposes hooks for custom decoding, and is far easier to debug than a compiled engine. The lesson is the boundary: the inference server owns batching and the GPU, the control plane owns rollout, and the engine underneath is replaceable precisely because that boundary is clean. One operational scar they call out: mismatched Triton and vLLM versions can stop a deployment loading, so pin them together.
...
**War story 1: The cold-start latency spike.** A team deployed a PyTorch model on GPU for a payment fraud endpoint. P99 latency was acceptable at steady state but spiked after the autoscaler added a new pod. Root cause: the first CUDA call on a fresh process pays for context creation, and the first inference then pays again for cuDNN/cuBLAS algorithm selection at that input shape. Do not budget a number for the context-creation cost from a document like this one — NVIDIA publishes none, developer-forum measurements span an order of magnitude, and the real figure moves with GPU memory size, driver version and whether persistence mode is on. What you can rely on is the *ordering*, and that is enough to design against: container image pull dominates, weight load is next, CUDA context creation is smaller than both, and all three land on whichever request arrives first. Measure your own by timing a `cudaFree(0)` in a fresh process with persistence mode on and off. Fix: warm-up requests sent during pod startup, gated behind the readiness probe so no user request is the one that pays.
