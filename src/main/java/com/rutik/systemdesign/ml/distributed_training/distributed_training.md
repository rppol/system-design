...
<!-- study-paths
senior: distributed_training.md
...
**Short:** DDP runs one process per GPU with symmetric NCCL AllReduce achieving 90-95% scaling efficiency, versus DataParallel's single master-GPU threading model at only 60-75%.
...
**Short:** Ring-AllReduce synchronizes gradients via ReduceScatter then AllGather around a logical ring, and NCCL over NVLink or InfiniBand is the right GPU backend versus Gloo for CPU training.
...
**Short:** Gradient accumulation delays the optimizer step across M micro-batches to reach a larger effective batch size, using model.no_sync() to skip AllReduce on intermediate steps.
...
**Short:** FSDP shards parameters, gradients, and optimizer states across N GPUs to roughly 1/N of DDP's per-GPU memory, temporarily AllGathering full parameters only during each forward pass.
...
**Short:** ZeRO Stage 1 shards optimizer states, Stage 2 adds gradient sharding, and Stage 3 additionally shards parameters, matching FSDP's full-shard memory profile.
...
**Short:** BF16 keeps FP32's 8 exponent bits so it avoids the gradient underflow and overflow that forces FP16 training to rely on a GradScaler.
...
**Short:** Distributed checkpointing must save only from rank 0, barrier before other ranks proceed, and unwrap DDP's .module state dict to avoid partial or mismatched saves.
...
**Short:** The linear scaling rule multiplies learning rate by N when data-parallel workers multiply by N, holding reliably up to about 8x before needing an LR warmup.
...
**Short:** Pipeline parallelism splits layers across GPUs and wastes (p-1)/m of each step as an idle bubble during fill and drain, which 1F1B interleaving shrinks further.
...
**Short:** Model FLOP utilization is achieved FLOP/s divided by peak hardware FLOP/s, with well-tuned LLM training on A100s reaching 38-45% and below 30% signaling a bottleneck.
...
**Short:** Gradient checkpointing recomputes activations from saved checkpoints during backward instead of storing them all, trading roughly 33% extra compute for large activation-memory savings.
...
**Short:** Communication dominates distributed training because network bandwidth has scaled far slower than GPU compute, making AllReduce time, not FLOPs, the limiting factor at scale.
...
**Short:** Synchronous DDP averages gradients identically across all replicas each step, while asynchronous parameter-server training risks stale gradients from workers updating without waiting.
...
**Short:** A straggler is the slowest worker that stalls a synchronous AllReduce step, mitigated by equal-size batches via drop_last=True and per-rank utilization profiling.
...
**Short:** NCCL hangs happen when ranks issue mismatched collectives, shapes, or batch counts, diagnosed with NCCL_DEBUG=INFO and TORCH_DISTRIBUTED_DEBUG=DETAIL.
...
**Short:** Ring-AllReduce keeps per-GPU data sent at roughly 2(N-1)/N of the gradient size regardless of GPU count, which is why it scales better than a naive parameter-server scheme.
...
**Short:** FP16 training keeps FP32 master weights and uses GradScaler to multiply the loss before backward so small gradients don't underflow to zero in FP16's narrow range.
...
**Short:** Weak scaling grows the global batch with GPU count to keep per-GPU work fixed, while strong scaling splits a fixed batch across more GPUs and quickly hits diminishing returns.
...
**Short:** ZeRO-Infinity offloads parameter and optimizer shards to CPU RAM and NVMe to enable trillion-parameter models, at the cost of PCIe and NVMe bandwidth becoming the new bottleneck.
...
## See Also
- [Training Infrastructure (LLM)](../../llm/training_infrastructure/training_infrastructure.md) — tensor/pipeline parallelism, ZeRO, FSDP at LLM scale (70B+ parameters)
- [ML Platform & GPU Infrastructure (DevOps)](../../devops/ml_platform_and_gpu_infrastructure/ml_platform_and_gpu_infrastructure.md) — GPU node lifecycle, cluster orchestration, cost management
