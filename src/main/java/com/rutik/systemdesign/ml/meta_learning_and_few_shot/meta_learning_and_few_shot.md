...
GPT-3-style in-context learning (Brown et al., 2020) hands a frozen model a prompt containing a handful of demonstrations (the support-set analogue) followed by a query, and reads off a prediction with ZERO gradient updates. Pretraining over trillions of tokens plays the role of meta-training over an enormous, implicit task distribution; the forward pass's attention over the demonstrations plays the role of the inner loop. The difference from MAML/Reptile is stark: those methods still run explicit gradient steps at meta-test time — see [Fine-Tuning](../../llm/fine_tuning/fine_tuning.md) for that explicit, gradient-based adaptation path — while in-context learning runs none; all of the "adaptation" is amortized into the pretrained weights and expressed purely through attention at inference time. Modern long-context models push this further with hundreds or thousands of in-context examples ("many-shot" in-context learning), continuing to improve accuracy well past the handful of examples classic few-shot benchmarks use.
...
**Q: What is task/class leakage in few-shot learning, and why does it silently inflate accuracy?**
**Short:** Task/class leakage happens when meta-test classes were already seen during meta-training, so the model recalls known classes instead of adapting from scratch.
...
**Q: Why does using an ImageNet-pretrained backbone for miniImageNet few-shot evaluation give misleadingly high numbers?**
**Short:** miniImageNet's 'novel' test classes are drawn from the same 1,000 ImageNet classes the backbone was fully supervised on, so few-shot accuracy reflects recall, not adaptation.
...
**Q: What does "training and testing conditions must match" mean in few-shot learning, and why does it matter?**
**Short:** Meta-training episodes must mirror the exact N-way K-shot structure expected at test time, or deployment accuracy can badly undershoot the benchmark number.
...
**Q: Why can transductive batch normalization inflate reported few-shot accuracy?**
**Short:** Computing BatchNorm statistics jointly over support and query images lets query-set information leak into the support-set forward pass, inflating accuracy.
...
**Q: Why is a single small meta-test run (for example, 100 episodes) an unreliable way to report few-shot accuracy?**
**Short:** Episode-sampling variance is large enough that a single 100-episode run can shift several accuracy points on a re-run, so 600+ episodes with a confidence interval are standard.
...
**Q: What is the difference between the support set and the query set in a few-shot episode?**
**Short:** The support set is the small labeled set the model adapts on, while the disjoint query set from the same classes evaluates that adaptation.
...
**Q: How does a Prototypical Network classify a new query example?**
**Short:** It averages each class's support embeddings into a prototype, then classifies the query by softmax over negative squared distance to every prototype, with no gradient step.
...
**Q: Why do Prototypical Networks use squared Euclidean distance instead of cosine similarity?**
**Short:** Squared Euclidean distance is a Bregman divergence giving the prototype a principled maximum-likelihood cluster-center interpretation that cosine similarity lacks.
...
**Q: What does MAML actually optimize for, and how is that different from ordinary supervised training?**
**Short:** MAML optimizes an initialization so a few gradient steps on any new task's support set yield low query loss, not the loss of the initialization itself.
...
**Q: Why does MAML require second-order gradients, and what does First-Order MAML (FOMAML) drop?**
**Short:** MAML's outer loss depends on the initialization through the inner-loop gradient step itself, requiring a Hessian-vector product that FOMAML approximates away.
...
**Q: How does Reptile differ from MAML, and why is it cheaper to compute?**
**Short:** Reptile moves the initialization toward weights reached by ordinary SGD on a task, with no query set and no second derivative at all, unlike MAML.
...
**Q: How does the LSTM meta-learner (Ravi and Larochelle) turn gradient descent itself into a learned model?**
**Short:** It maps the LSTM cell-state update onto the parameter-update rule, with forget and input gates acting as a learned per-step decay and learning rate.
...
**Q: How do Memory-Augmented Neural Networks (MANN) solve few-shot learning without any gradient-based adaptation?**
**Short:** MANN writes each support example into an external content-addressable memory and retrieves query labels by similarity, with no gradient step at test time.
...
**Q: What is the practical difference between transfer learning and meta-learning for a few-shot problem?**
**Short:** Meta-learning explicitly trains across simulated few-shot episodes matching the test protocol, but a well-tuned transfer-learning baseline often matches or beats it.
...
**Q: What is the connection between meta-learning and in-context learning (ICL) in large language models?**
**Short:** In-context learning is meta-learning with the inner loop removed, adapting entirely within one forward pass over prompt demonstrations with zero gradient updates.
...
**Q: What happens to few-shot accuracy as K (the number of shots per class) increases from 1 to 5 to 20, and why does it plateau?**
**Short:** Accuracy rises steeply from 1-shot to 5-shot as averaging reduces prototype variance, then plateaus past roughly 20 shots once intra-class noise dominates.
...
**Q: What happens to few-shot accuracy as N (the number of classes, or "way") increases?**
**Short:** Accuracy decreases monotonically as the number of ways N grows, since chance accuracy falls as 1/N and more confusable class boundaries appear.
...
**Q: How would you handle a query example whose true class is not among the episode's N support classes at all?**
**Short:** Standard closed-set N-way classification forces a label onto every query, so open-set recognition needs an explicit rejection threshold or outlier prototype.
...
**Q: Why is Omniglot considered an "easier" benchmark than miniImageNet, and what does that mean for interpreting a reported number?**
**Short:** Omniglot's low-variance, well-aligned characters push most methods into the high 90s, so a strong Omniglot score alone says little about realistic performance.
...
**Q: Why was tieredImageNet introduced as a benchmark when miniImageNet already existed?**
**Short:** tieredImageNet splits classes at the super-category level to stop near-identical classes from sitting on opposite sides of miniImageNet's flat train/test split.
...
## See Also
- [Self-Supervised and Contrastive Learning](../self_supervised_and_contrastive_learning/self_supervised_and_contrastive_learning.md) — the pretraining objectives (SimCLR, BYOL) that often supply the encoder a meta-learning method meta-trains further
- [Neural Network Fundamentals](../neural_network_fundamentals/neural_network_fundamentals.md) — backpropagation and the chain rule that MAML's second-order gradient extends one level further
- [Active Learning and Weak Supervision](../active_learning_and_weak_supervision/active_learning_and_weak_supervision.md) — a complementary answer to label scarcity: choosing what to label, rather than adapting from very few labels
- [Fine-Tuning (LLM)](../../llm/fine_tuning/fine_tuning.md) — the explicit, gradient-based adaptation counterpart to Section 4.5's in-context learning connection
