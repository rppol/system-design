...
<!-- study-paths
principal: causal_inference_and_ml.md
...
**Short:** We can never observe both potential outcomes for the same unit, so every causal method is a strategy for estimating that missing counterfactual.
...
**Short:** They are equal only when ignorability and positivity hold, as in a randomized experiment or when all confounders are measured and conditioned on.
...
**Short:** The propensity score e(x)=P(T=1|X=x) is a balancing score, so conditioning on this single number balances all observed covariates between treated and control.
...
**Short:** ATE averages the treatment effect over everyone, ATT averages it only over those actually treated, and CATE gives the effect as a function of covariates.
...
**Short:** Double ML partials confounders out of both outcome and treatment before regressing residuals, and cross-fitting prevents the overfitting bias that would otherwise correlate residuals with treatment.
...
**Short:** Validate with synthetic data of known effects, Qini/AUUC uplift curves, held-out A/B tests on top and bottom CATE quintiles, calibration checks, and placebo tests.
...
**Short:** Parallel trends requires treated and control groups to have moved identically absent treatment, and it fails whenever the groups were already trending differently before treatment.
...
**Short:** A valid instrument must be relevant to treatment, affect the outcome only through treatment, and be as-good-as-randomly assigned, identifying the LATE among compliers.
...
**Short:** CausalForest splits to maximize heterogeneity in treatment effects rather than variance reduction in Y, and uses honest splitting with separate samples for splits and leaf estimates.
...
**Short:** Use cohort-level difference-in-differences against a pre-rollout baseline, verifying parallel pre-period trends before trusting the estimate.
...
**Short:** Correlation is mere association, prediction estimates P(Y|X) to minimize loss, and causation is P(Y|do(X)); a highly accurate predictive model can still rest on anti-causal, spurious correlations.
...
**Short:** A post-treatment covariate is itself affected by treatment, so adjusting for it either removes part of the true effect or induces spurious collider bias.
...
**Short:** Positivity requires every covariate value to have both treated and control units, and violating it makes IPW weights explode for the unrepresented segment.
...
**Short:** The S-learner fits one model with treatment as a feature and can regularize the treatment signal away entirely when its effect is weak relative to the covariates.
...
**Short:** Use Rosenbaum bounds or the E-value to quantify how strong a hidden confounder would need to be to fully explain away the observed effect.
...
**Short:** Use DiD when you have before/after panel data and confounders are time-invariant, since differencing cancels fixed unmeasured differences that matching cannot handle.
...
**Short:** It combines an outcome model and a propensity model and stays consistent as long as either one alone is correctly specified.
