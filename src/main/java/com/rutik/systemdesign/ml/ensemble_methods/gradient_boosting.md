...
**Short:** Gradient boosting fits each new tree to the negative loss gradient rather than raw residuals, a framing that unifies regression, classification, ranking, and quantile losses under one algorithm.
...
**Short:** A smaller shrinkage learning rate needs more boosting rounds to reach the same training loss but generalizes better, like gradient descent with a smaller step size avoiding overshoot.
...
**Short:** AdaBoost is locked into reweighting samples under a fixed exponential loss with stumps, while gradient boosting fits any differentiable loss's negative gradient with standard trees, allowing robust losses like Huber.
...
**Short:** Boosting sequentially chases residuals so extra trees eventually fit noise and validation loss rises, while Random Forest's averaging of independent trees only shrinks variance and never worsens with more trees.
...
**Short:** Gradient boosting needs no feature scaling since trees are invariant to monotonic transforms, but high-cardinality categoricals should use ordinal or target encoding rather than one-hot, which starves splits.
...
**Short:** Stochastic gradient boosting trains each tree on a random row subsample, such as subsample=0.8, decorrelating trees and regularizing the model similarly to noise in stochastic gradient descent.
...
**Short:** In Random Forest max_depth barely matters since averaging controls variance, but in gradient boosting it sets how many-way feature interactions each tree captures, trading faster bias reduction for overfitting risk.
...
**Short:** The optimal number of boosting rounds is found by early stopping on validation loss for 50-100 non-improving rounds, not by fixing n_estimators from training time alone.
...
**Short:** Setting max_depth=1 turns each boosting tree into a single-feature decision stump with no interaction modeling, which is AdaBoost's default setup and a useful diagnostic for whether interactions matter.
...
**Short:** Huber loss caps the gradient contribution of large residuals beyond a delta threshold with a linear penalty instead of squared error's runaway quadratic weight, protecting boosting from outlier domination.
...
**Short:** Gradient boosting is functional gradient descent, where each added tree approximates the negative gradient of the loss with respect to the current prediction function, mirroring standard parameter updates.
...
**Short:** Gradient boosting handles K-class classification by training K trees per round, one per class's softmax pseudo-residual, so 500 rounds over 10 classes builds 5000 trees total.
...
**Short:** XGBoost's key regularization knobs are max_depth, min_child_weight, subsample, colsample_bytree, L2 lambda, L1 alpha, and gamma's minimum split-gain pruning threshold.
...
**Short:** Overfitting in gradient boosting is detected by a training-versus-validation loss gap and prevented in priority order by early stopping, a lower learning rate, shallower trees, and added regularization.
...
**Short:** HistGradientBoostingClassifier bins features once up front so each split search costs O(bins x features) instead of O(samples x features), making it 10-100x faster above 10K rows.
...
**Q: What is the deviance in gradient boosting, and where does sklearn expose the per-round training loss?**
**Short:** Deviance is the per-sample negative log-likelihood, exactly cross-entropy for binary classification; sklearn selects it as loss="log_loss" and records it per round in train_score_.
Deviance is the per-sample negative log-likelihood, which for binary classification is exactly cross-entropy and exactly what logistic regression minimises. Written out, deviance_i = -[y_i * log(p_i) + (1-y_i) * log(1-p_i)]. The term comes from generalised-linear-model theory, where it measures goodness of fit. In sklearn you ask for it by name as `GradientBoostingClassifier(loss="log_loss")`, which is also the default — "deviance" is the statistics vocabulary for the same quantity, not a separate objective. The common trap: there is no `staged_deviance_` attribute. The per-round training loss lives in `train_score_` (an ndarray of length n_estimators; with `subsample < 1` it is the in-bag loss, and `oob_improvement_` / `oob_scores_` carry the out-of-bag counterpart). Use those, or `staged_predict_proba`, for learning curves.

**Q: What is the difference between level-wise (depth-wise) and leaf-wise tree growth in gradient boosting?**
**Short:** XGBoost's level-wise growth expands every node at a depth for balanced trees, while LightGBM's leaf-wise growth always splits the highest-loss leaf for faster but more overfit-prone convergence.
