...
**Short:** Each bootstrap draw excludes a training sample with probability (1-1/N)^N, which converges to 1/e or about 36.8%, giving Random Forest a free OOB validation set on every tree.
...
**Short:** Restricting each split to m much less than p random candidate features, not just bootstrapping rows, forces different trees to pick different root splits, which is what actually lowers inter-tree correlation.
...
**Short:** MDI sums training-time impurity reduction per feature and is biased toward high-cardinality features, while permutation importance measures held-out metric drop from shuffling and is unbiased but costlier.
...
**Short:** Random Forest splits are invariant to any monotonic transform of a feature, so scaling or standardizing before training changes nothing and is a common wasted or leaky preprocessing step.
...
**Short:** Random Forest tuning priority is n_estimators first at 300-500 confirmed via OOB, then max_features as the primary regularization knob, then min_samples_leaf or split, then depth limits, then class_weight.
...
**Short:** sklearn's RandomForestClassifier rejects NaN outright, so missing values need simple imputation, a proximity-matrix-based RandomForestImputer, or a library like XGBoost or LightGBM that handles them natively.
...
**Short:** Extra Trees skips searching for the optimal split threshold and instead picks among random thresholds, making it faster and lower-variance than Random Forest at the cost of slightly higher bias per tree.
...
**Short:** Random Forest resists overfitting because adding trees only lowers variance, but it can still overfit via memorized bootstrap samples on tiny datasets or noise-feature splits when max_features=1.0.
...
**Short:** Random Forest averages many fully grown, low-bias high-variance trees, canceling variance while preserving each tree's low bias, which is why deeper individual trees help rather than hurt an RF.
...
**Short:** class_weight="balanced" computes fixed weights once from the full dataset, while "balanced_subsample" recomputes weights per bootstrap sample, adapting better to severe imbalance below about 5% positive rate.
...
**Short:** RandomForestRegressor defaults max_features to all features as plain bagging rather than sqrt, averages leaf values instead of voting, and reports OOB score as R^2 instead of accuracy.
...
**Short:** Comparing training error to OOB error diagnoses Random Forest fit: near-zero training error with high OOB signals overfitting per tree, while both being high signals underfitting from shallow trees or weak features.
...
**Short:** warm_start=True lets you add trees incrementally to an already-fitted Random Forest without retraining existing trees, useful for finding the n_estimators elbow via repeated OOB checks.
...
**Short:** BalancedRandomForestClassifier undersamples the majority class within each bootstrap to a 50/50 split before training, improving minority recall, while class_weight="balanced" keeps all data and reweights the loss.
...
**Short:** Random Forest predict_proba scores are averaged votes pulled toward the middle of the range in an S-shaped reliability curve, fixed by wrapping the model in CalibratedClassifierCV.
...
**Short:** Random Forest trees train independently on separate bootstrap samples with no cross-tree communication, so n_jobs=-1 parallelizes near-linearly across cores, unlike gradient boosting's sequential rounds.
...
**Short:** max_samples reduces the row count drawn per bootstrap below the full training size, speeding up training and increasing tree diversity on huge datasets at the cost of weaker individual trees.
