...
<!-- study-paths
senior: ensemble_methods.md, random_forests.md, gradient_boosting.md, xgboost_lightgbm.md
...
**Short:** Bagging averages low-bias high-variance models to cut variance via (1-rho)/B, while boosting sequentially reduces bias by correcting prior errors, risking higher variance if run too long.
...
**Short:** Random Forest averaging only shrinks variance so adding trees is always safe, while boosting's sequential residual-fitting eventually memorizes noise, which is why RF skips early stopping but boosting needs it.
...
**Short:** Boosting up-weights and can chase mislabeled points every round, especially under AdaBoost's exponential loss, while bagging dilutes any single noisy point's influence by averaging many trees.
...
**Short:** Tree ensembles optimize impurity or ranking rather than probability accuracy, so scores need Platt scaling or isotonic regression via CalibratedClassifierCV before being used as probabilities.
...
**Short:** Classification uses sqrt(p) candidate features per split because a categorical target needs few candidates, while regression needs more numeric candidates to find good split points, as in Breiman's p/3 rule.
...
**Short:** OOB error scores each sample only with the roughly 36.8% of trees that never saw it during bootstrapping, giving a free generalization estimate close to 3-fold cross-validation.
...
**Short:** MDI sums training-time impurity decrease and is biased toward high-cardinality features, while permutation importance measures the test-set metric drop from shuffling a feature and is unbiased but costlier.
...
**Short:** Gradient boosting fits the negative gradient of the loss with respect to current predictions, which equals raw residuals only for squared error and generalizes boosting to any differentiable loss.
...
**Short:** Shrinkage scales each new tree's contribution by a small learning rate so more trees are needed but generalization improves, trading training time for accuracy.
...
**Short:** XGBoost adds L1/L2-regularized leaf weights, second-order Taylor-approximated splits, cache-efficient column-block split finding, and native sparsity-aware handling of missing values.
...
**Short:** GOSS keeps all large-gradient samples and subsamples small-gradient ones to scan fewer rows per round, but it is not LightGBM's default sampling strategy out of the box.
...
**Short:** Stacking without out-of-fold predictions lets the meta-learner train on base-model predictions memorized from their own training data, which OOF prediction fixes by using only held-out base predictions.
...
**Short:** Soft voting averages predicted class probabilities before taking the argmax and beats hard voting's majority-vote rule whenever base models produce well-calibrated confidence scores.
...
**Short:** Ensemble variance reduction follows rho*sigma^2 + (1-rho)*sigma^2/B, so most of the gain comes from the first 3-5 diverse models, with a correlated-error floor that more models cannot remove.
...
**Short:** Extra Trees picks split thresholds randomly instead of searching for the best one, trading a bit of bias for faster training and lower variance versus Random Forest.
...
**Short:** Bagging resamples training subsets with replacement, giving a free roughly 36.8% out-of-bag holdout, while pasting samples without replacement and has no automatic held-out fraction.
...
**Short:** LightGBM's leaf-wise growth splits the highest-loss leaf for faster loss reduction but risks overfitting small data, while XGBoost's level-wise growth expands a full depth at a time for more balanced trees.
...
**Short:** DART randomly drops already-built trees when growing each new tree to prevent late trees from over-specializing on a narrow slice of residuals.
...
**Short:** Imbalanced classification is corrected by setting scale_pos_weight to the negative-to-positive ratio in XGBoost or class_weight="balanced" in sklearn ensembles, paired with a PR-AUC-based threshold.
