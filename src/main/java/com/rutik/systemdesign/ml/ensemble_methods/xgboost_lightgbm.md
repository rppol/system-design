...
Mental model for LightGBM: in a typical GBDT round, most samples have small gradients (already well-predicted) and contribute little to split quality. With the paper's defaults GOSS keeps the top 20% by |gradient| and draws a further 10% of N at random from the rest, so it scans about 30% of the rows — a 70% cut — and reweights the survivors so the gain estimate stays unbiased. Ke et al. (2017), Theorem 3.2, bound the resulting gain-approximation error and give its asymptotic rate as O(1/n_l(d) + 1/n_r(d) + 1/sqrt(n)), where n_l(d) and n_r(d) are the two child counts of the split and n is the **full** training-set size — so as long as the split is not badly unbalanced the 1/sqrt(n) term dominates and the error vanishes as data grows. They do not quote a fixed "% of information gain retained", so treat GOSS's accuracy cost as small-but-data-dependent rather than a fixed number.
...
The competition's top private-leaderboard score was **0.80570 AUC** (team "Home Aloan", per the team's own 1st-place writeup on Kaggle), in one of the largest fields Kaggle had run at that point — deepsense.ai, who finished 5th at roughly 0.80 AUC, describe it as drawing "more than 8,000 data scientists". Gradient-boosted trees — LightGBM above all — dominated the leaderboard, and the winning writeup is explicit that feature engineering mattered more than model tuning or stacking. Treat any specific "GOSS cut training from 6 hours to 1.5" figure with suspicion: no such measurement appears in the public writeups, and GOSS is not enabled by default, so a stock LightGBM run does not use it at all.
...
**Short:** XGBoost's four innovations are a regularized objective, second-order gradient/Hessian splits, column-block parallel histograms, and sparsity-aware default branches for missing values.
...
**Short:** GOSS keeps all high-gradient rows plus a random sample of low-gradient rows, scanning only about 30% of rows per round, and is off by default.
...
**Short:** EFB merges mutually exclusive sparse features, like one-hot columns, into one dense feature via greedy graph coloring, helping most on sparse high-cardinality data.
...
**Short:** Leaf-wise growth always splits the highest-gain leaf into deep asymmetric trees, converging faster but overfitting small datasets unless min_child_samples is raised.
...
**Short:** XGBoost's min_child_weight caps the summed Hessian per leaf, not a row count, while LightGBM's min_child_samples is a literal minimum row count.
...
**Short:** Halving learning_rate roughly doubles the trees needed to converge, so learning_rate=0.05 pairs with n_estimators=2000 plus early stopping.
...
**Short:** LightGBM's num_leaves caps total leaves regardless of depth, letting some branches go very deep, while XGBoost's max_depth bounds every path's depth uniformly.
...
**Short:** scale_pos_weight multiplies positive-class gradients by count(negative)/count(positive) to rebalance split scoring for imbalanced binary classification.
...
**Short:** Early stopping halts training when a held-out eval_metric hasn't improved for early_stopping_rounds rounds, recording the optimal round in best_iteration.
...
**Short:** Leaf-wise asymmetric trees reach many leaves in fewer comparisons and the node layout is cache-friendly; the popular "integer bin comparisons at inference" explanation is wrong.
Two factors hold up, and one widely repeated explanation does not. (1) Leaf-wise tree structure — for the same leaf count, LightGBM's asymmetric trees reach many leaves in fewer comparisons than a depth-balanced XGBoost tree. (2) Cache-friendly traversal — the node arrays are laid out for sequential access during descent. The myth is that inference compares integer bin indices: histogram binning is a **training**-time construct. `Tree::NumericalDecision` in `include/LightGBM/tree.h` compares the raw `double` feature value against a `double threshold_`, exactly as XGBoost does; the sibling `threshold_in_bin_` / `NumericalDecisionInner` path operates on the binned training dataset, not on the public `Predict` path. CatBoost's symmetric trees are faster still for prediction because all paths in the tree have the same depth and the same split feature at each depth level — the entire tree becomes a lookup table indexed by a bit vector.

**Q: How do you handle a dataset that is too large to fit in memory for XGBoost/LightGBM?**
**Short:** Datasets too large for RAM are handled via file-backed DMatrix/Dataset loading, two-round processing, chunked feature engineering, or Dask/MPI distributed training.
...
**Short:** XGBoost's second-order Taylor expansion uses each sample's gradient and Hessian to compute optimal leaf weights analytically, letting custom losses skip a per-node line search.
...
**Short:** XGBoost learns a default left/right branch for missing values by testing both directions during training, which breaks down when missingness patterns differ at inference.
...
**Short:** At billion-row scale, LightGBM's histogram binning and lower per-feature memory win CPU training speed, while XGBoost's Dask integration is the more production-proven distributed path.
...
**Short:** gamma pre-prunes splits by requiring a minimum loss reduction, while reg_lambda shrinks leaf-weight magnitudes via an L2 penalty without changing tree structure.
...
**Short:** colsample_bytree, colsample_bylevel, and colsample_bynode subsample features at the tree, depth-level, and node granularity respectively, and their ratios multiply.
...
**Short:** LightGBM's native categorical splits partition categories directly by gradient statistics in one split, avoiding the deep stacked splits one-hot encoding forces.
...
**Short:** DART randomly drops existing trees before fitting each new tree, mimicking neural-net dropout to reduce overfitting at the cost of no early stopping and slower inference.
...
**Short:** Monotone constraints restrict split-finding so predictions only increase or decrease with a chosen feature, trading 0.5-2% AUC for a regulatory-defensible model.
