...
**Short:** Stacking trains a meta-learner to combine base model predictions with learned weights and interactions, while voting applies a fixed rule like equal-weight averaging with no learning.
...
**Short:** Training base models on the data used to generate meta-features lets the meta-learner memorize near-perfect training predictions that don't exist at test time, which out-of-fold prediction fixes.
...
**Short:** All base models must share identical fold splits when generating OOF predictions, otherwise some rows carry honest predictions from one model and leaked in-fold predictions from another.
...
**Short:** Stacking should feed the meta-learner predicted probabilities rather than hard class labels, since probabilities carry confidence information that hard labels discard, typically worth 0.3-1% AUC.
...
**Short:** Stacking can underperform its best base model when the meta-learner overfits the meta-feature space, base models are highly correlated, or OOF estimates come from too little data.
...
**Short:** Blending trains each base model only once on a fixed holdout split, making it cheaper and simpler than K-fold stacking, which trains K times per model but preserves more training data.
...
**Short:** A good meta-learner is low-variance, well-calibrated, and fast to train, which is why L2-regularized Logistic Regression is the default choice over a more complex learner.
...
**Short:** Base model diversity is measured via the Pearson correlation of OOF predictions, and pairwise correlation above roughly 0.90-0.95 means a model adds negligible stacking value.
...
**Short:** Multi-level stacking adds meta-learners on top of prior OOF predictions, but level-2 typically adds only 0.1-0.5% AUC and level-3 under 0.1%, rarely justifying the added compute.
...
**Short:** Test predictions in K-fold stacking are standardly produced by averaging each fold's base model predictions on the test set, which reduces variance versus a single retrained model.
...
**Short:** Passthrough concatenates the original training features alongside base model OOF predictions as meta-learner input, useful when features help arbitrate between models but risky below about 10K rows.
...
**Short:** Stacking typically improves AUC by 0.5-2.0% over the best single model, with the gain driven mainly by base model diversity and sufficient training data rather than the meta-learner algorithm.
...
**Short:** A dominant base model is kept from overwhelming the meta-learner by excluding models more than about 2% AUC behind the best and by regularizing the meta-learner's weights toward equality.
...
**Short:** Stacking for time series must replace random K-fold OOF generation with TimeSeriesSplit so no fold's training window includes data from after its validation period.
...
**Short:** Poorly calibrated base model probabilities, like Random Forest's votes pulled toward the middle, distort the meta-learner's inputs, so calibrating each base model's OOF predictions before stacking helps.
...
**Short:** Production stacking with daily retraining must fit K times n_models training runs in the retraining window, version base and meta-learner atomically, and monitor each base model's AUC independently.
...
**Short:** Top Kaggle tabular finishes typically stack 3-5 diverse base models with shared 5-fold OOF splits and a simple Logistic Regression or Ridge meta-learner, rarely beyond two stacking levels.
...
**Short:** OOF generation defaults to 5 folds, moving to 10 for smaller datasets to reduce OOF variance at the cost of proportionally more base-model training runs.
...
**Short:** Stacking combines heterogeneous model families via a learned meta-learner, whereas bagging averages many instances of one algorithm and boosting sequentially corrects one algorithm's residuals.
...
**Short:** Stacking is an empirical, non-parametric approximation to Bayesian Model Averaging, learning combination weights discriminatively from OOF predictions instead of requiring a proper probabilistic model per base learner.
