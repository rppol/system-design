...
| CategoricalNB | Categorical, index-encoded (nominal) | Categorical | Discrete variables with k states |
...
Laplace smoothing in Naive Bayes = using a Dirichlet(alpha, ..., alpha) prior on the multinomial parameters; alpha = 1 is the uniform Dirichlet(1, ..., 1). The posterior mean is (count + alpha) / (total + alpha * V) where V = vocabulary size and alpha = smoothing parameter. (The posterior *mode* of that same posterior is (count + alpha - 1) / (total + alpha*V - V), which is why Section 2 states the mode identity against a Dirichlet(alpha+1) prior instead.)
...
**Email Spam Filtering (Multinomial NB)**: the Bayesian spam filter is the canonical published application — Sahami et al. (Microsoft Research, 1998) framed junk mail as Naive Bayes over word features, and Paul Graham's "A Plan for Spam" (2002) put the token-counting version into wide use, seeding the filters in SpamAssassin, Mozilla and Bogofilter. Despite the obvious incorrectness of the independence assumption (spam words co-occur: "free" and "money" and "click" appear together), these filters ran fast enough to score mail inline on early-2000s hardware, which is exactly why the technique spread. Modern large-provider filters are no longer Naive Bayes.

**Medical Diagnosis with Small Samples (Gaussian NB)**: illustrative scenario — a GaussianNB classifier trained on 120 patient records (40 per subtype of a rare genetic disorder) with 15 blood biomarker features reaches roughly 89% accuracy, while logistic regression on the same 120 samples overfits badly. GaussianNB's strong independence assumption acted as aggressive regularization, producing stable estimates from few data points.

**Bayesian A/B Testing (conversion optimisation platforms)**: VWO and several other experimentation vendors ship Bayesian A/B testing with Beta-Binomial conjugate models as the default engine, reporting "probability to beat baseline" instead of a p-value. The team computes P(variant > control) after each observation window and launches once it crosses a stated bar such as 0.95. This framework naturally incorporates prior knowledge (typical conversion rate ranges) and yields intuitive probabilities rather than p-values. The frequentist alternative for the same continuous-monitoring problem is a sequential test with an alpha-spending boundary, which is what Booking.com's in-house platform uses.

**Document Classification — retraining speed as the deciding constraint**: any abuse or vandalism classifier whose target drifts weekly is rebuilt constantly, and NB's single counting pass over the corpus is what makes an hourly or daily refresh trivial — 10M short documents count out in well under a minute on one CPU core. That speed is the reason NB survives as a baseline even where a stronger model exists. (Wikipedia's own ORES edit-quality models are not NB: revscoring trained a random forest, later gradient boosting, on edit and editor metadata rather than raw n-grams.)
...
A NB classifier on 20 features reported P(spam) = 0.999999999 for an email. The probability was so extreme that it was being used directly as a risk score by a downstream filter. The independence assumption causes the per-feature evidence to compound: if 15 features each vote with 80% confidence for spam, the spam-side product is 0.8^15 = 0.0352 against a ham-side product of 0.2^15 = 3.3e-11, and normalizing those two gives essentially 1.0 — far more certainty than any individual feature justifies. Fix: calibrate probabilities with sklearn CalibratedClassifierCV(method="isotonic") after training, or use logistic regression for probability outputs.
...
flowchart TD
    ...
    prep --> hv["HashingVectorizer<br/>n_features=2^20, alternate_sign=False<br/>stateless: no vocabulary to fit or ship"]
...
**On the hashing tradeoff**: `HashingVectorizer` is stateless — no vocabulary is fitted, stored or shipped, which is what makes `partial_fit` retraining and zero-downtime swaps trivial. The cost is collisions, and they are not rare: dropping a 1M-token vocabulary into 2^20 = 1,048,576 buckets puts roughly `1 - exp(-1,000,000 / 1,048,576) = 61%` of tokens in a bucket shared with at least one other token. Collisions are tolerable here because the colliding partners are overwhelmingly rare tokens carrying almost no likelihood mass, and `alternate_sign=False` keeps the summed counts non-negative as MultinomialNB requires. Widen to 2^22 buckets if per-word audit trails must be exact.
