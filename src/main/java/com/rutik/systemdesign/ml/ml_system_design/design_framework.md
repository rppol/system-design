...
**Netflix homepage construction**: the page is built in stages rather than as one ranked list — candidates are generated per row, a ranking model orders items inside each row, and a separate model orders the rows themselves, all under a page-level objective. This is how Netflix has described page construction in talks and blog posts, but no single publication states the three stages in these terms, so do not present the breakdown as a cited architecture. For a fully citable staged design, use YouTube instead (Covington et al., RecSys 2016 — candidate generation then ranking, with the objective and features spelled out). Either way the transferable lesson is the same: a "recommendation system" in production is several models with different objectives, each with its own training pipeline, serving infrastructure, and monitoring — not one.
