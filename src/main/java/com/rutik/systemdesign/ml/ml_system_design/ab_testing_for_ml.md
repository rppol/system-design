...
**Booking.com** runs thousands of A/B experiments simultaneously and has published extensively on their experimentation practice — most visibly the KDD 2019 paper "150 Successful Machine Learning Models: 6 Lessons Learned at Booking.com", where every model is validated through a randomized controlled trial rather than an offline metric. They have also published on using CUPED to raise the power of their experiments. On the failure rate, cite the source rather than the folklore: Booking.com has not published a measured denominator — the KDD 2019 paper counts 150 *successful* models and never says how many were tried — but Stefan Thomke's Harvard Business School case work on the company (*Building a Culture of Experimentation*, HBR 2020) states that for every online experiment that succeeds at Booking.com, nearly ten do not. Treat that as the reported order of magnitude, not a measured statistic, and take the design lesson regardless: most changes are not improvements, so an experimentation platform's job is to make finding that out cheap.
...
**Short:** Pre-calculate the required sample size from baseline rate, minimum detectable effect, alpha, and power before starting.
...
**Short:** SPRT evaluates evidence after every batch and stops only when the likelihood ratio crosses a pre-set boundary, keeping alpha at target.
...
**Short:** Statistical significance means the effect is unlikely due to chance, while practical significance means the effect is large enough to matter for the business.
...
**Short:** Guardrail metrics like latency and error rate must not regress even when the primary metric improves, so track them continuously.
...
**Short:** SRM is when the actual variant allocation differs from the planned one, detected with a chi-square goodness-of-fit test.
...
**Short:** Reduce revenue's variance with CUPED, winsorization, or a lower-variance proxy metric, or extend the experiment duration.
...
**Short:** Interleaving merges two models' ranked lists into one list so clicks compete head-to-head, making it far more sensitive than a standard A/B test.
...
**Short:** Pre-register one primary metric for the decision and apply Bonferroni or Benjamini-Hochberg correction to every secondary and guardrail metric.
...
**Short:** CUPED adjusts the metric using a correlated pre-experiment covariate, lowering variance so the same power needs fewer samples.
...
**Short:** Randomize on the hashed user_id so the same user always sees the same variant, and log the assignment at first exposure.
...
**Short:** The novelty effect is a short-term engagement bump from curiosity, detected by tracking whether the daily effect size decays over the run.
...
**Short:** The two-proportion z-test at alpha 0.05 and power 0.80 requires roughly 31,200 users per variant for that effect size.
...
**Short:** Bonferroni controls the family-wise error rate conservatively, while Benjamini-Hochberg controls the false discovery rate with more power.
...
**Short:** Compare per-query NDCG with a paired or two-sample t-test, but prefer interleaving first since NDCG is sensitive to position bias.
...
**Short:** Default to frequentist testing and switch to Bayesian only when its early-stopping rules or priors provide a concrete benefit.
...
**Short:** A holdback experiment keeps a permanent control group to measure the cumulative long-term value of many model improvements combined.
...
**Short:** Use cluster, geographic, or ego-network randomization so interacting users land in the same variant instead of biasing individual randomization.
...
**Short:** A ramp-up raises treatment traffic gradually so a catastrophic bug is caught on a small blast radius before full exposure.
...
- Revenue per user over the full 7-day window (CUPED): control = $24.80, treatment = $25.30, lift = +$0.50, p = 0.023. Significant.
  Recomputable from the stated inputs: raw revenue sd `$150` (the §6 figure), and a CUPED covariate correlation of `rho = 0.5` on revenue — deliberately weaker than the `0.7` this module uses for engagement metrics, because revenue autocorrelates poorly across periods (the LinkedIn caveat in §7). That gives adjusted sd `150 * sqrt(1 - 0.25) = $129.9`, `SE = 129.9 * sqrt(2/700,000) = $0.2196`, `z = 0.50 / 0.2196 = 2.28`, two-sided `p = 0.023`. Change `rho` to `0.7` and the same lift lands at `p = 0.006` — which is the real lesson: the significance of this row is a property of your covariate quality, not of the model you shipped.
