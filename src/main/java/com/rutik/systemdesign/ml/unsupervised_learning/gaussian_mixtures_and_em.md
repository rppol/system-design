...
**Short:** A GMM soft-assigns points to full-covariance ellipses, while k-means hard-assigns them to spherical centers.
...
**Short:** Yes, k-means is a GMM with covariance shrunk to zero and hard component assignments.
...
**Short:** A responsibility is the posterior probability that a given point came from a specific mixture component.
...
**Short:** The E-step computes soft responsibilities, and the M-step re-estimates means, covariances, and weights from them.
...
**Short:** EM never decreases log-likelihood because both steps only raise the ELBO, a tight lower bound on it.
...
**Short:** No, EM only reaches a local maximum, so practitioners run multiple restarts from different initializations.
...
**Short:** A component's covariance can collapse onto one point, diverging the likelihood; add reg_covar to prevent it.
...
**Short:** Choose K by minimizing BIC or AIC across a range of fitted GMMs, never by training likelihood alone.
...
**Short:** Covariance types trade flexibility for parameter count, from spherical's single value to full's rotated ellipses.
...
**Short:** The ELBO lower-bounds the log-likelihood, and EM performs coordinate ascent on it via the E-step and M-step.
...
**Short:** Score points by negative log-density under a fitted GMM and flag those above a chosen percentile.
...
**Short:** The E-step needs log-sum-exp because multiplying many small Gaussian densities underflows to zero otherwise.
...
**Short:** A GMM's E-step posterior is exact and categorical, while a VAE approximates an intractable posterior with an encoder.
...
**Short:** Yes, a GMM can overfit by adding near-degenerate components; control it with BIC, reg_covar, and simpler covariance types.
...
**Short:** Full-covariance EM costs O(N times K times D squared) per iteration, versus O(N times K times D) for k-means.
...
**Short:** A GMM captures multimodal data that a single Gaussian would badly misrepresent with one central mean.
...
**Short:** A full-covariance GMM fixes k-means' equal-size assumption by letting each cluster learn its own shape and weight.
