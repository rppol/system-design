...
Only the Instacart entry below is drawn from a published account; the remaining four are illustrative composites of common industry practice, not disclosed operational figures from the named companies.

**Instacart (COVID-19, March 2020):** Instacart has publicly described how the onset of COVID broke its item-availability model. Order volume and in-store stock levels moved faster than the model could track, cancellations caused by out-of-stock items spiked, and — in Instacart's own words — "the model architecture we were using at that time struggled to keep pace with these changes, resulting in a larger discrepancy between predicted availability and the actual outcome." That is the shape of concept drift rather than covariate drift: the input features (store, item, time of day) keep roughly their historical distribution while the P(Y|X) relationship they feed moves underneath the model — which is exactly the case input-distribution monitoring alone will not catch. The eventual fix was architectural, not a simple retrain. Instacart's later write-up on modernizing real-time availability describes the pre-existing pipeline as a batch scoring job that "ran every 2-4 hours" for the whole catalog, and replaces it with scoring stratified by item traffic: head items (about 1% of the catalog) get hourly trending scores plus real-time inference, while torso and tail items are scored daily — with real-time scoring further restricted to items whose last observed status is recent (their example: within 10 hours). The lesson is the shape of the fix, not the specific cadence: when P(Y|X) moves fastest on a small, high-traffic slice, refresh frequency should be spent there rather than raised uniformly.

**Card-issuer fraud (illustrative pattern):** near-continuous retraining of fraud models against a rolling 30-day window of labeled transactions. Label lag is 48-72 hours (dispute resolution time), so PSI monitoring on unlabeled transaction features carries the early-warning load and fires within hours of a new fraud pattern (new merchant category, new device type) appearing. Champion/challenger with a short shadow period is the promotion gate.

**Streaming recommendations (illustrative pattern):** score distribution is monitored per user cohort weekly. A major content release shifts the recommendation model's score distribution because the feature space changes (new title IDs appearing as viewed content). This registers as data drift but is benign — it is expected during a content event. The monitoring system carries context annotations (scheduled launches, marketing campaigns) that suppress alerts during known perturbation periods.

**Social-graph recommendations (illustrative pattern):** a scheduled weekly retrain for the graph-based recommendation model, with an emergency off-schedule trigger when AUC on a daily labeled validation set drops below (deployment_AUC - 0.03). The validation set is rebuilt daily from the previous day's connection requests and their outcomes (accepted vs not), giving a daily model-quality signal without waiting for offline batch evaluation.

**Delivery time estimation (illustrative pattern):** ETA models have strong seasonal drift (weather, holidays, restaurant busy periods). Monitoring includes a weather-conditioned quality report: on rainy days the model's MAE is expected to increase, so an alert fires only when the increase exceeds the historical rainy-day baseline by more than 2 standard deviations. This suppresses alert fatigue from predictable seasonal patterns while still catching genuine degradation.
...
| WhyLabs (whylogs) | Logging + drift profiling | Lightweight logging library; cloud aggregation | Sends to WhyLabs cloud; open core. The whylogs repo has had no upstream commits since January 2025 — check maintenance status before adopting |
...
**Q: How do you choose between PSI and KS test for drift detection?**
...
**Q: What is the champion/challenger pattern in model deployment?**
...
**Q: How do you handle label latency in a fraud model?**
...
**Q: Describe how you would detect a label feedback loop in a recommendation system.**
...
**Q: What is PSI and how do you compute it?**
...
**Q: How do you set up automated retraining that is safe to run without human review?**
...
**Q: How do you monitor a model that serves predictions in real-time with no label available for hours?**
...
**Q: What is virtual drift and how do you avoid false alerts from it?**
...
**Q: When would you choose online learning over periodic retraining?**
...
**Q: How do you distinguish between a bug in the data pipeline and genuine concept drift?**
...
**Q: What monitoring would you add to a churn prediction model and what are the SLOs?**
