...
Why it matters: in credit, employment, housing, and healthcare — all regulated domains in most jurisdictions — disparate impact on protected classes is illegal. GDPR Article 22 gives individuals the right not to be subject to a decision based *solely* on automated processing that produces legal or similarly significant effects, and Article 22(3) requires at minimum human intervention, the ability to express a point of view, and the ability to contest the decision. A standalone "right to explanation" appears in Recital 71, not in Article 22 itself, and whether the recital creates a binding right is genuinely contested (Wachter et al. 2017 vs Selbst and Powles 2017); the binding transparency duties sit in Articles 13(2)(f), 14(2)(g) and 15(1)(h), which require "meaningful information about the logic involved." Ignorance of these requirements is not a defense.

---

## 3. Core Principles

**Fairness is context-dependent.** There are multiple mathematical definitions of fairness that are mutually incompatible whenever the groups have different base rates: no single non-trivial model can then satisfy predictive parity, equalized odds and demographic parity at once (Chouldechova 2017; Kleinberg, Mullainathan and Raghavan 2016). The incompatibility is conditional on unequal prevalence — with equal base rates and a perfect classifier the definitions can coincide, which is exactly the degenerate case the theorems exclude. The appropriate definition depends on the deployment context, legal framework, and stakeholder agreement.
...
def explain_model(
    ...
    TreeExplainer is exact (not sampling-based) for GBDT models. TreeSHAP costs
    O(T * L * D^2) per prediction — T = number of trees, L = max leaves per tree,
    D = max depth — down from the O(T * L * 2^M) of naive exact Shapley over M features.
    """
...
def generate_adverse_action_explanation(
    model,
    df_train: pd.DataFrame,       # training frame INCLUDING the outcome column
    X_instance: pd.DataFrame,     # features only, the applicant being explained
    ...
    dice_ml.Data needs the full training frame WITH `outcome_name` present — it
    derives per-feature ranges and the MAD normalisation from it. Passing the single
    query row (and one without the outcome column) raises at construction time.
    """
    data = dice_ml.Data(
        dataframe=df_train,
...
**COMPAS recidivism algorithm (ProPublica, 2016):** Northpointe's COMPAS algorithm predicted recidivism risk for criminal defendants. ProPublica's analysis found that among defendants who did *not* re-offend, Black defendants were labelled higher-risk at nearly twice the rate of white defendants — a false positive rate of 44.85% vs 23.45%. Northpointe responded that the tool was calibrated (predictive parity held). Both were correct — equalized odds and predictive parity are mathematically incompatible when base rates differ. The case established that any high-stakes automated decision must explicitly document which fairness metric is being optimized and why.

**Amazon hiring tool (reported by Reuters, October 2018):** Amazon began building an ML model to score resumes in 2014, training it on resumes submitted to the company over a ten-year period. Because most of those resumes came from men, the model penalized resumes that included the word "women's" (e.g., "women's chess club captain") and favoured male-coded verbs. Recruiters saw its recommendations but Amazon says the tool was never the sole basis for a hiring decision; the team was disbanded in 2017 after Amazon concluded it could not guarantee the model was gender-neutral. Root cause: training data reflected the historical composition of the applicant pool; no fairness audit gated the work.

**Apple Card (Goldman Sachs, 2019):** In November 2019 viral allegations claimed male applicants received credit limits many times higher than women with similar financial profiles. New York DFS investigated and published its findings in March 2021: reviewing underwriting data for roughly 400,000 New York applicants, it found **no evidence of unlawful discrimination** under fair lending law, and reported that applications from men and women with similar credit characteristics generally had similar outcomes. DFS did criticise customer-service deficiencies and a perceived lack of transparency that "undermined consumer trust." The lesson is not that the model was biased — the regulator concluded otherwise — but that an unexplainable credit decision invites a regulatory investigation on its own, and that a viral anecdote is not a bias measurement.
...
**Payments fraud scoring (industry pattern, not a disclosure):** No major payments provider publishes a bias audit of its fraud models, and no numbers below are taken from one. The recurring internal pattern is: measure false positive rate — legitimate transactions wrongly declined — separately by merchant country, business type and transaction size, since a model trained on historical decline data reproduces whatever geography or business-category skew was in it; set a disparity-ratio review trigger; and route flagged segments to manual review. Pick the trigger ratio from your own decline-rate distribution rather than importing someone else's number.
...
**SHAP is slow for neural networks.** TreeSHAP is exact and fast — O(T·L·D²) per sample, for T trees of max depth D and L leaves. KernelSHAP is model-agnostic but approximation-based: it samples coalitions rather than enumerating all 2^M, and its regression step is roughly quadratic in the number of features. For a ResNet-50 with 25 million parameters, KernelSHAP on a single image takes minutes. Use Integrated Gradients or GradCAM for neural network explanations, and reserve SHAP for GBDT/linear models.
...
Disparate impact occurs when a neutral policy disproportionately affects members of a protected group. The four-fifths rule (Uniform Guidelines on Employee Selection Procedures, 29 CFR 1607, adopted 1978) operationalizes this: a selection rate below 80% of the highest group's rate is generally treated by federal enforcement agencies as evidence of adverse impact. Measure it as the selection rate ratio (group rate / highest group's rate) or as the demographic parity difference. A ratio < 0.8 or a difference > 0.1 warrants investigation and business justification. Note the compare-to-the-highest-group detail: comparing to "the majority group" gives a different answer whenever the majority group is not the best-selected one.

**Q: Explain the impossibility theorem for fairness metrics.**
...
**Q: What is the difference between demographic parity and equalized odds?**
...
**Q: How does SHAP work and why is it preferred over feature importance?**
...
**Q: A credit model is denied regulatory approval because adverse-action notices are insufficient. What do you do?**
...
**Q: What are proxy features and how do you detect them?**
Proxy features are features that are not protected attributes themselves but correlate with protected attributes, allowing the model to use them to discriminate. Common proxies: zip code → race (due to residential segregation), name → gender/ethnicity (name etymology), employment gap → gender (caregiving leave), "certain schools attended" → socioeconomic status. Detection: (1) compute point-biserial correlation between each feature and the protected attribute in the training data; (2) compute SHAP values for the protected attribute (if included) and for candidate proxies; (3) run a "feature audit": train a classifier to predict the protected attribute from model features — high accuracy indicates proxy features exist. Remove or adjust any feature where correlation with the protected attribute exceeds a threshold you have documented and agreed with legal — r > 0.3 is a common starting point in practice, but no regulator or standards body sets a numeric proxy-correlation cutoff, so the defensible artifact is the written rationale for whatever number you pick, not the number itself.

**Q: What is reweighing and when should you use it?**
...
**Q: Explain the difference between interpretability and explainability.**
...
**Q: How does GDPR Article 22 affect ML systems?**
GDPR Article 22 gives EU residents the right not to be subject to decisions based solely on automated processing that produce significant legal or similarly significant effects. Where such processing is nonetheless permitted (contract, explicit consent, or Union/Member State law), Article 22(3) requires the controller to safeguard the individual with at least: the right to obtain human intervention, the right to express a point of view, and the right to contest the decision. The duty to provide "meaningful information about the logic involved" comes from Articles 13(2)(f), 14(2)(g) and 15(1)(h), not from Article 22 — and the stronger "right to an explanation" of a specific decision sits only in non-binding Recital 71, which is why its legal force is disputed. In practice: any ML model making decisions on credit, employment, housing, or insurance for EU residents must have an explanation pipeline (SHAP/DiCE for adverse action), a human review escalation path, and a documented process for handling Article 22 requests. Systems that lack these are non-compliant and fall in the higher GDPR penalty tier: Article 83(5) caps fines at €20M or 4% of total worldwide annual turnover, whichever is higher — not whichever is lower.

**Q: What is differential privacy and when should you use it in ML?**
...
**Q: How do you handle a situation where your fairness audit finds that a previously deployed model has disparate impact?**
...
**Q: What is individual fairness and why is it hard to operationalize?**
...
**Q: How do you build a model card and what should it contain?**
A model card (Mitchell et al., 2019) is a structured document accompanying an ML model that documents: (1) model details — architecture, training data description, date, version; (2) intended use — primary intended uses, out-of-scope uses, and the specific population/context the model was designed for; (3) performance metrics — overall metrics (AUC, F1, ECE) and disaggregated metrics by subgroup (age band, gender, geography); (4) evaluation data — description of evaluation dataset and how it was collected; (5) ethical considerations — known biases, fairness audit results, sensitive use cases; (6) caveats and recommendations — known limitations, recommended thresholds, human oversight recommendations. Hugging Face supplies a model-card template and prompts authors to fill it in, but a card is not required to publish a model there (an organization can make one mandatory for its own uploads); model cards are best practice, and sometimes legally required, for any model making consequential decisions about people.

**Q: How do you detect and mitigate model amplification of bias over time?**
...
**Q: What is the 4/5ths rule and what are its limitations?**
The four-fifths rule (UGESP, 29 CFR 1607, 1978) states that a selection rate below 80% of the highest group's rate is generally treated by federal enforcement agencies as evidence of adverse impact. Limitations: (1) the 80% threshold is arbitrary — it was chosen for practicality, not statistical rigor; (2) it depends on the choice of which groups to compare and what counts as "selection"; (3) it is purely descriptive (says disparity exists) but not explanatory (doesn't say whether the disparity is due to legitimate predictors or bias); (4) it can be satisfied while still having large absolute disparity if the base selection rate is very low; (5) it was designed for employment, and courts have applied it inconsistently to algorithmic systems. A sixth limitation is now the enforcement posture itself: Executive Order 14281 (23 April 2025) directed federal agencies to deprioritize disparate-impact enforcement, and the EEOC has signalled an intent to rescind UGESP, so the federal-agency reading of the rule is in flux even though 29 CFR 1607 remains on the books and state law and private Title VII litigation are unaffected. Treat the four-fifths rule as a trigger for investigation, not as a complete fairness assessment or a stable compliance ceiling.
