...
**Short:** Contrastive learning pulls together representations of augmented views of the same image and pushes apart representations of different images, via the NT-Xent/InfoNCE loss.
...
**Short:** SimCLR uses only in-batch samples as negatives so small batches starve it of hard negatives, while MoCo maintains a 65,536-item queue independent of batch size.
...
**Short:** The momentum encoder is an exponential moving average copy of the main encoder that provides slowly changing, stable targets instead of unstable gradient-updated ones.
...
**Short:** BYOL's online network has an extra predictor MLP the target lacks, and the EMA-updated target changes too slowly for the online network to trivially match by collapsing.
...
**Short:** At 75% masking too little local context remains for texture-copying, forcing the encoder to learn global semantics; 50% masking lets nearby patches interpolate trivially.
...
**Short:** Linear probing freezes the backbone and trains only a linear head to test raw representation quality, while full fine-tuning adapts all weights to the downstream task.
...
**Short:** SimCLR uses RandomResizedCrop, flip, color jitter, grayscale, and blur, all chosen to be semantically invariant while still creating a non-trivial contrastive task.
...
**Short:** DINO centers the teacher's output by an EMA of the batch mean and sharpens it with a lower temperature than the student, and these two effects counteract each other.
...
**Short:** DINO's multi-crop local-to-global objective forces attention heads to specialize, so the CLS token's last-layer attention ends up highlighting the foreground object.
...
**Short:** The projection head absorbs the information the contrastive loss discards, so the pre-projection backbone features retain more fine-grained information for downstream use.
...
**Short:** Use SSL when you have abundant unlabeled domain data but few labels, need one backbone for many tasks, or are in a low-label fine-tuning regime under 1000 samples.
...
**Short:** BarlowTwins pushes the cross-correlation matrix of two augmented views toward identity, needing no negative pairs, large batches, or a memory queue unlike SimCLR or MoCo.
...
**Short:** Use k-NN probing for a fast no-training signal, linear probing as the community standard, or attentive probing, which works best for ViT patch tokens.
...
**Short:** SigLIP replaces CLIP's batch-wide softmax contrastive loss with a per-pair sigmoid binary cross-entropy loss, removing the need for very large batches.
...
**Short:** Instance discrimination treats every image as its own class, while clustering methods like SwAV assign images to learned prototype centroids as implicit positives.
...
**Short:** DINOv2's deduplication, quality filtering, and domain-balanced curation of 142M images matters as much as the objective; uncurated data drops linear probe accuracy 3-5%.
...
**Short:** Temperature scales similarity logits before the softmax; too large flattens the pull between positives, while too small makes gradients explode and training collapse-prone.
...
**Short:** Dimensional collapse leaves embeddings varying but confined to a low-dimensional subspace, unlike complete collapse where every input maps to the same constant vector.
...
**Short:** MAE's heavy encoder only processes the 25% visible patches while a lightweight discarded decoder reconstructs the rest, cutting pretraining compute roughly 3x.
...
**Short:** Register tokens are extra learnable input tokens that absorb high-norm scratch activations, cleaning up attention maps that vanilla ViTs otherwise hijack from background patches.
