...
**Short:** ViT splits the image into fixed patches, linearly projects each flattened patch to an embedding, prepends a CLS token, and adds learnable positional embeddings.
...
**Short:** Self-attention is permutation-invariant, so without positional embeddings shuffling all patches gives the same output and ViT loses about 15% ImageNet accuracy.
...
**Short:** The CLS token is a prepended learnable vector that attends to all patches through self-attention and becomes the global image representation fed to the classifier head.
...
**Short:** Swin's patch merging builds a ResNet-like multi-scale hierarchy with windowed local attention, unlike ViT's single-resolution feature map throughout.
...
**Short:** CLIP jointly aligns image and text embeddings via contrastive training, so at inference an image is classified by its highest cosine similarity to a class's text prompt.
...
**Short:** DeiT adds a distillation token learning from a CNN teacher, stochastic depth, and strong augmentation to reach ViT-level accuracy using only ImageNet-1k.
...
**Short:** Self-attention costs O(n^2*d) in sequence length, so doubling image resolution quadruples the token count and makes attention prohibitively expensive at high resolution.
...
**Short:** Start from a heavily pretrained checkpoint, linear-probe the head first, then unfreeze with layer-wise LR decay and strong augmentation plus stochastic depth.
...
**Short:** FlashAttention tiles attention computation to stay in GPU SRAM, cutting peak memory from O(n^2) to O(n) and roughly doubling training throughput.
...
**Short:** Absolute encodings assign a fixed embedding per position and need interpolation at new resolutions, while relative encodings like Swin's bias generalize across resolutions.
...
**Short:** Alternating blocks shift window partitions by half a window size, so tokens on a window's boundary in one block become interior tokens in the next, enabling cross-window flow.
...
**Short:** DINOv2 trains ViT with a self-distillation objective on a 142M curated dataset, producing linearly separable features that beat supervised ImageNet pretraining.
...
**Short:** CNN Grad-CAM heatmaps are reliably discriminative, while raw ViT CLS attention is noisier, so production explainability instead uses Attention Rollout across layers.
...
**Short:** An 8x8 patch size gives finer spatial resolution and about 2% higher top-1 accuracy than 16x16, but at four times the token count and attention compute.
...
**Short:** ViTs lack CNNs' built-in locality and translation-equivariance priors, so they trail ResNets from scratch and only pull ahead once pretrained at ImageNet-21k scale or beyond.
...
**Short:** Yes, global average pooling over patch tokens is a drop-in CLS-token alternative that often matches or slightly beats it, provided the head learning rate is tuned separately.
...
**Short:** ViTs keep gaining accuracy as data and parameters grow without the early plateau CNNs hit, but remain worse than a ResNet in low-data regimes under roughly 100k images.
