...
**Short:** The reparameterization trick rewrites VAE sampling as z = mu + epsilon*sigma so gradients flow through mu and sigma into the encoder, bypassing non-differentiable sampling.
...
**Short:** The VAE's ELBO objective sums a reconstruction term measuring decoder output quality and a KL term pulling the encoder's distribution toward the standard-Gaussian prior.
...
**Short:** Mode collapse happens when a GAN generator narrows to a small region that fools the discriminator, and WGAN-GP fixes it via a Lipschitz gradient-penalty under Wasserstein distance.
...
**Short:** DDPM's forward process adds Gaussian noise to an image over 1000 steps until it's pure noise, and the reverse process trains a U-Net to iteratively predict and remove that noise.
...
**Short:** Classifier-free guidance trains one diffusion model on both conditional and unconditional inputs, then extrapolates toward the condition at inference via a guidance scale, no separate classifier needed.
...
**Short:** FID measures the Frechet distance between Inception-v3 feature Gaussians fit to real versus generated images, with lower values better, needing about 50K samples for a reliable estimate.
...
**Short:** DDPM uses a stochastic 1000-step reverse process, while DDIM reformulates it as a deterministic ODE solver that samples in as few as 50 steps at nearly the same quality.
...
**Short:** Posterior collapse happens when a powerful decoder learns to ignore the latent code entirely, fixed by KL annealing, free-bits minimums, or a weaker decoder that forces reliance on z.
...
**Short:** A VQ-VAE replaces a VAE's continuous Gaussian latent with a discrete codebook of vectors, avoiding posterior collapse and enabling autoregressive priors over the discrete codes.
...
**Short:** Diagnosing GAN instability means checking discriminator accuracy stays near 50-75% and gradient norms are healthy, then fixing with WGAN-GP, feature matching, or label smoothing.
...
**Short:** VAE latent interpolation linearly blends two latent codes into smooth intermediate images because KL regularization forces the latent space into a densely populated, hole-free Gaussian structure.
...
**Short:** VAEs produce blurry samples because pixel-wise Gaussian reconstruction loss rewards predicting the per-pixel average of plausible outputs rather than any single sharp image.
...
**Short:** Inception Score only evaluates generated images and can reward an unrealistic generator, while FID also compares against real image features, making it the preferred metric.
...
**Short:** GANs are harder to train than diffusion models because their min-max adversarial game has no stable target, whereas diffusion trains on a stationary noise-prediction MSE regression.
...
**Short:** DDPM's forward process has a closed form because composed Gaussian noising stays Gaussian, letting training jump straight to any timestep x_t instead of running a sequential chain.
...
**Short:** Removing the KL term turns a VAE into an ordinary autoencoder whose latent space develops holes, since nothing pins encoder outputs near the prior that sampling relies on.
...
**Short:** A cosine noise schedule keeps more signal in the middle diffusion timesteps than a linear schedule, which collapses to near-pure noise too early and wastes model capacity.
...
**Short:** Autoregressive models generate one token or pixel at a time with exact likelihoods, while VAEs and diffusion models use latent variables to sample in one pass or fixed denoising steps.
