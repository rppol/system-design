...
<!-- study-paths
senior: information_retrieval_and_search.md
...
Bi-encoder / two-tower embeddings: a query tower and a document tower, independent, compared by dot product at query time — the same architecture as [Recommender Systems' two-tower](../recommender_systems/recommender_systems.md), applied to free text instead of user and item ids. ANN index families: HNSW (a navigable-small-world graph), IVF / IVF-PQ (cluster-and-quantize), and ScaNN (anisotropic quantization).
...
**Retrieval-Augmented Generation systems**: Every RAG pipeline (see [RAG Fundamentals](../../llm/rag_fundamentals/rag_fundamentals.md)) is this module wearing a different hat -- chunks take the place of documents, and the same hybrid BM25 + dense + rerank pipeline decides what context an LLM ever sees.
...
**Q: Why can a ranker with high offline accuracy still fail completely when it ships?**
...
**Q: Why must a learning-to-rank train/test split be grouped by query rather than randomized by row?**
...
**Q: Why does BM25 outperform raw TF-IDF cosine similarity for ranked retrieval?**
...
**Q: What do BM25's k1 and b parameters actually control?**
...
**Q: Why can't a reranker fix a document that first-stage retrieval never found?**
...
**Q: Why does including a document's shown position or click count as an LTR feature quietly leak the label?**
...
**Q: Why does dense retrieval often fail on rare identifiers, product codes, or legal citations?**
...
**Q: Why do offline NDCG gains sometimes fail to translate into online metric improvements?**
...
**Q: What is an inverted index and why is it faster than scanning every document?**
...
**Q: What is the difference between boolean and ranked retrieval?**
...
**Q: How does Reciprocal Rank Fusion combine a sparse and a dense ranking, and why does it use rank instead of score?**
...
**Q: Why is a cross-encoder more accurate than a bi-encoder, and why is it confined to reranking?**
...
**Q: How is NDCG computed, and why does it use a log2 discount?**
...
**Q: What is the difference between MAP and MRR?**
...
**Q: How does RankNet's loss work, and what specific problem does LambdaRank fix in it?**
...
**Q: What does LambdaMART add on top of LambdaRank?**
...
**Q: How does HNSW's graph structure trade memory and build time for search speed, and what does ef_search control?**
...
**Q: What is BM25F and why do multi-field documents need it?**
...
**Q: What is query understanding, and what failure mode can overly aggressive query rewriting cause?**
...
**Q: Why might a system choose IVF over HNSW despite HNSW's higher recall?**
...
**Q: When does hybrid retrieval outperform either pure sparse or pure dense retrieval alone?**
...
- [RAG Fundamentals](../../llm/rag_fundamentals/rag_fundamentals.md) -- this module's pipeline wearing a different hat: chunks replace documents, and the same hybrid retrieval plus reranking decides what context an LLM ever sees.
- [Search Engines](../../database/search_engines/search_engines.md) -- the systems and infrastructure view of running this module's inverted index at scale: segment merging, index lifecycle management, and the deep-pagination problem.
