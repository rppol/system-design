...
**Short:** Cross-entropy is the MLE objective for categorical outputs and its gradient is q-p, avoiding the vanishing gradient MSE gets from sigmoid's chain rule.
...
**Short:** Cross-entropy equals entropy plus KL divergence, H(p,q) = H(p) + KL(p||q), so minimizing cross-entropy is exactly minimizing KL(p||q).
...
**Short:** Forward KL(p||q) is mean-seeking and used in maximum likelihood, while reverse KL(q||p) is mode-seeking and used in variational inference like the VAE ELBO.
...
**Short:** Entropy measures average uncertainty and is maximized at log(k) for a uniform k-class distribution, hitting 0 when one outcome has probability 1.
...
**Short:** Information gain is the entropy reduction from a split, but it's biased toward high-cardinality features like a unique ID; gain ratio corrects for this.
...
**Short:** Mutual information captures any dependence including nonlinear relationships, while Pearson correlation only measures linear dependence and can be zero for a clear nonlinear link.
...
**Short:** The ELBO's KL term penalizes the encoder's posterior q(z|x) for diverging from the Gaussian prior p(z), regularizing the latent space to stay smooth.
...
**Short:** Only in the population limit, since cross-entropy is a proper scoring rule; in practice overparameterized networks trained with it are systematically overconfident.
...
**Short:** Shannon's source coding theorem sets the minimum code length at H(p) bits, and coding under a mismatched q wastes exactly KL(p||q) extra bits.
...
**Short:** Focal loss multiplies cross-entropy by (1-p_t)^gamma to down-weight easy examples, and is used for extreme class imbalance like object detection backgrounds.
...
**Short:** Perplexity is exp of the cross-entropy loss in nats, representing the effective number of tokens the model is choosing among at each position.
...
**Short:** Gini and entropy both measure node impurity and produce nearly identical trees, but Gini is cheaper since it skips the logarithm CART avoids.
...
**Short:** Label smoothing replaces one-hot targets with a higher-entropy distribution, preventing infinite logit gaps and improving calibration at some cost to distillation.
...
**Short:** Conditional entropy is the average uncertainty about Y remaining after observing X, and mutual information is exactly H(Y) minus that remaining uncertainty.
...
**Short:** Ranking features by mutual information with the target captures nonlinear relevance, but MI is computed one feature at a time and ignores redundancy and interactions.
...
**Short:** log-softmax uses the log-sum-exp trick for numerical stability, while computing softmax then its log overflows or underflows into inf or NaN gradients.
...
**Short:** The student minimizes KL divergence between temperature-softened teacher and student outputs, with the loss scaled by T^2 to offset softening's shrunken gradients.
...
**Short:** The data-processing inequality says no transformation of Y can increase information about X beyond what Y already retains, capping what feature engineering can add.
