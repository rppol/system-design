...
a `<!-- study-paths -->` block in that module's own page (`<module>.md`) naming the files each tier
...
5. Write a `<!-- study-paths -->` block at the top of the new module's page (`<module_name>.md`) naming the tiers it
...
3. If it belongs in a tier, add its path to that tier's line in the `<!-- study-paths -->`
   block at the top of `case_studies/case_studies.md` — that block drives the Level filter
