...
a `<!-- study-paths -->` block in that module's own README naming the files each tier
takes; listing a tier joins it, omitting the tier opts out, and `README.md` must always
be listed. Order is never declared — it comes from `STUDY_ORDER.ml` in `game/app.js`, so
...
block in `case_studies/README.md` (12 senior / 10 principal of the 24), driving the Level
...
5. Write a `<!-- study-paths -->` block in the new module's README naming the tiers it
