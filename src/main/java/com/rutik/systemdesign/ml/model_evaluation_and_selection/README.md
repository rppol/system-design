...
| Hyperopt              | Alternative Bayesian tuning (TPE, ATPE)      | Compatible with XGBoost; was dormant for years (0.2.7 was the stable release from Nov 2021), then shipped 0.3.0 in July 2026 |
...
**Short:** AUC-ROC is threshold-agnostic ranking quality, while AUC-PR is preferred on imbalanced data since AUC-ROC is misleadingly optimistic there.
...
**Short:** Plain KFold can produce folds with zero positives on rare classes, while StratifiedKFold preserves the positive rate in every fold.
...
**Short:** High bias shows as low, similar train and validation scores, while high variance shows as a large gap between a high train score and a low validation score.
...
**Short:** Calibration means a predicted probability of 0.7 is correct about 70% of the time, which matters whenever probabilities drive expected-value decisions.
...
**Short:** Use TimeSeriesSplit for temporally dependent data, with a purge gap between train and validation to stop autocorrelated features leaking across it.
...
**Short:** TPE models good and bad hyperparameter regions separately and samples from their ratio, focusing search where random search would waste trials.
...
**Short:** The Brier score is the mean squared error of probability predictions, decomposing into reliability, resolution, and an irreducible uncertainty term.
...
**Short:** A paired t-test on CV fold scores suits correlated per-fold comparisons, though k-fold's shared rows make it anti-conservative without a variance correction.
The paired t-test on CV fold scores is appropriate when you have multiple evaluation folds on the same data (each fold gives one observation per model) and you want to test whether mean CV performance differs. "Paired" accounts for fold-level correlation — fold 3 tends to be hard for all models, so comparing models on the same folds is more efficient than unpaired. Its assumptions are that the per-fold differences are independent and roughly normal; the independence assumption is **violated by construction**, because k-fold training sets share `(k-2)/(k-1)` of their rows, so the test underestimates the variance of the mean difference and is anti-conservative (rejects H0 too easily). The principled fix is Nadeau & Bengio's corrected resampled t-test, which scales the variance of the fold differences by `(1/(k*r) + n_test/n_train)` instead of `1/(k*r)`, where `r` is the number of CV repetitions; scikit-learn documents exactly this correction in its "Statistical comparison of models" example. Failing that, treat results as directional evidence and require p < 0.01 rather than p < 0.05.

**Q: What is McNemar's test and when do you use it instead of the t-test?**
**Short:** McNemar's test compares two classifiers on a single fixed test set using their discordant correct/incorrect disagreements, not continuous CV scores.
McNemar's test compares two classifiers on a fixed held-out test set based on the discordant pairs: cases where model A is correct and B is wrong (b), versus B correct and A wrong (c). The uncorrected statistic is `(b - c)^2 / (b + c)`, referred to a chi-squared distribution with 1 degree of freedom; the concordant cells are deliberately ignored. Its assumption is that the chi-squared approximation to the binomial is adequate, which fails when the discordant count is small — the usual rule is `b + c >= 25`, below which you must use the exact binomial test (`statsmodels`' `mcnemar(table, exact=True)`, as in the Section 6 code), and Edwards' continuity correction `(|b - c| - 1)^2 / (b + c)` is common in between. Use McNemar's when you have a single test set (not CV) and binary correct/incorrect outcomes; it measures disagreement directly rather than comparing continuous score differences. Note what it does *not* do: it tests only the given train/test split, so it captures test-set sampling noise but not variability from the choice of training set.

**Q: Explain the problem with using test set for both model selection and reporting.**
**Short:** Selecting the best of many models by test-set score and reporting that same score is selection bias that overestimates true performance.
...
**Short:** Standard choice is k=5 or k=10, trading lower bias at higher k against higher variance, with LOO-CV for very small datasets.
...
**Short:** MAPE is mean absolute percentage error, but it is undefined at zero actuals, asymmetric, and overly harsh on small actual values.
...
**Short:** Accuracy is dominated by the majority class, so predicting the majority for everyone yields high accuracy while ignoring the class that matters.
...
**Short:** Macro-F1 weights every class equally, micro-F1 is dominated by frequent classes, and weighted-F1 weights each class's F1 by its support.
...
**Short:** Discrimination is ranking ability measured by AUC, while calibration is whether predicted probabilities match observed frequencies, and a model can have one without the other.
...
**Short:** Resample the test set with replacement many times and take the percentile range of the recomputed metric to quantify its sampling noise.
