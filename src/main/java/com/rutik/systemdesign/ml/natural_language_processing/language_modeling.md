...
**Short:** A language model assigns a probability to a token sequence, equivalent to predicting the next token given everything before it.
...
**Short:** An unseen n-gram gets exactly zero maximum-likelihood probability, and since sentence probability is a product, one zero collapses the whole sentence.
...
**Short:** The Markov assumption says the next token depends only on the previous n-1 tokens, ignoring the rest of the sentence's history.
...
**Short:** Perplexity is the exponentiated average cross-entropy per token, so minimizing cross-entropy loss directly minimizes perplexity.
...
**Short:** Kneser-Ney estimates a word's backoff probability from how many distinct contexts it follows, not from its raw frequency.
...
**Short:** Backoff uses only the highest-order n-gram with a nonzero count, while interpolation always blends all orders with weights that sum to one.
...
**Short:** Higher-order n-gram contexts grow exponentially rarer, so a model runs out of training data before longer context actually helps.
...
**Short:** Add-one smoothing's huge vocabulary-sized denominator swamps real counts, making seen and unseen n-grams end up with nearly equal probability.
...
**Short:** Neural language models embed tokens as dense vectors, so statistically similar words generalize evidence even across unseen exact n-grams.
...
**Short:** Weight tying shares the input embedding matrix with the output softmax projection, cutting parameters and typically lowering perplexity.
...
**Short:** Fix a closed vocabulary and map every rarer word to a shared UNK symbol, applying the identical mapping at both training and test time.
...
**Short:** Perplexity is computed per token, so a subword tokenizer with shorter tokens shows lower per-token perplexity without being a better model.
...
**Short:** Good-Turing estimates the total probability of unseen events from how many n-grams in the corpus were seen exactly once.
...
**Short:** Kneser-Ney is absolute discounting with the lower-order term replaced by continuation probability, which is why it consistently performs better.
...
**Short:** Intrinsic evaluation measures perplexity in isolation, while extrinsic evaluation measures the model's effect on a real downstream task.
...
**Short:** Temperature rescales logits before softmax, with low values sharpening toward the most probable token and high values flattening the distribution.
...
**Short:** Memory scales with the number of distinct n-gram types, not tokens, which is why compressed tries like KenLM shrink large models to a few GB.
