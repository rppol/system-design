...
The result: a single pretrained model achieves state-of-the-art performance across 11 NLP tasks with minimal task-specific architecture. On the GLUE test set, BERT-base scored 79.6 and BERT-large 82.1, against 75.1 for OpenAI GPT and 74.0 for the pre-GPT state of the art; the abstract's headline 80.5% GLUE score (a 7.7-point absolute gain over the prior leaderboard best) is BERT-large on the public leaderboard. The delta wasn't engineering — it was bidirectionality.
...
**WordPiece tokenization:** Splits unknown words into subword pieces from a learned vocabulary of ~30K tokens. "unaffordable" → ["un", "##afford", "##able"]. The `##` prefix indicates a continuation subword. WordPiece picks the merge that most increases the likelihood of the training data under a unigram language model (unlike BPE, which simply merges the most frequent adjacent pair).
...
| **DeBERTa** (He et al., 2020) | Disentangled attention: separate content and position embeddings with two attention matrices; enhanced mask decoder for MLM pretraining | Best encoder for tasks requiring precise positional understanding |
| **DeBERTa-v3** (He et al., 2021) | RTD (Replaced Token Detection, ELECTRA-style) training objective instead of MLM; parameter-efficient | Top-tier NLU encoder; v3-large scores 91.4 GLUE average and 91.5/89.0 F1/EM on SQuAD 2.0, v3-base 90.6/90.7 MNLI-m/mm at 184M total params |
...
| **ModernBERT** (Warner et al., 2024) | Flash Attention 2, unpadding (removes padding tokens from computation), RoPE positional encoding, alternating local/global attention (128-token local window, global every third layer), extended context (8192 tokens), trained on 2T tokens | Long contexts and throughput; ~4x DeBERTa-v3-base's short-context variable-length inference throughput at comparable quality. NeoBERT (2025) is the newer entrant in the same class |
...
**BERT for finance:** FinBERT (ProsusAI/finbert) — BERT further pretrained on a financial corpus (Reuters TRC2-financial) and fine-tuned on the Financial PhraseBank for three-way sentiment. It is the standard open baseline for earnings-call and news sentiment scoring; the paper reports it beating general-purpose baselines on Financial PhraseBank, so treat any specific F1 delta as dataset-dependent and re-measure on your own labels.

**DeBERTa in medical NLP (illustrative):** DeBERTa-v3-large fine-tuned on clinical notes for ICD-10 code extraction. Disentangled attention is particularly valuable for clinical text where positional context matters (e.g., "history of" before a condition means it's past, not current). Whether it beats RoBERTa on a given clinical corpus, and by how much, has to be measured — there is no published cross-corpus constant.

**ModernBERT for code search:** ModernBERT-large processes 8192-token contexts, enabling retrieval over entire function bodies and docstrings — vs BERT's 512 limit that required truncation. It was pretrained with code in the mix and reports strong CodeSearchNet retrieval results, so a code-retrieval index can hold whole files rather than snippets.
...
| RoBERTa-large | 355M | 0.3x | 88.5 | Best quality in the original BERT family |
| ALBERT-xxlarge | 235M | 0.2x | 89.4 | Research, small parameter footprint |
| DeBERTa-v3-large | 435M (304M backbone + 131M embedding) | 0.4x | 91.4 | Production NLU, best quality/backbone ratio |
| DistilBERT | 67M | 1.6x | 77.0 | Latency-critical production |
| ModernBERT-large | 395M | 0.6x (FA2) | — | Long contexts (8K+), modern hardware |

GLUE numbers come from each model's own paper and are not strictly comparable: 79.6/82.1 and 77.0 are single-model GLUE *test* averages, 88.5 and 89.4 are GLUE leaderboard submissions, 91.4 is a dev-set average. Treat the column as a ranking, not as a measured delta.
...
| BertViz | Attention visualization for BERT | Interactive head-level attention patterns |
| ModernBERT | Long-context encoder (Warner et al., 2024) | Flash Attention 2, unpadding, 8192 token context |
...
**Short:** BERT masks 15% of tokens and uses an 80/10/10 mask/random/unchanged split so the model doesn't only learn to handle the [MASK] token.
...
**Short:** RoBERTa dropped NSP because it forced shorter training sequences, and its ablations showed removing it improved GLUE performance consistently.
...
**Short:** DeBERTa splits content and position into separate embeddings, computing attention as content-to-content plus content-to-position and back.
...
**Short:** DistilBERT distills a 6-layer student from BERT's soft output distribution, hidden-state similarity, and hard labels, keeping most of the quality.
...
**Short:** WordPiece merges subword pairs to maximize training-data likelihood, while BPE simply merges the most frequent adjacent pair each step.
...
**Short:** Pretrained [CLS] was only trained for NSP's coarse topic signal, so its untuned embeddings cluster together instead of reflecting true similarity.
...
**Short:** A legal classifier silently truncated contracts past 512 tokens, missing risk language that appeared later and misclassifying long documents as benign.
...
**Short:** MLM computes loss only on the roughly 15% masked positions, making it sample-inefficient compared to ELECTRA's replaced-token-detection over every token.
...
**Short:** Dynamic masking generates a fresh mask pattern every epoch instead of reusing a fixed set, acting as data augmentation that reduces overfitting.
Dynamic masking re-samples which tokens are masked every time a sequence is fed to the model, instead of masking once during preprocessing as original BERT did. Original BERT masked each sequence a fixed number of times (it duplicated the data 10x with different masks), so a given sequence saw at most 10 mask patterns across all of training. Dynamic masking generates a fresh mask on every epoch, so the model never sees the exact same masked input twice, which acts as data augmentation and prevents overfitting to specific mask placements. Combined with removing NSP, 8K-sequence batches, and 160GB of text against BERT's 16GB, dynamic masking was part of RoBERTa's recipe that lifted the GLUE leaderboard score to 88.5 without any architecture change.

**Q: Why is BERT fine-tuning unstable across random seeds on small datasets, and how do you stabilize it?**
**Short:** Small datasets plus a randomly initialized head make some seeds diverge to near-random accuracy, which lower learning rates and warmup fix.
...
- Monthly GPU cost: two reserved A10G-class instances, roughly an order of magnitude below per-token frontier-LLM API pricing at this clause volume (price both against current rates before quoting a figure)

**Key Decisions:**

- DeBERTa over BERT-base: +7 weighted F1 points for same inference cost
- Per-class evaluation caught silent failures on the "LIMITATION OF LIABILITY" clause type (only 120 examples: weighted F1 looked fine, that class's own F1 was 0.52) — the macro-vs-weighted gap in action
