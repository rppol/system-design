...
BLEU clips each n-gram count to its max count in any reference, takes the geometric mean of precisions p1–p4, and multiplies by the brevity penalty. Here the candidate (7 tokens) is longer than the reference (6 tokens) so BP=1, and BLEU ≈ 0.81 — the arithmetic is worked out token by token immediately below.
...
def bertscore(
    ...
    rescale_with_baseline: linearly rescales against a per-model baseline
    computed from randomly paired Common Crawl sentences, so a "no overlap"
    pair sits near 0. It does not clamp to [0, 1] — scores can go negative.
...
**CoNLL-2003 NER Benchmark:** The gold standard NER benchmark reports entity-level F1. Devlin et al. report 92.4 test F1 for BERT-base and 92.8 for BERT-large; later entity-aware encoders push past that — LUKE (Yamada et al., EMNLP 2020) reports 94.3 test F1. A 1-2 point gap matters for production because each false negative entity is a missed extraction — in financial document processing with 100K entities/day, 2 points is 2,000 fewer missed entities.

**Post-hoc calibration in content moderation (illustrative):** Moderation and safety classifiers built on fine-tuned encoders are routinely temperature-scaled before the confidence score is allowed to drive an automated action. Uncalibrated fine-tuned BERT typically shows ECE in the 0.15-0.25 band (predictions stated at 90% confidence are correct 70-80% of the time); a single fitted temperature usually pulls ECE under 0.03. Measure your own ECE — the band above is a rule of thumb, not a published constant.
...
**Short:** BLEU measures how much of the generated text matches the reference (precision), while ROUGE measures how much of the reference was covered (recall).
...
**Short:** The brevity penalty punishes candidates shorter than the reference, closing the loophole where one common word could otherwise score high precision.
...
**Short:** Sentence-level BLEU is unstable because short sentences can hit zero n-gram matches, so scores are aggregated across the whole corpus first.
...
**Short:** Clipping caps how many times a repeated word can count toward precision, closing the loophole of gaming BLEU through pure repetition.
...
**Short:** BERTScore's scale depends entirely on which embedding model produced the vectors, so scores from different models sit on incompatible scales.
...
**Short:** ROUGE-L scores the longest common subsequence between candidate and reference, rewarding in-order matches without requiring contiguous n-grams.
...
**Short:** EDA's synonym, insertion, swap, and deletion operations give small classification gains but can break token-label alignment in sequence labeling.
...
**Short:** BERTScore matches candidate and reference tokens by contextual embedding similarity, giving credit for valid paraphrases that BLEU scores as zero.
BERTScore (Zhang et al., 2020) scores a candidate against a reference by greedily matching contextual token embeddings on cosine similarity instead of comparing surface strings. Each candidate token takes its best reference partner (precision), each reference token takes its best candidate partner (recall), and the two combine as F1. The contextual embeddings mean "feline" and "cat" have high similarity (~0.87 in BERT space), so BERTScore gives partial credit for valid paraphrases that BLEU scores as zero. BERTScore is more informative than BLEU in three cases: (1) dialogue evaluation, where responses have high valid variation; (2) creative text generation, where exact n-gram matches are unlikely; (3) cross-lingual evaluation, where multilingual BERT can compare translations of different quality independent of exact word overlap.

**Q: Why does perplexity correlate poorly with downstream task performance for fine-tuned models?**
**Short:** Fine-tuning optimizes a task-specific objective that can diverge from next-token prediction, so lower perplexity need not mean better task accuracy.
...
**Short:** Entity-level F1 requires an entire span's boundaries and type to match exactly, while token-level F1 is inflated by the dominant O class.
...
**Short:** Beyond ROUGE, check factual consistency against the source, an abstractiveness ratio, and downstream signals like user read time or completion rate.
...
**Short:** Expected Calibration Error measures whether a model's stated confidence matches its actual accuracy, which matters whenever confidence drives decisions.
...
**Short:** Back-translation paraphrases text by round-tripping it through another language, but translation-quality bottlenecks and label-sensitive shifts limit it.
...
**Short:** The extractive oracle is the best ROUGE achievable by selecting source sentences directly, giving an upper bound to normalize a real model's score against.
The extractive oracle finds the subset of source sentences that maximizes ROUGE-2 against the reference summary. For CNN/DailyMail: pick the top 3 source sentences by greedy ROUGE-2 maximization. This gives the upper bound of any extractive summarization system on this dataset. Liu & Lapata (2019) measure it at ROUGE-1/2/L = 52.59 / 31.24 / 48.87, far above both the LEAD-3 baseline (40.42 / 17.62 / 36.67) and every published abstractive model (their best abstractive system reaches ROUGE-2 19.6; BART and PEGASUS land near 21). The headroom between 21 and 31 is the honest answer to "how much of this dataset is still on the table". Use the oracle to: (1) decide whether extraction can suffice — a high oracle means the references are largely reconstructible from source sentences, so an extractive system is a legitimate design choice; (2) normalize your scores — report your model as a fraction of the oracle rather than as a raw ROUGE number, since the oracle differs wildly by dataset; (3) calibrate expectations — a model at 65% of oracle on CNN/DailyMail is roughly state of the art, not broken. Computing oracle: `from rouge_score import rouge_scorer; scorer = rouge_scorer.RougeScorer(['rouge2']); greedy select sentences maximizing marginal ROUGE-2 gain`.
