...
**Short:** Without attention, an encoder-decoder must squeeze the whole source sentence into one fixed vector, which degrades badly past 15-20 tokens.
...
**Short:** Bahdanau's additive attention uses a learned tanh energy function, while Luong's multiplicative attention scores with a dot product or matrix.
...
**Short:** Dividing by sqrt(d_k) rescales dot-product variance back to about 1, keeping softmax out of the near-one-hot regime where gradients vanish.
...
**Short:** Beam search keeps the top-K partial sequences at each step but can fail through repetition, length bias, and beams collapsing to similar outputs.
...
**Short:** Coverage tracks how much attention each source position has already received, penalizing the decoder for over- or under-translating tokens.
...
**Short:** Teacher forcing trains on ground-truth previous tokens the model never sees at inference, causing compounding errors that scheduled sampling reduces.
...
**Short:** Beam search yields more consistent, idiomatic code, while nucleus sampling trades consistency for the diversity needed in pass@k evaluation.
...
**Short:** Label smoothing replaces one-hot targets with a soft distribution, preventing overconfidence and improving BLEU at the cost of higher perplexity.
...
**Short:** BLEU quality plateaus and can even decline past beam widths of 4-8, because extra hypotheses mostly duplicate paths already explored.
...
**Short:** Self-attention takes queries, keys, and values from the same sequence, while cross-attention takes queries from the decoder and keys/values from the encoder.
...
**Short:** A coverage penalty scores beams by how evenly attention spreads across source tokens, discouraging the repeated focus that causes repetition.
...
**Short:** Visualize attention as a heatmap to check for a near-diagonal pattern, then look for attention collapse or garbage-collection tokens as failure modes.
...
**Short:** Diverse beam search splits beams into groups with a diversity penalty between them, producing genuinely different outputs instead of near-duplicates.
...
**Short:** Sinusoidal positional encodings let relative position shifts be expressed as a linear transformation, giving limited extrapolation beyond training length.
...
**Short:** Additive attention's tanh naturally bounds its energies, while dot-product attention's variance grows with dimension and needs explicit rescaling.
...
**Short:** The encoder output depends only on the fixed source and is cached once, while the decoder must run step by step conditioning on its own past tokens.
...
**Short:** Local attention restricts scoring to a small window around a predicted alignment center, cutting cost when the true alignment is roughly monotonic.
