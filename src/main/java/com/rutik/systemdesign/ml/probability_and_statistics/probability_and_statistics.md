...
<!-- study-paths
senior: probability_and_statistics.md
...
**What is the precise statistical error introduced by peeking and why does it inflate Type I error?** Peeking exploits the fact that if you check a standard z-test p-value at multiple points during data collection, the probability of seeing p < 0.05 at least once is much higher than 5%. Formally, repeating a two-sided alpha=0.05 test at 14 equally spaced interim looks during a fixed-horizon experiment lifts the true Type I error to roughly 22%, and past 30% if you look 50 times — the effect first quantified by Armitage, McPherson and Rowe (1969). You do not have to take those figures on trust: simulate the test statistic as a random walk, check `|z| > 1.96` at each equally spaced look, and count how often it ever crosses. A 400,000-replication run reproduces 19.3%, 24.8% and 32.0% at 10, 20 and 50 looks (and 21.9% at 14), which is where the numbers above come from. This occurs because the p-value is not uniformly distributed under repeated observation; the test statistic is a random walk, and the probability that it ever crosses a fixed threshold is higher than the probability it exceeds the threshold at a single predetermined time.
