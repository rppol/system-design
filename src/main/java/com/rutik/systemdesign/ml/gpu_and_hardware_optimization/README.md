...
**Short:** Compute-bound GPU ops are limited by arithmetic throughput and fully saturate Tensor/CUDA cores, while memory-bound ops like elementwise kernels sit idle waiting on HBM bandwidth.
...
**Short:** Flash Attention fuses QK^T, softmax, and V matmuls into one kernel that never materializes the full N x N score matrix in HBM, cutting attention memory from O(N^2) to O(N).
...
**Short:** The roofline model plots achievable FLOP/s against arithmetic intensity, showing whether an op sits below the ridge point (memory-bound, fix via fusion) or above it (compute-bound).
...
**Short:** Gradient checkpointing stores activations only at sqrt(L) checkpoint boundaries and recomputes the rest during backward, cutting activation memory at roughly 33% extra compute.
...
**Short:** Tensor Cores multiply fixed-size tiles like 16x16 in one instruction, so dimensions not aligned to 8 or 16 force padding or a CUDA-core fallback that can cut throughput 50-70%.
...
**Short:** pin_memory=True locks CPU buffers so the GPU's DMA engine can copy them asynchronously with non_blocking=True, overlapping the next batch's transfer with current compute.
...
**Short:** torch.compile traces the model with TorchDynamo and compiles fused CUDA/Triton kernels via TorchInductor, typically giving 15-30% training speedup from one line of code.
...
**Short:** DataLoader worker count should scale to about 4 per GPU, since each worker duplicates the dataset in its own process and throughput plateaus past 8 workers from OS/IPC overhead.
...
**Short:** NVLink's roughly 600 GB/s bidirectional bandwidth versus PCIe's roughly 64 GB/s means an AllReduce adding under 25% step overhead on NVLink can add over 80% on PCIe.
...
**Short:** Gradient accumulation runs several micro-batch forward/backward passes before stepping the optimizer, paying only single micro-batch activation memory for a larger effective batch.
...
**Short:** Model FLOP Utilization is measured FLOP/s divided by peak hardware FLOP/s, and well-tuned LLM training on A100 typically reaches 38-45% MFU.
...
**Short:** GPU memory fragmentation happens when the caching allocator's freed-tensor pool develops holes from irregular allocation sizes, causing OOM even though reported usage looks low.
...
**Short:** A validation loop OOMs because tensors kept with their autograd graph attached accumulate activation memory across every batch, fixed by wrapping evaluation in torch.no_grad().
...
**Short:** Confirming a DataLoader bottleneck means checking whether GPU utilization stays below 80% while a CPU core is pegged at 100%, then benchmarking loader throughput in isolation.
