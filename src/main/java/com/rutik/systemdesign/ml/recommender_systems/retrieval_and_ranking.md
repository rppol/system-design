...
something genuinely harder, and the loss finally says so. The `10x` here comes from the bars in
Section 5.2, not from anyone's published study — the weights are always estimated from your own
randomized slice, never copied from someone else's paper.
...
is the regime the Best Practices section recommends, and anything in `0.5`-`0.7` behaves similarly.
...
**Airbnb Position Bias (no propensity model)**: Airbnb's search team attacked position bias without building an explicit propensity model at all. Instead they added the position of the result as an ordinary input feature to the ranking DNN — a control variable — and regularized it with dropout (rate 0.15) so relevance prediction could not lean on it; at scoring time the position feature is set to 0, which strips the bias term out of the score. Adding position naively cost about -1.3% NDCG on relevance, which is why the dropout matters. The online A/B test reported +0.7% bookings and +1.8% revenue. It is the cheapest position-bias fix in the literature: no randomized traffic slice required.

**Pinterest Diversity Re-Ranking**: Pinterest diversifies its home feed with a Determinantal Point Process rather than MMR — the DPP kernel puts item relevance on the diagonal and item-item similarity off it, so the selected subset is jointly relevant and spread out, and it has since moved to Sliding Spectrum Decomposition for numerical stability. Their published finding is the important one for the tradeoff: removing feed diversity raised immediate day-1 engagement but turned negative by the second week, with reduced session time — the diversity payoff shows up in the retention window, not the click.
