...
**PinSage (Pinterest)**: Graph convolutional network on a bipartite pin-board graph. Each pin's embedding is computed by aggregating feature vectors from its most-visited graph neighbors. Scales to a graph of 3 billion nodes (pins and boards) and 18 billion edges via importance-based neighborhood sampling (random walk visit counts determine neighbor weights). Pin embeddings capture visual style and conceptual similarity.
...
**Netflix CBF + CF Cascade**: Netflix's "More Like This" rail is content-based: given the film you just finished, recommend films with similar genre, tone, and themes using embeddings over Netflix's human-authored tag metadata (Netflix employs "taggers" who annotate titles). This CBF path operates independently of user history and provides immediate recommendations even for new titles added to the catalog hours ago. Their main homepage, by contrast, leans on collaborative signals (what similar users watch).

**Marketplace Listing Recommendation (Feature Augmentation Hybrid)**: A marketplace two-tower model embeds both the listing's content features (location, amenities, photos via a vision encoder such as CLIP, price range) and behavioral signals (listings other travelers booked in the same trip context) into a shared space. Airbnb's published listing-embedding work trains listing vectors from search-session sequences with a word2vec-style objective; the two-tower form generalizes it into a feature-augmentation hybrid — content features are inputs to the item tower, behavioral signals train the dot-product objective.

**Pinterest PinSage**: The Pinterest graph in the PinSage paper has 3 billion nodes (pins and boards) and 18 billion edges, with new pins added by users continuously. Pure CF fails because most pins have zero interactions when first created. PinSage uses graph convolutions on the pin-board bipartite graph, incorporating visual and text features. A new pin's embedding is computed from its image (a 4,096-dim VGG-16 feature vector) and its annotations (a word2vec-style embedding), then refined by aggregating embeddings of similar pins in the graph. Cold start is handled naturally — content features provide the initial embedding.
