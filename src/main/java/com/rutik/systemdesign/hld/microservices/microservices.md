...
For the full event-sourcing model — append-only event store, snapshots, projections, and replay mechanics — see [Event Sourcing & CQRS](../event_sourcing_cqrs/event_sourcing_cqrs.md).
...
For the full circuit-breaker state machine, bulkhead sizing, and retry/backoff/jitter mechanics, see [Resilience Patterns](../resilience_patterns/resilience_patterns.md).
...
For the full deep-dive — compensating-transaction design, the Outbox pattern for atomic event publication, and a complete choreography vs. orchestration walkthrough with code — see [Event Sourcing & CQRS](../event_sourcing_cqrs/event_sourcing_cqrs.md).

For full coverage of 2PC, 3PC, TCC, the outbox pattern, and idempotency keys alongside the saga orchestration vs. choreography tradeoffs, see [Distributed Transactions](../distributed_transactions/distributed_transactions.md).
...
For the full three-pillars (metrics, logs, traces) treatment, RED/USE methods, and the SLI/SLO/error-budget framework, see [Observability](../observability/observability.md).
...
For authentication/authorization patterns beyond service-to-service traffic (OAuth2, JWT, API keys, RBAC), see [Spring Security Architecture](../../spring/spring_security_architecture/spring_security_architecture.md) and [Auth & Authorization Systems](../../backend/auth_and_authorization_systems/auth_and_authorization_systems.md).

For the architectural overview of AuthN vs AuthZ, OAuth2/OIDC, JWT vs sessions, mTLS, and RBAC vs ABAC, see [Security and Authentication/Authorization](../security_and_auth/security_and_auth.md).
...
**Cross-references:** [backend/microservices_fundamentals](../../backend/microservices_fundamentals/microservices_fundamentals.md) (decomposition strategies, bounded contexts in code), [backend/api_gateway_patterns](../../backend/api_gateway_patterns/api_gateway_patterns.md), [backend/service_mesh_and_service_discovery](../../backend/service_mesh_and_service_discovery/service_mesh_and_service_discovery.md) (Istio/Envoy, sidecar pattern), [spring/spring_cloud_patterns](../../spring/spring_cloud_patterns/spring_cloud_patterns.md) (Spring Cloud Gateway, service discovery with Eureka/Consul), [event_sourcing_cqrs](../event_sourcing_cqrs/event_sourcing_cqrs.md) (Saga pattern deep dive).
