...
- A major equities venue *trades* roughly **8,000-11,000 symbols** (common stock, ETFs, preferred shares) — note that a venue trades the whole national market-system universe, which is far larger than the set of companies it *lists*. For estimation, assume **~10,000 active symbols**.
...
- Cluster-wide, the **consolidated market-data feed** (all symbols combined) can reach **10-20 million messages/sec** at extreme peak. Two real reference points bracket that figure, and it matters which one you cite: the U.S. **equity** SIPs are an order of magnitude *below* it — CTA's quote feed is provisioned at 392,000 messages per 100ms (~3.9M/sec) against a 2.4:1 capacity-to-peak ratio, so real equity-SIP peaks run in the **low millions** of messages/sec — while **OPRA**, the U.S. *options* consolidated feed, is well *above* it, with peak microbursts exceeding **40 million messages/sec**. The 10-20M/sec target used here is a design headroom figure for a venue whose per-symbol delta rates are options-like, not a published equity-SIP number.
...
| Per-order book memory footprint | ~200-320 KB (liquid symbol) |
...
public final class OrderBook {
    ...
    /**
     * Best bid/ask for market-data snapshots (§4.4). O(log P), NOT O(1):
     * TreeMap.firstEntry() calls getFirstEntry(), which walks the left spine
     * of the red-black tree ("while (p.left != null) p = p.left") -- there is
     * no cached first-node pointer. With P = 20-100 price levels (§2) that is
     * 5-7 pointer chases, which is why the fastest engines use an
     * array-indexed price ladder (§5) for genuine O(1).
     */
...
**Why `TreeMap` (red-black tree) for price levels**: insertion, deletion of an empty level, and "find best price" are all **O(log P)** where P is the number of distinct price levels (typically 20-100, §2) — effectively a small constant for the depths involved. Be precise about the last one, because it is the operation performed on *every* incoming order: `firstEntry()` is **O(log P), not O(1)**. The JDK implements it as `getFirstEntry()`, which walks the tree's left spine node by node (`while (p.left != null) p = p.left`) with no cached first-node pointer — cheap at P = 20-100 (5-7 pointer chases, each a potential cache miss), but it is a tree descent, not a constant-time array read. The alternative — a flat array indexed by price tick — is **faster** (true O(1) for everything) when the tick range is small and dense, which is why the highest-performance production engines often use **array-indexed price ladders** for liquid symbols (a fixed-size array covering a price band around the current market, with overflow handling for prices outside the band) rather than a tree. `TreeMap` is the right choice for an interview-level design because it handles **sparse, unbounded price ranges** (illiquid symbols, or a symbol that gaps 20% overnight) without any pre-sizing, at the cost of a constant-factor slower lookup than a pre-sized array — see §5 for the full comparison.
...
**The other two fan-outs off the same trade stream — drop copy and the regulatory tape** (the §1 trade-reporting/clearing-handoff requirement): the market-data feed above is public, anonymous, and best-effort. Two more consumers read the *same* `Trade` output (§4.1) with the opposite requirements, and conflating any of the three is a design error:

| Consumer | Audience | Delivery | Contents | Latency requirement |
|---|---|---|---|---|
| Market data (§4.4) | everyone | UDP multicast, best-effort, gap-filled | anonymized book deltas and prints | microseconds — on the critical path for participants |
| **Drop copy** | the two counterparties only | reliable, per-session (TCP), replayable | that firm's own fills, with its own order IDs and account attribution | milliseconds — a monitoring/reconciliation path, not a trading one |
| **Trade reporting (the tape)** | the regulator / trade reporting facility | reliable, exactly-once, durably acknowledged | fully attributed trade records (both counterparties, capacity, timestamps) | a regulatory SLA, commonly single-digit milliseconds — orders of magnitude looser than matching (§8) |

Three consequences follow. First, **none of these three sits in the matching hot path**: all are downstream consumers of the trade log, so a slow tape or a backed-up drop-copy session can never add latency to `OrderBook.process()` — the same isolation §1 demands for the market-data fan-out, applied to the reporting paths. Second, **the tape needs a durability guarantee market data does not**: a dropped multicast packet is recoverable by the subscriber (§4.4's gap detection), but a dropped trade report is a regulatory reporting failure, so the reporting path is built on the durable sequencer log (§4.3) with acknowledged, replayable delivery — the trade's sequence number is its dedup key, so a retry after an unacknowledged send is a no-op rather than a double-report. Third, **clearing is a consumer, not a step**: the clearinghouse subscribes to the same durable trade log and nets positions on its own T+1 cycle, which is exactly why §1 places clearing and settlement out of scope — the matching engine's obligation ends at "the trade is durably in the log," and everything downstream reads it rather than being called by it.
...
|---|---|---|
| Best-price lookup | O(log P) — `firstEntry()` walks the tree's left spine; ~5-7 pointer chases at P = 20-100, each a possible cache miss | O(1) always (fixed index, contiguous memory) |
...
|---|---|---|---|
| **Nasdaq (INET)** | INET matching engine (originated from the Island ECN acquisition, now the core of Nasdaq's exchange-technology platform) | Nasdaq markets its matching engine at **sub-40 microseconds** as standard, with the fastest production deployment cited at **~14 microseconds** door-to-door | Price-time priority (FIFO) as the baseline, as described in §4.1 — the textbook reference implementation for price-time matching at production scale |
...
| **London Stock Exchange (Millennium Exchange)** | Millennium Exchange (MillenniumIT platform, acquired and operated by LSEG) | The clearest **published** figure is LSE's own measurement from Turquoise's Millennium launch: **~126 microseconds** average order-entry latency on the native API, **~196 microseconds** over FIX 5.0. LSEG does not publish a current headline number, so treat any "low tens of microseconds" claim for this platform as unsourced | Price-time priority, with the same continuous-trading/auction session model (§4.7) as described in this design — included to illustrate that the architecture described here is not U.S.-specific; the same partitioned-symbol, single-writer matching model underlies major venues across regions |
...
| Leader election / symbol ownership | Raft (etcd), ZooKeeper-style ephemeral leases | §4.2 — cross-ref [`../consensus_algorithms/consensus_algorithms.md`](../consensus_algorithms/consensus_algorithms.md) |
...
| Observability | Custom nanosecond-resolution tracing through the order-entry-to-execution path; standard metrics/tracing stacks for everything outside the hot path | cross-ref [`../observability/observability.md`](../observability/observability.md) |
...
2. If it's a processing-time regression, check for **GC pauses** (if running on a managed-runtime matching engine) or **page faults / cache misses** (if the order book's working set, §2's ~200-320KB, has grown abnormally — e.g., an illiquid symbol suddenly receiving an enormous number of price levels from a quoting algorithm gone wrong).
3. If it's a genuine volume spike on a symbol not provisioned for top-100 treatment (§4.2's sharding), consider an **emergency symbol-ownership migration** — using the leader-election mechanism (§4.2, cross-ref [`../consensus_algorithms/consensus_algorithms.md`](../consensus_algorithms/consensus_algorithms.md)) to move the symbol to an instance with spare per-core capacity. This is a last resort: migration itself requires a brief pause (snapshot + handoff, §4.3) and is operationally risky mid-trading-day.
...
1. Detection: the consensus layer (§4.2, cross-ref [`../consensus_algorithms/consensus_algorithms.md`](../consensus_algorithms/consensus_algorithms.md)) observes a failed lease renewal for one or more symbols owned by the failed instance.
...
> **How to read these**: the two war stories below are **composite, illustrative incidents** — the failure mechanisms (a lock race deciding price-time priority, and a market-data client that applies deltas without checking sequence continuity) are real, recurring, and structural, but the specific message rates, latency numbers, and remediation timelines are constructed to make the mechanism concrete. Treat them as archetypes to reason from, not as citable public post-mortems of any named venue.
...
| Order-book memory (per liquid symbol) | 500-5,000 resting orders x ~40-64 bytes | ~200-320 KB |
...
A: A hot-standby instance, which has been continuously consuming the same sequencer log and applying it to its own in-memory book replica (§4.3, §8), detects the primary's lease has expired via the consensus layer (cross-ref [`../consensus_algorithms/consensus_algorithms.md`](../consensus_algorithms/consensus_algorithms.md)) and acquires ownership of the affected symbol(s). It confirms its replayed state is current up to the last sequence number the old primary is known to have processed (replaying any small gap from the durable log if needed), then begins accepting new orders for that symbol. No orders are lost — they were durably sequenced regardless of the primary's state — though clients may see a brief (hundreds of milliseconds to low seconds, §1) increase in acceptance-to-fill latency during the handoff. The crashed instance, once restored, rejoins only as a standby, never reclaiming primary status without going through lease acquisition again — preventing flapping and split-brain.
...
A: A hash map has no concept of order, so "find the best price" (the single most frequent operation, §4.1) would require a full scan — unacceptable. A sorted array gives O(1) best-price access but O(n) insertion/deletion of price levels in the general case (shifting elements). A `TreeMap` (red-black tree) gives O(log P) for *both* best-price access and insertion/removal of a price level, where P is the number of distinct price levels — typically 20-100 for a liquid symbol (§2), so O(log P) is effectively a small constant. Don't claim `firstEntry()` is O(1) in an interview: the JDK walks the tree's left spine to find it, with no cached pointer. The further refinement many production engines make (§5) is an **array-indexed price ladder** for a narrow "hot band" around the current market price, falling back to a tree (or simply rejecting/special-casing) for prices far outside that band — getting array-like O(1) for the overwhelming majority of activity while still handling the full price range correctly.
...
- **Leader election and symbol-ownership leases for matching-engine failover (§4.2, §8, §11)** -> [`../consensus_algorithms/consensus_algorithms.md`](../consensus_algorithms/consensus_algorithms.md)
...
- **Microsecond-scale latency SLOs, p99.9 tracking, and the acceptance-vs-fill latency split (§8, §10)** -> [`../observability/observability.md`](../observability/observability.md)
- **Graceful degradation via trading halts and matching-engine failover (§8, §9)** -> [`../resilience_patterns/resilience_patterns.md`](../resilience_patterns/resilience_patterns.md)
- **Strict ordering and replay determinism as a consensus/consistency problem (§1, §4.3, §11)** -> [`../../database/consistency_models_and_consensus/consistency_models_and_consensus.md`](../../database/consistency_models_and_consensus/consistency_models_and_consensus.md)
