...
This is the same tension flagged in §1's preview, and it is **not resolved by picking "the right" ID scheme in isolation** — it's resolved by deciding, separately, *whether the ID itself should also be the shard key* (cross-ref [`../../database/sharding_and_partitioning/README.md`](../../database/sharding_and_partitioning/sharding_and_partitioning.md)). If yes, time-ordering becomes a liability (§9's hotspotting discussion); if the shard key is something else entirely (e.g., `user_id`, `tenant_id`), time-ordering in the ID is close to a free win.
...
| Best fit | Fleets that already run ZooKeeper/etcd for other purposes (leader election, service discovery, Kafka controller — cross-ref [`../consensus_algorithms/README.md`](../consensus_algorithms/consensus_algorithms.md)) — the marginal cost of also using it for worker-ID allocation is small | Small, slowly-changing, manually-managed fleets where the operational discipline to keep a config registry correct is feasible |
...
| Worker-ID coordination | Apache ZooKeeper (ephemeral sequential znodes, §4.2); etcd (leases + monotonic revisions) | Cross-ref [`../consensus_algorithms/README.md`](../consensus_algorithms/consensus_algorithms.md) for the underlying consensus protocol (ZAB / Raft) backing these guarantees |
...
- **ZooKeeper/etcd ephemeral-node semantics, sessions, and ZAB/Raft consensus underlying worker-ID allocation (§4.2, §9, §11)** -> [`../consensus_algorithms/README.md`](../consensus_algorithms/consensus_algorithms.md)
- **ID-as-shard-key hotspotting and how to decouple shard key from primary key (§5, §11)** -> [`../../database/sharding_and_partitioning/README.md`](../../database/sharding_and_partitioning/sharding_and_partitioning.md)
- **Horizontal scaling and stateless-service principles underlying the embedded-library fleet model (§2, §3)** -> [`../scalability/README.md`](../scalability/scalability.md)
...
- **NTP offset/drift and clock-backward-jump metrics as part of a broader observability strategy (§8)** -> [`../observability/README.md`](../observability/observability.md)
- **Database-segment allocation as a key-value range-reservation pattern (§4.6)** -> [`../../database/key_value_stores/README.md`](../../database/key_value_stores/key_value_stores.md)
