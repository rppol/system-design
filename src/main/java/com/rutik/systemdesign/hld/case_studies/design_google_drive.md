...
3. **Browse / list folder**: the overwhelming majority of requests (§2's 10:1 read:write ratio) are reads against the **Metadata Tree Service** — "what's in this folder," "what's this file's name/size/modified-date" — answered entirely from the **Metadata DB** without ever touching blob storage. This is why the metadata tree has its own caching layer (cross-ref [`../caching/caching.md`](../caching/caching.md)).
4. **Sync**: each client holds a **cursor** (a logical timestamp / sequence number) representing "the last change I've seen." The **Sync/Notification Service** either pushes a "something changed past your cursor" notification (via a persistent connection, cross-ref [`../../backend/websockets_and_sse/websockets_and_sse.md`](../../backend/websockets_and_sse/websockets_and_sse.md)) or the client periodically polls; either way, the client then asks the Metadata Tree Service for "all changes since cursor X," applies them locally, downloads any new/changed chunks, and advances its cursor (§4.3).
...
2. A **cursor**: an opaque, monotonically-increasing token representing "the last change this client has processed" — conceptually a position in a per-account **change log** (cross-ref [`../event_sourcing_cqrs/event_sourcing_cqrs.md`](../event_sourcing_cqrs/event_sourcing_cqrs.md)), which is an append-only stream of `(timestamp, nodeId, changeType, newManifestId/newParentId/newName, ...)` records produced by every metadata write.
...
- **Box's enterprise permission and compliance model**: Box's permission system extends the inherited-vs-explicit ACL model (§4.6) with enterprise-specific overlays — classification labels, retention policies, and legal-hold flags that can override even an `editor`'s ability to permanently delete a file. Box also exposes an enterprise events API whose admin log records *access* events (preview, download, share) alongside permission and sharing *changes*, queryable by event type and retained for up to a year — so an auditor can reconstruct who actually opened a file, not just who was granted the right to. (Box does not publish a log line per internal permission evaluation; the audit surface is the access-and-change event stream.) This reflects a real product tradeoff: consumer Drive optimizes the ACL model for "share quickly, inherit by default," while Box's enterprise customers need the same underlying tree-and-ACL primitives (§4.2, §4.6) augmented with compliance metadata that can be queried independently of the file's actual access permissions.
...
- **Apple iCloud Drive**: iCloud Drive is *not* end-to-end encrypted under Apple's standard data protection — it is one of the categories that becomes end-to-end encrypted only when the user turns on **Advanced Data Protection** (introduced December 2022), which raises the count of end-to-end-encrypted iCloud data categories from 14 to 23 (iCloud Mail, Contacts, and Calendar stay out, for interoperability with the global email/contacts/calendar systems). Apple has not published iCloud Drive's chunking or content-hashing scheme, so treat the mechanism as unspecified — but the *constraint* is unavoidable and is the point worth carrying: once file bytes are encrypted under a per-user key the server never holds, identical plaintext from two users produces different ciphertext, so the server has nothing to match on and cross-user dedup (§4.1) is impossible for that data. That is the direct tension between §4.1's dedup savings and end-to-end encryption (cross-ref [`../security_and_auth/security_and_auth.md`](../security_and_auth/security_and_auth.md)): you can have server-side dedup, or you can have the server never seeing plaintext, but not both for the same data — which is why providers who want both apply E2EE selectively, per data category, exactly as Apple does.
...
| Metadata tree DB | Sharded relational or wide-column store, sharded by owning account | §4.2 — cross-ref [`../../database/sharding_and_partitioning/sharding_and_partitioning.md`](../../database/sharding_and_partitioning/sharding_and_partitioning.md) |
...
| Permission/ACL store | Sharded KV, key = `nodeId`, cached aggressively | §4.6 — cross-ref [`../../database/database_caching_patterns/database_caching_patterns.md`](../../database/database_caching_patterns/database_caching_patterns.md) |
| Change log / sync cursor stream | Append-only per-account event log (Kafka-style or log-structured store) | §4.3 — cross-ref [`../event_sourcing_cqrs/event_sourcing_cqrs.md`](../event_sourcing_cqrs/event_sourcing_cqrs.md) |
| Push notifications for sync | Persistent websocket / long-poll gateway | §4.3 — cross-ref [`../../backend/websockets_and_sse/websockets_and_sse.md`](../../backend/websockets_and_sse/websockets_and_sse.md) |
| Metadata-tree read cache | In-memory cache (Redis-class) for hot folders, recent files, ACL lookups | §4.2, §4.6 — cross-ref [`../caching/caching.md`](../caching/caching.md) |
...
2. If push delivery is healthy, check the **per-account change log's** consumer lag (cross-ref [`../observability/observability.md`](../observability/observability.md)) — if the change log itself is backed up, metadata writes are happening but devices can't discover them.
...
As a defense-in-depth measure, a **rate limit on conflicted-copy creation per file per device per hour** (cross-ref [`../rate_limiting/rate_limiting.md`](../rate_limiting/rate_limiting.md)) was added — if a device attempts to create more than a small number of conflicted copies of the same file within a short window, subsequent attempts are rejected with an error surfaced to the client (rather than silently creating more files), which would have turned this incident from "hundreds of files over an hour" into "a handful of files plus a visible error" even before the root cause was understood.
...
- At ~500 concurrent requests/instance (lightweight, mostly cache-hit reads against the change log): `11,250 / 500` ~= **~23 instances** for sync-check handling alone — small relative to the metadata-DB and chunk-hash-index fleets, because the *vast* majority of sync checks resolve to "no changes" from a hot cache (cross-ref [`../caching/caching.md`](../caching/caching.md)) without touching the underlying change-log store
...
- **Sharing/ACL model, encryption, and dedup-vs-privacy tradeoffs (§4.6, §6)** -> [`../security_and_auth/security_and_auth.md`](../security_and_auth/security_and_auth.md)
- **Metadata-tree sharding by owning account (§4.2, §10)** -> [`../../database/sharding_and_partitioning/sharding_and_partitioning.md`](../../database/sharding_and_partitioning/sharding_and_partitioning.md)
- **Push-notified delta sync over persistent connections (§4.3)** -> [`../../backend/websockets_and_sse/websockets_and_sse.md`](../../backend/websockets_and_sse/websockets_and_sse.md)
- **Metadata-tree and permission-check read caching (§4.2, §4.6, §8)** -> [`../caching/caching.md`](../caching/caching.md)
- **Chunk hash index and ACL store as sharded KV stores (§4.1, §4.6, §7)** -> [`../../database/key_value_stores/key_value_stores.md`](../../database/key_value_stores/key_value_stores.md)
- **Per-account change log as an event-sourced sync primitive (§4.3)** -> [`../event_sourcing_cqrs/event_sourcing_cqrs.md`](../event_sourcing_cqrs/event_sourcing_cqrs.md)
- **Conflicted-copy creation rate limiting (§9 War Story 2)** -> [`../rate_limiting/rate_limiting.md`](../rate_limiting/rate_limiting.md)
