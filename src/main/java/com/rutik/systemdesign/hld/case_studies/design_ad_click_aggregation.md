...
| Fraud signal pipeline | Same stream processor, separate Kafka topic for flagged events | §4.6 — cross-ref [`../rate_limiting/README.md`](../rate_limiting/rate_limiting.md) |
...
- **Idempotency keys and the outbox pattern as the general pattern behind exactly-once counting (§4.1, §4.4, §9 War Story 1)** -> [`../distributed_transactions/README.md`](../distributed_transactions/distributed_transactions.md)
- **Raw event log as the append-only source of truth, with rollups as projections (§3, §4.4, §4.5)** -> [`../event_sourcing_cqrs/README.md`](../event_sourcing_cqrs/event_sourcing_cqrs.md)
- **Per-IP/device rate limiting and anomaly detection for the click-fraud signal pipeline (§4.6)** -> [`../rate_limiting/README.md`](../rate_limiting/rate_limiting.md)
- **Watermark lag, discrepancy %, and OLAP query latency as RED-method/SLI metrics with alerting and runbooks (§8)** -> [`../observability/README.md`](../observability/observability.md)
- **Partition count and stream-processor parallelism as horizontal-scaling decisions (§10)** -> [`../scalability/README.md`](../scalability/scalability.md)
- **Real-time OLAP store as a specialized key-value/columnar store for rollup queries (§4.5, §10)** -> [`../../database/key_value_stores/README.md`](../../database/key_value_stores/key_value_stores.md)
