...
- **Average ingestion**: 1,000,000 events/sec (clicks + impressions combined; impressions outnumber clicks by roughly 200:1 on display placements, where average CTR sits under 0.5%, and by roughly 30:1 on search placements, where it is a few percent — but both flow through the same pipeline).
...
**Concrete numbers**: a standard HLL configuration uses **2^14 = 16,384 registers**, each storing a 6-bit counter — roughly **12 KB per sketch**, regardless of whether the sketch represents 100 distinct users or 10 billion. The relative standard error is `1.04 / sqrt(m)`, so at m = 16,384 it is approximately **0.81%** (one standard deviation) — the figure Redis publishes for its own 12 KB HLL. For an ad with 50 million unique daily viewers, the HLL estimate is typically within roughly ±405,000 of the true value. For the rollup hierarchy above, storing a 12 KB HLL sketch *per `(ad_id, dimension-combo, time-bucket)`* is dramatically cheaper than storing (or being able to reconstruct) the actual set of user IDs, which at billions of `(ad_id, user_id)` pairs (§2) would require many terabytes.
...
| **Global (correct)** | **433,350** (`SUM`) | **~292,700** (`merge`, ~0.81% error) |

A naive `SUM()` of the three regional unique-user *estimates* (301,700) overcounts the roughly 3% of users who clicked the same ad from two regions within the hour (travelers, VPN users, cross-border commuters) — each such user would be counted once per region they appeared in. The HLL `merge()` operation correctly accounts for this overlap, producing a global estimate (~292,700) close to the true deduplicated count; note the ~9,000-user correction is several times larger than the sketches' own ~0.81% error band (~2,400 users at this cardinality), which is what makes the merge worth doing rather than lost in the noise. Click counts have no such overlap problem — a single click event belongs to exactly one region's ingestion topic, so `SUM()` is exact for additive metrics at any scale (§4.5's rule: "additive metrics sum, unique-count metrics merge" applies at the cross-region layer too, not just within a region's own rollup hierarchy).
...
**Region outage handling**: if one region's rollups can't ship to the global aggregation layer on schedule, the global job proceeds with the regions that *did* report and marks that hour's global figure "partial — N of M regions reported," rather than blocking entirely (cross-ref [`../cap_theorem/cap_theorem.md`](../cap_theorem/cap_theorem.md) — an availability-over-consistency choice for the global *rollup* view specifically; the billing path, §4.4, reconciles each region's raw data independently and is never blocked by another region's outage). Once the degraded region recovers and ships its delayed rollups, the global job re-runs for the affected hours and the "partial" marker clears — the same late-correction-merge mechanism used within a region (§4.3), applied one layer up.
...
| Accuracy | Perfect | ~0.81% standard error at the 16,384-register configuration (§4.5) |
...
- **Metamarkets and Apache Druid**: **Apache Druid** was started in 2011 at **Metamarkets** — an ad-analytics company — by Eric Tschetter, Fangjin Yang, Gian Merlino and Vadim Ogievetsky, precisely to power real-time analytics dashboards over high-volume ad event streams; it was open-sourced in October 2012 and moved to the Apache License in February 2015. The "minute/hour rollup, sub-second group-by queries over time-windowed data" access pattern in §4.5 is Druid's core design target, and Druid remains one of the most widely deployed OLAP stores for exactly this kind of ad-analytics workload.
...
- **Meta (Facebook) and Scuba**: Meta's internal **Scuba** system is an in-memory, real-time, ad-hoc analytics database used heavily for ad-performance investigation — engineers can slice click/impression data by arbitrary dimension combinations within seconds, illustrating the "fast approximate exploration" half of the real-time-vs-batch tension this design's §3 OLAP layer addresses. The published VLDB 2013 paper describes Scuba ingesting **millions of rows per second and expiring data at the same rate**, holding roughly **70 TB of compressed data across 1,000+ tables entirely in memory** on hundreds of servers — a scale reachable only because old rows age out continuously rather than accumulating.
...
| Fraud signal pipeline | Same stream processor, separate Kafka topic for flagged events | §4.6 — cross-ref [`../rate_limiting/rate_limiting.md`](../rate_limiting/rate_limiting.md) |
...
> **How to read these**: the two war stories below are **composite, illustrative incidents**. The mechanisms and the fixes are real and recurring, but the specific percentages, durations and rates are constructed to make each mechanism concrete — they are archetypes to reason from, not citable post-mortems of any named company.
...
- Per §4.5: each HLL sketch is ~12 KB at the 16,384-register (2^14) configuration, giving ~0.81% standard error.
...
- **Idempotency keys and the outbox pattern as the general pattern behind exactly-once counting (§4.1, §4.4, §9 War Story 1)** -> [`../distributed_transactions/distributed_transactions.md`](../distributed_transactions/distributed_transactions.md)
- **Raw event log as the append-only source of truth, with rollups as projections (§3, §4.4, §4.5)** -> [`../event_sourcing_cqrs/event_sourcing_cqrs.md`](../event_sourcing_cqrs/event_sourcing_cqrs.md)
- **Per-IP/device rate limiting and anomaly detection for the click-fraud signal pipeline (§4.6)** -> [`../rate_limiting/rate_limiting.md`](../rate_limiting/rate_limiting.md)
- **Watermark lag, discrepancy %, and OLAP query latency as RED-method/SLI metrics with alerting and runbooks (§8)** -> [`../observability/observability.md`](../observability/observability.md)
- **Partition count and stream-processor parallelism as horizontal-scaling decisions (§10)** -> [`../scalability/scalability.md`](../scalability/scalability.md)
- **Real-time OLAP store as a specialized key-value/columnar store for rollup queries (§4.5, §10)** -> [`../../database/key_value_stores/key_value_stores.md`](../../database/key_value_stores/key_value_stores.md)
