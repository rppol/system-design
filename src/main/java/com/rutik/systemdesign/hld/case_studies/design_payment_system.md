...
This is the application-layer realization of the "at-most-once effect on top of at-least-once delivery" guarantee described in [`../distributed_transactions/distributed_transactions.md`](../distributed_transactions/distributed_transactions.md) §4.6. The schema is a dedup table with a **UNIQUE constraint** on the key — the database itself is the arbiter of "who got here first" under concurrent retries.
...
public class LedgerService {
    ...
    public void transfer(UUID transactionId, long fromAccount, long toAccount,
        ...
        // Credit the destination account. Only PSP_CLEARING is exempted from
...
The end-to-end "charge a customer" flow — **reserve funds -> call PSP -> confirm order** — is exactly the Saga pattern described in [`../distributed_transactions/distributed_transactions.md`](../distributed_transactions/distributed_transactions.md) §4.3/§4.4, applied here as:
...
- **Step 2 (Call PSP)**: The PSP adapter calls Stripe/Adyen with the tokenized payment method and the **same idempotency key**, threaded through as the PSP's own idempotency header (PSPs like Stripe support this natively — §6). This step is the one that can fail for reasons entirely outside our control (card declined, PSP timeout, 3-D Secure challenge) and is the "external network call" that [`../distributed_transactions/distributed_transactions.md`](../distributed_transactions/distributed_transactions.md)'s case study (§14) calls out as needing to live *outside* any database transaction.
...
Every state transition that downstream services need to know about (`PaymentInitiated`, `PaymentSucceeded`, `PaymentFailed`, `RefundIssued`) is written to the `outbox` table in the **same local transaction** as the state change itself — collapsing the dual-write problem exactly as described in [`../distributed_transactions/distributed_transactions.md`](../distributed_transactions/distributed_transactions.md) §4.5/§6.3.
...
> **How to read these**: the four war stories below are **composite, illustrative incidents** — the failure mechanisms are real and recurring in payment platforms, but the specific amounts, dates, row counts, and durations are constructed to make the mechanism concrete. Treat them as archetypes to reason from, not as citable public post-mortems of any named company.
...
- PSPs impose per-account rate limits (Stripe's documented global limit is **100 requests/sec in live mode** and 25/sec in a sandbox, applied across reads and writes per account, with individual endpoints capped at 25/sec unless noted; higher limits are negotiated case by case). At 580 TPS peak (§2), a single Stripe account is well past the default limit — capacity here is a commercial negotiation and a multi-PSP question, not just an engineering one.
...
A: Ensure raw card data (the PAN, CVV, expiry) **never reaches your servers at all**. The card input form is rendered by the PSP's hosted page or embedded iframe/hosted fields (e.g., Stripe Elements, Adyen Drop-in) directly in the client, which tokenizes the card and sends only an opaque token to your backend. Your servers handle tokens, never PANs, which is what qualifies an e-commerce merchant for the shortest self-assessment questionnaire instead of SAQ D ("we store/process/transmit cardholder data," which requires far more extensive controls, audits, and network segmentation). Be current on the detail here: under **PCI DSS v4.0.1**, whose revised SAQ A took effect **1 April 2025**, merchants whose payment page embeds a validated TPSP's iframe/form can use **SAQ A** — but only if they also confirm the page is not susceptible to script attacks, either by applying the Requirement 6.4.3 / 11.6.1 techniques themselves or by obtaining that confirmation from the PSP. Merchants whose own page still influences the payment form beyond a pure redirect/iframe fall to SAQ A-EP. This is why §3's architecture diagram explicitly labels the PSP Adapter as "tokenized card, no raw PAN ever touches us."
...
- **Saga, TCC, outbox pattern, idempotency keys (theory underlying §4.1, §4.3, §4.4)** -> [`../distributed_transactions/distributed_transactions.md`](../distributed_transactions/distributed_transactions.md)
- **PCI-DSS, tokenization, secrets management, HMAC webhook verification (§4.6, §7, §11)** -> [`../security_and_auth/security_and_auth.md`](../security_and_auth/security_and_auth.md)
- **PSP outage failover, retry with backoff, circuit breakers (§8 runbooks)** -> [`../resilience_patterns/resilience_patterns.md`](../resilience_patterns/resilience_patterns.md)
- **Event-driven propagation of payment status changes via the outbox/Kafka pipeline (§3, §4.4)** -> [`../event_sourcing_cqrs/event_sourcing_cqrs.md`](../event_sourcing_cqrs/event_sourcing_cqrs.md)
- **Database-internals view of cross-shard/cross-service transactions (ledger consistency at scale)** -> [`../../database/distributed_transactions/distributed_transactions.md`](../../database/distributed_transactions/distributed_transactions.md)
- **Production implementation of XA drivers, Kafka transactional producers, and `@Transactional` boundaries used in §4.2/§4.4** -> [`../../backend/distributed_transactions_and_consistency/distributed_transactions_and_consistency.md`](../../backend/distributed_transactions_and_consistency/distributed_transactions_and_consistency.md)
