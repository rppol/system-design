...
- **Hilbert curves have better locality than Z-order curves**: cells that are *consecutive along the curve* are always spatial neighbours, whereas a Z-order curve jumps across the grid at every power-of-two boundary (the discontinuity behind War Story 1). No space-filling curve can make the converse true — two cells that touch in 2D can still land far apart in the 1D ordering, which is why even S2 needs the neighbour-ring expansion of §4.4 — but Hilbert's average locality is measurably better, so far fewer neighbour pairs are split
        ...
        while (!forwardPq.isEmpty() || !backwardPq.isEmpty()) {
            if (!forwardPq.isEmpty()) {
                ...
                if (forwardSettled.add(cur.node())) {
                    // Meet on a node the OTHER search has merely REACHED, not
                    // necessarily settled. The two upward searches settle
                    // different node sets, so requiring "settled by both" can
                    // let the stopping condition fire before the true meeting
                    // node is ever settled backward -- returning a longer path.
                    if (backwardDist.containsKey(cur.node())) {
            ...
            if (!backwardPq.isEmpty()) {
                ...
                if (backwardSettled.add(cur.node())) {
                    if (forwardDist.containsKey(cur.node())) { // reached, not necessarily settled
...
- **Google Maps and the S2 Geometry Library**: Google open-sourced the S2 library used internally across Maps, Earth and local search — the cube-face/Hilbert-curve cell scheme described in §4.1 is the actual production indexing scheme, not a simplification for this case study.
...
| Geocoding index | Inverted index (Elasticsearch-style) over address tokens | §4.3 — cross-ref [`../../database/search_engines/search_engines.md`](../../database/search_engines/search_engines.md) |
| POI / Places store | Wide-column store sharded by S2 cell, in-memory replica per region | §4.4 — ~40GB total (§2) |
| Traffic ingestion | Kafka (200K pings/sec) + stream processor (windowed aggregation) | §4.6 — cross-ref [`../message_queues/message_queues.md`](../message_queues/message_queues.md) |
...
patterns in geospatial and navigation systems, written as concrete narratives so the
mechanism is visible. The numbers in them are internally consistent with §2 and §10, but
...
3. **Lag-driven autoscaling**: the Traffic Service's Kafka consumer group now autoscales based on consumer lag (cross-ref [`../observability/observability.md`](../observability/observability.md)), so a regional ping-volume spike triggers additional consumer capacity for that region's partitions before lag accumulates to a freshness-impacting level.
...
- Each routing node holds that region's CH-augmented graph in memory — ~172MB average base graph (§2), growing to about **~285MB** once shortcut edges are included (§5: shortcuts roughly double the *edge* count, and §2's split is 12GB of nodes against 22.4GB of edges, so the graph as a whole grows by ~65%, not 100%) — plus its slice of the POI index (~200MB average) and geocoding index, comfortably within a single modest instance's RAM.
...
- **Tile Rendering Service origin egress**: at the <1% miss rate (~2,000 req/sec) and the same ~20KB/tile, origin egress = `2,000 x 20KB` ~= 40 MB/sec ~= **~320 Mbps** — two orders of magnitude below total CDN egress, a concrete illustration of how much load the >99% hit rate absorbs.
...
| Road graph (CH-augmented, in-memory) | ~200 regions x ~285MB with shortcuts | ~57GB total, ~285MB/region |
...
When map coverage launches in a new country or region, the incremental update pipeline (§9, "Map Data Ingestion Pipeline") doesn't apply — every subsystem starts from zero, on a compressed timeline:
...
3. **POI and geocoding index population**: bulk-loaded into the new region's index shard (§4.3, §4.4) via bulk-insert APIs — typically faster per-record than the steady-state incremental "online apply" path (§9's ingestion pipeline), since bulk loads avoid per-diff overhead.
...
| Entire region's routing fleet fails | Circuit breaker trips for the region (cross-ref [`../resilience_patterns/resilience_patterns.md`](../resilience_patterns/resilience_patterns.md)) | Failover to cold-standby replica nodes holding the same region's CH graph snapshot, provisioned in an adjacent AZ/region | Reduced capacity (e.g., 2 standby nodes vs. 6-8 primary) — higher latency and lower throughput for that region until primary capacity is restored |
...
A: Whenever one direction settles a node that the other direction has already *reached* (has any tentative distance for), the sum of the two distances is a candidate shortest path, and the minimum such sum is tracked as `bestDist` (§4.5's `ContractionHierarchyRouter`). "Reached," not "settled by both," is the load-bearing detail: the two upward searches settle different node sets, so waiting for both to settle the meeting node can let the stopping condition fire first and return a longer path. The algorithm stops once the sum of the two frontiers' minimum remaining distances (`forwardPq.peek().dist() + backwardPq.peek().dist()`) is no longer less than `bestDist` — at that point, no undiscovered meeting point could possibly produce a shorter path, so further search is provably wasted work. This early-termination condition is what keeps the "few hundred to a few thousand nodes touched" bound tight in practice.

**Q: How would you scale the Geocoding Service to handle a sudden 10x spike in search-as-you-type traffic?**
A: Forward geocoding (§4.3) is built on an inverted index over address tokens — the same scaling techniques as full-text search apply (cross-ref [`../../database/search_engines/search_engines.md`](../../database/search_engines/search_engines.md)): shard the index by token or by region, replicate read-heavy shards, and add a caching layer for common partial-address prefixes (which overlaps significantly with the typeahead problem covered in [`./design_search_autocomplete.md`](./design_search_autocomplete.md)). Reverse geocoding (the other half of §4.3) scales differently — it's an S2-cell lookup (§4.1), so scaling it is really scaling the shared geospatial index, which is already replicated per-region for the routing and POI use cases (§4.4, §10) and absorbs additional read load cheaply.
...
A: Detection happens via a circuit breaker tripping for the region (§10, cross-ref [`../resilience_patterns/resilience_patterns.md`](../resilience_patterns/resilience_patterns.md)), and traffic fails over to **cold-standby nodes** that already hold a copy of that region's CH graph snapshot — at reduced capacity (e.g., 2 standby nodes vs. 6-8 primary) but without needing to regenerate the graph, which would otherwise take the hours-scale preprocessing time from §4.5/§10's cold-start analysis. Users already navigating continue receiving ETA updates from the ETA Service (§4.7), which only needs the *existing* route's segment list and live congestion factors — not a fresh CH search — so in-progress navigation degrades far more gracefully than a fresh "get directions" request would during the same outage.

**Q: A new country launches Maps coverage for the first time — what's the bottleneck in getting it live?**
A: Counter-intuitively, it's **not** tile rendering — pre-rendering zoom levels 0-14 for a country-sized region (~500K-1M tiles) takes only about 4 minutes at typical parallelism (§10's cold-start analysis). The bottleneck is **CH preprocessing** (§4.5): contracting an entire region's road graph from an unordered initial state takes tens of minutes to a few hours, an order of magnitude longer than the *incremental* re-preprocessing used for steady-state updates (§9's ingestion pipeline, War Story 4), because cold-start preprocessing has no prior contraction order to build on incrementally. A secondary risk is **under-provisioning the routing fleet** based on a traffic forecast — launch-day curiosity traffic routinely exceeds steady-state forecasts, so new regions are commonly over-provisioned by one tier (§10) for the first 1-2 weeks.

---

## Cross-References

- **Geospatial indexing as a sharding/partitioning key (§4.1, §4.4, §4.5, §10)** -> [`../consistent_hashing/consistent_hashing.md`](../consistent_hashing/consistent_hashing.md)
- **CDN-backed tile serving and the >99% hit-rate target (§3, §4.2, §10, War Story 3)** -> [`../cdn/cdn.md`](../cdn/cdn.md)
- **Live traffic overlay and historical-fallback as a caching problem (§4.6, §4.7, War Story 2)** -> [`../caching/caching.md`](../caching/caching.md)
- **Inverted index for forward geocoding (§4.3, §11)** -> [`../../database/search_engines/search_engines.md`](../../database/search_engines/search_engines.md)
- **Nearby-entity matching contrasted with Uber's H3-based driver dispatch (§4.1, §4.4, §6, §11)** -> [`./design_uber.md`](./design_uber.md)
- **Kafka-based GPS ping ingestion for the Traffic Service (§4.6, §10)** -> [`../message_queues/message_queues.md`](../message_queues/message_queues.md)
- **Circuit breakers and graceful degradation for regional routing-fleet failure (§10)** -> [`../resilience_patterns/resilience_patterns.md`](../resilience_patterns/resilience_patterns.md)
- **Location-data minimization and retention policy (§10)** -> [`../security_and_auth/security_and_auth.md`](../security_and_auth/security_and_auth.md)
