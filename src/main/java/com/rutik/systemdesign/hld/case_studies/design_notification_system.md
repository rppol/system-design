...
2. **Circuit breaker** — reactively trips if the provider starts erroring (5xx, timeouts) so we stop hammering a degraded provider and can fail over (e.g., SES -> SendGrid). The breaker's state machine (closed -> open -> half-open) is the same one used throughout the codebase — see [`../resilience_patterns/README.md`](../resilience_patterns/resilience_patterns.md) for the full state-machine definition; it is not redefined here.
...
Following the RED method (Rate, Errors, Duration) — see [`../observability/README.md`](../observability/observability.md) for the full framework — applied per channel and per priority tier:
...
- If each worker thread handles one in-flight message at a time (synchronous provider call per thread) and a worker pod runs ~200 threads, that's `10,000 / 200 = 50` worker pods needed at peak — sized with headroom (e.g., 65-70 pods) to keep W from creeping upward as utilization approaches 100% (queueing theory: latency degrades sharply as utilization -> 1; see [`../resilience_patterns/README.md`](../resilience_patterns/resilience_patterns.md) for the broader treatment of load-shedding and backpressure when a pool approaches saturation).
...
- **Per-channel Kafka topic isolation and consumer-group scaling (§4.3)** -> [`../message_queues/README.md`](../message_queues/message_queues.md)
- **Token-bucket rate limiting and provider-quota enforcement (§4.4)** -> [`../rate_limiting/README.md`](../rate_limiting/rate_limiting.md)
- **Circuit breaker state machine, retries with backoff, load shedding (§4.4, §8, §10)** -> [`../resilience_patterns/README.md`](../resilience_patterns/resilience_patterns.md)
- **RED-method monitoring, SLI/SLO framing, and the alert-fatigue war story referenced in §9** -> [`../observability/README.md`](../observability/observability.md)
- **Async messaging patterns (pub/sub, DLQ, outbox) underlying the producer/consumer pipeline** -> [`../../backend/messaging_patterns/README.md`](../../backend/messaging_patterns/messaging_patterns.md)
- **Kafka internals (partitions, consumer groups, offset management, rebalancing)** -> [`../../backend/kafka_deep_dive/README.md`](../../backend/kafka_deep_dive/kafka_deep_dive.md)
