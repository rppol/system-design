...
<!-- study-paths
principal: consensus_algorithms.md
...
**Short:** In Raft, a follower whose election timeout fires becomes a candidate and wins by getting a majority of votes.
...
**Short:** Raft prevents split-brain because a node with a stale term always defers to any message carrying a higher term.
...
**Short:** A majority quorum guarantees any two quorums overlap by at least one node, preserving committed state.
...
**Short:** When a Raft leader fails, a follower times out, wins a quorum vote, and becomes the new leader.
...
**Short:** Kafka's ISR with acks=all is a quorum-replication scheme requiring min.insync.replicas acknowledgments.
...
**Short:** etcd exists because Kubernetes needs linearizable reads/writes and a Watch API ordinary databases don't provide.
...
**Short:** FLP proves no deterministic algorithm guarantees both safety and liveness in a fully asynchronous system.
...
**Short:** Raft improved on Paxos by fully specifying leader election, log structure, and membership changes.
...
**Short:** Choose PBFT over Raft only when nodes may be Byzantine, since Raft tolerates crash faults but not malicious ones.
...
**Short:** After a partition heals, Raft's new leader overwrites a stale follower's log from the first point of disagreement.
...
**Short:** A 3-node etcd cluster suffices for most Kubernetes deployments; 5 nodes handle very large, high-throughput clusters.
...
**Short:** A leader lease lets a Raft leader serve reads locally, skipping the quorum round-trip ReadIndex normally requires.
...
**Short:** Flexible Paxos lets Phase 1 and Phase 2 use different quorum sizes as long as they are guaranteed to overlap.
...
**Short:** KRaft replaces ZooKeeper with a built-in Raft metadata quorum, enabling faster propagation and more partitions.
...
**Short:** Raft's term number makes stale leaders harmless, since any higher-term message forces an immediate step-down.
...
**Short:** A fencing token is a monotonically increasing lock ID that lets storage reject writes from an expired lock holder.
...
**Short:** ReadIndex lets a Raft leader confirm via a quorum heartbeat that it is still leader before serving a linearizable read.
...
**Short:** A 4-node Raft cluster tolerates the same single failure as a 3-node cluster, while costing an extra machine.
...
**Short:** Consensus algorithms are CP systems: the minority partition refuses writes rather than risk diverging from the majority.
...
**Cross-references:** [database/consistency_models_and_consensus](../../database/consistency_models_and_consensus/consistency_models_and_consensus.md) (how consensus underpins linearizable reads/writes), [backend/kafka_deep_dive](../../backend/kafka_deep_dive/kafka_deep_dive.md) (ISR as a Raft-like replication mechanism), [database/distributed_transactions](../../database/distributed_transactions/distributed_transactions.md).
