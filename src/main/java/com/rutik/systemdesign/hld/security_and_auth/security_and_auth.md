...
These are handled by different components, at different points in the request path, and conflating them is itself a common design mistake (§10). A request typically passes through: **TLS termination** (encrypts the wire) -> **AuthN** (who is this — validate a token/session/cert) -> **AuthZ** (can this identity do this specific thing) -> business logic. Around all of this sits **defense in depth**: encryption at rest, secrets management, and protection against the OWASP Top 10:2025 attack classes — broken access control (A01), security misconfiguration (A02), software supply chain failures (A03), and authentication failures (A07) are the ones this module touches directly.
...
> See [Spring Security Architecture](../../spring/spring_security_architecture/spring_security_architecture.md) and [Spring Security: JWT & OAuth2](../../spring/spring_security_jwt_oauth/spring_security_jwt_oauth.md) for filter-chain implementation details, and [Backend: Auth & Authorization Systems](../../backend/auth_and_authorization_systems/auth_and_authorization_systems.md) and [Backend Security & OWASP](../../backend/backend_security_owasp/backend_security_owasp.md) for the full OWASP Top 10 treatment and production-grade implementation patterns.
...
**Cross-references:** [Spring Security Architecture](../../spring/spring_security_architecture/spring_security_architecture.md), [Spring Security: JWT & OAuth2](../../spring/spring_security_jwt_oauth/spring_security_jwt_oauth.md), [Backend: Auth & Authorization Systems](../../backend/auth_and_authorization_systems/auth_and_authorization_systems.md) (JWT internals, OAuth2 flows, OIDC, RBAC vs ABAC, token revocation), [Backend Security & OWASP](../../backend/backend_security_owasp/backend_security_owasp.md) (OWASP Top 10, injection, broken access control), [Microservices](../microservices/microservices.md) (§6.4 Security — service-to-service identity propagation), [Resilience Patterns](../resilience_patterns/resilience_patterns.md) (rate limiting and circuit breakers as defense against credential-stuffing and abuse).
