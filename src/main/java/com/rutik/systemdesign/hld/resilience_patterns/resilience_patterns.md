...
<!-- study-paths
senior: resilience_patterns.md
principal: resilience_patterns.md
...
Rate limiting (token bucket, sliding window, leaky bucket — see [Rate Limiting](../rate_limiting/rate_limiting.md) for the full treatment) is primarily discussed as a way to protect *your* service from *its callers*. In the resilience context, the same mechanisms apply in the *outbound* direction: capping the rate at which your service calls a downstream dependency, so that even a sudden burst of incoming traffic to you doesn't translate into a burst that overwhelms a downstream dependency that can't handle it. This module doesn't re-derive the algorithms — see [Rate Limiting](../rate_limiting/rate_limiting.md) for those.
...
While redundancy itself is covered in [Load Balancing](../load_balancing/load_balancing.md) and [Scalability](../scalability/scalability.md), the *resilience* angle is **how failover is triggered and how quickly**:
...
sequenceDiagram
    ...
    Note over AG: t=20ms, ~60ms margin reserved,<br/>~900ms per parallel call
    par
...
| reserved margin | Slack this hop keeps for merging/serializing the response (`~60ms`) |
...
```
  ...
  Aggregator: overhead 20 ms, reserves ~60 ms for merge/sort/serialize
      remaining = 980 - 20 - 60                =   900 ms  per parallel call
...
### 6.6 Retry Budgets — Bounding Retries in Aggregate

Backoff and jitter (§6.2) shape *when* retries land. A circuit breaker (§6.1) stops them once a
dependency is clearly failing. Neither bounds the thing that actually hurts during a *partial*
degradation: what fraction of the traffic hitting the dependency is retry traffic. At a 30%
error rate with 3 attempts per call, every individual client is behaving perfectly correctly —
correct backoff, correct jitter, breaker legitimately still closed at 30% against a 50%
threshold — and the dependency is nonetheless receiving ~60% more load than the users generated.
That is Q6's retry amplification arriving through the front door.

A **retry budget** closes it with a third control: a client-side token bucket shared across all
calls, not per call. Retries spend tokens; successes replenish them; when the bucket empties the
client stops retrying entirely and returns the first error, while first attempts continue
unaffected. It is the same token bucket as [Rate Limiting §3](../rate_limiting/rate_limiting.md), turned
inward and applied to your own retry traffic.

Two production implementations, both worth knowing by name:

| | gRPC `retryThrottling` | AWS SDK `standard` retry mode |
|---|---|---|
| Bucket | `maxTokens`, per server name | 500 tokens, per SDK client |
| Charge | failed RPC: `-1` | transient retry: `-14`; throttling retry: `-5` |
| Refill | successful RPC: `+tokenRatio` | successful retry restores its own cost; first-try success `+1` |
| Cut-off | retries disabled while `token_count <= maxTokens / 2` | retries disabled when the budget reaches zero |

The asymmetric AWS charges encode a judgement worth stealing: a transient 5xx costs almost 3x a
throttling response, because a 5xx usually means a service-wide problem where more retries will
not help, while a `ThrottlingException` is the service telling you it will succeed later. With
its default of 3 max attempts, that budget only begins to drain once sustained transient
failures exceed roughly 22% of requests (about 32% for throttling) — below that, successes
replenish it faster than retries drain it, so the budget is invisible in normal operation and
engages exactly when it should.

How it relates to the breaker: a breaker is **per dependency and binary** — all traffic or none,
once one aggregate failure rate crosses a line. A budget is **client-wide and proportional** —
it throttles retries smoothly, and it catches the cases a breaker structurally cannot see: a
dependency failing only for some keys, tenants, or shards, where no single breaker's failure
rate ever reaches its threshold but the retry load is real. Ship both.
...
- **Istio / Envoy — Outlier Detection** — service meshes implement circuit breaking at the **sidecar proxy** layer, transparently to application code. Envoy's "outlier detection" ejects (stops routing to) an upstream endpoint that trips any of its detectors: consecutive 5xx, consecutive gateway failures (502/503/504), consecutive local-origin failures (connection timeouts and resets), a success rate that deviates statistically from the cluster mean, or a configured failure-percentage threshold. Note what is *not* on that list — Envoy does not eject on latency percentiles, so "slow but returning 200s" is a failure mode outlier detection will not catch for you — effectively a circuit breaker applied per-instance rather than per-logical-dependency, useful for catching "this one of twenty pods is unhealthy" without the application needing to know.
...
- **Retrying non-idempotent operations without an idempotency key** — see §10, War Story 4, and [Distributed Transactions §4.6](../distributed_transactions/distributed_transactions.md). This is one of the most common sources of "the resilience pattern caused the incident."
...
| .NET ecosystem | **Polly** | The .NET equivalent of Resilience4j — circuit breaker, retry, timeout, fallback, hedging and a concurrency limiter, composed as a *resilience pipeline* via `ResiliencePipelineBuilder`. The concurrency limiter is how bulkhead isolation is expressed: it is a thin layer over `System.Threading.RateLimiting`, and sits outermost in the pipeline so a full pool rejects before retry or breaker logic runs. |
...
**Short:** Requiring every one of N independent dependencies to succeed multiplies their availabilities together, worsening the aggregate.
...
**Short:** An unreliable dependency needs a lower breaker threshold so it trips before its small bulkhead saturates with slow calls.
...
**Q17: Backoff, jitter and a circuit breaker are all correctly configured, and a partially degraded dependency is still drowning in retries — what's missing?**
**Short:** A client-wide retry budget caps what fraction of outbound traffic can be retries, something no per-call backoff or breaker bounds.

A: A retry budget — a client-wide cap on how much of your outbound traffic is allowed to be retries, rather than a per-call rule. At a 30% error rate with 3 attempts, every client is behaving correctly (correct backoff, correct jitter, breaker legitimately closed at 30% against a 50% threshold) and the dependency still sees roughly 60% more load than users generated; nothing in §6.1 or §6.2 bounds that ratio. The budget is a token bucket shared across all calls: retries spend tokens, successes replenish them, and when it empties the client stops retrying and returns the first error while first attempts continue untouched. gRPC ships this as `retryThrottling` (failed RPC costs 1 token, success adds `tokenRatio`, retries pause while `token_count <= maxTokens/2`) and the AWS SDK's `standard` mode as a 500-token retry quota that charges 14 for a transient retry and 5 for a throttling retry — asymmetric because a 5xx usually signals a service-wide problem that retrying will not fix. Ship it alongside the breaker rather than instead of it: a breaker is per-dependency and binary, a budget is client-wide and proportional, and only the budget catches a dependency that is failing for some tenants or shards while its aggregate failure rate never crosses any breaker's threshold.
...
**Cross-references:** [Backend: Fault Tolerance Patterns](../../backend/fault_tolerance_patterns/fault_tolerance_patterns.md) (circuit breaker state machine and Resilience4j implementation in depth), [Spring: Resilience4j Patterns](../../spring/case_studies/cross_cutting/resilience4j_patterns.md) (annotation-driven circuit breaker/retry/bulkhead configuration), [Microservices](../microservices/microservices.md) (§6.1 Circuit Breaker in a service-mesh context, and §10 War Story 1 — the retry-storm scenario this module's War Story 2 complements from a design-tuning angle), [Rate Limiting](../rate_limiting/rate_limiting.md) (rate limiting as a defensive, outbound-facing resilience measure), [Distributed Transactions](../distributed_transactions/distributed_transactions.md) (§4.6 idempotency keys — the prerequisite for safe retries, and §10 War Story 2 — the duplicate-charge failure class this module's War Story 4 revisits from the retry-policy angle), [Load Balancing](../load_balancing/load_balancing.md) (health-check-driven failover and active-active/active-passive redundancy that complements, but is distinct from, the patterns in this module).
