...
<!-- study-paths
principal: database_types_deep_dive.md
...
**Short:** B+trees optimize for fast random reads, while LSM-trees optimize for fast sequential writes at the cost of read amplification.
...
**Short:** PostgreSQL's MVCC tags each row with creating and deleting transaction IDs so readers and writers never block each other.
...
**Short:** Cassandra's LSM-tree engine makes every write sequential across a node ring, avoiding the random I/O and VACUUM pressure PostgreSQL faces.
...
**Short:** High-cardinality fields in aggregations cause fielddata OOMs and bucket explosion; fix with keyword type and cardinality aggregation.
...
**Short:** Neo4j stores direct pointers between related nodes, making each traversal hop O(1) regardless of total database size.
...
**Short:** Choose MongoDB for hierarchical, schema-fluid documents accessed by ID; choose PostgreSQL for normalized data needing joins and strong transactions.
...
**Short:** A well-designed partition key needs high cardinality, even write distribution, and single-node query isolation to avoid hot partitions.
...
**Short:** Redis Cluster shards data across 16384 hash slots, but multi-key operations and Lua scripts require all keys to share a slot.
...
**Short:** Spanner's TrueTime clock lets it wait out clock uncertainty at commit, guaranteeing commit timestamps reflect real-world transaction order.
...
**Short:** HTAP databases like TiDB, SingleStore, and TimescaleDB run OLTP and OLAP on the same data without an ETL delay.
...
**Short:** RDB is a fast, compact snapshot with more data-loss risk, while AOF logs every write for much better durability.
...
**Short:** Decide by access pattern, schema stability, and cross-entity consistency needs, not by team familiarity.
...
**Short:** Consistent hashing places nodes on a token ring, and virtual nodes split each node's range into many small pieces for better rebalancing.
...
**Short:** CAP theorem forces a choice between consistency and availability during a network partition, since partition tolerance is not optional.
...
**Short:** LSM-tree compaction can rewrite Cassandra data 10-30x, burning through an SSD's finite write endurance far faster than expected.
...
- [Database Internals and Indexing](../database_internals_and_indexing/database_internals_and_indexing.md)
- [Query Optimization](../query_optimization/query_optimization.md)
- [Caching Strategies Deep Dive](../caching_strategies_deep_dive/caching_strategies_deep_dive.md)
- [Kafka Deep Dive](../kafka_deep_dive/kafka_deep_dive.md)
- [Distributed Transactions and Consistency](../distributed_transactions_and_consistency/distributed_transactions_and_consistency.md)
