...
**Thread pool bulkhead:** Executes each call to a dependency in a dedicated, isolated thread pool. When the pool is saturated, new calls are rejected. Adds a hand-off to another thread — lmbench's `lat_ctx` reports context-switch latency in microseconds, and published runs land anywhere from a few microseconds to tens of microseconds depending on working-set size and run-queue depth, with cache-locality loss on top — but provides complete isolation, since a hung downstream call cannot even hold a thread from the shared server thread pool.
...
DoorDash's publicly documented May 12, 2022 outage is the canonical modern example of a misconfigured breaker making things worse rather than better. Routine database maintenance briefly raised read/write latency; that latency propagated upstream as timeouts, and the elevated load tripped **Envoy circuit breakers left at their defaults**. DoorDash's own postmortem timeline runs from the 4:12 pm alert on hitting Envoy circuit-breaker limits — the first such alert in over a year of running Envoy, and one whose significance the team says was not well understood — through turning consumer traffic off at the edge at 5:30 pm, a 30-minute gradual ramp back, and a partial recovery to 80% of normal order volume between 6:00 and 6:22 pm. End to end it ran about three and a half hours: a system-wide outage in which Dashers could not accept deliveries, merchants cooked meals that went undelivered, and DoorDash afterwards refunded consumers, paid merchants for cancelled orders and compensated Dashers.
