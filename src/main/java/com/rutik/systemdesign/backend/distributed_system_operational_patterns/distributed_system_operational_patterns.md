...
<!-- study-paths
principal: distributed_system_operational_patterns.md
...
**Short:** A bulkhead isolates resources per dependency to limit blast radius, while a circuit breaker detects and stops calls to a failing one.
...
**Short:** The strangler fig pattern routes traffic through a facade to gradually extract functionality into new services until the monolith is decommissioned.
...
**Short:** An ACL translates a legacy or external system's model into your domain model, keeping foreign concepts out of your codebase.
...
**Short:** Carry the correlation ID via an HTTP header or Kafka message header, store it in MDC, and restore it explicitly across thread and async boundaries.
...
**Short:** A sidecar extends the main app's own capabilities, while an ambassador proxies its outbound calls on its behalf.
...
**Short:** A preStop sleep bridges the gap while endpoint removal propagates, so a pod stops receiving new traffic before it drains in-flight requests.
...
**Short:** Evaluate flags client-side against downloaded rules from a dedicated flag service, and give every flag an owner and a removal date.
...
**Short:** MDC is a reused ThreadLocal, so clearing it only around the whole batch lets an exception mid-batch leak one record's ID into the rest.
...
**Short:** @RefreshScope proxies a bean and discards it on a refresh event, so Spring lazily rebuilds it from the environment on next access.
...
**Short:** Refreshing a @RefreshScope DataSource tears down the live connection pool, abruptly failing every in-flight transaction on it.
...
**Short:** A kill switch is a permanent, instantly-disablable safety flag, unlike a rollout flag that ramps to 100% and then gets deleted.
...
**Short:** Require an expiry date and owner on every flag, forbid flags gating other flags, and automate removal once telemetry shows zero reads.
...
**Short:** Readiness failure and traffic removal must happen first, then HTTP drain, then background resources like Kafka flush shut down last.
...
**Short:** Use a sidecar for logic that must deploy independently or span languages, and a library for sub-millisecond in-process access.
...
**Short:** Size a bulkhead to measured peak concurrent calls plus roughly 20% headroom, or it rejects healthy requests as if the dependency failed.
