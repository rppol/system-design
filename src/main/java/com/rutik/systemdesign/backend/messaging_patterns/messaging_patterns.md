...
<!-- study-paths
principal: messaging_patterns.md
...
**Short:** The outbox pattern writes an event to a DB table in the same transaction as business data, then relays it reliably.
...
**Short:** Exactly-once delivery needs an idempotent producer, acks=all, read_committed consumers, and a transactional outbox/inbox.
...
**Short:** A DLQ holds messages that exhausted retries; alert on every one, fix the root cause, then replay in controlled batches.
...
**Short:** The transactional inbox records processed message IDs in the same transaction as business logic to block double-processing.
...
**Short:** BACKWARD compatibility lets a newer schema read data written under an older schema, enabling consumers-first deployment.
...
**Short:** A stalled Debezium connector holds PostgreSQL WAL files via its replication slot, risking disk exhaustion and an outage.
...
**Short:** Choose CDC for near-real-time low-latency publishing, and polling when operational simplicity matters more than latency.
...
**Short:** FORWARD compatibility lets an old schema read new data, needed when producers must deploy before consumers can update.
...
**Short:** A poison-pill message fails identically on every retry attempt, so a fixed backoff schedule just delays quarantining it.
...
**Short:** A safe replay confirms the root cause is fixed, replays in rate-limited batches, and relies on idempotency to dedupe.
...
**Short:** The claim-check pattern stores a large payload in blob storage and publishes only a small reference on the message bus.
...
**Short:** Switch to claim-check between roughly 100KB and 1MB, and use an S3 lifecycle rule to expire blobs no message references.
...
**Short:** Retain inbox idempotency rows for a few days, matching the realistic redelivery window, then purge them on a schedule.
...
**Short:** Split a hot aggregate's events across a bounded set of sub-keys, guaranteeing order only within each sub-key.
