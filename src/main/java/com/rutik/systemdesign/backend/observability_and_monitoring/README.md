...
- **Netflix**: Atlas, their open-sourced in-memory dimensional time-series backend, is the primary telemetry store for operational metrics
...
| Client-side percentiles | Exact at the source, no query-time computation | Fixed percentiles, cannot re-aggregate; per-instance memory overhead in the app |
...
- Store metrics for 15 days at full resolution (Prometheus' own default retention) and longer at reduced resolution — Prometheus itself never downsamples, so use recording rules to pre-aggregate, or ship blocks to Thanos, whose compactor writes 5-minute and 1-hour downsampled copies
