...
- [Messaging Patterns](../messaging_patterns/messaging_patterns.md)
- [Event-Driven Fundamentals](../event_driven_fundamentals/event_driven_fundamentals.md)
...
| Exactly-once (producer) | `enable.idempotence=true`, `acks=all`, `transactional.id=x` | ~3% throughput loss at 1 KB records committing every 100 ms; materially worse with smaller records or shorter commit intervals. This is Confluent's own measurement, but note its age — it is from the November 2017 post introducing transactions in Kafka 0.11, and neither Confluent nor the Apache project has published a re-benchmark against KRaft-mode 4.x. Treat 3% as the right order of magnitude for the *shape* of the overhead (per commit, not per message) rather than a current number | None within Kafka cluster |
