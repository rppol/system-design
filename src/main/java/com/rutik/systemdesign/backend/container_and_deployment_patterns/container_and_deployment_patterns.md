...
<!-- study-paths
senior: container_and_deployment_patterns.md
...
**Short:** A multi-stage build compiles in a JDK image but ships only the JRE and artifacts, shrinking the final image sharply.
...
**Short:** maxUnavailable caps how many pods may go down during a rollout, while maxSurge caps how many extra pods may run above the desired count.
...
**Short:** CPU requests guide scheduling, while CPU limits are enforced by CFS cgroups and can throttle latency-sensitive JVM apps.
...
**Short:** Choose blue-green when you need instant rollback, an atomic schema cutover, or no two versions running simultaneously.
...
**Short:** HPA scales replicas from polled metrics every 15 seconds, but flapping, slow reaction, and rollout thrashing are common pitfalls.
...
**Short:** For containerized Java services, the most critical 12-factor rules are env-based config, statelessness, disposability, dev/prod parity, and logs as streams.
...
**Short:** KEDA scales on external event-source metrics like Kafka lag or queue depth, and supports true scale-to-zero, which HPA cannot do.
...
**Short:** KEDA polls Kafka lag externally at zero replicas, avoiding compute cost during idle periods until lag crosses a trigger threshold.
...
**Short:** The drain blocks indefinitely because the eviction API refuses any eviction that would violate the PDB's minAvailable.
...
**Short:** BestEffort pods are OOM-killed first under memory pressure, then Burstable pods furthest over their request, with Guaranteed pods last.
...
**Short:** Choose canary for gradual, metrics-gated exposure, and blue-green when the change needs an atomic, all-or-nothing cutover.
...
**Short:** Mounted secret volumes are generally safer than environment variables, which can leak into process listings, logs, and crash dumps.
