...
<!-- study-paths
principal: service_mesh_and_service_discovery.md
...
**Short:** A service mesh is a sidecar-proxy layer providing mTLS, uniform retries/circuit breaking, tracing, and traffic control.
...
**Short:** Liveness failure restarts the container; readiness failure only removes the pod from load-balancer endpoints.
...
**Short:** Client-side discovery has the client query a registry and load balance itself; server-side puts that behind a load balancer.
...
**Short:** Istio's DestinationRule outlierDetection ejects an unhealthy upstream instance at the Envoy sidecar level.
...
**Short:** mTLS means both client and server present certificates, giving mutual authentication plus in-cluster encryption.
...
**Short:** Shift traffic weight to the canary subset in steps, watching error rate and latency gates before each increase.
...
**Short:** VirtualService controls where a request routes; DestinationRule controls how the chosen connection behaves.
...
**Short:** Deploy PERMISSIVE mTLS first, verify coverage and reroute health checks, then switch the namespace to STRICT.
...
**Short:** Self-preservation stops Eureka from evicting instances when too many clients stop renewing leases at once.
...
**Short:** The JVM caches DNS lookups for 30 seconds regardless of TTL, so clients keep hitting an IP after it changes.
...
**Short:** A startup probe budget sized for normal boot time can kill the container mid-migration during a slow deploy.
...
**Short:** A startupProbe already gates when liveness begins evaluating, making a nonzero initialDelaySeconds mostly redundant.
