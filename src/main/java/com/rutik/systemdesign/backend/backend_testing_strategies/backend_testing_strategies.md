...
<!-- study-paths
principal: backend_testing_strategies.md
...
**Short:** The testing pyramid favors many fast unit tests, fewer integration tests, and minimal E2E tests to keep the suite fast.
...
**Short:** A stub returns pre-programmed responses without verification, while a mock also verifies a specific interaction occurred.
...
**Short:** Consumer-driven contract testing lets each consumer define its expected provider API, catching breaking changes in CI before deployment.
...
**Short:** Mutation testing injects small bugs into code and checks whether the test suite catches them, scoring true test sensitivity.
...
**Short:** Test asynchronous Kafka processing with Awaitility polling assertions against an embedded or Testcontainers broker.
...
**Short:** Common test anti-patterns include testing implementation details, over-mocking, non-determinism, and meaningless assertions.
...
**Short:** Achieve database test isolation with transactional rollback, cleanup SQL scripts, or a fresh Testcontainers schema per test class.
...
**Short:** Property-based testing generates many random inputs to verify an invariant holds, rather than asserting one fixed example.
...
**Short:** Test Spring Security with @WithMockUser or a JWT post-processor in @WebMvcTest to verify 200, 401, and 403 responses.
...
**Short:** Static @Container fields create one container per test class instead of a costly new one per test method.
...
**Short:** Google's test size annotations cap small, medium, and large tests by allowed I/O and runtime, keeping fast tests on every commit.
...
**Short:** WireMock stubs HTTP responses in-process for external dependencies, while Testcontainers runs the real dependency in Docker.
