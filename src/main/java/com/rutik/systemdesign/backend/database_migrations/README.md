...
**Q: What is Liquibase's built-in rollback and how does it differ from Flyway's approach to undoing migrations?**
Liquibase can auto-generate or explicitly define a rollback changeset for most changes, so `liquibase rollback` undoes them without a hand-written undo script. Flyway's community edition has no undo mechanism at all — reverting requires writing a new forward migration that manually reverses the change; only Flyway Teams (a paid tier) adds versioned undo scripts (`U8__...sql`). This makes Liquibase the safer default when a team expects frequent rollbacks in lower environments, while Flyway's philosophy favors a new forward-fix migration over automated undo. Choose Liquibase over Flyway specifically when built-in rollback tooling is a hard requirement.

**Q: When should you choose a blue-green database cutover instead of expand-contract for a schema change?**
Choose blue-green cutover for major restructuring where expand-contract's incremental phases cannot apply, such as splitting a database or switching engines entirely. Blue-green stands up a second, fully independent database, replicates or backfills all data into it, then cuts application traffic over in one coordinated switch — accepting a short cutover window measured in minutes rather than expand-contract's zero-downtime guarantee. The tradeoff is risk: rollback means switching traffic back to a now-stale primary, whereas expand-contract's small reversible steps let you abort at any phase. Reserve blue-green for restructuring so large that expand-contract's phased column-level changes cannot express it, not for routine renames or type changes.
