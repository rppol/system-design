...
- [Java 8 Features](../java8_features/java8_features.md) — lambda syntax, Optional, and stream overview as the entry point
- [Java Streams — Deep Dive](../java_streams/java_streams.md) — Spliterator internals, parallel stream mechanics, all terminal ops
- [Generics & Type System](../generics_and_type_system/generics_and_type_system.md) — Function/Supplier/Consumer type parameters, wildcard bounds in functional APIs
- [LLD: Strategy Pattern](../../lld/behavioral/strategy/strategy.md) — the GoF pattern that lambdas and method references implement without the boilerplate of a `Strategy` interface hierarchy
