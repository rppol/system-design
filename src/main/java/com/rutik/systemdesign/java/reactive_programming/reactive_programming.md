...
- The workload is simple request/response and you are on **Java 21+** — virtual threads give the same scalability with readable, debuggable, blocking code (see [`../structured_concurrency_and_loom/`](../structured_concurrency_and_loom/structured_concurrency_and_loom.md)).
...
**Short:** Mono emits 0-1 elements and Flux emits 0..N, and both are lazy and backpressured unlike the eager CompletableFuture.
...
**Short:** subscribeOn picks the thread for the source and only its first call counts; publishOn switches downstream threads and its position matters.
...
**Short:** Backpressure lets a slow subscriber request only as many elements as it can handle, via `Subscription.request(n)`.
...
**Short:** flatMap runs inner publishers concurrently with no order guarantee, concatMap runs them sequentially preserving order, and switchMap cancels the previous one.
...
**Short:** Blocking an event-loop thread freezes every connection that thread multiplexes, since only a handful of threads serve thousands of clients.
...
**Short:** A cold publisher reruns its work for every subscriber, while a hot publisher multicasts one stream and late subscribers miss earlier emissions.
...
**Short:** onErrorReturn substitutes a static fallback value, while onErrorResume switches to an entirely new fallback publisher.
...
**Short:** Reactor propagates trace data via an immutable Context that flows upstream, because a ThreadLocal breaks across thread hops.
...
**Short:** Virtual threads now handle plain blocking request/response at reactive-level scale, so reactive is reserved for streaming, push, and cross-network backpressure.
...
**Short:** Reactive Streams defines Publisher, Subscriber, Subscription, and Processor, which Java 9 copied verbatim into java.util.concurrent.Flow.
...
**Short:** StepVerifier subscribes to a publisher and asserts its exact signal sequence, and virtual time lets long delays verify in microseconds.
...
**Short:** Subscribing multiple times to a cold Flux re-executes the whole sequence independently for each subscriber, including any side effects.
...
**Short:** parallel() is fixed-size for CPU-bound work, boundedElastic is an elastic pool for wrapping blocking I/O, and single() serializes work on one thread.
...
**Short:** Operators like flatMap and publishOn request elements upstream in prefetch batches, replenishing demand once about 75% is consumed.
...
**Short:** zip combines elements positionally and completes at the shortest source, merge interleaves sources concurrently, and concat runs them sequentially.
...
**Short:** BlockHound is a Java agent that throws an error when it detects a blocking call on a thread meant to stay non-blocking.
...
**Short:** flatMap's unordered concurrent execution let payment ledger entries commit out of order, fixed by switching to sequential concatMap.
...
**Short:** flatMap's concurrency parameter caps how many inner publishers run at once, acting as a built-in backpressure limiter on downstream fan-out.
...
**See also:**
- [Structured Concurrency & Loom](../structured_concurrency_and_loom/structured_concurrency_and_loom.md) — virtual threads as the simpler default for request/response; the reactive-vs-VT decision
- [Concurrency](../concurrency/concurrency.md) — `CompletableFuture`, the eager async primitive reactive replaces
- [Spring WebFlux](../../spring/spring_webflux/spring_webflux.md) — Reactor applied in a Spring server, Netty, R2DBC, WebClient

---

## Related / See Also

- [Structured Concurrency & Loom](../structured_concurrency_and_loom/structured_concurrency_and_loom.md) — virtual threads: the modern alternative for I/O-bound request/response work
- [Concurrency](../concurrency/concurrency.md) — `CompletableFuture`, executors, and the thread primitives underneath schedulers
- [Java 9–21 Features](../java9_to_21_features/java9_to_21_features.md) — `java.util.concurrent.Flow` (Reactive Streams in the JDK)
- [Functional Programming](../functional_programming/functional_programming.md) — function composition and laziness, the conceptual basis of operator chains
- [Spring WebFlux](../../spring/spring_webflux/spring_webflux.md) — Reactor in a Spring application: Netty, RouterFunction, WebClient, R2DBC
- [Backend: Async & Concurrency Patterns](../../backend/async_and_concurrency_patterns/async_and_concurrency_patterns.md) — reactive backpressure in the broader concurrency landscape
