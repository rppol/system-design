...
- [JVM Internals](../jvm_internals/jvm_internals.md) — GC algorithms, tri-color marking, and the reference-processing phase that decides when a `Cleaner` action can run at all
- [Java Memory Model](../java_memory_model/java_memory_model.md) — happens-before rules; safe publication of the state a cleanup action reads from the cleaner's own thread
- [Performance & Tuning](../performance_and_tuning/performance_and_tuning.md) — heap dump, JFR, and async-profiler methodology used to hunt the leaks this module diagnoses
- [Concurrency](../concurrency/concurrency.md) — `ThreadLocal`, thread pools, and the pooled-thread lifecycle that makes `ThreadLocalMap` leaks possible
