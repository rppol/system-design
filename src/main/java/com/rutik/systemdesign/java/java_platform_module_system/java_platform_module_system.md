...
- [Java 9-21 Features](../java9_to_21_features/java9_to_21_features.md) — where JPMS sits alongside records, sealed classes, and virtual threads in the broader Java 9-21 arc; a condensed directive summary lives there
- [JVM Internals](../jvm_internals/jvm_internals.md) — class loading (bootstrap/platform/application loaders) that the module system's readability graph is layered on top of
- [Bytecode & Classfile](../bytecode_and_classfile/bytecode_and_classfile.md) — `module-info.class`'s bytecode-level `Module` attribute, and `javap -v` to inspect it directly
