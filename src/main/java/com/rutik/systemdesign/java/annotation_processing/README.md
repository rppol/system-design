...
```
  javac
   |
   v
 ROUND 1: original .java ---> [Processor.process] --(Filer)--> Generated1.java
   |                                                                |
   v                                                                |
 ROUND 2: Generated1.java --> [Processor.process] --(Filer)--> Generated2.java
   |                                                                |
   v                                                                |
 ROUND 3: Generated2.java --> [Processor.process] --> (no new files)
   |
   v
 FINAL ROUND: processingOver()==true  --> last chance to emit, then COMPILE ALL
```
...
```
  MapStruct (compile time):
    build:  @Mapper read --> generate UserMapperImpl.java (plain getters/setters)
    run:    new UserMapperImpl().toDto(u)   -> direct field copies   (no reflection)

  Reflective mapper (run time):
    build:  nothing
    run:    for each field: field.setAccessible(true); field.get(...) / set(...)
            -> reflection per field, per call                        (slow, opaque)
```
...
```
  spec-compliant processor:   read model --> Filer.write(NEW file)        [additive]
  Lombok:                      read model --> mutate javac's existing AST  [in place]
                               (adds getX()/equals()/builder() to YOUR class via com.sun.*)
```
