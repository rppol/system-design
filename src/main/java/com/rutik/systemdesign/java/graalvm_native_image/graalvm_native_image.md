...
<!-- study-paths
principal: graalvm_native_image.md
...
This module is the pure-Java view of native-image: the closed-world model, the reachability-analysis mechanics, the metadata file formats, the tracing agent, class-initialization timing, and the CLI — all independent of any framework. [Spring Native & GraalVM](../../spring/spring_native_graalvm/spring_native_graalvm.md) is the layer built *on top* of these exact primitives: Spring Boot's AOT engine auto-generates most of this same metadata from `@Configuration`/`@Bean`/`@Transactional` so you rarely write it by hand in a Spring app. Here, there is no framework doing that for you — you either use the tracing agent, write the JSON, or call the low-level `Feature` API directly, which is also the API those frameworks are built on.
...
**Reading a GraalVM version number.** Releases are numbered independently of the JDK they embed, in two trains: **25.0 is the LTS train** (based on Java 25, receiving security and bug fixes), and **25.1+ are Innovation releases** shipping monthly with new features. Both Oracle GraalVM and Community Edition follow this scheme. There is deliberately no GraalVM release for a non-LTS JDK, so pin to the LTS train for anything you ship and treat an Innovation build as the place to try a flag early, not as a deployment target.
...
The current format consolidates all five concerns into a single `reachability-metadata.json` under the same `META-INF/native-image/<group>/<artifact>/` path, and that is what the tracing agent emits. The per-concern files above are still parsed, which is why both forms turn up in real codebases and in library-shipped metadata. The unified document merges the same underlying concepts — reflection, resources, JNI, serialization, proxies — and folds some of them together: JNI registration, for instance, is now an attribute (`"jniAccessible"`) inside the `"reflection"` section rather than a section of its own. Treat the exact key layout as generated, version-specific output, and regenerate it with the agent for the GraalVM version you actually build with rather than hand-copying an example between versions.
...
`beforeAnalysis` runs *during* the build, as ordinary Java — it can read files, walk a classpath, or (as here) just enumerate a fixed list, and every `RuntimeReflection.register(...)` call adds that element to the reachable set before the fixed-point loop runs. This is exactly the mechanism [Spring's `RuntimeHintsRegistrar`](../../spring/spring_native_graalvm/spring_native_graalvm.md) and its AOT-generated bean definitions compile down to — Spring's contribution is generating these registrations automatically from `@Configuration`/`@Bean`, not inventing a different underlying mechanism. [Annotation Processing](../annotation_processing/annotation_processing.md) is the sibling approach for the same underlying problem: generate ordinary, statically-analyzable Java at *compile* time so there is no reflective call left for `native-image` to worry about at all.

### 6.6 Build-time vs. runtime class initialization

By default, `native-image` initializes **application classes at run time** — the policy exists so that class initialization is a performance question rather than a correctness trap — except for the subset its analysis can *prove* safe to also run at build time (safe supertypes, no calls into anything unsafe). Most **JDK and GraalVM SDK classes** (the garbage collector, the deoptimizer, core `java.lang` machinery) *are* initialized at build time by default, because they are engineered to be build-time-safe. You override either direction explicitly:
...
- **AWS Lambda custom runtimes** package Java functions as native executables to remove cold start from the request's critical path — the same technique [Spring's native module](../../spring/spring_native_graalvm/spring_native_graalvm.md) applies inside a Spring Boot function, but usable with a plain `Main` class and no framework at all.
...
**Avoid native image when**: you run a long-lived, throughput-heavy service where a warmed JIT's peak performance is the metric that matters most; your codebase depends on unbounded runtime reflection, dynamic classloading, or libraries that rewrite already-loaded classes (CGLIB, hand-rolled ASM codegen, Mockito's default inline mock maker) and cannot be hinted away; you build a genuine plugin system that loads arbitrary *untrusted* third-party jars at run time — the closed world fundamentally cannot support classes it has never seen; your CI cannot absorb multi-minute, multi-GB-RAM native builds on every change; or your team depends on mature JVM-level profiling and debugging tooling that native's ecosystem does not yet fully match.
...
A test suite ran on Mockito's default **inline** mock maker, which works by attaching a Java agent and redefining already-loaded classes. Under `nativeTest` this failed outright: a native image has no instrumentation and no class redefinition, so there is nothing for the agent to attach to. **Fix**: switch that module to the `mockito-subclass` artifact. The subclass mock maker generates mock *subclasses* through Byte Buddy, which does have native-image support, and the Native Build Tools plugin adds the required build arguments automatically when it detects Byte Buddy on the classpath. The distinction is the lesson: generating a new subclass ahead of the closed world's boundary is a solvable packaging problem, whereas rewriting an existing loaded class at run time is not — which is also why CGLIB and hand-rolled ASM `defineClass` codegen remain genuinely impossible here, and no amount of reachability metadata fixes them.
...
**Q: Why does a native image sometimes crash at runtime with a `ClassNotFoundException` even though the build succeeded and every unit test passed?**
**Short:** A clean native-image build proves nothing about reflective paths your tests never actually exercised.
...
**Q: What is the closed-world assumption and why can't GraalVM native-image just "support reflection" the way the JVM does?**
**Short:** The closed-world assumption requires every reachable class and method to be known at build time, with nothing addable later.
...
**Q: What actually differs between GraalVM Community Edition, Oracle GraalVM, and Mandrel, and why does the difference matter for a native-image build?**
**Short:** GraalVM Community Edition, Oracle GraalVM, and Mandrel differ mainly in license and whether PGO and G1 GC are available.

They are three distributions of the same underlying native-image technology with different licenses and feature sets, not three different tools. GraalVM Community Edition (OpenJDK-based, GPL) and Mandrel (Red Hat's downstream of Community Edition, built specifically for Quarkus) both lack Profile-Guided Optimization and the G1 GC option; Oracle GraalVM (Oracle-JDK-based, free for production under the GraalVM Free Terms and Conditions) adds both. A `--pgo` or `--gc=G1` flag that works on your machine and errors "not supported" on a teammate's is almost always this exact distribution mismatch.

**Q: Why might every instance of a native image end up sharing an identical "random" seed, timestamp, or generated key that was supposed to be unique per process?**
**Short:** Native images share a frozen "random" value when a static initializer captured environment state at build time.
...
**Q: Why can't a native image use CGLIB, raw ASM class generation, or Mockito's default inline mock maker the way a JVM application can?**
**Short:** A native image has no live classloader accepting new bytecode and no instrumentation to redefine classes already in the image.

Every class that will ever exist in the process was decided during the build, so there is no live classloader willing to accept freshly generated bytecode and no instrumentation able to redefine a class already baked into the image. That rules out CGLIB, hand-rolled ASM `defineClass` codegen, and Mockito's default *inline* mock maker, which attaches a Java agent and rewrites loaded classes. It does **not** rule out everything that generates classes: Byte Buddy has native-image support, so Mockito's `mockito-subclass` maker — which produces mock *subclasses* rather than rewriting existing ones — works under `nativeTest`, with the Native Build Tools plugin adding the required build arguments once it detects Byte Buddy. Sort the library by which of the two it needs before concluding it is impossible; if it truly rewrites loaded classes, the fix is a compile-time-generated equivalent or a hand-written fake, not a hint.

**Q: Why is a native image's peak throughput usually lower than a long-running JVM's, even after applying PGO?**
**Short:** A native image's peak throughput is usually lower than a warmed JVM's because there is no runtime JIT to keep re-optimizing it.

There is no runtime JIT to keep re-optimizing the program based on the traffic it is actually seeing right now. A warmed HotSpot JVM continuously profiles and recompiles hot methods with speculative, traffic-specific optimizations (see [JVM Internals](../jvm_internals/jvm_internals.md)); native-image compiles once, ahead of time, with at best a *recorded* profile from an earlier PGO run, so it typically lands around 70-90% of a warmed JVM's peak throughput. PGO narrows that gap by biasing the AOT compiler toward previously-hot code, but it is feeding the compiler yesterday's traffic pattern, not today's.

**Q: What is a common way JNI interacts badly with a native image in production, and why does reachability metadata not fix it?**
**Short:** UnsatisfiedLinkError can occur even with correct JNI hints because the native shared library must still ship separately.
...
**Q: Why do native-image builds need so much memory and take so long compared to `javac`?**
**Short:** Native-image builds need heavy memory and time because reachability analysis is a whole-program fixed-point computation.
...
**Q: What actually happens during GraalVM's static reachability (points-to) analysis?**
**Short:** Static reachability analysis computes a fixed point of every class, method, and field the program can possibly touch.
...
**Q: What are the different kinds of reachability metadata and what does each one unlock?**
**Short:** Reachability metadata comes in five kinds: reflection, resource, proxy, serialization, and JNI hints.
...
**Q: What is the unified `reachability-metadata.json` and how does it relate to the older `reflect-config.json`/`resource-config.json`/`proxy-config.json` files?**
**Short:** All five per-concern config files consolidate into one reachability-metadata.json, which is what the tracing agent emits.

The five per-concern configuration files consolidate into a single `reachability-metadata.json`, and that is what the tracing agent writes. It merges the same reflection/resource/JNI/serialization/proxy concepts into one document — JNI access, for example, becomes a `"jniAccessible"` attribute inside the `"reflection"` section rather than its own file. The older per-file format is still parsed by the build, so both forms show up in real codebases and in library-shipped metadata. Always regenerate the file with the tracing agent for the GraalVM version you build with rather than hand-copying an example between versions, since the exact layout is generated output, not a hand-authored contract.

**Q: What is the GraalVM tracing agent and what is its most important limitation?**
**Short:** The GraalVM tracing agent records metadata it observes during a JVM run, limited to what it actually saw executed.
...
**Q: What is the `Feature` API and how does it relate to what a framework like Spring Boot's AOT engine does automatically?**
**Short:** The Feature API is the build-time Java API for registering reachability metadata programmatically instead of hand-writing JSON.
...
**Q: What is Substrate VM and what does it provide that a plain compiled executable would not have?**
**Short:** Substrate VM is the minimal runtime GraalVM links into a native image, supplying its own GC and threading.
...
**Q: What is an "isolate" in Substrate VM terms?**
**Short:** An isolate is an independent Java heap and runtime state that Substrate VM can run inside a single OS process.
...
**Q: What garbage collectors can a native image use and how do you pick one?**
**Short:** Native image offers serial, G1, and epsilon garbage collectors, selected at build time with the --gc flag.
...
**Q: What is the difference between build-time and runtime class initialization, and what is the default?**
**Short:** Application classes default to runtime initialization while most JDK classes default to build-time initialization.

Build-time initialization runs a class's static initializer once during the build and bakes the result into the image heap; runtime initialization defers it to run once per process at startup. The default for **application classes** is runtime initialization unless the analysis proves a class safe for build time, while most **JDK/GraalVM SDK classes** default to build-time initialization because they are engineered to be safe there; you override either direction explicitly with `--initialize-at-build-time=<class>` or `--initialize-at-run-time=<class>`. Get this wrong for a class that captures a clock reading, a random seed, or an environment variable, and every replica cut from the same image silently shares one build-time value.

**Q: What is Profile-Guided Optimization (PGO) for native image, and who can actually use it?**
**Short:** Profile-Guided Optimization feeds a recorded execution profile back into the AOT compiler, available only on Oracle GraalVM.
...
**Q: What is "quick build mode" (`-Ob`) and when should you use it?**
**Short:** Quick build mode (-Ob) skips expensive optimization passes to speed up the edit-build-test loop, not for shipped binaries.
...
**Q: When would you choose native image over the JIT JVM, and when would you deliberately stay on the JVM?**
**Short:** Native image wins when startup latency or memory footprint dominates; the JVM wins for long-lived, throughput-bound services.
...
10. **Avoid libraries that rewrite loaded classes** (CGLIB, hand-rolled ASM codegen, Mockito's inline mock maker) in anything you intend to compile native; prefer compile-time-generated alternatives, or the subclass-generating variants that Byte Buddy's native-image support already covers.
...
- [JVM Internals](../jvm_internals/jvm_internals.md) — the JIT tiers, garbage collectors, and classloading model that native-image replaces with Substrate VM and a build-time closed world.
- [Bytecode & Class-File Format](../bytecode_and_classfile/bytecode_and_classfile.md) — the `.class` structure, verification, and `invokedynamic`/runtime codegen that native-image's static reachability analysis operates over (and why runtime bytecode generation cannot survive into a native image).
- [Annotation Processing](../annotation_processing/annotation_processing.md) — the compile-time codegen alternative to reflection: generate statically-analyzable Java so there is no dynamic call left for `native-image` to hint at all.
- [Spring Native & GraalVM (AOT)](../../spring/spring_native_graalvm/spring_native_graalvm.md) — how Spring Boot's AOT engine auto-generates most of the reachability metadata this module shows you how to write and reason about by hand.
