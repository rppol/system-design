...
**Short:** Native image can't use CGLIB or runtime bytecode generation because there is no JIT or live classloader after the build.
...
**Short:** Since GraalVM for JDK 23, the five separate config files consolidate by default into one reachability-metadata.json.
