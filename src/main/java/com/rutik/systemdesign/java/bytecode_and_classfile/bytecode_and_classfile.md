...
This module is pure Java — no Spring. It pairs with [JVM Internals](../jvm_internals/jvm_internals.md) (which loads, links, and JIT-compiles the bytes this module produces) and with [Annotation Processing](../annotation_processing/annotation_processing.md) (compile-*time* codegen, the alternative to bytecode-*time* manipulation).
...
- **Symbolic references, resolved lazily**: A `Methodref` names a class/method/descriptor by pool indices. The JVM resolves the symbolic reference to a direct one on first use (link phase — see [JVM Internals](../jvm_internals/jvm_internals.md)).
...
Verification happens once, at link time, before initialization — so a bytecode defect never reaches your `<clinit>`. Full details of the load/link/init lifecycle live in [JVM Internals](../jvm_internals/jvm_internals.md).
...
Because generics are erased (see [Generics & Type System](../generics_and_type_system/generics_and_type_system.md)), the compiler injects **bridge methods** to preserve polymorphism. `class Box implements Comparable<Box> { public int compareTo(Box b){...} }` produces *two* methods in the class file:
...
**Prefer compile-time codegen ([Annotation Processing](../annotation_processing/annotation_processing.md)) when:**
...
- [JVM Internals](../jvm_internals/jvm_internals.md) — how the bytes this module produces are loaded, verified, linked, and JIT-compiled; Metaspace and classloader leaks from codegen.
- [Annotation Processing](../annotation_processing/annotation_processing.md) — compile-time codegen (JSR 269, JavaPoet, Lombok, MapStruct): the debuggable, fail-early alternative to bytecode-time manipulation.
- [Generics & Type System](../generics_and_type_system/generics_and_type_system.md) — erasure, bridge methods, and `Signature` attributes as they appear in the class file; `MethodHandle` vs reflection.
- [Strings & Text](../strings_and_text/strings_and_text.md) — `invokedynamic` string concatenation (JEP 280) and the constant pool's role in string interning.
