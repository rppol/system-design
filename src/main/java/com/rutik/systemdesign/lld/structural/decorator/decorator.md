...
**Q: What breaks in client code that uses instanceof or getClass on a decorated object?**
**Short:** Type-based checks fail, because the wrapper is a different class and only implements the shared interface, hiding any capability the target had.
A: Any logic that asks what the object *is* rather than what it *does*. A client checking `if (x instanceof Closeable)` gets false when the decorator implements only `Component`, a framework that reads annotations off `getClass()` sees the wrapper's class with none of the target's annotations, and a registry keyed by concrete class misses entirely. There is no general fix, only three partial ones: have the decorator implement every interface it wraps, which does not scale; provide an explicit unwrap escape hatch, the approach JDBC standardised with `Wrapper.unwrap` and `isWrapperFor`; or design the interface so callers never need the concrete type. The last is the real answer, and the reason a capability that clients must detect belongs on the interface as a query method rather than being inferred from a class. Expect this to be the first thing that breaks when you introduce decoration into an existing codebase.

**Q: When you close the outermost decorator in a stream chain, what happens to the ones underneath?**
**Short:** A well-behaved decorator flushes and closes its delegate, which is why you must close the outermost wrapper and never the underlying stream.
A: Each decorator's `close()` releases its own resources and then closes the object it wraps, so closing the outermost one cascades down the whole chain — this is why `try (var out = new BufferedOutputStream(new FileOutputStream(f)))` is correct and closing the `FileOutputStream` yourself is not. Get it backwards and you lose data: closing the inner stream leaves the buffer unflushed, so the last few kilobytes written are silently discarded, and abandoning the outer wrapper without closing it does the same at garbage-collection time with no error anywhere. The rule for writing a decorator is that it must delegate `close` and, if it buffers, flush before delegating; the rule for using one is that the outermost object owns the chain. Watch for decorators that deliberately break the chain, such as a wrapper that must not close a shared stream, and name that behaviour in the class documentation because callers cannot see it.

**Q: Does wrapping a collection with Collections.synchronizedList make iteration thread-safe?**
**Short:** No, each method call is synchronized individually, so iterating still requires the caller to hold the wrapper's lock manually.
A: No, and this is the JDK's most-cited decorator gotcha. The wrapper synchronizes each individual method, so `add` and `get` are atomic, but an iteration is many calls, and another thread can mutate the list between two of them, producing `ConcurrentModificationException` or a skipped element. The documented requirement is that the caller manually synchronizes on the wrapper object for the whole loop, `synchronized (list) { for (var e : list) ... }`, which is easy to forget and impossible for the wrapper to enforce. The general lesson is that a decorator can only add behaviour at the granularity of the methods it intercepts, so any invariant spanning multiple calls has to be the caller's responsibility or needs a different design — which is why `CopyOnWriteArrayList` and `ConcurrentHashMap` exist as purpose-built classes rather than as wrappers. Prefer those concurrent collections and keep the synchronized wrappers for legacy interop.

**Q: Can you add or remove a decorator from an object after clients already hold a reference to it?**
**Short:** No, the chain is fixed at construction, so reconfiguring it means rebuilding the stack and replacing every reference clients already hold.
A: No. Each decorator holds its delegate in a field set at construction, and clients hold a reference to the outermost object, so inserting a layer means building a new chain and somehow swapping every existing reference — which you cannot do for references already handed out. This is the practical limit of "add behaviour at runtime": you choose the composition when you create the object, not afterwards. The workarounds each cost something: hand clients a stable holder or proxy whose delegate can be replaced, which adds a level of indirection to every call; make a decorator's behaviour conditional on a mutable flag, which is really a feature toggle rather than a decoration; or recreate objects at a natural boundary such as a request or a reload. Decide the composition point up front, and if runtime reconfiguration is a requirement, a proxy with a swappable target is the honest structure.

**Q: How do you apply a Decorator to a bean managed by a dependency-injection container?**
**Short:** Register the decorator as the injected bean and give it the raw implementation as its delegate, marking one primary so injection stays unambiguous.
A: You make the decorator the bean that everyone else receives and pass it the real implementation as a constructor argument, so no client is aware of the layering. In Spring that means two beans of the same interface plus a disambiguation strategy — the decorator marked `@Primary` and the delegate injected by `@Qualifier` — or a `@Bean` method that constructs the chain explicitly, which is clearer once there are several layers. The failure mode to avoid is a decorator that also injects the interface it implements without qualification, which either fails at startup as an ambiguous dependency or, worse, self-injects and recurses infinitely on the first call. Prefer explicit `@Bean` wiring for anything more than one layer, since the ordering of decorators is significant and an annotation-scattered chain hides it; and remember that framework-generated proxies for concerns like transactions are applied around whichever bean is registered, so their position relative to your decorators is decided by how you wire them.
