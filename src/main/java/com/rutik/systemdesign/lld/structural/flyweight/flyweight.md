...
**Q: Why does comparing two boxed Integers with == work for 100 and fail for 1000?**
**Short:** Small values come from a shared cache so both references point at one flyweight, while larger values are new objects that are never reference-equal.
A: Because autoboxing routes through `Integer.valueOf`, which returns a cached shared instance for values in the small range the specification requires to be interned and allocates a fresh object outside it — so `==`, which compares references, is true for 100 and false for 1000 even though `equals` is true for both. This is the flyweight pattern producing a correctness trap: the sharing is an implementation detail that accidentally makes reference comparison appear to work, and the code passes every test written with small numbers before failing in production on a real identifier. The rule is absolute — compare boxed values with `equals`, or unbox to primitives first — and the same applies to any flyweight factory you write, so document that reference identity is not part of the contract. If callers genuinely need cheap identity comparison, guarantee interning for the entire value domain rather than a subrange, and say so explicitly.

**Q: When does a flyweight cache become a memory leak?**
**Short:** As soon as its keys come from user input, since the pool grows without bound and every entry is strongly reachable forever.
A: As soon as the set of distinct intrinsic values is unbounded or attacker-controlled. A flyweight factory is a map that never removes anything, which is exactly right for a closed domain such as chess piece types or CSS colours, and exactly wrong for user-supplied strings, tenant identifiers, or request paths — each new value adds a permanent entry, and a caller can grow your heap by sending distinct values. This is why interning arbitrary user strings is a documented anti-pattern rather than an optimisation. The defences are to bound the cache with an eviction policy, which turns it into an ordinary cache and forfeits the identity guarantee, to hold values through weak or soft references so the collector can reclaim unused flyweights, or simply not to apply the pattern when the key space is open. Decide by asking who controls the set of distinct intrinsic values, and if the answer is "the caller", assume unbounded.

**Q: What does deserialization or reflection do to a flyweight's uniqueness guarantee?**
**Short:** Both bypass the factory and create duplicate instances with equal state, so any code relying on reference identity silently breaks.
A: Both create instances directly without consulting the factory, so a deserialized flyweight is a second object with the same intrinsic state and any `==` comparison against the canonical instance is false — the same weakness that forces a classic singleton to define `readResolve`. The fix is the same: implement `readResolve` to return the factory's canonical instance for that state, so the deserialized copy is discarded during construction of the object graph. Reflection can also invoke a private constructor and produce a duplicate, which you can only mitigate by making the constructor throw when an instance for that key already exists, and cannot prevent entirely. The broader point for an interview is that uniqueness is only as strong as the paths into the object, so unless every path is closed, treat sharing as a memory optimisation and never as an identity guarantee.

**Q: Should a flyweight pool use weak or soft references, and what is the trap with WeakHashMap?**
**Short:** Weak values let unused flyweights be collected, but a WeakHashMap fails if the value references its own key, which keeps the entry alive forever.
A: Use them when the key space is large or open and you want unused flyweights reclaimed; skip them for a small closed domain where the pool is trivially bounded and weak references only add collection overhead and unpredictability. The classic trap is reaching for `WeakHashMap`, whose keys are weak: if the value holds a reference to its key — which a flyweight very often does, because the key is derived from the intrinsic state stored in the flyweight — then the value keeps the key strongly reachable, the entry is never collected, and you have written an ordinary leaking map with extra steps. The correct shape is a map with weak or soft *values*, such as a `ConcurrentHashMap` holding `WeakReference` values with explicit cleanup, or a cache library configured for weak values. Soft references are a poor default because they are only cleared under memory pressure, which turns a leak into a latency spike at the worst moment.

**Q: How do you decide whether Flyweight will actually save memory before implementing it?**
**Short:** Compare the shared object's footprint against a reference plus map overhead per instance, and multiply by the duplication ratio you measured.
A: Estimate rather than assume, because the pattern replaces N copies of an object with N references plus one copy plus the pool's own bookkeeping. On a 64-bit HotSpot JVM with compressed ordinary object pointers, an object header is 12 bytes padded to an 8-byte boundary and a reference is 4 bytes, so an object carrying only a couple of fields is not much larger than the reference that would replace it, and a hash map entry costs more than either. The win comes from two multipliers: how large the shared intrinsic state is, and how duplicated it is — a million particles sharing 50 distinct sprite objects is an enormous win, a million objects with 900,000 distinct values is a loss. Measure the real distribution with a heap dump before committing, since the duplication ratio is the term people guess wrong, and remember the extrinsic state you now pass on every call has its own cost in parameter passing and in the awkwardness of the API.
