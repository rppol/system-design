...
**Q: Where does the transaction boundary belong when a Facade orchestrates several subsystems?**
**Short:** On the facade method, since it is the only place that knows the whole unit of work; per-subsystem transactions cannot be rolled back together.
A: On the facade method, because it is the one component that knows where the business operation begins and ends — annotating `placeOrder` with `@Transactional` gives the inventory reservation and the ledger write a single commit boundary, while transactions started inside each subsystem commit independently and leave you with stock reserved for an order that was never paid. Two caveats matter in practice. Anything not transactional must not be inside the boundary: sending an email or publishing to a broker inside the transaction either fires for a transaction that later rolls back, or holds a database connection open across a network call, so defer those to after-commit hooks. And a long facade method holds its connection for its entire duration, so keep external calls and expensive computation outside the annotated method. If subsystems span separate databases or services, a single transaction is not available at all and the facade needs an explicit saga with compensations.

**Q: What should a Facade do when one of three subsystem calls fails halfway through?**
**Short:** It must define and document the outcome, either compensating the completed steps or exposing partial success, because callers cannot see the subsystems.
A: It has to own the answer, because the caller made one call and cannot inspect or repair the intermediate state. Three defensible contracts exist and you must pick one explicitly: all-or-nothing, which requires a transaction or explicit compensation of the steps already applied; best-effort, where the facade continues past failures and returns a result describing what succeeded; and fail-fast with documented partial effects, which is the honest version of doing nothing but is only acceptable when the partial state is safe or self-healing. The same discipline applies to the exceptions themselves — a facade that lets `SQLException`, `IOException`, and a vendor SDK error all escape has published its subsystems' vocabulary as its own API, so translate them into a small, meaningful set and keep the originals as causes. Write the chosen policy into the facade's documentation, since nothing in the signature reveals it.

**Q: How does a Facade hide performance problems, and what do you do about it?**
**Short:** One convenient call can trigger many expensive subsystem calls, so add instrumentation and document the cost that the simplified signature conceals.
A: By design it hides how much work happens, so a caller reasonably assumes `getCustomerDashboard(id)` is one lookup when it fans out to six services, and calling it inside a loop over 200 customers produces the N+1 problem at the facade level with no visible cause at the call site. The cost is real but the answer is not to expose the subsystems again — it is to make the cost visible and to offer bulk entry points. Instrument the facade so each method reports its own latency and the number of downstream calls it made, which turns an invisible problem into a dashboard; add batch methods such as `getDashboards(ids)` that let the facade fan out efficiently instead of the caller iterating; and name expensive operations honestly, since a method called `loadFullHistory` sets expectations that `getDetails` does not. Where results are stable, caching inside the facade is legitimate precisely because the facade is the one place that knows the whole operation.

**Q: How does Facade map onto an API gateway or backend-for-frontend in a distributed system?**
**Short:** A gateway is a facade at the network edge, and a backend-for-frontend is one facade per client type rather than one shared for all.
A: Directly, and the same tradeoffs reappear a layer up. An API gateway gives external clients one endpoint, one authentication scheme, and one error format over many internal services, exactly as a facade gives one entry point over many classes — including the God Object risk, since a gateway that accumulates business logic becomes the distributed monolith everyone was avoiding. The backend-for-frontend pattern is the recognised fix for the other failure mode: rather than one facade serving a web app, a mobile app, and partners with a union of everything, each client gets its own facade shaped to its needs and owned by its team, which keeps each one small and lets them evolve independently. The cost is duplication across facades, which is acceptable because the alternative is a shared interface no team can change. Mention the parallel in an interview, then name the specific risk you would watch: logic creeping into the gateway.

**Q: What changes about a Facade once it is a published API that other teams depend on?**
**Short:** It becomes a compatibility contract, so changes must be additive and removals need a deprecation period rather than a coordinated edit.
A: It stops being a convenience you can refactor freely and becomes a contract. Internally, adding a parameter to a facade method is a compile error you fix in one commit; once other teams or external clients depend on it, the same change breaks builds you do not control, so evolution has to be additive — new overloads or new methods rather than changed signatures, new optional fields rather than required ones, and a published deprecation window before anything is removed. This also constrains the subsystems, since a facade method's semantics must survive a subsystem being replaced, which is the point but only holds if the facade's types are its own rather than the subsystems' types passed through. Practical measures are versioning the facade interface when a breaking change is unavoidable, keeping a contract test suite that any implementation must pass, and resisting the temptation to expose subsystem enums directly, because you have then published their evolution schedule too.
