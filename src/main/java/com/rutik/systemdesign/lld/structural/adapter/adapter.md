...
- `java.nio.channels.Channels.newInputStream(ReadableByteChannel)` / `Channels.newChannel(InputStream)` — bidirectional adapters between the stream API and the NIO channel API.
- `Enumeration.asIterator()` plus `Collections.list(Enumeration)` and `Collections.enumeration(Collection)` — adapters between `Enumeration` and `Iterator`/`Collection`.
- `org.apache.logging.slf4j.Log4jLogger` (in `log4j-slf4j2-impl`) — adapts a Log4j 2 `ExtendedLogger` to the SLF4J `Logger` interface, so application code can stay on SLF4J while the backend is Log4j 2. `org.slf4j.bridge.SLF4JBridgeHandler` is the mirror image: a `java.util.logging.Handler` that routes JUL records into SLF4J.
...
**Q: Should an adapter let the adaptee's exceptions escape to the client?**
**Short:** No, it must translate them into the target's exception vocabulary, or callers end up coupled to the very implementation the adapter hides.
A: No — an adapter that converts the method signatures but lets a `SQLException` or a vendor SDK's error type propagate has only half-adapted, because every caller must now catch a type that belongs to the implementation you were isolating them from, and swapping the adaptee breaks all of them. The adapter is the correct and only place for that translation: catch the adaptee's exceptions, map them to the target's declared exceptions, and preserve the original as the cause so nothing is lost for debugging. Spring's data access layer is the reference example, translating vendor-specific `SQLException` error codes into a single unchecked `DataAccessException` hierarchy so application code expresses "duplicate key" once rather than per database. Map deliberately rather than blanket-wrapping everything into one type, since collapsing a retryable timeout and a permanent constraint violation into the same exception destroys the caller's ability to react.

**Q: What do you do when the adaptee simply cannot do something the target interface requires?**
**Short:** Fail loudly with UnsupportedOperationException or narrow the target interface; never fake success, which turns a missing feature into silent data loss.
A: You have three honest options and one dishonest one. Throw `UnsupportedOperationException` from that method, which is what the JDK does when `Arrays.asList` produces a fixed-size view whose `add` cannot work — the caller finds out immediately and loudly. Narrow the target interface so the unsupported operation is not part of the contract, splitting a read-write port into a read-only one when some adaptees are read-only, which is Interface Segregation applied to the problem. Or emulate the behaviour in the adapter when it is genuinely feasible, accepting that the adapter now holds logic and needs its own tests. The dishonest option is a silent no-op that returns success, which converts a missing capability into corrupted data discovered weeks later. If more than one or two methods need stubbing out, the interfaces are too far apart and you should question the abstraction rather than the adapter.

**Q: How do you adapt a push-based callback API to a pull-based interface?**
**Short:** You need a buffer between them, because the producer decides when data arrives and the consumer decides when to read, so the queue must be bounded.
A: With a queue and a handoff, because the two styles disagree about who controls timing: the callback API calls you whenever it likes, while an `Iterator` or `InputStream` is read on the consumer's schedule. The adapter registers a listener that enqueues, and `next()` blocks on the queue until an element or an end-of-stream sentinel arrives, which also means propagating a producer-side error as a poison entry so the consumer sees an exception rather than hanging forever. The critical design decision is the queue bound, since an unbounded queue turns a fast producer into an out-of-memory error and a bounded one forces you to choose whether to block the producer's callback thread, drop, or fail. This is by far the most expensive adapter shape — it introduces a thread boundary, a buffer, and a shutdown protocol — so before writing it, check whether the library already offers a pull-style or reactive API.

**Q: Is an adapter instance safe to share between threads?**
**Short:** Only if both the adapter and the adaptee are stateless; a buffering adapter carries partial state between calls and is not thread-safe.
A: Only if it holds no state of its own and the adaptee is itself thread-safe. Many useful adapters are stateful by nature — `InputStreamReader` keeps a charset decoder holding the leading bytes of a partially read multi-byte character, so two threads reading through one instance interleave those bytes and produce corrupted text or a decoding error, not merely out-of-order output. A pure signature-translating adapter with only a `final` adaptee field is fine to share, and is the shape to aim for. Where the adapter must buffer, treat it as a per-thread or per-operation object exactly as you would the stream itself, and say so in its documentation, because callers reasonably assume a thin wrapper inherits the adaptee's thread-safety and it usually does not.

**Q: In a ports-and-adapters architecture, which module owns the adapter and which way do the dependencies point?**
**Short:** The domain owns the port interface and depends on nothing; the adapter lives in an outer module that depends inward and implements it.
A: The domain defines the port — an interface written in the domain's own vocabulary, such as `PaymentGateway` with domain types and domain exceptions — and knows nothing about who implements it. The adapter lives in an infrastructure module that depends on both the domain and the third-party SDK, implements the port, and is wired in at the composition root. That direction is the whole point, and it is Dependency Inversion made concrete: the arrow points inward, so replacing the vendor changes one outer module and recompiles nothing in the core. The mistake that undoes it is letting vendor types appear in the port's signature, at which point the domain depends on the SDK through the interface and the adapter is decoration. Check by asking whether the domain module still compiles with the vendor jar removed; if not, the boundary is in the wrong place.
