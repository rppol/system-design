...
**Q: What happens when implementation details leak through the Abstraction's interface?**
**Short:** The two hierarchies stop being independent, since any client using the escape hatch is now bound to one implementor and cannot switch.
A: The bridge stops bridging. The moment the abstraction exposes something implementor-specific — a raw handle, an untyped options map, or an `unwrap` returning the underlying object — client code starts using it, and swapping implementors breaks those clients even though the interface did not change. JDBC shows both sides: the abstraction is clean enough that most code runs on any driver, yet `unwrap` and vendor-specific SQL dialects exist because some capabilities have no portable expression, and every use of them pins that code to one database. Leaks are sometimes unavoidable, so make them visible and countable — put escape hatches on a separate, explicitly named interface rather than on the main abstraction, so a code search tells you exactly how portable you really are. Treat a growing set of implementor-specific methods as a sign the abstraction was drawn at the wrong level.

**Q: Who chooses and creates the Implementor, and why must it not be the Abstraction?**
**Short:** The caller or a factory injects it, because an Abstraction that constructs a concrete implementor is coupled to it and cannot be swapped or tested.
A: Someone outside the abstraction — a factory, a configuration reader, or a dependency-injection container — should construct the implementor and pass it in, usually through the abstraction's constructor. If the abstraction does `this.impl = new OpenGlRenderer()`, the compile-time dependency you removed from the client has reappeared inside the abstraction, tests cannot substitute a fake renderer, and adding an implementor means editing the abstraction hierarchy. The common variants are constructor injection for a fixed choice, a factory when the choice depends on runtime data such as the platform, and service discovery when implementors ship separately. Whichever you use, resolve it once at the composition root rather than letting each abstraction subclass pick its own, since scattered selection is how a codebase ends up with two different implementors live at the same time.

**Q: What do you do when there are three independent dimensions rather than two?**
**Short:** Chain bridges or give the abstraction several implementor references, and reconsider, because three genuinely orthogonal axes are rarer than they look.
A: You either hold several implementor references in the abstraction — a shape that also knows a renderer and a persistence format — or you bridge again, letting one implementor hold its own implementor. Both work, and the arithmetic still favours it heavily, since three axes of four options each is 64 classes with inheritance and 12 with composition. The practical warning is that three-way independence is often illusory: if some combinations are invalid or meaningless, you now need runtime validation of combinations that inheritance would have made unrepresentable, and the object graph becomes harder to construct and to read in a stack trace. Before adding a third axis, check whether one of them is really a Strategy on one implementor, and if the axes are genuinely orthogonal, hide the assembly behind a factory or builder so callers never wire three collaborators by hand.

**Q: How do you refactor an existing m-times-n inheritance explosion into a Bridge?**
**Short:** Extract the varying implementation calls into an Implementor interface, then replace each leaf subclass with a composition of one abstraction and one implementor.
A: Work bottom-up from the leaves. First identify which methods differ along the implementation axis rather than the abstraction axis, extract them into an implementor interface, and create one implementor per implementation variant by copying the bodies out of the leaf classes. Then collapse the leaves: `WindowsCircle` and `LinuxCircle` become one `Circle` holding an implementor, and every construction site changes from `new WindowsCircle()` to `new Circle(windowsRenderer)`. The risk concentrates in two places — leaf classes that quietly differ in more than the intended axis, which is why you compare bodies rather than assume symmetry, and the construction sites, which are numerous and easy to miss. Do it behind the existing public API with a factory that preserves the old names, so the change is invisible to callers, and lean on characterisation tests written before the refactor since the compiler cannot verify behaviour preservation here.

**Q: Is Bridge just dependency injection with extra vocabulary?**
**Short:** No, injection is the wiring mechanism, while Bridge is the decision to model two hierarchies that vary independently and to keep them separate.
A: No, though they are easy to confuse because the code looks the same: an object holding an injected interface. Dependency injection is a technique for supplying a collaborator, and it applies whether or not either side is a hierarchy. Bridge is a design decision about the shape of a domain — that there are two axes of variation, each with its own family of types, and that both will keep growing — and the injected reference is just how the two families meet. The practical difference shows up in what you expect to add: with plain injection you usually expect more implementations of one interface, while with Bridge you expect growth on both sides and you evaluate every interface change against "does this force all m abstractions or all n implementors to change". So use injection to build a bridge, but only call it Bridge when both hierarchies genuinely exist.
