...
*The Spring and Hibernate mechanics below are real; the traffic figures and timings are illustrative for one hypothetical service, not published measurements.*

A typical Spring Boot service runs at ~20k `@Transactional` method invocations/sec across `OrderService`, `PaymentService`, `InventoryService`. Each call passes through a CGLIB-generated proxy that opens (or joins) a database transaction, executes the target method, then commits or rolls back. Per-call cost is on the order of a microsecond — nearly all of it the transaction advice (thread-local transaction status plus begin/commit bookkeeping) rather than the proxy dispatch, which is nanosecond-scale — amortized below noise vs. typical 5-20ms DB query times. Bytecode generation cost is paid once per proxied class at startup (~5ms per class; ~150 classes ~ 750ms added to startup).
...
- **Spring AOP**: `org.springframework.aop.framework.JdkDynamicAopProxy` (interface-based) and `org.springframework.aop.framework.CglibAopProxy` (class-based). Activated by `@EnableTransactionManagement`, `@EnableCaching`, `@EnableAsync`. Plain Spring Framework picks the JDK proxy when the target implements an interface and CGLIB otherwise; Spring Boot has set `spring.aop.proxy-target-class=true` by default since Boot 2.0, so a Boot app gets CGLIB either way.
...
- Proxy *dispatch* is a nanosecond-scale cost, not a microsecond one: a JIT-warm JDK dynamic proxy whose handler does a real `Method.invoke` measures ~2-3ns of overhead over a direct call (50M-call loop, JDK 23, 8 distinct proxy identities to keep the call site from going monomorphic). CGLIB is in the same band and avoids per-call reflection; neither is meaningfully "the fast one" — benchmark your own workload with JMH.
- What actually costs ~1µs on a `@Transactional` call is the *advice*, not the dispatch: transaction-status thread-local lookup, `TransactionInterceptor` bookkeeping, and commit/rollback plumbing. Still negligible vs. the 5-20ms DB query inside the method.
- CGLIB bytecode generation: ~5ms per proxied class at startup; ~750ms total for a ~150-bean Spring service. Mitigated by AOT (Spring Boot's AOT processing + GraalVM native image moves this to build time).
...
**Q: Which methods can a proxy never intercept, and why does the annotation then do nothing?**
**Short:** Private, final, and static methods cannot be overridden, so a subclass proxy cannot intercept them and the annotation is silently ignored.
A: A subclass-based proxy works by overriding, so anything that cannot be overridden cannot be advised: `private` methods are invisible to the subclass, `final` methods and `final` classes cannot be overridden at all, and `static` methods are not dispatched virtually. With an interface-based proxy the restriction is different but just as sharp — only methods declared on the proxied interface are intercepted, so a public method absent from the interface is invisible. The consequence is that `@Transactional` or `@Cacheable` on such a method compiles, deploys, and does nothing, with no warning, which is the same class of silent failure as self-invocation. Check the modifiers first when an aspect "is not firing", make advised methods public and non-final on a class that is not final, and if you genuinely need to advise private or final methods, that requires load-time or compile-time weaving rather than a proxy.

**Q: What is the most common bug in a hand-written InvocationHandler?**
**Short:** Letting InvocationTargetException escape, so callers see a reflection wrapper instead of the exception the target actually threw.
A: Forgetting to unwrap `InvocationTargetException`. `Method.invoke` wraps whatever the target threw, so a handler that simply propagates it hands callers a reflection exception whose message says nothing useful, breaks every `catch (BusinessException e)` in the codebase, and turns a checked exception into something the proxy's interface never declared. The fix is to catch it in the handler and rethrow `e.getCause()`, preserving the original type and stack. Two neighbouring mistakes belong with it: swallowing exceptions in a logging handler so failures disappear, and adding frames that make stack traces unreadable, which is why it is worth keeping handler logic thin. Wrap the delegation in `try/catch` around `InvocationTargetException` as the very first thing you write, and test it with a target that throws a checked exception.

**Q: What happens to equals, hashCode, and getClass on a dynamic proxy?**
**Short:** They run against the proxy, not the target, so a proxy never equals the object it wraps and its class carries none of the target's annotations.
A: They are the proxy's, not the target's. A dynamic proxy routes `equals`, `hashCode`, and `toString` to the invocation handler, and unless the handler explicitly forwards them the proxy uses identity semantics, so `proxy.equals(target)` is false and putting both in a set yields two entries. `getClass()` returns the generated class, so reflective code sees a name like a synthetic subclass or `$Proxy0`, `instanceof` against the concrete target type fails for interface-based proxies, and annotations declared on the target class are not necessarily visible on it — which is why frameworks provide a helper to resolve the real target class rather than reading `getClass()` directly. Practical guidance: never use a proxied object as a map key unless the handler forwards `equals` and `hashCode` consistently, and use the framework's target-class utility whenever code reflects over a bean.

**Q: Why does Spring proxy @Configuration classes, and what changes if you turn that off?**
**Short:** The CGLIB proxy makes one @Bean method calling another return the shared singleton; without it the second call constructs a brand-new object.
A: Because `@Bean` methods are ordinary Java methods, and one calling another directly would just run the method again and build a second instance. Spring subclasses the configuration class so those inter-bean calls are intercepted and served from the container, preserving singleton semantics — that is the whole reason `@Configuration` is proxied while `@Component` is not. Setting `proxyBeanMethods = false` removes the proxy, which speeds startup and is the right choice for configurations whose `@Bean` methods never call one another, and it is a footgun otherwise: the code looks identical and quietly produces duplicate beans, so a repository configured twice ends up with two connection pools. The proxy also explains why a `@Configuration` class and its `@Bean` methods must not be `final` or private. Prefer passing the dependency as a method parameter over calling another `@Bean` method, since that works identically under both settings.

**Q: What is wrong with making a remote proxy look exactly like a local call?**
**Short:** It hides latency and partial failure, so callers write loops and assume success against an operation that can be slow or fail halfway.
A: Perfect transparency is the trap, because a network call differs from a local one in ways the interface cannot express: it takes milliseconds rather than nanoseconds, it can time out, it can fail after the server already applied the change, and it can succeed while the response is lost. Code written against a local-looking interface then does the wrong things — calling in a loop over 500 items, retrying a non-idempotent operation after a timeout and double-charging, and having no timeout at all because a local call never needed one. The lessons that came out of the transparent-RPC era are to make remoteness visible in the API through coarse-grained, batch-friendly methods and an explicitly asynchronous or timeout-bearing signature, to make operations idempotent so a retry is safe, and to surface remote failure as a distinct exception type rather than as whatever the local interface declared. Use a remote proxy to remove the plumbing, not to pretend the network is not there.
