...
|---------|-----------|-----------------|
| Adapter | [adapter/](adapter/adapter.md) | Convert an incompatible interface into the one the client expects |
| Bridge | [bridge/](bridge/bridge.md) | Separate abstraction from implementation so both can vary independently |
| Composite | [composite/](composite/composite.md) | Treat individual objects and tree compositions uniformly via a shared interface |
| Decorator | [decorator/](decorator/decorator.md) | Attach additional responsibilities to an object dynamically at runtime without subclassing |
| Facade | [facade/](facade/facade.md) | Provide a single simplified interface to a complex subsystem of many components |
| Flyweight | [flyweight/](flyweight/flyweight.md) | Share common intrinsic state across many fine-grained objects to reduce memory |
| Proxy | [proxy/](proxy/proxy.md) | Control access to an object — add lazy loading, security checks, logging, or remote forwarding |
...
|---------|------|--------|---------------------|-------------|
| Adapter | [adapter/](adapter/adapter.md) | Convert one interface to another | `InputStreamReader` wrapping `InputStream`; `Arrays.asList()` | Class adapter (`extends Adaptee implements Target`) is legal in Java but locks you to one adaptee class — prefer the object adapter |
| Bridge | [bridge/](bridge/bridge.md) | Separate abstraction from implementation so both can vary independently | JDBC: `java.sql.Driver` (abstraction) + vendor driver (implementation) | Often mistaken for Adapter — Bridge is designed upfront; Adapter retrofits |
| Composite | [composite/](composite/composite.md) | Treat individual objects and compositions uniformly via a tree | `java.awt.Container` + `Component`; `jakarta.faces.component.UIComponent` | Leaf-specific operations (e.g., `add()`) don't make sense on leaves; breaks type safety |
| Decorator | [decorator/](decorator/decorator.md) | Attach additional responsibilities to an object dynamically | `BufferedReader(new InputStreamReader(socket.getInputStream()))` | Deep stacking makes debugging hard; ordering of decorators affects behavior |
| Facade | [facade/](facade/facade.md) | Provide a simplified interface to a complex subsystem | `jakarta.faces.context.FacesContext`; Spring's `JdbcTemplate` | Facade can become a God Class if it exposes too much; doesn't prevent direct subsystem access |
| Flyweight | [flyweight/](flyweight/flyweight.md) | Share common state across many fine-grained objects | `Integer.valueOf()` cache (-128 to 127); `String.intern()`; `Character` cache | Intrinsic/extrinsic state boundary must be clear; thread-safety of shared state required |
| Proxy | [proxy/](proxy/proxy.md) | Control access to an object — add lazy loading, logging, security, remote access | Spring AOP (JDK dynamic proxy / CGLIB); `java.lang.reflect.Proxy` | Proxy must be type-compatible with the subject (shared interface for JDK proxies, generated subclass for CGLIB); CGLIB cannot proxy `final` classes or advise `final` methods |
...
**Short:** Proxy controls access transparently for the caller; Decorator adds behavior the caller explicitly composes.
...
**Short:** Adapter retrofits two already-existing incompatible interfaces; Bridge is designed upfront to prevent that need.
...
**Short:** Spring uses a JDK dynamic proxy when the bean implements an interface, and a CGLIB subclass otherwise.
...
**Short:** Adapter converts one interface to another one-to-one; Facade simplifies many subsystem calls into one, many-to-one.
...
**Short:** GoF offers two designs: transparency, which puts add() on Component, or safety, which restricts it to Composite.
...
**Short:** Intrinsic state is shared and stored inside the Flyweight; extrinsic state is context-specific and passed in per call.
...
**Short:** A Decorator stack becomes unmanageable once it grows deep, order-sensitive, or is assembled conditionally at runtime.
...
**Short:** Calling this.method() inside a bean bypasses the Spring proxy entirely, so transactional advice never runs.
...
**Short:** Integer.valueOf caches Integer instances for values -128 to 127 and reuses them as shared Flyweight objects.
...
**Short:** Bridge replaces N times M subclasses with N plus M by separating an abstraction from its implementation.
...
**Short:** A remote proxy represents an object living in another JVM and marshals calls to it over the network, as RMI stubs do.
...
**Short:** Facade is the wrong choice when clients still need full, unrestricted access to the underlying subsystem.
...
**Short:** Object adapter, which delegates via composition, is preferred in Java over the more restrictive class adapter.
...
**Short:** JdbcTemplate hides the entire JDBC resource-and-exception boilerplate behind simple query and update calls.
...
**Short:** Yes, a Composite can contain other Composites, modeling a recursive part-whole hierarchy like a file system.
...
**Short:** Proxy, Decorator-vs-Proxy, and Adapter-vs-Bridge are tested most because each combines mechanics with a real framework.
