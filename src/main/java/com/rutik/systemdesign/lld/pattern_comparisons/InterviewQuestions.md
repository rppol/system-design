...
**Short:** Singleton gives a class one instance and a global access point, made thread-safe via double-checked locking, a holder class, or an enum.
...
**Short:** Factory Method defines a creation interface but lets subclasses decide which concrete class to instantiate.
...
**Short:** Observer defines a one-to-many dependency so all subscribers are notified automatically when a subject's state changes.
...
**Short:** Strategy encapsulates interchangeable algorithms as composed objects, letting behavior change at runtime unlike inheritance.
...
**Short:** Decorator attaches additional responsibilities to an object dynamically, avoiding a subclass explosion from stacked features.
...
**Short:** Builder separates constructing a complex object from its representation, avoiding telescoping constructors with many parameters.
...
**Short:** Adapter converts one class's interface into another that clients expect, letting incompatible classes work together.
...
**Short:** Command encapsulates a request as an object, enabling queuing, logging, and undoable operations.
...
**Short:** Proxy provides a surrogate that controls access to another object; virtual, protection, and remote are common variants.
...
**Short:** Composite arranges objects into tree structures so clients treat individual objects and compositions uniformly.
...
**Short:** Chain of Responsibility passes a request along a chain of handlers until one of them processes it.
...
**Short:** Template Method defines an algorithm's skeleton in a base class while letting subclasses override individual steps.
...
**Short:** State lets an object change its behavior by delegating to a current-state object instead of branching on conditionals.
...
**Short:** Facade provides a simplified, unified interface to a complex subsystem without hiding its individual classes.
...
**Short:** Memento captures and externalizes an object's internal state so it can be restored later without breaking encapsulation.
...
**Short:** The Holder idiom is generally preferred over double-checked locking because JVM class-loading guarantees its thread safety.
...
**Short:** Slow observers are handled with async dispatch on a thread pool, per-observer queues, or reactive streams with back-pressure.
...
**Short:** Builder is best for four or more parameters, optional fields, or multi-step construction where a plain constructor gets unwieldy.
...
**Short:** Decorator avoids class explosion because n features need only n decorator classes instead of 2^n subclasses.
...
**Short:** In a file-system Composite, a Directory sums its children's sizes recursively while a File returns its own size directly.
...
**Short:** Bridge separates an abstraction from its implementor so a new shape or renderer needs only one new class, not a product of both.
...
**Short:** Command-based undo reverses actions with stored inverses, while Memento-based undo restores a full saved state snapshot.
...
**Short:** A Proxy typically creates and controls the real subject it wraps, while a Decorator always receives an already-built object.
...
**Short:** Observer suits one-to-many broadcast with independent reactions, while Mediator suits many-to-many coordination needing central control.
...
**Short:** Flyweight shares intrinsic, context-independent state across many fine-grained objects to cut memory usage.
...
**Short:** Visitor uses double dispatch so a new operation becomes a new visitor class, with no changes to the element hierarchy.
...
**Short:** Which pattern replaces a type-keyed switch depends on what it varies on: polymorphism, State, Strategy, or Visitor.
...
**Short:** Each middleware handler processes a request or calls next.handle() to pass it down the chain, short-circuiting on failure.
...
**Short:** Use Factory Method for one product with a subclass-decided implementation; use Abstract Factory for a family of related products.
...
**Short:** An event bus maps event types to subscriber lists and dispatches each published event, often as a Command, to its handlers.
...
**Short:** A logging framework combines Chain of Responsibility for level filtering, Strategy for appenders and formatters, and Decorator for enrichment.
...
**Short:** External iteration lets the client drive the loop, while internal iteration lets the aggregate call a client-supplied callback.
...
**Short:** Template Method embodies the Hollywood Principle because the base class calls subclass hooks, not the other way around.
...
**Short:** Interpreter represents a grammar as a class hierarchy so sentences can be evaluated by walking the resulting AST.
...
**Short:** Prototype creates new objects by copying an existing instance, but clone() falls short with circular references or unclonable fields.
...
**Short:** A plugin architecture combines Strategy for capabilities, a registry Factory for lookup, Observer for events, and Facade for isolation.
...
**Short:** Branching undo/redo replaces the linear history stack with a tree of Command nodes, branching whenever a new action follows an undo.
...
**Short:** An order-processing system uses State for the order lifecycle, Command for actions, and an Observer event bus to decouple services.
...
**Short:** Refactoring a God Object means extracting each responsibility into its own class, then coordinating them behind a thin Facade.
...
**Short:** A pattern-based rate limiter uses Strategy for the limiting algorithm, Proxy to intercept requests, and Chain of Responsibility for rules.
...
**Short:** A caching layer wraps the real data source with a Cache Proxy and uses Strategy for the eviction policy.
...
**Short:** A multi-channel notification service uses Strategy per channel, Command for queued sends, and Chain of Responsibility for routing checks.
...
**Short:** A connection pool is an Object Pool guarded by a Singleton, with clients borrowing a Proxy connection that returns itself on close().
...
**Short:** A workflow engine models tasks as a Composite tree of Commands, with State for task lifecycle and Mediator for orchestration.
...
**Short:** A hand-built DI container is a type-to-factory registry where singleton scope memoizes the first resolved instance.
...
**Short:** A chess game combines State for game phase, Strategy for per-piece move generation, and Command for moves with undo.
...
**Short:** A distributed pub/sub system extends Observer with a broker as the Subject, treating each message as a serializable Command.
...
**Short:** A URL shortener uses Strategy for the encoding algorithm and a Cache Proxy to check for a long URL before hitting the database.
...
**Short:** Circuit breaker combines State for CLOSED/OPEN/HALF-OPEN transitions with a Proxy that checks state before delegating the real call.
...
**Short:** Needing to touch fifteen classes for one feature signals an Open/Closed violation, fixable with Visitor, Decorator, or Mediator.
