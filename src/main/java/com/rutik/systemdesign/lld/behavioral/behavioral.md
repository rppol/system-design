...
| # | Pattern | Module |
|---|---------|--------|
| 1 | Chain of Responsibility | [chain_of_responsibility/](chain_of_responsibility/chain_of_responsibility.md) |
| 2 | Command | [command/](command/command.md) |
| 3 | Interpreter | [interpreter/](interpreter/interpreter.md) |
| 4 | Iterator | [iterator/](iterator/iterator.md) |
| 5 | Mediator | [mediator/](mediator/mediator.md) |
| 6 | Memento | [memento/](memento/memento.md) |
| 7 | Observer | [observer/](observer/observer.md) |
| 8 | State | [state/](state/state.md) |
| 9 | Strategy | [strategy/](strategy/strategy.md) |
| 10 | Template Method | [template_method/](template_method/template_method.md) |
| 11 | Visitor | [visitor/](visitor/visitor.md) |
...
|---------|--------|---------------------|-------------|
| [Chain of Responsibility](chain_of_responsibility/chain_of_responsibility.md) | Pass a request along a handler chain until one handles it | Middleware pipelines; auth/validation/logging layers | Silent drop when no handler matches |
| [Command](command/command.md) | Encapsulate an action, its receiver, and its parameters as an object | Undo/redo; task queuing; deferred execution | Command objects becoming fat with business logic |
| [Interpreter](interpreter/interpreter.md) | Define a grammar and interpret sentences in it | Simple DSLs; expression parsers; SQL WHERE clauses | Performance degrades for complex grammars |
| [Iterator](iterator/iterator.md) | Provide sequential access to elements without exposing the container | Any traversal abstraction over a collection or cursor | ConcurrentModificationException on structural modification |
| [Mediator](mediator/mediator.md) | Route communication between objects through a central hub | N objects with M*N connections; chat rooms; UI controllers | Mediator becomes a god object |
| [Memento](memento/memento.md) | Capture and restore an object's state without exposing its internals | Undo history; save points; transactional rollback | Deep copy cost on large object graphs |
| [Observer](observer/observer.md) | Notify multiple subscribers when a subject's state changes | Domain events; UI data binding; Spring ApplicationEvent | Listener leaks; synchronous I/O in observers |
| [State](state/state.md) | Allow an object to alter its behavior when its internal state changes | Vending machines; order lifecycles; TCP connections | Class explosion for large FSMs |
| [Strategy](strategy/strategy.md) | Define a family of algorithms and make them interchangeable | Sorting; payment methods; discount rules | Strategy interface leaking implementation details |
| [Template Method](template_method/template_method.md) | Define an algorithm skeleton in a base class; let subclasses fill in steps | Spring Batch readers/processors; framework hooks | Tight coupling to base class; hard to test |
| [Visitor](visitor/visitor.md) | Add new operations to an object structure without modifying the elements | AST traversal; type-specific reporting; compiler passes | Breaking change when a new element type is added |
...
1. [Observer](observer/observer.md) — Foundation for all event-driven thinking. Once you understand publish/subscribe, every framework's event system makes sense. Study first.

2. [Strategy](strategy/strategy.md) — The most practically useful pattern. Appears everywhere as lambdas in Java 8+. Teaches the core idea: extract the varying part into an interface.

3. [Command](command/command.md) — Builds on Strategy by adding history and deferral. Once you see Runnable as a Command, task queues and undo systems become obvious.

**Phase 2 — Control Flow**

4. [Chain of Responsibility](chain_of_responsibility/chain_of_responsibility.md) — Builds on Command by adding routing. After studying Command, CoR feels like a natural extension: a linked list of commands.

5. [Template Method](template_method/template_method.md) — Complements Strategy (inheritance vs composition). Study alongside Strategy to understand when each applies.

**Phase 3 — State and Coordination**

6. [State](state/state.md) — FSM thinking. Builds on Strategy structurally; differs in who drives the switch. Study after Strategy so the structural similarity is obvious.

7. [Iterator](iterator/iterator.md) — Simple and ubiquitous. Java hides this with for-each and Streams, but knowing the underlying protocol matters for custom data structures.

8. [Mediator](mediator/mediator.md) — Study after Observer. Mediator solves the problem that arises when you have many observers that also need to talk to each other.

**Phase 4 — Advanced**

9. [Memento](memento/memento.md) — Simple state capture. Study alongside Command (Command-with-undo is an alternative to Memento for behavioral state).

10. [Visitor](visitor/visitor.md) — Advanced; requires understanding of type hierarchies and the limits of single dispatch. Study after you are comfortable with polymorphism.

11. [Interpreter](interpreter/interpreter.md) — Most specialized. Only relevant when building DSLs or expression parsers. Study last.
...
| Pattern | See Also |
|---------|---------|
| Observer | [`../../spring/spring_events_and_scheduling/`](../../spring/spring_events_and_scheduling/spring_events_and_scheduling.md) — Spring ApplicationEvent is a managed Observer implementation |
| Chain of Responsibility | [`../../spring/filters_and_interceptors/`](../../spring/filters_and_interceptors/filters_and_interceptors.md) — Servlet FilterChain is CoR in production |
| Template Method | [`../../spring/spring_batch/`](../../spring/spring_batch/spring_batch.md) — ItemReader/Processor/Writer follow Template Method skeleton |
| Strategy | [`../../java/functional_programming/`](../../java/functional_programming/functional_programming.md) — lambdas are Strategy objects; Comparator.comparing() is a factory for them |
| State | [`../../hld/microservices/`](../../hld/microservices/microservices.md) — service lifecycle (starting, running, degraded, stopped) modeled as FSM |
| Command | [`../../java/concurrency/`](../../java/concurrency/concurrency.md) — Runnable is Command; ExecutorService is the task queue that receives Commands |
| Iterator | [`../../java/java_streams/`](../../java/java_streams/java_streams.md) — Stream is a composable, lazy, parallelizable alternative to external Iterator |
| Visitor | [`../pattern_comparisons/`](../pattern_comparisons/pattern_comparisons.md) — see Decorator_vs_Proxy.md for intent analysis methodology applicable to Visitor |
...
**Short:** Strategy lets the client pick the algorithm; State lets the object drive its own transitions internally.
...
**Short:** Observer broadcasts one-to-many; Mediator centralizes many-to-many communication through one hub.
...
**Short:** Command encapsulates an action to run later with its receiver and parameters; Strategy encapsulates an algorithm to run now.
...
**Short:** Template Method varies behavior through inheritance; Strategy varies it through composition, which Effective Java prefers.
...
**Short:** It becomes an antipattern without a terminal handler, letting unmatched requests silently fall off the chain.
...
**Short:** Java's enhanced for-each loop desugars into calls on `hasNext()` and `next()`, the textbook Iterator pattern.
...
**Short:** Spring's ApplicationContext acts as the event bus, decoupling the publisher from every registered @EventListener.
...
**Short:** Deep copies of large object graphs become expensive in memory and CPU, and can fail on non-serializable fields.
...
**Short:** Java resolves overloads by the argument's declared type at compile time, so double-dispatch is needed to reach the correct visit method.
...
**Short:** Observer/Listener is the most abused pattern, mainly from blocking I/O inside listeners and never-unregistered subscriptions leaking memory.
...
**Short:** One class per state becomes unmanageable past roughly 50 states, favoring a state-transition table instead.
...
**Short:** Iterator is caller-driven external iteration; Stream is library-driven internal iteration via functional pipelines.
...
**Short:** Type safety, separation of concerns, and open/closed compliance are the three forces that push you toward Visitor.
...
**Short:** Mediator turns into a God Object when it centralizes so much coordination logic that it knows every participant's behavior.
...
**Short:** Interpreter's one-object-per-grammar-node design creates a deep object graph that is memory-intensive and slow to evaluate.
...
**Short:** Memento snapshots one object's state locally, while Event Sourcing reconstructs state system-wide from an append-only event log.
