...
**Q: What stops a Mediator from looping forever when a colleague reacts to its own notification?**
**Short:** Nothing structural, so a mediator needs a reentrancy guard or a directed event graph, or A notifies B which notifies A endlessly.
A: Nothing in the pattern prevents it — the mediator's job is to route, and if colleague A's change triggers a notification that makes B change, which triggers a notification that changes A, the cycle runs until the stack overflows. This is the standard bug in UI mediators: setting a slider updates a text field, the text field's change handler updates the slider, and dragging the slider freezes the window. Three fixes are used in practice: a per-event reentrancy flag on the colleague so a programmatic update does not re-fire its own handler, an equality check that suppresses a notification when the value did not actually change, and modelling the dependencies as a directed acyclic graph so the mediator can refuse a cyclic wiring at registration time. Prefer the equality check as the default because it costs nothing and also removes redundant work, and add the guard flag only where a genuine round-trip is unavoidable.

**Q: How do you tell a real Mediator from an ordinary service class that someone renamed?**
**Short:** A Mediator exists so colleagues never reference each other; a service that its callers still call directly is just a service.
A: The test is whether removing the mediator would force the participants to hold references to one another. In a genuine Mediator the colleagues know only the mediator, communication flows in both directions through it, and the coordination rules — who reacts to what, in what order — live in the mediator and nowhere else. A class named `OrderMediator` whose callers already know and invoke the same collaborators directly is not a mediator, it is a service with a misleading name, and the pattern's decoupling benefit is absent. This matters in an interview because the payoff you claim, turning n-squared peer wiring into n spokes, only exists if the peers really stopped wiring to each other; if they did not, say so and call it an orchestration service instead.

**Q: How does a Mediator dispatch to the right colleague without a chain of instanceof checks?**
**Short:** Register colleagues by an explicit event type or channel key so dispatch is a map lookup instead of a growing instanceof ladder.
A: By keying the routing on data rather than on runtime type — a `Map<EventType, List<Colleague>>` or a `Map<String, Handler>` populated at registration, so adding a colleague adds a map entry rather than a branch. The `instanceof` ladder is the usual first draft and it fails the Open/Closed Principle immediately: every new colleague type edits the mediator, subclass ordering starts to matter, and the ladder becomes the God Object the pattern was supposed to prevent. The two alternatives worth naming are double dispatch, where the colleague calls a typed method on the mediator such as `mediator.buttonClicked(this)`, which keeps type safety at the cost of a wider mediator interface, and a generic `notify(sender, event)` with a registry, which keeps the interface narrow at the cost of stringly-typed events. Choose double dispatch for a small fixed set of colleague types and the registry when colleagues are added dynamically or by plugins.

**Q: If a mediator notifies three colleagues and the second one throws, what state is the system in?**
**Short:** Partially coordinated, since the first colleague already reacted and the third never heard, so the mediator must define its failure policy.
A: Partially coordinated, and that is the pattern's quiet weakness: the mediator is not a transaction, so colleague one has already applied its change, colleague two failed, and colleague three was never called. You must choose a policy explicitly rather than inherit whatever the exception does — best-effort, where each notify is wrapped in try/catch and failures are logged so every colleague still hears the event; fail-fast, where the mediator aborts and propagates, accepting the partial state; or compensating, where the mediator tracks which colleagues succeeded and instructs them to roll back. Best-effort suits notification-style coordination such as UI updates, while compensating is required when colleagues are mutating durable data. Whichever you pick, document it on the mediator interface, because callers cannot tell from the signature.

**Q: How does Mediator map onto orchestration versus choreography in distributed systems?**
**Short:** An orchestrating saga coordinator is a distributed Mediator, while a choreographed saga where services react to each other's events is Observer.
A: Directly: a saga orchestrator holds the workflow, calls each service in turn, and the services never call one another — that is Mediator promoted to a network boundary, with the same benefits and the same risk of one component knowing everything. Choreography is the opposite arrangement, where each service publishes events and others subscribe, which is Observer at scale and gives you loose coupling at the cost of no single place that describes the flow. The tradeoff repeats the in-process one exactly: the orchestrator is easy to reason about, easy to instrument end-to-end, and a single point of failure and change, while choreography scales team autonomy but makes "what happens when an order is placed" a question you answer by grepping across repositories. Reach for orchestration when the workflow has real business meaning and needs compensation, and for choreography when the participants are genuinely independent reactions to a fact.
