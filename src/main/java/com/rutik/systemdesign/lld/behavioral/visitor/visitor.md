...
**Q: Does pattern matching for switch over a sealed hierarchy replace Visitor in modern Java?**
**Short:** Largely yes, because a sealed interface plus an exhaustive switch gives compile-time completeness without accept methods or double dispatch.
A: For a hierarchy you control, largely yes. Declaring `sealed interface Node permits Add, Mul, Literal` and switching over it with type patterns gives you the property Visitor was invented for — the compiler rejects the switch when a new subtype appears and no branch handles it — without adding an `accept` method to every element, without a visitor interface, and with the operation readable as one method instead of a class. Visitor still wins in three situations: the hierarchy is not sealed or not yours to modify, so exhaustiveness cannot be checked; you need the operation to be a first-class object that can be passed, composed, or configured; or the traversal itself is complex and belongs with the elements. Say the switch is the default in current Java and keep Visitor for the extensible and pluggable cases, rather than reciting the GoF version as though nothing changed.

**Q: Who owns the traversal — the elements or the visitor — and how do you skip a subtree?**
**Short:** Whoever recurses controls it, so put traversal in the visitor or return a control value if visitors must skip subtrees or change order.
A: If each element's `accept` recurses into its children, traversal is fixed by the structure and every visitor gets the same order, which is simple but means no visitor can prune. If the visitor recurses, each one chooses pre-order, post-order, or partial traversal, at the cost of duplicating walking code in every visitor — usually solved with an abstract base visitor that implements the default walk. The third option is the one the JDK chose: `java.nio.file.FileVisitor` methods return a `FileVisitResult`, so a visitor answers `CONTINUE`, `SKIP_SUBTREE`, `SKIP_SIBLINGS`, or `TERMINATE` and the walker obeys, which keeps traversal in one place while still giving control. Decide this before writing the interface, because retrofitting a return value onto `void visit(...)` touches every visitor you have.

**Q: Can one visitor instance be reused across traversals or shared between threads?**
**Short:** Not if it accumulates results in fields; a stateful visitor must be created per traversal or made to return values instead.
A: Not if it accumulates. The usual visitor collects into an instance field — a running total, a list of matches — which means a second traversal continues from the first one's numbers and two threads corrupt each other, so a stateful visitor is a per-traversal object, not a shared service. Two ways out: make the visitor stateless and generic, `interface Visitor<R> { R visit(Add n); ... }`, so each `visit` returns a value the caller combines and nothing is stored; or keep the accumulator but construct the visitor at the call site and discard it, treating it as a one-shot. The generic-return form composes far better, since results fold naturally over a tree, but it is awkward for operations that genuinely have no result. Document which one a given visitor is, because the two look identical from the outside and only one is safe to cache in a field.

**Q: What breaks when a Visitor runs over a graph or a DAG instead of a tree?**
**Short:** Shared nodes get visited once per path and a cycle recurses forever, so the visitor needs an identity-based visited set.
A: A node reachable by two paths is visited twice, which silently doubles counts in an aggregating visitor, and a genuine cycle recurses until the stack overflows. The fix is to carry a `Set<Node>` of already-visited nodes and return early on a repeat, using an identity set such as `Collections.newSetFromMap(new IdentityHashMap<>())` when distinct nodes may be `equals` to each other, as literal nodes in an AST often are. Whether double-visiting is a bug depends on the operation — a "print every path" visitor wants both visits, a "sum node sizes" visitor does not — so the visited set belongs in the specific visitor or in an explicit graph-walking base class rather than being forced on all of them. Deep but acyclic structures have the related problem of recursion depth, which an explicit stack solves if trees can reach thousands of levels.

**Q: Can a Visitor modify the structure it is visiting?**
**Short:** Not in place during traversal without corrupting it; return replacement nodes and rebuild the tree, which is how compiler transforms work.
A: Not safely in place. Removing a child while the parent is iterating its list throws `ConcurrentModificationException`, replacing a node mid-walk means the visitor may process the new node, the old one, or neither depending on traversal order, and the result depends on implementation details no caller can see. The standard alternative is a transformer visitor whose methods return a node — `Node visit(Add n)` returns either `n` unchanged or a new node built from transformed children — so the traversal produces a fresh tree and the original is untouched, which is exactly how compiler and query-plan rewriters work and why it makes optimisation passes composable. If mutation is unavoidable, collect the intended edits into a list during the read-only pass and apply them afterwards. Immutable node classes make the whole question moot and are worth the constructor boilerplate in an AST.
