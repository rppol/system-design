...
**Q: What is the fragile base class problem here, and what does it cost to convert a Template Method to Strategy later?**
**Short:** Changing the skeleton silently changes every subclass, and converting to composition means rewriting every subclass into a separate collaborator.
A: Because subclasses depend on the base class's internal call order, a change to the skeleton — reordering steps, adding a call, making a hook conditional — alters the behaviour of every subclass without a single compile error, and the base class author usually cannot see all of them. The coupling is also exclusive: Java has single inheritance, so a class that already extends a template cannot extend another one, and the first time a subclass needs two templates you are stuck. Converting to Strategy afterwards is real work, not a refactor script — each subclass becomes a separate collaborator object, protected step methods become interface methods, any shared protected field must be passed explicitly, and every construction site changes from `new PdfReport()` to assembling a context plus strategies. Prefer composition when you expect several axes of variation, and keep Template Method for a genuinely fixed skeleton with one axis.

**Q: Is a Template Method base class safe to share across concurrent calls?**
**Short:** Only if the steps keep no per-invocation state in instance fields, since a servlet or Spring bean template is a shared singleton.
A: Only if the steps pass data through parameters and return values rather than through instance fields. This bites hard precisely where Template Method is most common: an `HttpServlet` is instantiated once and serves every concurrent request, and a Spring bean template is a singleton by default, so a `protected String currentUser` assigned in one step and read by a later step is shared mutable state and races under load. The corrupted-data symptom is the same one that plagues chain handlers — user A's report renders user B's rows — and it never reproduces in a single-threaded test. Thread the per-invocation data through an explicit context object passed to every step, or make the steps return values the skeleton composes; reserve instance fields for immutable configuration set at construction.

**Q: Can you implement Template Method with interface default methods, and what do you lose?**
**Short:** Yes, a default method can hold the skeleton calling abstract methods, but it cannot be final or hold state, so nothing protects the algorithm.
A: Yes — a `default` method can call the interface's abstract methods and so express the skeleton, which is useful when implementers already extend another class. What you give up is the enforcement: `default` methods cannot be declared `final`, so any implementer can override the skeleton itself and quietly discard the algorithm, and interfaces cannot hold instance state, so anything a step needs must be a parameter or another abstract accessor. You also lose `protected` visibility, since every interface member is public, meaning the individual steps become part of the published API and callers can invoke them out of order. Use the interface form when multiple inheritance of behaviour is the real constraint, and an abstract class with a `final` template method plus `protected` steps when protecting the algorithm matters more.

**Q: How do you unit-test an abstract Template Method class?**
**Short:** Define a minimal concrete subclass inside the test that records each step call, so the test asserts the skeleton's order and conditions.
A: Write a small concrete subclass in the test source whose steps append to a list, then invoke the template method once and assert the recorded sequence — that verifies the thing only the base class owns, which is the order, the conditional hook calls, and the exception path. Do not test it indirectly through a production subclass, since a failure then cannot be attributed to the skeleton or to the step. Test each real subclass separately by exercising its steps against known inputs, and if a step is hard to reach in isolation, that usually means the step is doing too much and should be extracted to a collaborator. The awkwardness of needing a subclass just to test the skeleton is itself the usual argument for Strategy, where the algorithm and the steps are separately instantiable.

**Q: What do you do when a subclass needs to skip a step or run the steps in a different order?**
**Short:** Add an explicit hook the skeleton consults, or split the template, but never let a subclass override the template method to reorder steps.
A: Add a boolean hook such as `shouldValidate()` that the skeleton consults, so the skip becomes part of the published algorithm rather than a subversion of it. What you must not do is let the subclass override the template method itself to reorder or omit steps, because the skeleton's guarantees — that validation always runs before persistence, that cleanup always happens — are exactly what the pattern sells, which is why the template method is declared `final`. If several subclasses want genuinely different orders, the honest conclusion is that there is no single algorithm here, and you should split into two templates or move to Strategy where the composition order is the caller's decision. Treat repeated pressure to reorder as design feedback, not as a hook to add.
