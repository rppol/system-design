...
**Q: Does a Chain of Responsibility built from recursive handler calls risk a StackOverflowError?**
**Short:** Yes, each hop is a live stack frame, so a very deep or accidentally cyclic chain overflows the thread stack.
A: Yes, whenever the chain is expressed as "handler calls next handler" rather than as a loop. In the wrapping form the frames stay live until the innermost handler returns, which is exactly why a stack trace thrown inside a servlet shows one `ApplicationFilterChain.internalDoFilter` frame per filter. With a default thread stack around 512 KB to 1 MB (`-Xss`), a few dozen handlers cost nothing; the real killer is a cycle introduced when the chain is assembled from configuration and someone points a handler's `next` at a handler already upstream, which recurses until the stack dies. The iterative form — a `List<Handler>` plus an index, which is how most modern middleware runners work — has flat stacks and gives far more readable traces. Prefer the iterative form for chains built dynamically, and validate for cycles at assembly time rather than at request time.

**Q: If a handler throws midway through the chain, does the rest of the chain still run?**
**Short:** No, the exception unwinds every handler, so post-processing after the downstream call runs only if it sits in a finally block.
A: No — the exception propagates straight up through every caller and skips all remaining work, both downstream handlers and the post-processing code of every upstream one. In a wrapping chain the code before `chain.doFilter(request, response)` is the pre-phase and the code after it is the post-phase, and the post-phase is ordinary sequential code with no special protection. This produces two classic production bugs: a latency-metrics filter that times `doFilter` loses every failed request from its histogram — precisely the requests worth measuring — and an MDC or SLF4J context filter that clears the thread-local after `doFilter` leaks that context into the next request served by the same pooled thread. Put every post-phase action in a `finally`, and decide explicitly per handler whether it swallows, translates, or rethrows.

**Q: Is it safe to share one handler instance across concurrent requests?**
**Short:** Only if the handler is stateless; a singleton handler that keeps per-request data in a field corrupts data under load.
A: Only if the handler holds no mutable per-request state. Servlet `Filter`s, Spring Security filters, and OkHttp `Interceptor`s are all effectively singletons — one instance serves every concurrent request — so an instance field is shared memory. The failure mode is subtle: a field assigned in the pre-phase and read in the post-phase gets overwritten by another request in between, so the handler logs or bills the wrong user, and it only shows up under concurrency. Keep per-request state in the request object itself (`ServletRequest.setAttribute`) or in a local variable whose scope spans the downstream call, and if you must use a `ThreadLocal`, clear it in a `finally`. Fields set once at construction, such as configuration or collaborators, are fine and expected.

**Q: What changes when the chain is asynchronous rather than blocking?**
**Short:** Each hop returns a future instead of a value, so short-circuiting means completing that future early rather than simply returning.
A: The handler signature becomes something like `CompletionStage<Response> handle(Request req, Chain next)` and each hop composes with `thenCompose` instead of calling down and waiting. Two things that worked in the blocking chain stop working: `try/finally` no longer sees downstream failures, because the method returns before the work completes, so post-processing must move into `whenComplete` or `handle` on the returned stage; and `ThreadLocal` context evaporates across the async boundary, because the continuation may run on a different pool thread, so context must be passed explicitly or carried by a context-propagating executor. Netty's `ChannelPipeline` avoids the composition problem entirely by making handlers void and pushing events forward with `ctx.fireChannelRead`, with the response written back down the pipeline rather than returned. Pick one shape for the whole chain — a single blocking handler in an async chain reintroduces thread blocking exactly where you were trying to remove it.

**Q: How is Chain of Responsibility different from Observer, given both hand one request to many objects?**
**Short:** Observer notifies every listener unconditionally with no ordering or veto, while a chain is ordered and any handler can stop it.
A: Observer broadcasts a fact — order among listeners is unspecified, no listener can prevent delivery to the others, and the publisher ignores what they return. Chain of Responsibility delegates a responsibility — order is explicit and load-bearing, a handler may consume the request, mutate it for those downstream, or reject it outright, and the caller does care about the outcome. The diagnostic question is whether you would be upset if one participant did not run, or whether participant two depends on what participant one did: either "yes" means you have a chain, not an observer set. Real systems use both for different jobs — Spring's `ApplicationEventPublisher` is Observer, while its `SecurityFilterChain` is Chain of Responsibility — so name the one you mean rather than saying "the request is passed around".
