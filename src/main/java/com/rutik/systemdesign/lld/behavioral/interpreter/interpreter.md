...
**Q: What is the security risk of exposing an expression language to users, and how do you contain it?**
**Short:** A user-supplied expression is code execution unless sandboxed, so restrict the grammar, forbid method and type access, and bound time and memory.
A: An interpreter evaluates whatever it is given, so any user-controlled expression is untrusted code running with your process's privileges — the well-documented failure mode is an expression language that permits method invocation and type references, letting an input reach runtime facilities and execute arbitrary commands. Containment is layered: prefer a purpose-built grammar that has no notion of method calls, field access, or class literals over a general-purpose engine with features disabled; evaluate against a whitelist of variables in the Context rather than exposing live domain objects whose getters can be chained; and bound the evaluation with a timeout and a recursion or node-count limit, since even an arithmetic-only grammar can express an expression that never finishes or that allocates enormous intermediate values. Never build the expression by concatenating user input into a template, which is the string-injection version of the same bug, and log the expression source with every evaluation so an abusive input is attributable.

**Q: What goes wrong if an AndExpression evaluates both of its children?**
**Short:** Short-circuiting is lost, so the right side runs even when the left is false, producing avoidable errors and unwanted side effects.
A: You lose short-circuit semantics, and with it the idiom every user of the language relies on: `user != null && user.isActive()` throws a null-pointer error because the right child is evaluated regardless. The naive `interpret()` that computes `left.interpret(ctx) && right.interpret(ctx)` in Java actually short-circuits by accident, but the moment you evaluate into locals first, or add logging, tracing, or a uniform "evaluate all children then combine" base class, the behaviour silently changes. The same rule governs a conditional node, which must evaluate its test and then exactly one branch, never both — an eager conditional in a language with side effects, or with expensive subexpressions, is both a correctness and a performance bug. Write the laziness into those node classes explicitly and unit-test it with a child that throws when evaluated, so a later refactor cannot quietly remove it.

**Q: Does the Interpreter pattern cover parsing, and where do precedence and associativity live?**
**Short:** No, Interpreter only evaluates an existing AST; precedence and associativity are decided by the parser that builds the tree's shape.
A: No — GoF's Interpreter starts from an AST that already exists and says nothing about how to build it, which is the single most common misunderstanding of the pattern. Precedence and associativity are entirely properties of the tree's shape: once `2 + 3 * 4` has been parsed into an addition whose right child is a multiplication, evaluation is a trivial post-order walk with no precedence logic anywhere, and if the parser built it the other way the interpreter would happily compute the wrong answer. That is why hand-written interpreters usually pair with a recursive-descent parser with one method per precedence level, and why the honest answer to "implement a calculator" in an interview is to describe both halves and name which one is Interpreter. Keep the two separable, because you will replace the parser long before you replace the evaluator.

**Q: Is a parsed AST safe to evaluate from multiple threads, and should you cache it?**
**Short:** Yes if nodes are immutable and all mutable state lives in a per-evaluation Context, and caching the parsed tree is where the real speedup is.
A: Yes, provided the node objects are immutable after parsing and every piece of evaluation state — variable bindings, accumulators, output buffers — lives in the Context passed into `interpret()`, so concurrent evaluations share only read-only structure. The bug is caching a partial result or the last value on the node itself as a micro-optimisation, which turns a thread-safe tree into shared mutable state and produces wrong answers under load rather than an exception. Caching is worth it in the other direction: parsing usually dominates a single evaluation, so parse each distinct expression once, keep the tree in a bounded map keyed by the source string, and reuse it — the same reason expression-language integrations advise holding the parsed expression rather than re-parsing per call. Bound that cache, because keying on user-supplied strings is an unbounded-growth risk.

**Q: How do you produce useful error messages from a tree-walking interpreter?**
**Short:** Record the source line and column on every node at parse time, then wrap evaluation failures in an error that reports that position.
A: By carrying source positions through the AST: the parser stamps each node with the line, column, and source text span it came from, and `interpret()` catches low-level failures and rethrows them as a domain error naming the position and the offending subexpression. Without that, users get a bare `NullPointerException` or `ClassCastException` from deep inside the tree with a stack trace of internal node classes, which tells them nothing about which part of their expression was wrong. Two refinements pay for themselves: run a separate validation pass over the tree before evaluation to catch unbound variables and type mismatches statically, so an obviously broken expression fails at save time rather than at 3am in a scheduled job; and include the full expression with a caret under the failing span, the convention compilers use because it is the fastest thing for a human to read. Budget for this from the start — retrofitting positions onto an existing node hierarchy touches every class.
