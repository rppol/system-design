...
**Q: Where do side effects belong in a State machine, and what breaks if a transition fails halfway?**
**Short:** Put side effects in explicit entry and exit actions, and change the state field only after the effect succeeds, or the two diverge.
A: In explicit `onEntry`/`onExit` hooks on each state rather than scattered through the action methods, so it is obvious what happens on every arrival and departure and no path can skip the cleanup. The failure that matters is a transition that both mutates the state field and performs an external effect — charge a card, then set state to `PAID`. If the effect succeeds and the assignment throws, or the process dies between them, the machine and the outside world disagree permanently, and a retry double-charges. Order the work so the irreversible effect happens last and the state change is the cheap, local, final step, make the effect idempotent with an operation key so a retry is safe, or record an intermediate `PAYMENT_PENDING` state that a reconciliation job can resolve. A state machine gives you no atomicity for free; that is the caller's problem to solve.

**Q: How do you persist a State machine's current state across restarts?**
**Short:** Persist a stable state identifier, never a serialized state object, and keep old identifiers readable when the class is renamed or removed.
A: Store a stable identifier — an enum name or a short code column — and rehydrate by mapping that identifier back to a state instance on load, never by serializing the state object itself, which drags implementation details into your data. The trap arrives at the first refactor: rename `AWAITING_SHIP` to `READY_TO_SHIP` and every in-flight row still carries the old string, so `Enum.valueOf` throws on load and the orders are stuck. Treat persisted state identifiers as a public schema — add new ones freely, keep retired ones parseable and mapped to a terminal or migration state, and write a data migration when you truly must remove one. Storing an ordinal instead of a name is worse still, because reordering the enum silently reinterprets existing rows.

**Q: When is a full State class hierarchy over-engineering compared with a switch on an enum?**
**Short:** When there are only a few states and one behaviour that varies, a switch keeps the whole machine visible in one readable place.
A: When the machine has a handful of states, one or two state-dependent operations, and no plan to grow — then a `switch` over an enum shows the entire behaviour on one screen, while the class hierarchy spreads it over a file per state and forces a reader to open all of them to answer "what happens on cancel". The crossover is roughly when states carry their own data or when the number of operations grows, because a switch must then be duplicated in every method and the compiler stops helping you keep them in sync; an exhaustive `switch` over an enum with no default does at least fail compilation when a state is added, which is the main reason to keep it. Unwinding in the other direction is the expensive one: once callers hold `State` references and states drive their own transitions, collapsing back to a switch means finding every transition scattered across the concrete states. Start with the enum switch and promote to classes when a state needs its own fields.

**Q: What do you do when several states share most of their behaviour but differ in one transition?**
**Short:** Use a hierarchical state machine, where a superstate holds the shared handling and substates override only the transition that differs.
A: Model it as a hierarchical state machine: a superstate implements the common handling, substates inherit it and override only what differs, and an event unhandled by a substate bubbles up to its parent. This is what UML statecharts add over a flat finite-state machine, and it is the real answer to state explosion, because a global event like `cancel` or `disconnect` is handled once on the superstate instead of copied into fifteen leaf states. In plain Java it is an abstract base state class with default implementations plus concrete subclasses, which is Template Method living inside State. Reach for it as soon as you notice yourself copying the same handler into several states, and reach for a statechart library or an explicit transition table once you also need orthogonal regions or history states.

**Q: When does a state machine belong in a workflow engine rather than in State classes?**
**Short:** Once transitions must survive restarts, wait days for a human, or be visible to non-engineers, durable workflow tooling beats in-memory State objects.
A: Once the machine is long-running and durable rather than in-memory and short-lived. State classes hold the current state in a field, so anything that must survive a deploy, wait days for a human approval, retry with backoff, fire timers, or produce an audit trail of every transition ends up with you reimplementing a workflow engine badly around them. The signals are timers and deadlines as first-class events, compensation across multiple services, and business people asking to see or change the flow. Keep State for machines whose whole lifecycle fits inside one process and one request — a TCP connection, a media player, a document editor — and move to a durable workflow or saga orchestrator, with the state persisted per instance, when the lifecycle is measured in days.
