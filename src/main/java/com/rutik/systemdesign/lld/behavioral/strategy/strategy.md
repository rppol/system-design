...
**Short:** Strategy defines a family of interchangeable algorithms behind a common interface that a Context delegates to.
...
**Short:** Template Method varies algorithm steps via inheritance, while Strategy injects a whole algorithm object via composition.
...
**Short:** Yes, any Java functional interface like Comparator or Predicate is effectively a Strategy implementable by a lambda.
...
**Short:** A registry Map from key to Strategy instance is the most flexible way to select a strategy at runtime.
...
**Short:** Strategy embodies favoring composition over inheritance and the Open/Closed Principle for algorithm variation.
...
**Short:** No, a plain field is not thread-safe when swapped concurrently; use volatile or AtomicReference instead.
...
**Short:** If-else suits a few stable options, a Map dispatch table scales to many, and plain Strategy suits one fixed algorithm per Context.
...
**Short:** Yes, well-designed strategies are stateless, so a single instance can be safely shared across many Contexts and threads.
...
**Short:** Negligible in practice, since the JIT can devirtualize or inline a monomorphic strategy call almost like a direct call.
...
**Short:** Inject a stub, lambda, or mock Strategy via constructor injection to test the Context without a real algorithm.
...
**Q: If the Context no longer has an if-else, but the factory that picks the strategy does, have you actually gained anything?**
**Short:** Yes, because the branch shrinks to one line per option in one place, but only if selection is data-driven rather than a growing ladder.
A: Yes, but less than people claim, and only if you keep the selection honest. Moving a fat conditional out of the Context is already a win — the branch no longer surrounds the business logic, each arm is one mapping line rather than an algorithm, and the Context becomes testable with a stubbed strategy. The gain evaporates if the factory grows its own logic, with nested conditions choosing among strategies and special cases per customer, which is the original problem relocated. Keep selection declarative: a `Map<Enum, Strategy>` built once, or dependency injection keyed on a discriminator, so adding a strategy adds a registry entry rather than a branch. If you genuinely cannot avoid conditional selection logic, that is a sign the choice itself is a business rule worth naming as its own class.

**Q: How is Strategy different from State, given both delegate to an interchangeable object?**
**Short:** The client picks a Strategy and it does not change itself, whereas a State object decides the Context's next state, so the object drives transitions.
A: The difference is who chooses and whether the object knows about its siblings. A Strategy is selected from outside by the client or a factory, is normally unaware that other strategies exist, and does not replace itself; a State is typically chosen from inside, knows the states it can transition to, and swaps the Context's current state as part of handling a request. That gives them different graph shapes: strategies form a flat set of alternatives, states form a directed graph of transitions with legal and illegal edges. The structural code looks nearly identical — a Context holding an interface reference — so name the intent in an interview: "interchangeable algorithms for the same task" is Strategy, "behaviour that changes as the object moves through a lifecycle" is State.

**Q: How do you wire a set of strategies with Spring so a new one registers itself?**
**Short:** Inject every implementation as a List or Map of the strategy interface; Spring supplies all beans of that type, keyed by bean name for a Map.
A: Declare each strategy as a bean and inject `List<PaymentStrategy>` or `Map<String, PaymentStrategy>` into the selector — Spring populates the collection with every bean implementing the interface, and for the `Map` form the key is the bean name, so `@Component("card")` becomes the lookup key. Adding a strategy then means adding one annotated class with no edit to the selector, which is the Open/Closed payoff made real. Two gotchas: relying on bean names as business keys makes a rename a silent behaviour change, so prefer having each strategy expose its own `supports(x)` or `getKey()` method and build the map from that; and list order is not guaranteed unless you add `@Order` or implement `Ordered`, which matters if the selector picks the first match. Validate at startup that the map is non-empty and has no unexpected duplicate keys, so a misconfiguration fails on boot rather than on the first request.

**Q: What do you do when different strategies need different inputs?**
**Short:** Pass a single context or parameter object rather than widening the interface, and bind strategy-specific configuration at construction time.
A: Do not widen the interface to the union of every strategy's needs — that is how you end up with `execute(order, taxRate, couponCode, carrierId)` where each implementation ignores most arguments and a new strategy forces every caller to change. Two clean options: define one immutable parameter object, such as a `PricingRequest`, that carries everything any strategy might read, so adding a field is additive; or bind the strategy-specific values at construction, so a `CouponStrategy` receives its coupon rules as constructor arguments and the runtime call stays narrow. Prefer constructor binding for configuration that varies per deployment and the parameter object for data that varies per invocation. If one strategy genuinely needs input the others cannot even see, that is evidence you have two different abstractions sharing an interface by accident.

**Q: When is an enum with per-constant bodies a better Strategy than separate classes?**
**Short:** When the set of strategies is small, fixed, and stateless, an enum makes them exhaustive, serializable by name, and impossible to forget in a switch.
A: When the strategies are a closed, known set — arithmetic operations, comparison directions, retry policies — an enum where each constant overrides the abstract method gives you the whole family in one readable file, a guaranteed single instance per constant, safe deserialization by name, and the ability to iterate `values()` for validation or documentation. It is the form Effective Java recommends for exactly this shape, and it composes well with a `switch` that the compiler can check for exhaustiveness. Its limits are the flip side of its strengths: constants are effectively singletons, so a strategy needing per-instance configuration or constructor-injected collaborators does not fit, and third parties cannot add a constant, so any plugin or extension story rules it out. Start with the enum when the list is closed and move to classes the moment an implementation needs injected dependencies.
