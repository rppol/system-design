...
**Q: When is Command the wrong pattern, and what does it cost to unwind once it is in a codebase?**
**Short:** Command is wrong when nothing ever needs to defer, queue, undo, or log the call, and unwinding it means inlining every class back into its caller.
A: Command is the wrong choice when the operation is invoked immediately, exactly once, from one place, and nobody needs to store, replay, or reverse it — then a plain method call says the same thing with one class instead of three. The tell that you over-applied it is a command class whose `execute()` body is a single delegating line and whose only caller constructs it and immediately runs it; you have added an allocation, an indirection, and a file for no capability. Unwinding is genuinely expensive because the pattern is viral: every call site now speaks in command objects, any persisted queue or undo history holds serialized commands, and the invoker's logging, retry, and metrics are all keyed on the command type. Adopt Command when you can name the second capability it buys — a queue, an undo stack, an audit log, a retry — and stay with direct calls until then.

**Q: How is a Command object different from an event in event sourcing or CQRS?**
**Short:** A command is an imperative request that can still be rejected, while an event is an immutable past-tense fact that already happened.
A: A command is a request in the imperative mood — `PlaceOrder`, `CancelShipment` — addressed to one handler, validated on arrival, and legitimately refusable; an event is a statement in the past tense — `OrderPlaced`, `ShipmentCancelled` — that records something already true, is broadcast to any number of subscribers, and cannot be rejected because it is history. The practical consequences differ sharply: a command may be retried and so needs idempotency keys, whereas an event is appended once and replayed forever, which means an event's schema must stay readable by future code while a command's can change with its single handler. Mixing the two is a common design error — persisting commands as the source of truth means a replay re-runs validation against today's rules and can produce a different history. Use Command for the in-process invoker/undo machinery this pattern is about, and use events when the record itself is the system of record.

**Q: What invalidates a redo stack, and why must executing a new command clear it?**
**Short:** Any new command executed after an undo clears the redo stack, because the redone states no longer follow from the current one.
A: Executing a fresh command after one or more undos must discard the redo stack, because redo entries are only meaningful as a continuation of the state they were undone from, and the new command has moved the document to a different state. Skipping this is the classic undo bug: the user undoes a paragraph delete, types something new, presses redo, and the editor replays a deletion against text whose offsets no longer exist, corrupting the document or throwing an index exception. The standard implementation keeps two stacks — `execute` pushes to undo and clears redo, `undo` pops undo and pushes redo, `redo` pops redo and pushes undo — and the clear is the line people forget. The other invalidator is state changed behind the invoker's back, such as a background job editing the same document, so route every mutation through the invoker or accept that undo is unsound.

**Q: What stops an undo history from becoming a memory leak?**
**Short:** Bound the history and store deltas rather than snapshots, since each command pins its receiver and captured state for as long as it is retained.
A: Nothing, unless you bound it, because every retained command holds strong references to its receiver and to whatever state it captured for reversal, so an unbounded history keeps the entire edit history of the session reachable. The cost is dominated by what you capture: a command that snapshots a whole document per keystroke grows linearly in document size times edit count, while one that stores a delta — the inserted text and its offset — stores bytes. Standard mitigations are a fixed-depth deque that discards the oldest entry, coalescing consecutive small commands of the same type into one, and clearing the history at a natural boundary such as save or document close. Choose the depth deliberately and store deltas, and where a full snapshot is unavoidable use a Memento holding an immutable, structurally shared copy rather than a deep clone.

**Q: What happens if the third command inside a macro command fails halfway through?**
**Short:** The macro must undo the commands it already executed in reverse order, or it leaves the system in a partially applied state.
A: The macro is left partially applied unless it explicitly rolls back, so `MacroCommand.execute()` must track which children succeeded and call `undo()` on them in reverse order when a later child throws. This is a compensating rollback, not a transaction, and it has two holes worth naming in an interview: an irreversible child, such as one that already charged a card or sent an email, cannot be undone and needs its own compensating action, and the rollback itself can fail, which you must log and surface rather than swallow inside the catch block. Ordering matters too — put the reversible, cheap children first and the irreversible ones last, so the most likely failures happen while rollback is still possible. If genuine all-or-nothing semantics are required across a database, use the database's transaction rather than a macro command's best-effort unwind.
