...
**Q: What happens if an Observer attaches or detaches itself while the Subject is notifying?**
**Short:** With a plain list it throws ConcurrentModificationException; with a copy-on-write snapshot it survives, but the change takes effect only next round.
A: With an `ArrayList` iterated directly it throws `ConcurrentModificationException`, because the observer's `update()` runs inside the subject's loop and structurally modifies the list under the iterator. The usual fix is to iterate a snapshot — `CopyOnWriteArrayList`, or copying the list before the loop — which stops the exception but changes the semantics in a way people forget: a newly attached observer does not receive the event currently being dispatched, and a just-detached observer still does, because the snapshot was taken first. A self-detaching "notify me once" observer therefore fires exactly once, which is what you wanted, while an observer that attaches a child observer will see that child miss the triggering event. Decide which semantics you need and write it in the subject's contract, since both are defensible and callers cannot infer it.

**Q: Why does a weak-reference listener registry silently drop lambda observers?**
**Short:** A lambda has no other strong reference, so a weak registry lets the collector reclaim it and the listener stops firing at an unpredictable time.
A: Because the only reference to `subject.addListener(e -> repaint())` is the weak one you just stored — the lambda is an anonymous object nobody else holds, so the next garbage collection reclaims it and notifications stop, at a moment that depends on heap pressure and is therefore unreproducible in tests. The same trap catches any inline anonymous class and, in JavaFX, bindings whose intermediate objects are only weakly held. Weak registries only work when the observer's lifetime is anchored elsewhere, typically because it is a field of a longer-lived object such as a view or controller. Either keep a strong field for every registered listener and detach it explicitly, or accept strong references plus a mandatory `detach()` in a `finally` or lifecycle hook — a weak registry that quietly stops working is worse than a leak, because a leak is at least visible in a heap dump.

**Q: How do you stop cascading updates from looping or from observing a half-updated state?**
**Short:** Update all subject state before notifying, and either guard against reentrancy or batch changes so observers never see an intermediate value.
A: Two separate problems, both caused by notifying too eagerly. Looping happens when observer B's reaction mutates subject A, which notifies B again; the defences are the same as in Mediator — suppress a notification when the value did not change, and add a reentrancy flag for genuine round trips. Observing a half-updated state, sometimes called a glitch, happens when a subject notifies after mutating field one but before mutating field two, so an observer that reads both sees an inconsistent pair, or when an observer derived from two subjects is notified twice per logical change and computes nonsense in between. The fixes are to complete all state changes before the notify call, and for multi-source dependencies to batch changes into one transaction that notifies once at commit, which is exactly what reactive frameworks buy you with their topological update ordering. Do not paper over it with an `if (updating) return;` at the top of every observer.

**Q: What is the backpressure risk in an asynchronous Observer, and how does the Flow API address it?**
**Short:** A slow observer behind an unbounded queue grows until the heap dies, which is why Flow makes the subscriber request n items.
A: If notifications are handed to an executor or queue so the subject never blocks, a subscriber that consumes slower than the subject produces makes that queue grow without bound until the process runs out of memory — and because the subject is decoupled, nothing signals the problem until the heap does. `java.util.concurrent.Flow` inverts this by making demand explicit: the subscriber calls `subscription.request(n)` and the publisher may only emit that many items, so the slowest consumer sets the pace. Without a reactive library the equivalent controls are a bounded queue plus an explicit overflow policy — block the producer, drop oldest, drop newest, or shed the subscriber — chosen per observer rather than globally. Always bound the queue and name the drop policy; an unbounded `LinkedBlockingQueue` in a notification path is a latent outage.

**Q: Why is registering an Observer from the Subject's constructor dangerous?**
**Short:** It publishes this before construction finishes, so another thread can call the observer's methods while final fields are still unset.
A: Because it lets `this` escape before the object is fully constructed: once the subject hands itself to a registry or an observer inside its own constructor, any other thread can reach a partially initialised object whose subclass fields and even `final` fields are not yet guaranteed visible. The same hazard applies in reverse — an observer that registers itself in its own constructor can receive an `update()` callback before its fields are assigned, and reads a null collaborator. It is also a subclass trap in a single thread: a superclass constructor that registers and fires a notification runs before the subclass constructor body, so the subclass observes itself in a zeroed state. Register after construction, either from a factory method that constructs and then subscribes, or from an explicit lifecycle hook such as Spring's `@PostConstruct`.
...
|-----------|-------------------|---------------------|
| Spring `ApplicationEventPublisher` + `@EventListener` | Container-managed observers with type-based dispatch, ordering via `@Order`, and `@Async` for off-thread delivery | The default inside a Spring application |
| `@TransactionalEventListener(phase = AFTER_COMMIT)` | Delivery deferred until the publishing transaction commits, so observers never react to work that gets rolled back | Any side effect (email, outbound message, cache invalidation) triggered by a database change |
| Guava `EventBus` / `AsyncEventBus` | A standalone bus with `@Subscribe` handlers and `DeadEvent` for events nobody listened to | Non-Spring JVM applications; `DeadEvent` catches an unregistered subscriber immediately |
